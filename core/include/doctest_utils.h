...
inline bool validateDoctestArguments(int argc, char** argv) {
    ...
    static const std::vector<std::string> valid_prefixes = {
        ...
        "-dt-v", "--dt-version",
        "-dt-c", "--dt-count",
        "-dt-ltc", "--dt-list-test-cases",
        "-dt-lts", "--dt-list-test-suites",
        "-dt-lr", "--dt-list-reporters",
        "-dt-tc", "--dt-test-case",
        "-dt-tce", "--dt-test-case-exclude", 
        "-dt-sf", "--dt-source-file",
        "-dt-sfe", "--dt-source-file-exclude",
        "-dt-ts", "--dt-test-suite",
        "-dt-tse", "--dt-test-suite-exclude",
        "-dt-sc", "--dt-subcase",
        "-dt-sce", "--dt-subcase-exclude",
        "-dt-r", "--dt-reporters",
        "-dt-o", "--dt-out",
        "-dt-ob", "--dt-order-by",
        "-dt-rs", "--dt-rand-seed", 
    ...
    auto isValidDoctestArgument = [&](const std::string& arg) -> bool {
        ...
        for (const auto& prefix : valid_prefixes) {
            if (arg == prefix || (arg.length() > prefix.length() && 
                                  arg.substr(0, prefix.length()) == prefix && 
                                  arg[prefix.length()] == '=')) {
                return true;
            }
        }
...
inline bool checkTestFiltersMatchTests(int argc, char** argv) {
    ...
    if (output.find("unskipped test cases passing the current filters: 0") != std::string::npos || 
        output.find("number of tests: 0") != std::string::npos) {
        // Check if any test filters were actually specified
        bool has_test_filters = false;
        for (int i = 1; i < argc; ++i) {
            std::string arg(argv[i]);
            if (arg.find("-tc=") == 0 || arg.find("--test-case=") == 0 ||
                arg.find("-ts=") == 0 || arg.find("--test-suite=") == 0 ||
                arg.find("-sc=") == 0 || arg.find("--subcase=") == 0 ||
                arg.find("-dt-tc=") == 0 || arg.find("--dt-test-case=") == 0 ||
                arg.find("-dt-ts=") == 0 || arg.find("--dt-test-suite=") == 0 ||
                arg.find("-dt-sc=") == 0 || arg.find("--dt-subcase=") == 0) {
 ...
 * @param argc Number of command line arguments  
 * @param argv Array of command line argument strings
 * @param test_executable_name Name of the test executable for better error messages
 * @return Test result code from doctest
 */
template<typename ContextType = doctest::Context>
inline int runDoctestWithValidation(int argc, char** argv, const std::string& test_executable_name = "") {
