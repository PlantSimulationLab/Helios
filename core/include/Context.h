...
class Texture {
   public:
    ...
    /** \param[in] "texture_file" Path to texture file. Note that file paths can be absolute, or relative to the
...
class CompoundObject {
   public:
    ...
    //! Override the color in the texture map for all primitives in the Compound Object, in which case the primitives
    //! will be colored by the constant RGB color, but will apply the transparency channel in the texture to determine
    //! its shape
    void overrideTextureColor();

    //! For all primitives in the Compound Object, use the texture map to color the primitives rather than the constant
    //! RGB color. This is function reverses a previous call to overrideTextureColor(). Note that using the texture
    //! color is the default behavior.
     ...
     * \note While this function still works for scalar data, it is typically prefereable to use the scalar versions of
     * this function.
     */
   ...
   protected:
    ...
    //! Flag to indicate whether all object primitives are in tact. If any primitives have been deleted, this flag will
    //! be set to false.
...
class Tile : public CompoundObject {
   public:
    //! Default constructor
    Tile(uint a_OID, const std::vector<uint>& a_UUIDs, const int2& a_subdiv, const char* a_texturefile,
...
class Box : public CompoundObject {
   public:
    //! Default constructor
    Box(uint a_OID, const std::vector<uint>& a_UUIDs, const int3& a_subdiv, const char* a_texturefile,
        helios::Context* a_context);
...
class Disk : public CompoundObject {
   public:
    //! Default constructor
    Disk(uint a_OID, const std::vector<uint>& a_UUIDs, uint a_subdiv, const char* a_texturefile,
         helios::Context* a_context);
...
class Cone : public CompoundObject {
   public:
    //! Default constructor
    Cone(uint a_OID, const std::vector<uint>& a_UUIDs, const vec3& a_node0, const vec3& a_node1, float a_radius0,
...
//! Primitive class
/**
 * All primitive objects inherit this class, and it provides functionality universal to all primitives.  There may be
 * additional functionality associated with specific primitive elements.
 * @private
 */
class Primitive {
   public:
    ...
    //! Override the color in the texture map, in which case the primitive will be colored by the constant RGB color,
    //! but will apply the transparency channel in the texture to determine its shape
    void overrideTextureColor();

    //! Use the texture map to color the primitive rather than the constant RGB color. This is function reverses a
    //! previous call to overrideTextureColor(). Note that using the texture color is the default behavior.
    void useTextureColor();
...
/**
 * Position is given with respect to its center. Patches can only be added through the Context member function \ref
 * Context::addPatch(). \image html doc/images/Patch.png "Sample image of a Patch." width=3cm
 * @private
 */
...
/**
 * A Triangle is specified by the positions of its three vertices. Triangles can only be added through the Context
 * member function \ref Context::addTriangle(). \image html doc/images/Triangle.png "Sample image of a Triangle."
 * width=3cm
 * @private
 */
class Triangle : public Primitive {
   public:
    ...
    Triangle(const helios::vec3& vertex0, const helios::vec3& vertex1, const helios::vec3& vertex2,
             const helios::RGBAcolor& color, uint UUID);

    //! Triangle constructor
    Triangle(const helios::vec3& vertex0, const helios::vec3& vertex1, const helios::vec3& vertex2,
             const char* texturefile, const std::vector<helios::vec2>& uv, float solid_fraction, uint UUID);
   ...
   private:
    ...
    void makeTransformationMatrix(const helios::vec3& vertex0, const helios::vec3& vertex1,
                                  const helios::vec3& vertex2);
};

//! Parallelpiped geometric object filled with a participating medium
/** Position is given with respect to its center. Voxels can only be added through the Context member function \ref
 Context::addVoxel().
 @private
 */
...
/** The Context provides an interface to global information about the application environment.   It allows access to
 * application-level operations such as adding geometry, running models, and visualization. After creation, the Context
 * must first be initialized via a call to initializeContext(), after which geometry and models can be added and
 * simulated.
 */
class Context {
   private:
    ...
    void writeDataToXMLstream(const char* data_group, const std::vector<std::string>& data_labels, void* ptr,
   ...
   public:
    ...
    //! Run a self-test of the Context. The Context self-test runs through validation checks of Context-related
    ...
    //! Add new default Patch geometric primitive, which is centered at the origin (0,0,0), has unit length and width,
    //! horizontal orientation, and black color
     ...
     */
    uint addPatch(const helios::vec3& center, const helios::vec2& size, const helios::SphericalCoord& rotation,
                  const char* texture_file, const helios::vec2& uv_center, const helios::vec2& uv_size);
    ...
    //! Add new Triangle geometric primitive
    /** Function to add a new Triangle to the Context given its the (x,y,z) coordinates of its vertices and diffuse
     RGBcolor. \param[in] "vertex0" 3D coordinate of Triangle vertex #0 \param[in] "vertex1" 3D coordinate of Triangle
     vertex #1 \param[in] "vertex2" 3D coordinate of Triangle vertex #2 \param[in] "color" diffuse R-G-B color of
     Triangle \return UUID of Triangle \ingroup primitives
     */
    uint addTriangle(const helios::vec3& vertex0, const helios::vec3& vertex1, const helios::vec3& vertex2,
                     const helios::RGBcolor& color);
    ...
    //! Add new Triangle geometric primitive
    /** Function to add a new Triangle to the Context given its the (x,y,z) coordinates of its vertices and diffuse
     RGBAcolor. \param[in] "vertex0" 3D coordinate of Triangle vertex #0 \param[in] "vertex1" 3D coordinate of Triangle
     vertex #1 \param[in] "vertex2" 3D coordinate of Triangle vertex #2 \param[in] "color" diffuse R-G-B-A color of
     Triangle \return UUID of Triangle \ingroup primitives
     */
    uint addTriangle(const helios::vec3& vertex0, const helios::vec3& vertex1, const helios::vec3& vertex2,
                     const helios::RGBAcolor& color);
    ...
    /** Function to add a new Triangle to the Context given its the (x,y,z) coordinates of its vertices and diffuse
     RGBcolor. \param[in] "vertex0" 3D coordinate of Triangle vertex #0 \param[in] "vertex1" 3D coordinate of Triangle
     vertex #1 \param[in] "vertex2" 3D coordinate of Triangle vertex #2 \param[in] "texture_file" path to image file
     (JPEG or PNG) to be used as texture \param[in] "uv0" u-v texture coordinates for vertex0 \param[in] "uv1" u-v
     texture coordinates for vertex1 \param[in] "uv2" u-v texture coordinates for vertex2 \return UUID of Triangle \note
     Assumes a default color of black. \ingroup primitives
     */
    uint addTriangle(const helios::vec3& vertex0, const helios::vec3& vertex1, const helios::vec3& vertex2,
    ...
    //! Rotate a group of primitives about an arbitrary line (not necessarily passing through the origin) using a vector
    //! of UUIDs
    /**
     * \param[in] "UUID" Unique universal identifier (UUID) of primitive to be translated
     * \param[in] "rot" Rotation angle in radians
     ...
     * \param[in] "currentUUID" uint unique universal identifier (UUID) of primitive that is the destination for data
     * copying
     */
    ...
    /**
     ...
     \param[in] "data" Primitive data value (scalar)
     */
    void setPrimitiveData(const std::vector<std::vector<std::vector<uint>>>& UUIDs, const char* label,
                          const uint& data);
    ...
    //! Get the path to texture map file for primitive. If primitive does not have a texture map, the result will be an
    //! empty string.
    /**
    ...
    //! Get the transparency channel pixel data from primitive texture map. If transparency channel does not exist, an
    ...
    /**
     ...
     \param[in] "data" Object data value (scalar)
     */
    void setObjectData(const std::vector<std::vector<std::vector<uint>>>& objIDs, const char* label,
                       const double& data);
     ...
     * \param[in] "comparator" Points will be filtered if "object_data (comparator) threshold", where (comparator) is
     * one of ">", "<", or "="
     */
    std::vector<uint> filterObjectsByData(const std::vector<uint>& ObjIDs, const char* object_data, float threshold,
    ...
    //! Rotate multiple compound objects about an arbitrary line (not necessarily passing through the origin) based on a
    //! vector of UUIDs
    /**
     * \param[in] "ObjIDs" Vector of object IDs to translate
     * \param[in] "rot" Rotation angle in radians
    ...
    /**
     * \param[in] "ObjectIDs" object IDs of the tile objects to change
     * \param[in] "area_ratio" the approximate ratio between individual tile object area and individual subpatch area
     * desired
     */
    ...
    /** A `tube' or `snake' compound object comprised of Triangle primitives
     ...
     \param[in] "Ndivs" Number of radial divisions of the Tube. E.g., Ndivs = 3 would be a triangular prism, Ndivs = 4
     would be a rectangular prism, etc. \param[in] "nodes" Vector of (x,y,z) positions defining Tube segments.
     ...
     */
    uint addDiskObject(uint Ndivs, const helios::vec3& center, const helios::vec2& size,
                       const helios::SphericalCoord& rotation, const helios::RGBcolor& color);
     ...
     */
    uint addDiskObject(uint Ndivs, const helios::vec3& center, const helios::vec2& size,
                       const helios::SphericalCoord& rotation, const helios::RGBAcolor& color);
     ...
     */
    uint addDiskObject(uint Ndivs, const helios::vec3& center, const helios::vec2& size,
                       const helios::SphericalCoord& rotation, const char* texture_file);
    ...
    /** A `cone' or `cone frustum' or 'cylinder' compound object comprised of Triangle primitives
     \image html doc/images/Tube.png "Sample image of a Tube compound object." width=0.1cm
     \param[in] "Ndivs" Number of radial divisions of the Cone. E.g., Ndivs = 3 would be a triangular prism, Ndivs = 4
     would be a rectangular prism, etc. \param[in] "node0" (x,y,z) position defining the base of the cone \param[in]
     "node1" (x,y,z) position defining the end of the cone \param[in] "radius0" Radius of the cone at the base node.
    ...
    /**\param[in] "label" Name of timeseries variable (e.g., temperature)
     \param[in] "index" Index of timeseries data point. The index starts at 0 for the earliest data point, and moves
     chronologically in time. \ingroup timeseries
     */
    ...
    /**This function interpolates the timeseries data to provide a value at exactly `date' and `time'.  Thus, `date' and
     `time' must be between the first and last timeseries values. \param[in] "label" Name of timeseries variable (e.g.,
     temperature) \param[in] "date" Date vector corresponding to the time of `value' (see \ref helios::Date "Date")
     \param[in] "time" Time vector corresponding to the time of `value' (see \ref helios::time "Time")
    ...
    /**This function returns timeseries data by index, and is typically used when looping over all data in the
     timeseries.  See \ref getTimeseriesLength() to get the total length of the timeseries data. \param[in] "label" Name
     of timeseries variable (e.g., temperature) \param[in] "index" Index of timeseries data point. The index starts at 0
     for the earliest data point, and moves chronologically in time. \return Value of timeseries data point \ingroup
     timeseries
     */
    ...
    /**\param[in] "label" Name of timeseries variable (e.g., temperature)
     \param[in] "index" Index of timeseries data point. The index starts at 0 for the earliest data point, and moves
     chronologically in time. \return Time of timeseries data point \ingroup timeseries
     */
    ...
    /**\param[in] "label" Name of timeseries variable (e.g., temperature)
     \param[in] "index" Index of timeseries data point. The index starts at 0 for the earliest data point, and moves
     chronologically in time. \return Date of timeseries data point \ingroup timeseries
     */
    ...
    /** \param[in] "UUIDs" Subset of primitive UUIDs for bounding box calculation.
     ...
     \param[out] "zbounds" Domain bounds in x-direction (zbounds.x=min bound, zbounds.y=max bound)
     */
    void getDomainBoundingBox(const std::vector<uint>& UUIDs, helios::vec2& xbounds, helios::vec2& ybounds,
                              helios::vec2& zbounds) const;
     ...
     * \param[in] "quiet" If set to true, command line output will be disabled. Optional argument - default value is
     ...
     * \param[in] "column_format" Vector of strings with primitive data labels - the order of the text file columns will
     * be determined by the order of the labels in the vector. If primitive data doesn not exist, an error will be
     * thrown. \param[in] "print_header" Flag specifying whether to print the name of the primitive data in the column
     * header.
     */
    void writePrimitiveData(std::string filename, const std::vector<std::string>& column_format,
     ...
     * \param[in] "column_format" Vector of strings with primitive data labels - the order of the text file columns will
     * be determined by the order of the labels in the vector. If primitive data doesn not exist, an error will be
     * thrown. \param[in] "UUIDs" Unique universal identifiers for primitives to include when writing data to file.
    ...
    /** \param[in] "filename" name of OBJ file.
     ...
     \ingroup context
     */
    std::vector<uint> loadOBJ(const char* filename, const vec3& origin, float height, const SphericalCoord& rotation,
                              const RGBcolor& default_color);
     ...
     */
    std::vector<uint> loadOBJ(const char* filename, const vec3& origin, float height, const SphericalCoord& rotation,
                              const RGBcolor& default_color, const char* upaxis);
