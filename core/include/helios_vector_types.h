...
inline float vec2::operator*(const vec2& a) const { return a.x * x + a.y * y; }
...
inline vec2 vec2::operator+(const float a) const { return {a + x, a + y}; }

inline vec2 operator+(const float a, const vec2& v) { return {a + v.x, a + v.y}; }
...
inline vec2 vec2::operator-(const float a) const { return {x - a, y - a}; }
...
inline vec3 make_vec3(float X[3]) { return {X[0], X[1], X[2]}; }
...
inline float vec3::operator*(const vec3& a) const { return a.x * x + a.y * y + a.z * z; }
...
inline vec3 vec3::operator+(const float a) const { return {x + a, y + a, z + a}; }

inline vec3 operator+(const float a, const vec3& v) { return {a + v.x, a + v.y, a + v.z}; }
...
inline vec4 make_vec4(float X[4]) { return {X[0], X[1], X[2], X[3]}; }

inline float vec4::operator*(const vec4& a) const { return a.x * x + a.y * y + a.z * z + a.w * w; }
...
inline vec4 vec4::operator+(const float a) const { return {x + a, y + a, z + a, w + a}; }

inline vec4 operator+(const float a, const vec4& v) { return {a + v.x, a + v.y, a + v.z, a + v.w}; }
...
struct RGBcolor {
   public:
    ...
    //! Constructor given a vec3 denoting R-G-B components.  \note If arguments are outside of the range 0 to 1, values
    //! are clamped.
...
/** \param[in] "r" Red component
    ...
    \ingroup vectors
*/
inline RGBcolor make_RGBcolor(float r, float g, float b) { return {r, g, b}; }
...
/** \param[in] "r" Red component
    ...
    \ingroup vectors
*/
inline RGBAcolor make_RGBAcolor(float r, float g, float b, float a) { return {r, g, b, a}; }
...
/** \param[in] "color" RGBcolor
    ...
    \ingroup vectors
*/
inline RGBAcolor make_RGBAcolor(RGBcolor color, float a) { return {color.r, color.g, color.b, a}; }
...
struct Date {
    ...
    */
    Date(int a_day, int a_month, int a_year) {
        if (a_day < 1 || a_day > 31) {
            throw(std::runtime_error("ERROR (Date constructor): Day of month is out of range (day of " +
                                     std::to_string(a_day) + " was given)."));
        } else if (a_month < 1 || a_month > 12) {
            throw(std::runtime_error("ERROR (Date constructor): Month of year is out of range (month of " +
                                     std::to_string(a_month) + " was given)."));
        } else if (a_year < 1000) {
...
inline Date make_Date(int day, int month, int year) {
    if (day < 1 || day > 31) {
        throw(std::runtime_error("ERROR (make_Date): Day of month is out of range (day of " + std::to_string(day) +
                                 " was given)."));
    } else if (month < 1 || month > 12) {
        throw(std::runtime_error("ERROR (make_Date): Month of year is out of range (month of " + std::to_string(month) +
                                 " was given)."));
    } else if (year < 1000) {
...
inline Date Julian2Calendar(int JulianDay, int year) {
    ...
    if (JulianDay < 1 || JulianDay > 366) {
        throw(std::runtime_error("ERROR (Julian2Calendar): Julian day of " + std::to_string(JulianDay) +
...
inline Date make_Date(int JulianDay, int year) {
    if (JulianDay < 1 || JulianDay > 366) {
        throw(std::runtime_error("ERROR (make_Date): Julian day is out of range (Julian day of " +
                                 std::to_string(JulianDay) + " was given)."));
    } else if (year < 1000) {
        throw(std::runtime_error("ERROR (make_Date): Year should be specified in YYYY format."));
    }
...
/** \param[in] "elevation" Elevation angle (radians)
    \param[in] "azimuth" Azimuthal angle (radians)
  */
inline SphericalCoord make_SphericalCoord(float a_elevation, float a_azimuth) { return {1, a_elevation, a_azimuth}; }
