...
//    }
...

    {
        ...
        if (!context_test.doesPrimitiveExist(UUID)) {
            ...
            if (fabsf(area - expected_area) > errtol) {
                error_count++;
...
// ====== TEST Voxel::getNormal() ======
    ...
    { //  Test Voxel::calculateSolidFraction()
        ...
        if (!context_test.doesPrimitiveExist(UUID)) {
            ...
            if (fabsf(solidFraction - 1.0f) > errtol) {
                error_count++;
    ...
    { // Test Patch::rotate()
        ...
        if (!context_test.doesPrimitiveExist(UUID)) {
            ...
            vec3 normal = context_test.getPrimitiveNormal(UUID);
            if (fabsf(normal.x) > errtol || fabsf(normal.y) > errtol || fabsf(normal.z - 1.0) > errtol) {
                error_count++;
    ...
    { //  Test Voxel::rotate()
        ...
        if (!context_test.doesPrimitiveExist(UUID)) {
            ...
            vec3 normal = context_test.getPrimitiveNormal(UUID);
            if (fabsf(normal.x) > errtol || fabsf(normal.y) > errtol || fabsf(normal.z) > errtol) {
                error_count++;
    ...
    { //  Triangle::getCenter()
        ...
        if (!context_test.doesPrimitiveExist(UUID)) {
            ...
            if (fabsf(center.x - expected_center.x) > errtol || fabsf(center.y - expected_center.y) > errtol || fabsf(center.z - expected_center.z) > errtol) {
                error_count++;
...

    {
        ...
        if (!context_test.doesPrimitiveExist(UUID)) {
            ...
            if (fabsf(new_normal.x - 0.0f) > 1e-5 || fabsf(new_normal.y - 0.0f) > 1e-5 || fabsf(new_normal.z - 1.0f) > 1e-5) {
                error_count++;
...

    {
        ...
        if (fabsf(retrieved_data - 23.5f) > 1e-5) {
            error_count++;
...

    {
        ...
        if (objID == 0) {  // Assuming valid IDs start from 1
            error_count++;
...

    {
        ...
        if (fabsf(obj_center.x - expected_center.x) > 1e-5 ||
            fabsf(obj_center.y - expected_center.y) > 1e-5 ||
            fabsf(obj_center.z - expected_center.z) > 1e-5) {
            error_count++;
...

    {
        ...
        if (!context_test.doesPrimitiveExist(UUID)) {
            ...
            if (fabsf(T[0] - scaleFactor.x) > 1e-6 ||
                fabsf(T[5] - scaleFactor.y) > 1e-6 ||
                fabsf(T[10] - scaleFactor.z) > 1e-6) {
                error_count++;
...

    {
        ...
        objIDs.push_back(context_test.addPatch(make_vec3(1, 1, 1), make_vec2(2, 2)));
        objIDs.push_back(context_test.addPatch(make_vec3(2, 2, 2), make_vec2(3, 3)));
...

    {
        ...
        if (!context_test.doesPrimitiveExist(UUID)) {
            error_count++;
            std::cerr << "failed: Primitive creation failed." << std::endl;
        } else {
            CompoundObject object;
            object.setPrimitiveUUIDs({UUID});
...

    {
        ...
        if (!ObjIDs.empty()) {
            ...
            if (fabsf(T[3] - shift.x) > 1e-7 || fabsf(T[7] - shift.y) > 1e-7 || fabsf(T[11] - shift.z) > 1e-7) {
                error_count++;
...

    {
        ...
        if (ObjID1 == 0 || ObjID2 == 0) {
            ...
            for (uint ObjID : ObjIDs) {
                ...
                if (fabsf(T[0] - cos_r) > 1e-6 || fabsf(T[2] - sin_r) > 1e-6 ||
                    fabsf(T[8] + sin_r) > 1e-6 || fabsf(T[10] - cos_r) > 1e-6) {
...

    {
        ...
        if (ObjID1 == 0 || ObjID2 == 0) {
            ...
            for (uint ObjID : ObjIDs) {
                ...
                if (fabsf(T[0] - cos_r) > 1e-7 || fabsf(T[2] - sin_r) > 1e-7 ||
                    fabsf(T[8] + sin_r) > 1e-7 || fabsf(T[10] - cos_r) > 1e-7) {
                    error_count++;
...

    {
        ...
        if (ObjID == 0) {
            ...
            if (fabsf(T[5] - cos_r) > 1e-7 || fabsf(T[6] + sin_r) > 1e-7 ||
                fabsf(T[9] - sin_r) > 1e-7 || fabsf(T[10] - cos_r) > 1e-7) {
                error_count++;
                std::cerr << "failed: rotateObject - Rotation matrix does not match expected values." << std::endl;
            }
        }
...

    {
        ...
        if (ObjID == 0) {
            ...
            bool match = (fabsf(T[0] - cos_r) < 1e-6) && (fabsf(T[1] - sin_r) < 1e-6) &&
                         (fabsf(T[4] + sin_r) < 1e-6) && (fabsf(T[5] - cos_r) < 1e-6);
...

    {
        ...
        if (ObjID1 == 0 || ObjID2 == 0) {
            ...
            for (uint ObjID : ObjIDs) {
                ...
                if (fabsf(new_pos.x - expected_new_pos.x) > 1e-6 ||
                    fabsf(new_pos.y - expected_new_pos.y) > 1e-6 ||
                    fabsf(new_pos.z - expected_new_pos.z) > 1e-6) {
                    success = false;
...

    {
        ...
        if (!context_test.doesPrimitiveExist(UUID)) {
            ...
            if (fabsf(min_corner.x - expected_min.x) > 1e-6 ||
                fabsf(min_corner.y - expected_min.y) > 1e-6 ||
                fabsf(min_corner.z - expected_min.z) > 1e-6) {
                error_count++;
            ...
            if (fabsf(max_corner.x - expected_max.x) > 1e-6 ||
                fabsf(max_corner.y - expected_max.y) > 1e-6 ||
                fabsf(max_corner.z - expected_max.z) > 1e-6) {
                error_count++;
...

    {
        ...
        if (ObjID == 0) {
            ...
            if (fabsf(T[0] - scale_factor.x) > 1e-6 || fabsf(T[5] - scale_factor.y) > 1e-6 || fabsf(T[10] - scale_factor.z) > 1e-6) {
                error_count++;
...

    {
        ...
        if (!context_test.doesObjectExist(sphereID)) {
            ...
            if (!sphere) {
                ...
                if (fabsf(computed_volume - expected_volume) > 1e-5) {
                    error_count++;
...

    {
        ...
        if (!context_test.doesObjectExist(tubeID)) {
            ...
            if (!tube) {
                ...
                if (fabsf(computed_length - expected_length) > 1e-5) {
                    error_count++;
...

    {
        ...
        if (!context_test.doesObjectExist(tubeID)) {
            ...
            if (!tube) {
                ...
                if (fabsf(computed_segment_volume - expected_segment_volume) > 1e-5) {
                    error_count++;
...

    {
        ...
        if (!tube) {
            ...
            if (fabsf((new_nodes[1] - new_nodes[0]).magnitude() - 3.0f) > 1e-5) {
                error_count++;
        ...
        if (!tube) {
            ...
            for (size_t segment = 0; segment < scaled_vertices.size(); segment++) {
                for (size_t j = 0; j < scaled_vertices[segment].size(); j++) {
                    ...
                    if (fabsf(new_distance - expected_distance) > error_tolerance) {
                        test_failed = true;
...

    {
        ...
        if (!context_test.doesPrimitiveExist(UUID)) {
            ...
            std::cout << "Testing printPrimitiveInfo() output for UUID: " << UUID << std::endl;
...

    {
        ...
        if (fabsf(center.x - expected_center.x) > 1e-5 ||
            fabsf(center.y - expected_center.y) > 1e-5 ||
            fabsf(center.z - expected_center.z) > 1e-5) {
            error_count++;
...

    {
        ...
        bool isCorrect = true;
        for (int i = 0; i < 16; i++) {
            if (fabsf(T[i] - expected_T[i]) > 1e-5) {
                isCorrect = false;
                break;
            }
        }
...

    {
        ...
        bool isCorrect = true;
        for (int i = 0; i < 16; i++) {
            if (fabsf(retrievedTransform[i] - newTransform[i]) > 1e-5) {
                isCorrect = false;
                break;
            }
        }
...
///
...

    {
        ...
        if (!context_test.doesObjectExist(ObjID)) {
            ...
            if (fabsf(volume - expected_volume) > 1e-5) {
                error_count++;
