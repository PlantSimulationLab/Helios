...
// =================================================================================
// Suite 6: Data Management
//
// Tests for the data management system, including global, object, and primitive
...
TEST_SUITE("Data Management") {
    ...
    TEST_CASE("Object Data") {
        Context ctx;
        uint objID = ctx.addBoxObject(make_vec3(0, 0, 0), make_vec3(1, 1, 1), make_int3(1, 1, 1));

        ctx.setObjectData(objID, "test_uint", (uint) 42);
        std::vector<vec3> vec_data = {make_vec3(1, 2, 3), make_vec3(4, 5, 6)};
        ctx.setObjectData(objID, "test_vec3_vec", vec_data);

        DOCTEST_CHECK(ctx.doesObjectDataExist(objID, "test_uint"));
        DOCTEST_CHECK(ctx.doesObjectDataExist(objID, "test_vec3_vec"));
        DOCTEST_CHECK(!ctx.doesObjectDataExist(objID, "non_existent"));

        uint ui;
        ctx.getObjectData(objID, "test_uint", ui);
        DOCTEST_CHECK(ui == 42);

        std::vector<vec3> v_r;
        ctx.getObjectData(objID, "test_vec3_vec", v_r);
        DOCTEST_CHECK(v_r.size() == 2);
        DOCTEST_CHECK(v_r[1].y == doctest::Approx(5.0));

        DOCTEST_CHECK(ctx.getObjectDataType(objID, "test_uint") == HELIOS_TYPE_UINT);
        DOCTEST_CHECK(ctx.getObjectDataSize(objID, "test_vec3_vec") == 2);

        std::vector<std::string> labels = ctx.listObjectData(objID);
        DOCTEST_CHECK(labels.size() == 2);

        ctx.clearObjectData(objID, "test_uint");
        DOCTEST_CHECK(!ctx.doesObjectDataExist(objID, "test_uint"));
    }
    ...
    TEST_CASE("Primitive Data Types") {
        ...
        ctx.setPrimitiveData(p, "test_uint", (uint) 1);
        ctx.setPrimitiveData(p, "test_double", 2.0);
        ctx.setPrimitiveData(p, "test_vec2", make_vec2(1, 2));
        ctx.setPrimitiveData(p, "test_vec3", make_vec3(1, 2, 3));
        ctx.setPrimitiveData(p, "test_vec4", make_vec4(1, 2, 3, 4));
        ctx.setPrimitiveData(p, "test_int2", make_int2(1, 2));
        ctx.setPrimitiveData(p, "test_int3", make_int3(1, 2, 3));
        ctx.setPrimitiveData(p, "test_int4", make_int4(1, 2, 3, 4));
        ctx.setPrimitiveData(p, "test_string", "world");

        uint r_ui;
        ctx.getPrimitiveData(p, "test_uint", r_ui);
        DOCTEST_CHECK(r_ui == 1);

        double r_d;
        ctx.getPrimitiveData(p, "test_double", r_d);
        DOCTEST_CHECK(r_d == doctest::Approx(2.0));

        vec2 r_v2;
        ctx.getPrimitiveData(p, "test_vec2", r_v2);
        DOCTEST_CHECK(r_v2 == make_vec2(1, 2));

        vec3 r_v3;
        ctx.getPrimitiveData(p, "test_vec3", r_v3);
        DOCTEST_CHECK(r_v3 == make_vec3(1, 2, 3));

        vec4 r_v4;
        ctx.getPrimitiveData(p, "test_vec4", r_v4);
        DOCTEST_CHECK(r_v4 == make_vec4(1, 2, 3, 4));

        int2 r_i2;
        ctx.getPrimitiveData(p, "test_int2", r_i2);
        DOCTEST_CHECK(r_i2 == make_int2(1, 2));

        int3 r_i3;
        ctx.getPrimitiveData(p, "test_int3", r_i3);
        DOCTEST_CHECK(r_i3 == make_int3(1, 2, 3));

        int4 r_i4;
        ctx.getPrimitiveData(p, "test_int4", r_i4);
        DOCTEST_CHECK(r_i4 == make_int4(1, 2, 3, 4));

        std::string r_s;
        ctx.getPrimitiveData(p, "test_string", r_s);
        DOCTEST_CHECK(r_s == "world");

        // Test all supported vector types
        std::vector<int> v_i = {1, 2, 3};
        ctx.setPrimitiveData(p, "test_v_int", v_i);
        std::vector<int> r_v_i;
        ctx.getPrimitiveData(p, "test_v_int", r_v_i);
        DOCTEST_CHECK(r_v_i == v_i);

        std::vector<uint> v_ui = {4, 5, 6};
        ctx.setPrimitiveData(p, "test_v_uint", v_ui);
        std::vector<uint> r_v_ui;
        ctx.getPrimitiveData(p, "test_v_uint", r_v_ui);
        DOCTEST_CHECK(r_v_ui == v_ui);

        std::vector<double> v_d = {1.1, 2.2};
        ctx.setPrimitiveData(p, "test_v_double", v_d);
        std::vector<double> r_v_d;
        ctx.getPrimitiveData(p, "test_v_double", r_v_d);
        DOCTEST_CHECK(r_v_d == v_d);

        std::vector<vec2> v_v2 = {make_vec2(1, 2), make_vec2(3, 4)};
        ctx.setPrimitiveData(p, "test_v_vec2", v_v2);
        std::vector<vec2> r_v_v2;
        ctx.getPrimitiveData(p, "test_v_vec2", r_v_v2);
        DOCTEST_CHECK(r_v_v2 == v_v2);

        std::vector<vec3> v_v3 = {make_vec3(1, 2, 3), make_vec3(4, 5, 6)};
        ctx.setPrimitiveData(p, "test_v_vec3", v_v3);
        std::vector<vec3> r_v_v3;
        ctx.getPrimitiveData(p, "test_v_vec3", r_v_v3);
        DOCTEST_CHECK(r_v_v3 == v_v3);

        std::vector<vec4> v_v4 = {make_vec4(1, 2, 3, 4), make_vec4(5, 6, 7, 8)};
        ctx.setPrimitiveData(p, "test_v_vec4", v_v4);
        std::vector<vec4> r_v_v4;
        ctx.getPrimitiveData(p, "test_v_vec4", r_v_v4);
        DOCTEST_CHECK(r_v_v4 == v_v4);

        std::vector<int2> v_i2 = {make_int2(1, 2), make_int2(3, 4)};
        ctx.setPrimitiveData(p, "test_v_int2", v_i2);
        std::vector<int2> r_v_i2;
        ctx.getPrimitiveData(p, "test_v_int2", r_v_i2);
        DOCTEST_CHECK(r_v_i2 == v_i2);

        std::vector<int3> v_i3 = {make_int3(1, 2, 3), make_int3(4, 5, 6)};
        ctx.setPrimitiveData(p, "test_v_int3", v_i3);
        std::vector<int3> r_v_i3;
        ctx.getPrimitiveData(p, "test_v_int3", r_v_i3);
        DOCTEST_CHECK(r_v_i3 == v_i3);

        std::vector<int4> v_i4 = {make_int4(1, 2, 3, 4), make_int4(5, 6, 7, 8)};
        ctx.setPrimitiveData(p, "test_v_int4", v_i4);
        std::vector<int4> r_v_i4;
        ctx.getPrimitiveData(p, "test_v_int4", r_v_i4);
        DOCTEST_CHECK(r_v_i4 == v_i4);

        std::vector<std::string> v_s = {"hello", "world"};
        ctx.setPrimitiveData(p, "test_v_string", v_s);
        std::vector<std::string> r_v_s;
        ctx.getPrimitiveData(p, "test_v_string", r_v_s);
...
TEST_SUITE("Data Management") {
    ...
    TEST_CASE("Primitive Data Types") {
        ...
        // Test all supported scalar types
        ctx.setPrimitiveData(p, "test_int", 1);
        ctx.setPrimitiveData(p, "test_uint", 1u);
        ctx.setPrimitiveData(p, "test_float", 2.f);
        ctx.setPrimitiveData(p, "test_double", 2.0);
        ctx.setPrimitiveData(p, "test_vec2", make_vec2(1, 2));
        ctx.setPrimitiveData(p, "test_vec3", make_vec3(1, 2, 3));
        ctx.setPrimitiveData(p, "test_vec4", make_vec4(1, 2, 3, 4));
        ...
        int r_i;
        ctx.getPrimitiveData(p, "test_int", r_i);
        DOCTEST_CHECK(r_i == 1);
        ...
        float r_f;
        ctx.getPrimitiveData(p, "test_float", r_f);
        ...
        ctx.setPrimitiveData(p, "test_v_float", v_f);
        std::vector<float> r_v_f;
        ctx.getPrimitiveData(p, "test_v_float", r_v_f);
