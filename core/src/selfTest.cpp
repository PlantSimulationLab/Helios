...
int Context::selfTest() {
    ...
    if (type != PRIMITIVE_TYPE_PATCH) {
        error_count++;
        std::cerr << "failed: uint addPatch( const vec3& center, const vec2& size ). Patch::getType did not return "
                     "`PRIMITIVE_TYPE_PATCH'."
                  << std::endl;
    }
    if (center_r.x != center.x || center_r.y != center.y || center_r.z != center.z) {
        std::cerr << "failed: uint addPatch( const vec3& center, const vec2& size ). Patch::getCenter returned "
                     "incorrect value."
                  << std::endl;
    }
    if (size_r.x != size.x || size_r.y != size.y) {
        error_count++;
        std::cerr
            << "failed: uint addPatch( const vec3& center, const vec2& size ). Patch::getSize returned incorrect value."
            << std::endl;
    }
    if (normal_r.x != 0.f || normal_r.y != 0.f || normal_r.z != 1.f) {
        error_count++;
        std::cerr << "failed: uint addPatch( const vec3& center, const vec2& size ). Patch::getNormal did not return "
                     "correct value."
                  << std::endl;
    }
    if (vertices.at(0).x != vertices_r.at(0).x || vertices.at(0).y != vertices_r.at(0).y ||
        vertices.at(0).z != vertices_r.at(0).z || vertices.at(1).x != vertices_r.at(1).x ||
        vertices.at(1).y != vertices_r.at(1).y || vertices.at(1).z != vertices_r.at(1).z ||
        vertices.at(2).x != vertices_r.at(2).x || vertices.at(2).y != vertices_r.at(2).y ||
        vertices.at(2).z != vertices_r.at(2).z || vertices.at(3).x != vertices_r.at(3).x ||
        vertices.at(3).y != vertices_r.at(3).y || vertices.at(3).z != vertices_r.at(3).z) {
        error_count++;
        std::cerr << "failed: uint addPatch( const vec3& center, const vec2& size ). Patch::getVertices did not return "
                     "correct value."
                  << std::endl;
    }
    if (fabsf(area_r - size.x * size.y) > 1e-5) {
        error_count++;
        std::cerr << "failed: uint addPatch( const vec3& center, const vec2& size ). Patch::getArea did not return "
                     "correct value."
                  << std::endl;
    }
    if (color_r.r != 0.f || color_r.g != 0.f || color_r.b != 0.f) {
        error_count++;
        std::cerr << "failed: uint addPatch( const vec3& center, const vec2& size ). Patch::getColor did not return "
    ...
    if (fabsf(center_cpy.x - center.x - shift.x) > errtol || fabsf(center_cpy.y - center.y - shift.y) > errtol ||
        fabsf(center_cpy.z - center.z - shift.z) > errtol || center_r.x != center.x || center_r.y != center.y ||
        center_r.z != center.z) {
        error_count++;
    ...
    if (fabsf(rotation_r.elevation - rotation.elevation) > errtol ||
        fabsf(rotation_r.azimuth - rotation.azimuth) > errtol) {
        error_count++;
    ...
    if (type != PRIMITIVE_TYPE_TRIANGLE) {
        error_count++;
        std::cerr << "failed: uint addTriangle(const vec3& v0, const vec3& v1, const vec3& v2, const RGBcolor &color "
                     "). Triangle::getType did not return `PRIMITIVE_TYPE_TRIANGLE'."
                  << std::endl;
    }
    ...
    if (normal_r.x != normal.x || normal_r.y != normal.y || normal_r.z != normal.z) {
        error_count++;
        std::cerr << "failed: uint addTriangle(const vec3& v0, const vec3& v1, const vec3& v2, const RGBcolor &color "
                     "). Triangle::getNormal did not return correct value."
                  << std::endl;
    }
    if (vertices.at(0).x != vertices_r.at(0).x || vertices.at(0).y != vertices_r.at(0).y ||
        vertices.at(0).z != vertices_r.at(0).z || vertices.at(1).x != vertices_r.at(1).x ||
        vertices.at(1).y != vertices_r.at(1).y || vertices.at(1).z != vertices_r.at(1).z ||
        vertices.at(2).x != vertices_r.at(2).x || vertices.at(2).y != vertices_r.at(2).y ||
        vertices.at(2).z != vertices_r.at(2).z) {
        error_count++;
        std::cerr << "failed: uint addTriangle(const vec3& v0, const vec3& v1, const vec3& v2, const RGBcolor &color "
                     "). Triangle::getVertices did not return correct value."
                  << std::endl;
    }
    vec3 A(v1 - v0);
    vec3 B(v2 - v0);
    ...
    if (area_r != sqrtf(s * (s - a) * (s - b) * (s - c))) {
        error_count++;
        std::cerr << "failed: uint addTriangle(const vec3& v0, const vec3& v1, const vec3& v2, const RGBcolor &color "
                     "). Triangle::getArea did not return correct value."
                  << std::endl;
    }
    if (color_r.r != color.r || color_r.g != color.g || color_r.b != color.b) {
        error_count++;
        std::cerr << "failed: uint addTriangle(const vec3& v0, const vec3& v1, const vec3& v2, const RGBcolor &color "
                     "). Triangle::getColor did not return correct color."
                  << std::endl;
    }
    ...
    if (vertices.at(0).x != vertices_cpy.at(0).x || vertices.at(0).y != vertices_cpy.at(0).y ||
        vertices.at(0).z != vertices_cpy.at(0).z || vertices.at(1).x != vertices_cpy.at(1).x ||
        vertices.at(1).y != vertices_cpy.at(1).y || vertices.at(1).z != vertices_cpy.at(1).z ||
        vertices.at(2).x != vertices_cpy.at(2).x || vertices.at(2).y != vertices_cpy.at(2).y ||
        vertices.at(2).z != vertices_cpy.at(2).z) {
        error_count++;
    ...
    if (vertices.at(0).x != (vertices_cpy.at(0).x - shift.x) || vertices.at(0).y != (vertices_cpy.at(0).y - shift.y) ||
        vertices.at(0).z != (vertices_cpy.at(0).z - shift.z) || vertices.at(1).x != (vertices_cpy.at(1).x - shift.x) ||
        vertices.at(1).y != (vertices_cpy.at(1).y - shift.y) || vertices.at(1).z != (vertices_cpy.at(1).z - shift.z) ||
        vertices.at(2).x != (vertices_cpy.at(2).x - shift.x) || vertices.at(2).y != (vertices_cpy.at(2).y - shift.y) ||
        vertices.at(2).z != (vertices_cpy.at(2).z - shift.z)) {
        error_count++;
    ...
    for (uint UUIDp : UUIDs) {
        ...
        if (fabsf(rotation_r.zenith - rotation.zenith) > errtol ||
            fabsf(rotation_r.azimuth - rotation.azimuth) > errtol) {
            error_count++;
    ...
    if (bboxmin_patch.x != -1.25f || bboxmax_patch.x != 1.25 || bboxmin_patch.y != -0.25f || bboxmax_patch.y != 0.25f ||
        bboxmin_patch.z != 0.f || bboxmax_patch.z != 0.f) {
        error_count++;
    ...
    if (data_return != data) {
        error_count++;
        std::cerr
            << "failed: set/getPrimitiveData (setting/getting through primitive). Get data did not match set data."
            << std::endl;
    ...
    if (data_return != data) {
        error_count++;
        std::cerr << "failed: set/getPrimitiveData (setting/getting through Context). Get data did not match set data."
                  << std::endl;
        std::cout << data_return << std::endl;
    }
    ...
    if (data_return != data) {
        error_count++;
        std::cerr << "failed: set/getPrimitiveData (setting/getting scalar data). Get data did not match set data."
                  << std::endl;
        std::cout << data_return << std::endl;
    }
    ...
    uint UUIDp = context_test.addPatch(make_vec3(2, 3, 4), sizep, make_SphericalCoord(0, 0), texture, 0.5 * (uv0 + uv2),
                                       uv2 - uv0);
    ...
    if (texture2 != texture) {
        error_count++;
        std::cerr << "failed: textures - queried texture file does not match that provided when adding primitive."
                  << std::endl;
    }
    ...
    if (uv.at(0).x != uv0.x || uv.at(0).y != uv0.y || uv.at(1).x != uv1.x || uv.at(1).y != uv1.y ||
        uv.at(2).x != uv2.x || uv.at(2).y != uv2.y || uv.at(3).x != uv3.x || uv.at(3).y != uv3.y) {
        error_count++;
        std::cerr << "failed: Queried texture (u,v) coordinates do not match that provided when adding primitive."
                  << std::endl;
    }
    ...
    uint UUIDp2 = context_test.addPatch(make_vec3(2, 3, 4), sizep, make_SphericalCoord(0, 0), texture,
                                        0.5 * (uv2 + uv0), uv2 - uv0);
    ...
    uint UUIDp3 = context_test.addTriangle(make_vec3(2, 3, 4), make_vec3(2, 3 + sizep.y, 4),
                                           make_vec3(2 + sizep.x, 3 + sizep.y, 4), texture, uv0, uv1, uv2);
    ...
    if (gdata_return != gdata) {
        error_count++;
        std::cerr << "failed: set/getGlobalData (setting/getting through primitive). Get data did not match set data."
                  << std::endl;
    }
    ...
    for (uint i = 0; i < 5; i++) {
        if (gdata_return_v.at(i) != gdata_v.at(i)) {
            error_count++;
            std::cerr << "failed: set/getGlobalData (setting/getting vector data). Get data did not match set data."
                      << std::endl;
            break;
        }
    }
    ...
    if (gdata_return != gdata) {
        error_count++;
        std::cerr << "failed: set/getGlobalData (setting/getting scalar data). Get data did not match set data."
                  << std::endl;
    }
    ...
    uint IDtile =
        context_test.addTileObject(make_vec3(0, 0, 0), make_vec2(3, 1), make_SphericalCoord(0, 0), make_int2(3, 3));
    ...
    if (objdata_return != objdata) {
        error_count++;
        std::cerr << "failed: set/getObjectData (getting through object pointer). Get data did not match set data."
                  << std::endl;
    }
    ...
    if (objdata_return != objdata) {
        error_count++;
        std::cerr << "failed: set/getObjectData (setting/getting scalar data). Get data did not match set data."
                  << std::endl;
    }
    ...
    if (fabsf(sph1.radius - sph2.radius) > errtol || fabsf(sph1.elevation - sph2.elevation) > errtol ||
        fabsf(sph1.zenith - sph2.zenith) > errtol || fabsf(sph1.azimuth - sph2.azimuth) > errtol) {
    ...
    // context_ts.queryTimeseriesData( "timeseries", date_ts, make_Time(0,0,0) ); //this is to test that querying
    // outside of dataset does not throw a segmentation fault.

    context_ts.queryTimeseriesData("timeseries", date_ts,
                                   make_Time(time0_ts.hour, time0_ts.minute, time0_ts.second + 10));
    ...
    //------- setTileObjectSubdivisionCount, getTileObjectAreaRatio, cropDomain & writeXML & loadXML for compound
    //objects  --------//

    Context context_to;

    // non-textured tile object single subpatch
    uint to1 = context_to.addTileObject(make_vec3(0, 0, 0), make_vec2(1, 0.3),
                                        make_SphericalCoord(float(M_PI) * 0.25, float(M_PI) * 0.75), make_int2(1, 1));
    // non-textured tile object multiple subpatches
    uint to2 = context_to.addTileObject(make_vec3(1, 1, 1), make_vec2(1, 1),
                                        make_SphericalCoord(float(M_PI) * 0.25, float(M_PI) * 0.75), make_int2(5, 3));
    // textured tile object single subpatch disk
    uint to3 = context_to.addTileObject(make_vec3(2, 2, 2), make_vec2(1, 0.3),
                                        make_SphericalCoord(float(M_PI) * 0.25, float(M_PI) * 0.75), make_int2(1, 1),
                                        "lib/images/disk_texture.png");
    // textured tile object multiple subpatches disk
    uint to4 = context_to.addTileObject(make_vec3(3, 3, 3), make_vec2(1, 0.3),
                                        make_SphericalCoord(float(M_PI) * 0.25, float(M_PI) * 0.75), make_int2(5, 3),
                                        "lib/images/disk_texture.png");
    // textured tile object single subpatch diamond
    uint to5 =
        context_to.addTileObject(make_vec3(-1, -1, -1), make_vec2(1, 1), make_SphericalCoord(M_PI * 0.25, M_PI * 0.75),
                                 make_int2(1, 1), "lib/images/diamond_texture.png");
    // textured tile object multiple subpatches diamond
    uint to6 =
        context_to.addTileObject(make_vec3(-2, -2, -2), make_vec2(1, 1), make_SphericalCoord(M_PI * 0.25, M_PI * 0.75),
                                 make_int2(5, 3), "lib/images/diamond_texture.png");
    ...
    if (err_1 >= errtol) {
        std::cerr << "failed. tile object area ratio returned by getTileObjectAreaRatio is not correct for "
                     "non-textured single subpatch tile."
                  << std::endl;
        error_count++;
    }
    ...
    if (err_2 >= errtol) {
        std::cerr << "failed. tile object area ratio returned by getTileObjectAreaRatio is not correct for "
                     "non-textured multi-subpatch tile."
                  << std::endl;
        error_count++;
    }
    double err_3 = fabs(context_to.getTileObjectAreaRatio(to5) - 1.0);
    if (err_3 >= errtol) {
        std::cerr << "failed. tile object area ratio returned by getTileObjectAreaRatio is not correct for textured "
                     "single subpatch tile."
                  << std::endl;
        error_count++;
    }
    ...
    //        std::cerr << "failed. setTileObjectSubdivisionCount (subiv version) not producing the correct number of
    //        primitives." << std::endl; error_count ++;
    //    }

    if (allObjectIDs_to_after.size() != allObjectIDs_to.size()) {
        std::cerr << "failed. setTileObjectSubdivisionCount(subiv version) is  changing the number of objects in the "
                     "context (it shouldn't)."
                  << std::endl;
        error_count++;
    }
    ...
    if (allObjectIDs_to_after2 != allObjectIDs_to) {
        std::cerr << "failed. setTileObjectSubdivisionCount  (area ratio version)  is changing the number of objects "
                     "in the context (it shouldn't)."
                  << std::endl;
        error_count++;
    }

    if (context_to.getTileObjectPointer(to1)->getPrimitiveUUIDs().size() == uint(49)) {
        std::cerr << "failed. setTileObjectSubdivisionCount  (area ratio version)  did not result in the correct "
    ...
    if (context_to_PrimitiveCount != context_to2_PrimitiveCount) {
        std::cerr << "failed. number of primitives before writing and loading xml does not match number after"
                  << std::endl;
        error_count++;
    }

    if (context_to_ObjectCount != context_to2_ObjectCount) {
        std::cerr << "failed. number of Objects before writing and loading xml does not match number after"
                  << std::endl;
        error_count++;
    }
