...
float Texture::getSolidFraction() const { return solidfraction; }

void Context::markGeometryClean() { isgeometrydirty = false; }

void Context::markGeometryDirty() { isgeometrydirty = true; }

bool Context::isGeometryDirty() const { return isgeometrydirty; }
...
vec3 Voxel::getNormal() const { return make_vec3(0, 0, 0); }
...
RGBcolor Primitive::getColor() const { return make_RGBcolor(color.r, color.g, color.b); }

RGBcolor Primitive::getColorRGB() const { return make_RGBcolor(color.r, color.g, color.b); }
...
void Primitive::setColor(const helios::RGBcolor& newcolor) {
    ...
    //   std::cout << "WARNING (Primitive::setColor): Cannot set the color of individual primitives within a compound
    //   object. Use the setter function for objects." << std::endl; return;
    // }
...
void Primitive::overrideTextureColor() {
    // if( parent_object_ID!=0 ){
    //   std::cout << "WARNING (Primitive::overrideTextureColor): Cannot set the texture options of individual
    //   primitives within a compound object. Use the setter function for objects." << std::endl; return;
    // }
...
void Primitive::useTextureColor() {
    // if( parent_object_ID!=0 ){
    //   std::cout << "WARNING (Primitive::useTextureColor): Cannot set the texture options of individual primitives
    //   within a compound object. Use the setter function for objects." << std::endl; return;
    // }
...
void Primitive::scale(const vec3& S) {
    if (parent_object_ID != 0) {
        std::cout << "WARNING (Primitive::scale): Cannot scale individual primitives within a compound object. Use the "
                     "setter function for objects."
                  << std::endl;
        return;
    }
...
void Primitive::translate(const helios::vec3& shift) {
    if (parent_object_ID != 0) {
        std::cout << "WARNING (Primitive::translate): Cannot translate individual primitives within a compound object. "
                     "Use the setter function for objects."
                  << std::endl;
        return;
    }
...
void Patch::rotate(float rot, const char* axis) {
    if (parent_object_ID != 0) {
        std::cout << "WARNING (Patch::rotate): Cannot rotate individual primitives within a compound object. Use the "
                     "setter function for objects."
                  << std::endl;
        return;
    }
...
void Triangle::rotate(float rot, const char* axis) {
    if (parent_object_ID != 0) {
        std::cout << "WARNING (Triangle::rotate): Cannot rotate individual primitives within a compound object. Use "
                     "the setter function for objects."
                  << std::endl;
        return;
    }
...
void Voxel::rotate(float rot, const char* axis) {
    if (parent_object_ID != 0) {
        std::cout << "WARNING (Voxel::rotate): Cannot rotate individual primitives within a compound object. Use the "
                     "setter function for objects."
                  << std::endl;
        return;
    }
...
void Voxel::rotate(float rot, const helios::vec3& axis) {
    std::cout
        << "WARNING (Voxel::rotate) - Voxels can only be rotated about the z-axis. Ignoring this call to rotate()."
        << std::endl;
}
...
void Triangle::makeTransformationMatrix(const helios::vec3& vert0, const helios::vec3& vert1,
                                        const helios::vec3& vert2) {
    // We need to construct the Affine transformation matrix that transforms some generic triangle to a triangle with
    // vertices at vertex0, vertex1, vertex2.

    // V1 is going to be our generic triangle.  This is the triangle that we'll intersect in the OptiX ray intersection
    // program.  We just need to pass the transformation matrix to OptiX so that we'll end up with the right triangle.

    // We'll assume our generic triangle has vertices
    // v0 = (0,0,0)
    // v1 = (0,1,0)
    // v2 = (1,1,0)
    // this needs to match up with the triangle in triangle_intersect() and triangle_bounds() (see
    // primitiveIntersection.cu). Note that the matrix is padded with 1's to make it 4x4
    ...
    inv[0] = V1[5] * V1[10] * V1[15] - V1[5] * V1[11] * V1[14] - V1[9] * V1[6] * V1[15] + V1[9] * V1[7] * V1[14] +
             V1[13] * V1[6] * V1[11] - V1[13] * V1[7] * V1[10];

    inv[4] = -V1[4] * V1[10] * V1[15] + V1[4] * V1[11] * V1[14] + V1[8] * V1[6] * V1[15] - V1[8] * V1[7] * V1[14] -
             V1[12] * V1[6] * V1[11] + V1[12] * V1[7] * V1[10];

    inv[8] = V1[4] * V1[9] * V1[15] - V1[4] * V1[11] * V1[13] - V1[8] * V1[5] * V1[15] + V1[8] * V1[7] * V1[13] +
             V1[12] * V1[5] * V1[11] - V1[12] * V1[7] * V1[9];

    inv[12] = -V1[4] * V1[9] * V1[14] + V1[4] * V1[10] * V1[13] + V1[8] * V1[5] * V1[14] - V1[8] * V1[6] * V1[13] -
              V1[12] * V1[5] * V1[10] + V1[12] * V1[6] * V1[9];

    inv[1] = -V1[1] * V1[10] * V1[15] + V1[1] * V1[11] * V1[14] + V1[9] * V1[2] * V1[15] - V1[9] * V1[3] * V1[14] -
             V1[13] * V1[2] * V1[11] + V1[13] * V1[3] * V1[10];

    inv[5] = V1[0] * V1[10] * V1[15] - V1[0] * V1[11] * V1[14] - V1[8] * V1[2] * V1[15] + V1[8] * V1[3] * V1[14] +
             V1[12] * V1[2] * V1[11] - V1[12] * V1[3] * V1[10];

    inv[9] = -V1[0] * V1[9] * V1[15] + V1[0] * V1[11] * V1[13] + V1[8] * V1[1] * V1[15] - V1[8] * V1[3] * V1[13] -
             V1[12] * V1[1] * V1[11] + V1[12] * V1[3] * V1[9];

    inv[13] = V1[0] * V1[9] * V1[14] - V1[0] * V1[10] * V1[13] - V1[8] * V1[1] * V1[14] + V1[8] * V1[2] * V1[13] +
              V1[12] * V1[1] * V1[10] - V1[12] * V1[2] * V1[9];

    inv[2] = V1[1] * V1[6] * V1[15] - V1[1] * V1[7] * V1[14] - V1[5] * V1[2] * V1[15] + V1[5] * V1[3] * V1[14] +
             V1[13] * V1[2] * V1[7] - V1[13] * V1[3] * V1[6];

    inv[6] = -V1[0] * V1[6] * V1[15] + V1[0] * V1[7] * V1[14] + V1[4] * V1[2] * V1[15] - V1[4] * V1[3] * V1[14] -
             V1[12] * V1[2] * V1[7] + V1[12] * V1[3] * V1[6];

    inv[10] = V1[0] * V1[5] * V1[15] - V1[0] * V1[7] * V1[13] - V1[4] * V1[1] * V1[15] + V1[4] * V1[3] * V1[13] +
              V1[12] * V1[1] * V1[7] - V1[12] * V1[3] * V1[5];

    inv[14] = -V1[0] * V1[5] * V1[14] + V1[0] * V1[6] * V1[13] + V1[4] * V1[1] * V1[14] - V1[4] * V1[2] * V1[13] -
              V1[12] * V1[1] * V1[6] + V1[12] * V1[2] * V1[5];

    inv[3] = -V1[1] * V1[6] * V1[11] + V1[1] * V1[7] * V1[10] + V1[5] * V1[2] * V1[11] - V1[5] * V1[3] * V1[10] -
             V1[9] * V1[2] * V1[7] + V1[9] * V1[3] * V1[6];

    inv[7] = V1[0] * V1[6] * V1[11] - V1[0] * V1[7] * V1[10] - V1[4] * V1[2] * V1[11] + V1[4] * V1[3] * V1[10] +
             V1[8] * V1[2] * V1[7] - V1[8] * V1[3] * V1[6];

    inv[11] = -V1[0] * V1[5] * V1[11] + V1[0] * V1[7] * V1[9] + V1[4] * V1[1] * V1[11] - V1[4] * V1[3] * V1[9] -
              V1[8] * V1[1] * V1[7] + V1[8] * V1[3] * V1[5];

    inv[15] = V1[0] * V1[5] * V1[10] - V1[0] * V1[6] * V1[9] - V1[4] * V1[1] * V1[10] + V1[4] * V1[2] * V1[9] +
              V1[8] * V1[1] * V1[6] - V1[8] * V1[2] * V1[5];
    ...
    for (int i = 0; i < 16; i++) invV1[i] = inv[i] * det;
...
void Primitive::getPrimitiveData(const char* label, int& data) const {
    if (!doesPrimitiveDataExist(label)) {
        throw(std::runtime_error("ERROR (getPrimitiveData): Primitive data " + std::string(label) +
    ...
    if (type == HELIOS_TYPE_INT) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type int, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Primitive::getPrimitiveData(const char* label, uint& data) const {
    ...
    if (type == HELIOS_TYPE_UINT) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type uint, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Primitive::getPrimitiveData(const char* label, float& data) const {
    ...
    if (type == HELIOS_TYPE_FLOAT) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type float, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Primitive::getPrimitiveData(const char* label, double& data) const {
    ...
    if (type == HELIOS_TYPE_DOUBLE) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type double, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Primitive::getPrimitiveData(const char* label, vec2& data) const {
    ...
    if (type == HELIOS_TYPE_VEC2) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type vec2, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Primitive::getPrimitiveData(const char* label, vec3& data) const {
    ...
    if (type == HELIOS_TYPE_VEC3) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type vec3, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Primitive::getPrimitiveData(const char* label, vec4& data) const {
    ...
    if (type == HELIOS_TYPE_VEC4) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type vec4, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Primitive::getPrimitiveData(const char* label, int2& data) const {
    ...
    if (type == HELIOS_TYPE_INT2) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type int2, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Primitive::getPrimitiveData(const char* label, int3& data) const {
    ...
    if (type == HELIOS_TYPE_INT3) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type int3, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Primitive::getPrimitiveData(const char* label, int4& data) const {
    ...
    if (type == HELIOS_TYPE_INT4) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type int4, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Primitive::getPrimitiveData(const char* label, std::string& data) const {
    ...
    if (type == HELIOS_TYPE_STRING) {
        ...
        throw(std::runtime_error("ERROR (getPrimitiveData): Attempted to get data for type string, but data " +
                                 std::string(label) + " for primitive " + std::to_string(UUID) +
...
void Context::setPrimitiveData(const uint& UUID, const char* label, const int& data) {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (setPrimitiveData): UUID of " + std::to_string(UUID) +
...
void Context::setPrimitiveData(const std::vector<std::vector<uint>>& UUIDs, const char* label,
                               const helios::vec2& data) {
    for (const auto& j : UUIDs) {
        for (const auto& UUID : j) {
            setPrimitiveData(UUID, label, data);
        }
...
void Context::setPrimitiveData(const std::vector<std::vector<std::vector<uint>>>& UUIDs, const char* label,
                               const int& data) {
    for (const auto& j : UUIDs) {
        for (const auto& UUID : j) {
            setPrimitiveData(UUID, label, data);
...
void Context::getPrimitiveData(uint UUID, const char* label, int& data) const {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (getPrimitiveData): UUID of " + std::to_string(UUID) +
                                 " does not exist in the Context."));
    }
...
void Context::setObjectData(const uint objID, const char* label, const int& data) {
    if (objects.find(objID) == objects.end()) {
        throw(std::runtime_error("ERROR (setObjectData): objID of " + std::to_string(objID) +
                                 " does not exist in the Context."));
    }
...
void Context::setObjectData(const std::vector<std::vector<std::vector<uint>>>& objIDs, const char* label,
                            const int& data) {
    for (const auto& j : objIDs) {
        for (const auto& i : j) {
...
void Context::getObjectData(const uint objID, const char* label, int& data) const {
    if (objects.find(objID) == objects.end()) {
        throw(std::runtime_error("ERROR (getObjectData): objID of " + std::to_string(objID) +
                                 " does not exist in the Context."));
    }
...
HeliosDataType Context::getObjectDataType(const uint objID, const char* label) const {
    if (objects.find(objID) == objects.end()) {
        throw(std::runtime_error("ERROR (getObjectDataType): objID of " + std::to_string(objID) +
                                 " does not exist in the Context."));
    }
...
uint Context::getObjectDataSize(const uint objID, const char* label) const {
    if (objects.find(objID) == objects.end()) {
        throw(std::runtime_error("ERROR (getObjectDataSize): objID of " + std::to_string(objID) +
                                 " does not exist in the Context."));
    }
...
bool Context::doesObjectDataExist(const uint objID, const char* label) const {
    if (objects.find(objID) == objects.end()) {
        throw(std::runtime_error("ERROR (doesObjectDataExist): objID of " + std::to_string(objID) +
                                 " does not exist in the Context."));
    }
...
void Context::clearObjectData(const uint objID, const char* label) {
    if (objects.find(objID) == objects.end()) {
        throw(std::runtime_error("ERROR (clearObjectData): objID of " + std::to_string(objID) +
                                 " does not exist in the Context."));
    }
...
void Context::getGlobalData(const char* label, int& data) const {
    if (!doesGlobalDataExist(label)) {
        throw(std::runtime_error("ERROR (getGlobalData): Global data " + std::string(label) +
                                 " does not exist in the Context."));
    }
    ...
    if (gdata.type == HELIOS_TYPE_INT) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type int, but data '" +
                                 std::string(label) + "' does not have type int."));
    }
}
...
void Context::getGlobalData(const char* label, uint& data) const {
    ...
    if (gdata.type == HELIOS_TYPE_UINT) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type uint, but data '" +
                                 std::string(label) + "' does not have type uint."));
    }
}
...
void Context::getGlobalData(const char* label, float& data) const {
    ...
    if (gdata.type == HELIOS_TYPE_FLOAT) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type float, but data '" +
                                 std::string(label) + "' does not have type float."));
    }
}
...
void Context::getGlobalData(const char* label, double& data) const {
    ...
    if (gdata.type == HELIOS_TYPE_DOUBLE) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type double, but data '" +
                                 std::string(label) + "' does not have type double."));
    }
}
...
void Context::getGlobalData(const char* label, helios::vec2& data) const {
    ...
    if (gdata.type == HELIOS_TYPE_VEC2) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type vec2, but data '" +
                                 std::string(label) + "' does not have type vec2."));
    }
}
...
void Context::getGlobalData(const char* label, helios::vec3& data) const {
    ...
    if (gdata.type == HELIOS_TYPE_VEC3) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type vec3, but data '" +
                                 std::string(label) + "' does not have type vec3."));
    }
}
...
void Context::getGlobalData(const char* label, helios::vec4& data) const {
    ...
    if (gdata.type == HELIOS_TYPE_VEC4) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type vec4, but data '" +
                                 std::string(label) + "' does not have type vec4."));
    }
}
...
void Context::getGlobalData(const char* label, helios::int2& data) const {
    ...
    if (gdata.type == HELIOS_TYPE_INT2) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type int2, but data '" +
                                 std::string(label) + "' does not have type int2."));
    }
}
...
void Context::getGlobalData(const char* label, helios::int3& data) const {
    ...
    if (gdata.type == HELIOS_TYPE_INT3) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type int3, but data '" +
                                 std::string(label) + "' does not have type int3."));
    }
}
...
void Context::getGlobalData(const char* label, helios::int4& data) const {
    ...
    if (gdata.type == HELIOS_TYPE_INT4) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type int4, but data '" +
                                 std::string(label) + "' does not have type int4."));
    }
}
...
void Context::getGlobalData(const char* label, std::string& data) const {
    ...
    if (gdata.type == HELIOS_TYPE_STRING) {
        ...
        throw(std::runtime_error("ERROR (getGlobalData): Attempted to get global data for type string, but data '" +
                                 std::string(label) + "' does not have type string."));
    }
}
...
HeliosDataType Context::getGlobalDataType(const char* label) const {
    if (!doesGlobalDataExist(label)) {
        throw(std::runtime_error("ERROR (getGlobalDataType): Global data " + std::string(label) +
                                 " does not exist in the Context."));
    }
...
size_t Context::getGlobalDataSize(const char* label) const {
    if (!doesGlobalDataExist(label)) {
        throw(std::runtime_error("ERROR (getGlobalDataSize): Global data " + std::string(label) +
                                 " does not exist in the Context."));
    }
...
Triangle::Triangle(const vec3& a_vertex0, const vec3& a_vertex1, const vec3& a_vertex2, const RGBAcolor& a_color,
                   uint a_UUID) {
...
Triangle::Triangle(const vec3& a_vertex0, const vec3& a_vertex1, const vec3& a_vertex2, const char* a_texturefile,
                   const std::vector<vec2>& a_uv, float a_solid_fraction, uint a_UUID) {
    makeTransformationMatrix(a_vertex0, a_vertex1, a_vertex2);
...
void Context::setDate(int day, int month, int year) {
    if (day < 1 || day > 31) {
        throw(std::runtime_error("ERROR (setDate): Day of month is out of range (day of " + std::to_string(day) +
                                 " was given)."));
    } else if (month < 1 || month > 12) {
        throw(std::runtime_error("ERROR (setDate): Month of year is out of range (month of " + std::to_string(month) +
                                 " was given)."));
    } else if (year < 1000) {
...
void Context::setDate(Date date) {
    if (date.day < 1 || date.day > 31) {
        throw(std::runtime_error("ERROR (setDate): Day of month is out of range (day of " + std::to_string(date.day) +
                                 " was given)."));
    } else if (date.month < 1 || date.month > 12) {
        throw(std::runtime_error("ERROR (setDate): Month of year is out of range (month of " +
                                 std::to_string(date.month) + " was given)."));
...
float Context::randn(float mean, float stddev) { return mean + norm_distribution(generator) * fabsf(stddev); }
...
uint Context::addPatch(const vec3& center, const vec2& size, const SphericalCoord& rotation, const char* texture_file,
                       const helios::vec2& uv_center, const helios::vec2& uv_size) {
    // texture must have type PNG or JPEG
    std::string fn = texture_file;
    if (fn.substr(fn.find_last_of(".") + 1) != "png" && fn.substr(fn.find_last_of(".") + 1) != "PNG" &&
        fn.substr(fn.find_last_of(".") + 1) != "jpg" && fn.substr(fn.find_last_of(".") + 1) != "jpeg" &&
    ...
    if (uv_center.x - 0.5 * uv_size.x < -1e-3 || uv_center.y - 0.5 * uv_size.y < -1e-3 ||
        uv_center.x + 0.5 * uv_size.x - 1.f > 1e-3 || uv_center.y + 0.5 * uv_size.y - 1.f > 1e-3) {
        throw(
            std::runtime_error("ERROR (addPatch): Invalid texture coordinates. uv_center-0.5*uv_size should be >=0 and "
                               "uv_center+0.5*uv_size should be <=1."));
    }
...
uint Context::addTriangle(const helios::vec3& vertex0, const helios::vec3& vertex1, const helios::vec3& vertex2,
                          const char* texture_file, const helios::vec2& uv0, const helios::vec2& uv1,
    ...
    if (textures.at(texture_file).hasTransparencyChannel()) {
        const std::vector<std::vector<bool>>* alpha = textures.at(texture_file).getTransparencyData();
        int2 sz = textures.at(texture_file).getSize();
        int2 uv_min((int)lroundf(fmin(fmin(uv[0].x, uv[1].x), uv[2].x) * float(sz.x)),
                    (int)lroundf(fmin(fmin(uv[0].y, uv[1].y), uv[2].y) * float(sz.y)));
        int2 uv_max((int)lroundf(fmax(fmax(uv[0].x, uv[1].x), uv[2].x) * float(sz.x)),
                    (int)lroundf(fmax(fmax(uv[0].y, uv[1].y), uv[2].y) * float(sz.y)));
        int A = 0;
        int At = 0;
        vec2 xy;
        for (int j = uv_min.y; j < uv_max.y; j++) {
...
void Context::scalePrimitive(uint UUID, const helios::vec3& S) { getPrimitivePointer_private(UUID)->scale(S); }
...
void Context::deletePrimitive(uint UUID) {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (deletePrimitive): UUID of " + std::to_string(UUID) +
...
uint Context::copyPrimitive(uint UUID) {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (copyPrimitive): UUID of " + std::to_string(UUID) +
                                 " not found in the context."));
    }
    ...
    if (type == PRIMITIVE_TYPE_PATCH) {
        ...
        if (!p->hasTexture()) {
            ...
            tri_new = (new Triangle(vertices.at(0), vertices.at(1), vertices.at(2), texture_file.c_str(), uv,
                                    solid_fraction, currentUUID));
        }
        float transform[16];
        p->getTransformationMatrix(transform);
...
Primitive* Context::getPrimitivePointer(uint UUID) const {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (getPrimitivePointer): UUID of " + std::to_string(UUID) +
                                 " does not exist in the Context."));
    }
...
Patch* Context::getPatchPointer(uint UUID) const {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (getPatchPointer): UUID of " + std::to_string(UUID) +
                                 " does not exist in the Context."));
...
helios::vec2 Context::getPatchSize(uint UUID) const {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (getPatchSize): UUID of " + std::to_string(UUID) +
                                 " does not exist in the Context."));
    } else if (primitives.at(UUID)->getType() != PRIMITIVE_TYPE_PATCH) {
...
helios::vec3 Context::getPatchCenter(uint UUID) const {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (getPatchCenter): UUID of " + std::to_string(UUID) +
                                 " does not exist in the Context."));
    } else if (primitives.at(UUID)->getType() != PRIMITIVE_TYPE_PATCH) {
...
Triangle* Context::getTrianglePointer(uint UUID) const {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (getTrianglePointer): UUID of " + std::to_string(UUID) +
                                 " does not exist in the Context."));
    } else if (primitives.at(UUID)->getType() != PRIMITIVE_TYPE_TRIANGLE) {
        throw(
            std::runtime_error("ERROR (getTrianglePointer): UUID of " + std::to_string(UUID) + " is not a triangle."));
    }
...
helios::vec3 Context::getTriangleVertex(uint UUID, uint number) const {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (getTriangleVertex): UUID of " + std::to_string(UUID) +
                                 " does not exist in the Context."));
    } else if (primitives.at(UUID)->getType() != PRIMITIVE_TYPE_TRIANGLE) {
...
Voxel* Context::getVoxelPointer(uint UUID) const {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (getVoxelPointer): UUID of " + std::to_string(UUID) +
                                 " does not exist in the Context."));
...
helios::vec3 Context::getVoxelSize(uint UUID) const {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (getVoxelSize): UUID of " + std::to_string(UUID) +
                                 " does not exist in the Context."));
    } else if (primitives.at(UUID)->getType() != PRIMITIVE_TYPE_VOXEL) {
...
helios::vec3 Context::getVoxelCenter(uint UUID) const {
    if (primitives.find(UUID) == primitives.end()) {
        throw(std::runtime_error("ERROR (getVoxelCenter): UUID of " + std::to_string(UUID) +
                                 " does not exist in the Context."));
    } else if (primitives.at(UUID)->getType() != PRIMITIVE_TYPE_VOXEL) {
...
uint Context::getPrimitiveCount() const { return primitives.size(); }
...
void Context::addTimeseriesData(const char* label, float value, const Date& date, const Time& time) {
    ...
    if (timeseries_data.find(label) == timeseries_data.end()) {  // does not exist
        ...
        if (N == 1) {
            ...
            if (date_value <
                timeseries_datevalue[label].front()) {  // check if data should be inserted at beginning of timeseries
                timeseries_data[label].insert(it_data, value);
                timeseries_datevalue[label].insert(it_datevalue, date_value);
                return;
            } else if (date_value >
                       timeseries_datevalue[label].back()) {  // check if data should be inserted at end of timeseries
                timeseries_data[label].push_back(value);
                timeseries_datevalue[label].push_back(date_value);
                return;
            }
            ...
            for (uint t = 0; t < N - 1; t++) {
                ...
                if (date_value > timeseries_datevalue[label].at(t) &&
                    date_value < timeseries_datevalue[label].at(t + 1)) {
...
void Context::setCurrentTimeseriesPoint(const char* label, uint index) {
    if (timeseries_data.find(label) == timeseries_data.end()) {  // does not exist
        throw(std::runtime_error("ERROR (setCurrentTimeseriesPoint): Timeseries variable `" + std::string(label) +
                                 "' does not exist."));
    }
...
float Context::queryTimeseriesData(const char* label, const Date& date, const Time& time) const {
    if (timeseries_data.find(label) == timeseries_data.end()) {  // does not exist
        throw(std::runtime_error("ERROR (setCurrentTimeseriesData): Timeseries variable `" + std::string(label) +
                                 "' does not exist."));
    }
    ...
    if (date_value < tmin) {
        std::cerr << "WARNING (queryTimeseriesData): Timeseries date and time is outside of the range of the data. "
                     "Using the earliest data point in the timeseries."
                  << std::endl;
        return timeseries_data.at(label).front();
    } else if (date_value > tmax) {
        std::cerr << "WARNING (queryTimeseriesData): Timeseries date and time is outside of the range of the data. "
                     "Using the latest data point in the timeseries."
                  << std::endl;
        return timeseries_data.at(label).back();
    }

    if (timeseries_datevalue.at(label).empty()) {
        std::cout << "WARNING (queryTimeseriesData): timeseries " << label << " does not contain any data."
                  << std::endl;
        return 0;
        ...
        for (i = 0; i < timeseries_data.at(label).size() - 1; i++) {
            if (date_value >= timeseries_datevalue.at(label).at(i) &&
                date_value <= timeseries_datevalue.at(label).at(i + 1)) {
                success = true;
                break;
            }
        }
        ...
        if (!success) {
            throw(
                std::runtime_error("ERROR (queryTimeseriesData): Failed to query timeseries data for unknown reason."));
        }
...
Date Context::queryTimeseriesDate(const char* label, const uint index) const {
    if (timeseries_data.find(label) == timeseries_data.end()) {  // does not exist
        throw(std::runtime_error("ERROR (queryTimeseriesDate): Timeseries variable `" + std::string(label) +
                                 "' does not exist."));
    }
...
uint Context::getTimeseriesLength(const char* label) const {
    if (timeseries_data.find(label) == timeseries_data.end()) {  // does not exist
        throw(std::runtime_error("ERROR (getTimeseriesDate): Timeseries variable `" + std::string(label) +
                                 "' does not exist."));
    } else {
...
void Context::getDomainBoundingSphere(vec3& center, float& radius) const {
    ...
    radius = 0.5f *
             sqrtf(powf(xbounds.y - xbounds.x, 2) + powf(ybounds.y - ybounds.x, 2) + powf((zbounds.y - zbounds.x), 2));
}
...
void Context::cropDomainX(const vec2& xbounds) {
    ...
    if (getPrimitiveCount() == 0) {
        std::cout
            << "WARNING (cropDomainX): No primitives were inside cropped area, and thus all primitives were deleted."
            << std::endl;
    }
}
...
void Context::cropDomainY(const vec2& ybounds) {
    ...
    if (getPrimitiveCount() == 0) {
        std::cout
            << "WARNING (cropDomainY): No primitives were inside cropped area, and thus all primitives were deleted."
            << std::endl;
    }
}
...
void Context::cropDomainZ(const vec2& zbounds) {
    ...
    if (getPrimitiveCount() == 0) {
        std::cout
            << "WARNING (cropDomainZ): No primitives were inside cropped area, and thus all primitives were deleted."
            << std::endl;
    }
}

void Context::cropDomain(const std::vector<uint>& UUIDs, const vec2& xbounds, const vec2& ybounds,
                         const vec2& zbounds) {
    std::vector<vec3> vertices;
    ...
    for (uint UUID : UUIDs) {
        ...
        for (auto& vertex : vertices) {
            if (vertex.x < xbounds.x || vertex.x > xbounds.y || vertex.y < ybounds.x || vertex.y > ybounds.y ||
                vertex.z < zbounds.x || vertex.z > zbounds.y) {
                deletePrimitive(UUID);
                delete_count++;
                break;
            }
    ...
    if (delete_count == UUIDs.size()) {
        std::cout << "WARNING (cropDomain): No specified primitives were entirely inside cropped area, and thus all "
                     "specified primitives were deleted."
                  << std::endl;
    }
}
...
RGBcolor CompoundObject::getColor() const { return make_RGBcolor(color.r, color.g, color.b); }

RGBcolor CompoundObject::getColorRGB() const { return make_RGBcolor(color.r, color.g, color.b); }

RGBAcolor CompoundObject::getColorRGBA() const { return color; }
...
void CompoundObject::getObjectData(const char* label, int& data) const {
    if (!doesObjectDataExist(label)) {
        throw(std::runtime_error("ERROR (getObjectData): Object data " + std::string(label) +
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_INT) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type int, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type int."));
    }
}
...
void CompoundObject::getObjectData(const char* label, uint& data) const {
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_UINT) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type uint, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type uint."));
    }
}
...
void CompoundObject::getObjectData(const char* label, float& data) const {
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_FLOAT) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type float, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type float."));
    }
}
...
void CompoundObject::getObjectData(const char* label, double& data) const {
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_DOUBLE) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type double, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type double."));
    }
}
...
void CompoundObject::getObjectData(const char* label, vec2& data) const {
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_VEC2) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type vec2, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type vec2."));
    }
}
...
void CompoundObject::getObjectData(const char* label, vec3& data) const {
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_VEC3) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type vec3, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type vec3."));
    }
}
...
void CompoundObject::getObjectData(const char* label, vec4& data) const {
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_VEC4) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type vec4, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type vec4."));
    }
}
...
void CompoundObject::getObjectData(const char* label, int2& data) const {
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_INT2) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type int2, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type int2."));
    }
}
...
void CompoundObject::getObjectData(const char* label, int3& data) const {
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_INT3) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type int3, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type int3."));
    }
}
...
void CompoundObject::getObjectData(const char* label, int4& data) const {
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_INT4) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type int4, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type int4."));
    }
}
...
void CompoundObject::getObjectData(const char* label, std::string& data) const {
    ...
    if (object_data_types.at(label) == HELIOS_TYPE_STRING) {
        ...
        throw(std::runtime_error("ERROR (getObjectData): Attempted to get data for type string, but data '" +
                                 std::string(label) + "' for object " + std::to_string(OID) +
                                 " does not have type string."));
    }
}
...
bool Context::areObjectPrimitivesComplete(uint objID) const { return getObjectPointer(objID)->arePrimitivesComplete(); }
...
CompoundObject* Context::getObjectPointer(uint ObjID) const {
    if (objects.find(ObjID) == objects.end()) {
        throw(std::runtime_error("ERROR (getObjectPointer): ObjectID of " + std::to_string(ObjID) +
                                 " does not exist in the Context."));
    }
...
void Context::deleteObject(uint ObjID) {
    if (objects.find(ObjID) == objects.end()) {
        throw(std::runtime_error("ERROR (deleteObject): Object ID of " + std::to_string(ObjID) +
                                 " not found in the context."));
    }
...
uint Context::copyObject(uint ObjID) {
    if (objects.find(ObjID) == objects.end()) {
        throw(std::runtime_error("ERROR (copyObject): Object ID of " + std::to_string(ObjID) +
                                 " not found in the context."));
    }
    ...
    if (type == OBJECT_TYPE_TILE) {
        ...
        auto* tube_new =
            (new Tube(currentObjectID, UUIDs_copy, nodes, radius, colors, subdiv, texturefile.c_str(), this));
...
std::vector<uint> Context::filterObjectsByData(const std::vector<uint>& IDs, const char* object_data, float threshold,
                                               const char* comparator) const {
    ...
    for (uint i = 0; i < IDs.size(); i++) {
        if (doesObjectDataExist(IDs.at(i), object_data)) {
            ...
            if (type == HELIOS_TYPE_UINT) {
                ...
                std::cout << "WARNING: Object data not of type UINT, INT, or FLOAT. Filtering for other types not yet "
                             "supported."
                          << std::endl;
            }
        }
...
void Context::translateObject(uint ObjID, const vec3& shift) { getObjectPointer(ObjID)->translate(shift); }
...
void Context::rotateObject(uint ObjID, float rot, const char* axis) { getObjectPointer(ObjID)->rotate(rot, axis); }
...
void Context::rotateObject(uint ObjID, float rot, const vec3& axis) { getObjectPointer(ObjID)->rotate(rot, axis); }
...
helios::ObjectType Context::getObjectType(uint ObjID) const { return getObjectPointer(ObjID)->getObjectType(); }
...
float Context::getTileObjectAreaRatio(const uint& ObjectID) const {
    if (getObjectPointer(ObjectID)->getObjectType() != OBJECT_TYPE_TILE) {
        std::cerr << "WARNING (getTileObjectAreaRatio): ObjectID " << ObjectID << " is not a tile object. Skipping..."
                  << std::endl;
        return 0.0;

    } else {
        if (!(getObjectPointer(ObjectID)->arePrimitivesComplete())) {
            std::cerr << "WARNING (getTileObjectAreaRatio): ObjectID " << ObjectID
                      << " is missing primitives. Area ratio calculated is area of non-missing subpatches divided by "
                         "the area of an individual subpatch."
                      << std::endl;
        }
...
void Context::setTileObjectSubdivisionCount(const std::vector<uint>& ObjectIDs, int2 new_subdiv) {
    ...
    for (uint OBJID : ObjectIDs) {
        ...
        if (getObjectPointer(OBJID)->getObjectType() != OBJECT_TYPE_TILE) {
            std::cerr << "WARNING (setTileObjectSubdivisionCount): ObjectID " << OBJID
    ...
    for (uint j = 0; j < tex.size(); j++) {
        ...
        uint object_template =
            addTileObject(make_vec3(0, 0, 0), make_vec2(1, 1), nullrotation, new_subdiv, tex.at(j).c_str());
...
void Context::setTileObjectSubdivisionCount(const std::vector<uint>& ObjectIDs, float area_ratio) {
    ...
    // if this is not true then the copying method won't work well because a new template will need to be created for
    // each texture/aspect ratio combination
    ...
    for (uint j = 0; j < tex.size(); j++) {
        ...
        float option_1_AR =
            (tile_area / (tile_size.x / ceil(subpatch_per_x) * tile_size.y / floor(subpatch_per_y))) - area_ratio;
        float option_2_AR =
            (tile_area / (tile_size.x / floor(subpatch_per_x) * tile_size.y / ceil(subpatch_per_y))) - area_ratio;
    ...
    for (uint i = 0; i < textured_tile_ObjectIDs.size(); i++) {
        ...
        // getPrimitivePointer_private(current_object_pointer->getPrimitiveUUIDs().at(0))->getTextureFile();
        std::string current_texture_file = current_object_pointer->getTextureFile();
        // std::cout << "current_texture_file for ObjID " << textured_tile_ObjectIDs.at(i) << " = " <<
        // current_texture_file << std::endl;
        std::vector<uint> UUIDs_old = current_object_pointer->getPrimitiveUUIDs();
...
Tile::Tile(uint a_OID, const std::vector<uint>& a_UUIDs, const int2& a_subdiv, const char* a_texturefile,
...
Tile* Context::getTileObjectPointer(uint ObjID) const {
    if (objects.find(ObjID) == objects.end()) {
        throw(std::runtime_error("ERROR (getTileObjectPointer): ObjectID of " + std::to_string(ObjID) +
                                 " does not exist in the Context."));
    }
...
Sphere::Sphere(uint a_OID, const std::vector<uint>& a_UUIDs, uint a_subdiv, const char* a_texturefile,
               helios::Context* a_context) {
    makeIdentityMatrix(transform);
...
Sphere* Context::getSphereObjectPointer(uint ObjID) const {
    if (objects.find(ObjID) == objects.end()) {
        throw(std::runtime_error("ERROR (getSphereObjectPointer): ObjectID of " + std::to_string(ObjID) +
                                 " does not exist in the Context."));
    }
...
Tube::Tube(uint a_OID, const std::vector<uint>& a_UUIDs, const std::vector<vec3>& a_nodes,
           const std::vector<float>& a_radius, const std::vector<helios::RGBcolor>& a_colors, uint a_subdiv,
           const char* a_texturefile, helios::Context* a_context) {
    makeIdentityMatrix(transform);
...
Tube* Context::getTubeObjectPointer(uint ObjID) const {
    if (objects.find(ObjID) == objects.end()) {
        throw(std::runtime_error("ERROR (getTubeObjectPointer): ObjectID of " + std::to_string(ObjID) +
                                 " does not exist in the Context."));
    }
...
std::vector<helios::vec3> Tube::getNodes() const {
    ...
    for (uint i = 0; i < nodes.size(); i++) {
        nodes_T.at(i).x =
            transform[0] * nodes.at(i).x + transform[1] * nodes.at(i).y + transform[2] * nodes.at(i).z + transform[3];
        nodes_T.at(i).y =
            transform[4] * nodes.at(i).x + transform[5] * nodes.at(i).y + transform[6] * nodes.at(i).z + transform[7];
        nodes_T.at(i).z =
            transform[8] * nodes.at(i).x + transform[9] * nodes.at(i).y + transform[10] * nodes.at(i).z + transform[11];
    }
...
void Tube::setSubdivisionCount(uint a_subdiv) { subdiv = a_subdiv; }
...
Box* Context::getBoxObjectPointer(uint ObjID) const {
    if (objects.find(ObjID) == objects.end()) {
        throw(std::runtime_error("ERROR (getBoxObjectPointer): ObjectID of " + std::to_string(ObjID) +
                                 " does not exist in the Context."));
    }
...
Disk* Context::getDiskObjectPointer(uint ObjID) const {
    if (objects.find(ObjID) == objects.end()) {
        throw(std::runtime_error("ERROR (getDiskObjectPointer): ObjectID of " + std::to_string(ObjID) +
                                 " does not exist in the Context."));
    }
...
uint Disk::getSubdivisionCount() const { return subdiv; }

void Disk::setSubdivisionCount(uint a_subdiv) { subdiv = a_subdiv; }
...
Polymesh* Context::getPolymeshObjectPointer(uint ObjID) const {
    if (objects.find(ObjID) == objects.end()) {
        throw(std::runtime_error("ERROR (getPolymeshObjectPointer): ObjectID of " + std::to_string(ObjID) +
                                 " does not exist in the Context."));
    }
...
Cone* Context::getConeObjectPointer(const uint ObjID) const {
    if (objects.find(ObjID) == objects.end()) {
        throw(std::runtime_error("ERROR (getConeObjectPointer): ObjectID of " + std::to_string(ObjID) +
                                 " does not exist in the Context."));
    }
...
helios::vec3 Cone::getNode(int number) const {
    ...
    node_T.x = transform[0] * nodes.at(number).x + transform[1] * nodes.at(number).y +
               transform[2] * nodes.at(number).z + transform[3];
    node_T.y = transform[4] * nodes.at(number).x + transform[5] * nodes.at(number).y +
               transform[6] * nodes.at(number).z + transform[7];
    node_T.z = transform[8] * nodes.at(number).x + transform[9] * nodes.at(number).y +
               transform[10] * nodes.at(number).z + transform[11];
...
helios::vec3 Cone::getAxisUnitVector() const {
    ...
    helios::vec3 axis_unit_vector = helios::make_vec3(
        nodes_T.at(1).x - nodes_T.at(0).x, nodes_T.at(1).y - nodes_T.at(0).y, nodes_T.at(1).z - nodes_T.at(0).z);
...
float Cone::getLength() const {
    ...
    float length = powf(powf(nodes_T.at(1).x - nodes_T.at(0).x, 2) + powf(nodes_T.at(1).y - nodes_T.at(0).y, 2) +
                            powf(nodes_T.at(1).z - nodes_T.at(0).z, 2),
                        0.5);
    return length;
}
...
void Cone::scaleLength(float S) {
    ...
    vec3 axis_unit_vector = helios::make_vec3(nodes_T.at(1).x - nodes_T.at(0).x, nodes_T.at(1).y - nodes_T.at(0).y,
                                              nodes_T.at(1).z - nodes_T.at(0).z);
    float length = powf(powf(axis_unit_vector.x, 2) + powf(axis_unit_vector.y, 2) + powf(axis_unit_vector.z, 2), 0.5);
    axis_unit_vector = axis_unit_vector / length;
    ...
    // only rotate if the cone is not alread aligned with the z axis (i.e., angle is not zero. If zero, the axis of
    // rotation is 0,0,0 and we end up with problems)
    if (angle != float(0.0)) {
...
uint Context::addSphereObject(uint Ndivs, const vec3& center, float radius, const RGBcolor& color) {
    ...
    for (int j = 0; j < Ndivs; j++) {
        for (int i = 1; i < Ndivs - 1; i++) {
            vec3 v0 = center + sphere2cart(make_SphericalCoord(radius, -0.5f * float(M_PI) + float(i) * dtheta,
                                                               float(j) * dphi));
            vec3 v1 = center + sphere2cart(make_SphericalCoord(radius, -0.5f * float(M_PI) + float(i + 1) * dtheta,
                                                               float(j) * dphi));
            vec3 v2 = center + sphere2cart(make_SphericalCoord(radius, -0.5f * float(M_PI) + float(i + 1) * dtheta,
                                                               float(j + 1) * dphi));
            vec3 v3 = center + sphere2cart(make_SphericalCoord(radius, -0.5f * float(M_PI) + float(i) * dtheta,
                                                               float(j + 1) * dphi));
...
uint Context::addSphereObject(uint Ndivs, const vec3& center, float radius, const char* texturefile) {
    ...
    for (int j = 0; j < Ndivs; j++) {
        ...
        vec2 uv0 = make_vec2(
            1.f - atan2f(sinf((float(j) + 0.5f) * dphi), -cosf((float(j) + 0.5f) * dphi)) / float(2.f * M_PI) - 0.5f,
            1.f - n0.z * 0.5f - 0.5f);
    ...
    for (int j = 0; j < Ndivs; j++) {
        ...
        vec2 uv0 = make_vec2(
            1.f - atan2f(sin((float(j) + 0.5f) * dphi), -cos((float(j) + 0.5f) * dphi)) / float(2.f * M_PI) - 0.5f,
            1.f - n0.z * 0.5f - 0.5f);
        vec2 uv1 = make_vec2(1.f - atan2f(n1.x, -n1.y) / float(2.f * M_PI) - 0.5f, 1.f - n1.z * 0.5f - 0.5f);
        vec2 uv2 = make_vec2(1.f - atan2f(n2.x, -n2.y) / float(2.f * M_PI) - 0.5f, 1.f - n2.z * 0.5f - 0.5f);
...
uint Context::addTileObject(const vec3& center, const vec2& size, const SphericalCoord& rotation, const int2& subdiv,
    ...
    for (uint j = 0; j < subdiv.y; j++) {
        for (uint i = 0; i < subdiv.x; i++) {
            subcenter = make_vec3(-0.5f * size.x + (float(i) + 0.5f) * subsize.x,
                                  -0.5f * size.y + (float(j) + 0.5f) * subsize.y, 0.f);
...
uint Context::addBoxObject(const vec3& center, const vec3& size, const int3& subdiv, const RGBcolor& color,
    ...
    if (reverse_normals) {  // normals point inward
        ...
        U = addTile(subcenter, make_vec2(size.x, size.z), make_SphericalCoord(0.5f * M_PI, M_PI),
                    make_int2(subdiv.x, subdiv.z), color);
        UUID.insert(UUID.end(), U.begin(), U.end());

        // left
        subcenter = center - make_vec3(0, 0.5f * size.y, 0);
        U = addTile(subcenter, make_vec2(size.x, size.z), make_SphericalCoord(0.5f * M_PI, 0),
                    make_int2(subdiv.x, subdiv.z), color);
        UUID.insert(UUID.end(), U.begin(), U.end());
        ...
        U = addTile(subcenter, make_vec2(size.y, size.z), make_SphericalCoord(0.5f * M_PI, 1.5f * M_PI),
                    make_int2(subdiv.y, subdiv.z), color);
        UUID.insert(UUID.end(), U.begin(), U.end());

        // back
        subcenter = center - make_vec3(0.5f * size.x, 0, 0);
        U = addTile(subcenter, make_vec2(size.y, size.z), make_SphericalCoord(0.5f * M_PI, 0.5f * M_PI),
                    make_int2(subdiv.y, subdiv.z), color);
        UUID.insert(UUID.end(), U.begin(), U.end());
        ...
        U = addTile(subcenter, make_vec2(size.x, size.y), make_SphericalCoord(M_PI, 0), make_int2(subdiv.x, subdiv.y),
                    color);
        UUID.insert(UUID.end(), U.begin(), U.end());

        // bottom
        subcenter = center - make_vec3(0, 0, 0.5f * size.z);
        U = addTile(subcenter, make_vec2(size.x, size.y), make_SphericalCoord(0, 0), make_int2(subdiv.x, subdiv.y),
                    color);
        UUID.insert(UUID.end(), U.begin(), U.end());
...
uint Context::addBoxObject(vec3 center, const vec3& size, const int3& subdiv, const char* texturefile,
                           bool reverse_normals) {
    // texture must have type PNG or JPEG
    std::string fn = texturefile;
    if (fn.substr(fn.find_last_of(".") + 1) != "png" && fn.substr(fn.find_last_of(".") + 1) != "PNG" &&
    ...
    if (reverse_normals) {  // normals point inward
        ...
        // right
        subcenter = center + make_vec3(0, 0.5f * size.y, 0);
        U = addTile(subcenter, make_vec2(size.x, size.z), make_SphericalCoord(0.5 * M_PI, M_PI),
                    make_int2(subdiv.x, subdiv.z), texturefile);
        UUID.insert(UUID.end(), U.begin(), U.end());

        // left
        subcenter = center - make_vec3(0, 0.5f * size.y, 0);
        U = addTile(subcenter, make_vec2(size.x, size.z), make_SphericalCoord(0.5 * M_PI, 0),
                    make_int2(subdiv.x, subdiv.z), texturefile);
        UUID.insert(UUID.end(), U.begin(), U.end());
        ...
        // front
        subcenter = center + make_vec3(0.5f * size.x, 0, 0);
        U = addTile(subcenter, make_vec2(size.y, size.z), make_SphericalCoord(0.5 * M_PI, 1.5 * M_PI),
                    make_int2(subdiv.y, subdiv.z), texturefile);
        UUID.insert(UUID.end(), U.begin(), U.end());

        // back
        subcenter = center - make_vec3(0.5f * size.x, 0, 0);
        U = addTile(subcenter, make_vec2(size.y, size.z), make_SphericalCoord(0.5 * M_PI, 0.5 * M_PI),
                    make_int2(subdiv.y, subdiv.z), texturefile);
        UUID.insert(UUID.end(), U.begin(), U.end());
...
uint Context::addDiskObject(uint Ndivs, const vec3& center, const vec2& size, const SphericalCoord& rotation,
    ...
    for (int i = 0; i < Ndivs; i++) {
        ...
        UUID.at(i) = addTriangle(
            make_vec3(0, 0, 0), make_vec3(size.x * cosf(dtheta * float(i)), size.y * sinf(dtheta * float(i)), 0),
            make_vec3(size.x * cosf(dtheta * float(i + 1)), size.y * sinf(dtheta * float(i + 1)), 0), color);
...
uint Context::addDiskObject(uint Ndivs, const vec3& center, const vec2& size, const SphericalCoord& rotation,
    ...
    for (int i = 0; i < Ndivs; i++) {
        ...
        UUID.at(i) = addTriangle(
            make_vec3(0, 0, 0), make_vec3(size.x * cosf(dtheta * float(i)), size.y * sinf(dtheta * float(i)), 0),
            make_vec3(size.x * cosf(dtheta * float(i + 1)), size.y * sinf(dtheta * float(i + 1)), 0), texture_file,
            make_vec2(0.5, 0.5),
            make_vec2(0.5f * (1.f + cosf(dtheta * float(i))), 0.5f * (1.f + sinf(dtheta * float(i)))),
            make_vec2(0.5f * (1.f + cosf(dtheta * float(i + 1))), 0.5f * (1.f + sinf(dtheta * float(i + 1)))));
        getPrimitivePointer_private(UUID.at(i))->rotate(rotation.elevation, "y");
        getPrimitivePointer_private(UUID.at(i))->rotate(rotation.azimuth, "z");
        getPrimitivePointer_private(UUID.at(i))->translate(center);
    }
...
std::vector<uint> Context::addTile(const vec3& center, const vec2& size, const SphericalCoord& rotation,
                                   const int2& subdiv) {
    RGBcolor color = make_RGBcolor(0.f, 0.75f, 0.f);  // Default color is green
...
std::vector<uint> Context::addTile(const vec3& center, const vec2& size, const SphericalCoord& rotation,
                                   const int2& subdiv, const RGBcolor& color) {
    std::vector<uint> UUID;
    ...
    for (uint j = 0; j < subdiv.y; j++) {
        for (uint i = 0; i < subdiv.x; i++) {
            subcenter = make_vec3(-0.5f * size.x + (float(i) + 0.5f) * subsize.x,
                                  -0.5f * size.y + (float(j) + 0.5f) * subsize.y, 0);
...
std::vector<uint> Context::addTile(const vec3& center, const vec2& size, const SphericalCoord& rotation,
                                   const int2& subdiv, const char* texturefile) {
    // texture must have type PNG or JPEG
    std::string fn = texturefile;
    if (fn.substr(fn.find_last_of(".") + 1) != "png" && fn.substr(fn.find_last_of(".") + 1) != "PNG" &&
        fn.substr(fn.find_last_of(".") + 1) != "jpg" && fn.substr(fn.find_last_of(".") + 1) != "jpeg" &&
    ...
    for (uint j = 0; j < subdiv.y; j++) {
        for (uint i = 0; i < subdiv.x; i++) {
            ...
            uv[0] = make_vec2(float(i) * uv_sub.x, 1.f - float(j) * uv_sub.y);
            uv[1] = make_vec2(float(i + 1) * uv_sub.x, 1.f - float(j) * uv_sub.y);
            uv[2] = make_vec2(float(i + 1) * uv_sub.x, 1.f - float(j + 1) * uv_sub.y);
            uv[3] = make_vec2(float(i) * uv_sub.x, 1.f - float(j + 1) * uv_sub.y);
            ...
            if (textures.at(texturefile).hasTransparencyChannel()) {
                ...
                int2 uv_min(floor(uv[0].x * float(sz.x - 1)), floor((1.f - uv[0].y) * float(sz.y - 1)));
                int2 uv_max(floor(uv[2].x * float(sz.x - 1)), floor((1.f - uv[2].y) * float(sz.y - 1)));
...
void Context::loadOsubPData(pugi::xml_node p, uint ID) {
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_int"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_int")) {
        ...
        for (pugi::xml_node data = prim_data.child("data"); data; data = data.next_sibling("data")) {
            if (u >= prim_UUIDs.size()) {
                std::cerr << "WARNING (Context::loadXML): There was a problem with reading object primitive data \""
                          << label
                          << "\". The number of data values provided does not match the number of primitives contained "
                             "in this object. Skipping remaining data values."
                          << std::endl;
                break;
            }
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_uint"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_uint")) {
        const char* label = prim_data.attribute("label").value();
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_float"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_float")) {
        const char* label = prim_data.attribute("label").value();
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_double"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_double")) {
        const char* label = prim_data.attribute("label").value();
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_vec2"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_vec2")) {
        const char* label = prim_data.attribute("label").value();
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_vec3"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_vec3")) {
        const char* label = prim_data.attribute("label").value();
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_vec4"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_vec4")) {
        const char* label = prim_data.attribute("label").value();
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_int2"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_int2")) {
        const char* label = prim_data.attribute("label").value();
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_int3"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_int3")) {
        const char* label = prim_data.attribute("label").value();
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_int4"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_int4")) {
        const char* label = prim_data.attribute("label").value();
    ...
    for (pugi::xml_node prim_data = p.child("primitive_data_string"); prim_data;
         prim_data = prim_data.next_sibling("primitive_data_string")) {
        const char* label = prim_data.attribute("label").value();
...
std::vector<uint> Context::loadXML(const char* filename, bool quiet) {
    ...
    // error checking
    if (!result) {
        throw(std::runtime_error("failed.\n XML [" + std::string(filename) + "] parsed with errors, attr value: [" +
    ...
    // if primitives are added that belong to an object, store there UUIDs here so that we can make sure their UUIDs are
    // consistent
    ...
    for (pugi::xml_node p = helios.child("patch"); p; p = p.next_sibling("patch")) {
        ...
        if (strlen(transform_str) == 0) {
            ...
            if (i != 16) {
                if (!quiet) {
                    std::cout << "WARNING (Context::loadXML): Transformation matrix does not have 16 elements. "
                                 "Assuming identity matrix."
                              << std::endl;
                }
                makeIdentityMatrix(transform);
            }
        ...
        if (strlen(texUV) > 0) {
            ...
            if (c != 0) {
                std::cerr
                    << "WARNING (loadXML): textureUV for patch does not contain an even number of elements. Skipping..."
                    << std::endl;
                uv.resize(0);
            }
            if (uv.size() != 4) {
                std::cerr << "WARNING (loadXML): textureUV for patch does not contain four pairs of (u,v) coordinates. "
                             "Skipping..."
                          << std::endl;
                uv.resize(0);
            }
        ...
        if (strcmp(texture_file.c_str(), "none") == 0) {
            ...
            if (uv.empty()) {
                ...
                ID = addPatch(make_vec3(0, 0, 0), make_vec2(1, 1), make_SphericalCoord(0, 0), texture_file.c_str(),
                              0.5 * (uv.at(2) + uv.at(0)), uv.at(2) - uv.at(0));
            }
        }
    ...
    for (pugi::xml_node tri = helios.child("triangle"); tri; tri = tri.next_sibling("triangle")) {
        ...
        if (strlen(texUV) > 0) {
            ...
            if (uv.size() != 3) {
                std::cerr << "WARNING (loadXML): textureUV for triangle does not contain three pairs of (u,v) "
                             "coordinates. Skipping..."
                          << std::endl;
                uv.resize(0);
            }
        }
        ...
        if (strcmp(texture_file.c_str(), "none") == 0 || uv.empty()) {
            ...
            ID = addTriangle(vert_pos.at(0), vert_pos.at(1), vert_pos.at(2), texture_file.c_str(), uv.at(0), uv.at(1),
                             uv.at(2));
        }
        getPrimitivePointer_private(ID)->setTransformationMatrix(transform);
    ...
    for (pugi::xml_node p = helios.child("tile"); p; p = p.next_sibling("tile")) {
        ...
        if (strlen(texUV) > 0) {
            ...
            if (uv.size() != 4) {
                std::cerr << "WARNING (loadXML): textureUV for tile does not contain four pairs of (u,v) coordinates. "
                             "Skipping..."
                          << std::endl;
                uv.resize(0);
            }
        ...
        if (strlen(subdiv_str) == 0) {
            std::cerr << "WARNING (loadXML): Number of subdivisions for tile was not provided. Assuming 1x1."
                      << std::endl;
        ...
        if (strcmp(texture_file.c_str(), "none") == 0) {
            if (strlen(color_str) == 0) {
                ...
                ID = addTileObject(patch.getCenter(), patch.getSize(), cart2sphere(patch.getNormal()), subdiv,
                                   make_RGBcolor(color.r, color.g, color.b));
            }
        } else {
            ID = addTileObject(patch.getCenter(), patch.getSize(), cart2sphere(patch.getNormal()), subdiv,
                               texture_file.c_str());
        ...
        if (objID > 0 && object_prim_UUIDs.find(objID) != object_prim_UUIDs.end()) {
            deletePrimitive(
                getObjectPrimitiveUUIDs(ID));  // \todo This is fairly inefficient, it would be nice to have a way to do
                                               // this without having to create and delete a bunch of primitives.
    ...
    for (pugi::xml_node p = helios.child("sphere"); p; p = p.next_sibling("sphere")) {
        ...
        const char* subdiv_str = subdiv_node.child_value();
        if (strlen(subdiv_str) == 0) {
            std::cerr << "WARNING (loadXML): Number of subdivisions for sphere was not provided. Assuming 5."
                      << std::endl;
            subdiv = 5;
        } else {
    ...
    for (pugi::xml_node p = helios.child("tube"); p; p = p.next_sibling("tube")) {
        ...
        const char* subdiv_str = subdiv_node.child_value();
        if (strlen(subdiv_str) == 0) {
            std::cerr << "WARNING (loadXML): Number of subdivisions for tube was not provided. Assuming 5."
                      << std::endl;
            subdiv = 5;
        } else {
            subdiv = std::stoi(subdiv_str);
    ...
    for (pugi::xml_node p = helios.child("disk"); p; p = p.next_sibling("disk")) {
        ...
        const char* subdiv_str = subdiv_node.child_value();
        if (strlen(subdiv_str) == 0) {
            std::cerr << "WARNING (loadXML): Number of subdivisions for disk was not provided. Assuming 5."
                      << std::endl;
            subdiv = 5;
        } else {
            subdiv = std::stoi(subdiv_str);
        ...
        if (strcmp(texture_file.c_str(), "none") == 0) {
            if (strlen(color_str) == 0) {
                ...
                ID = addDiskObject(subdiv, disk.getCenter(), disk.getSize(), nullrotation,
                                   make_RGBcolor(color.r, color.g, color.b));
            }
        } else {
    ...
    for (pugi::xml_node p = helios.child("cone"); p; p = p.next_sibling("cone")) {
        ...
        const char* subdiv_str = subdiv_node.child_value();
        if (strlen(subdiv_str) == 0) {
            std::cerr << "WARNING (loadXML): Number of subdivisions for cone was not provided. Assuming 5."
                      << std::endl;
            subdiv = 5;
        } else {
            subdiv = std::stoi(subdiv_str);
        ...
        if (strlen(nodes_str) > 0) {
            ...
            if (c != 3) {
                throw(
                    std::runtime_error("ERROR (loadXML): Loading of cone failed. Cone end nodes must be specified as "
                                       "pairs of 3 x,y,z coordinates."));
            }
            c = 0;
            while (data_stream >> tmp.at(c)) {
                c++;
        ...
        // * Add the Cone * //
        if (texture_file == "none") {
            ID = addConeObject(subdiv, nodes.at(0), nodes.at(1), radii.at(0), radii.at(1),
                               make_RGBcolor(color.r, color.g, color.b));
        } else {
        ...
        for (pugi::xml_node d = p.child("datapoint"); d; d = d.next_sibling("datapoint")) {
            ...
            if (strlen(time_str) > 0) {
                ...
                if (time_.x < 0 || time_.x > 23) {
                    throw(std::runtime_error("ERROR (loadXML): Invalid hour of " + std::to_string(time_.x) +
                                             " given in timeseries. Hour must be positive and not greater than 23."));
                } else if (time_.y < 0 || time_.y > 59) {
                    throw(std::runtime_error("ERROR (loadXML): Invalid minute of " + std::to_string(time_.y) +
                                             " given in timeseries. Minute must be positive and not greater than 59."));
                } else if (time_.z < 0 || time_.z > 59) {
                    throw(std::runtime_error("ERROR (loadXML): Invalid second of " + std::to_string(time_.z) +
            ...
            if (strlen(Jdate_str) > 0) {
                ...
                if (date_.x < 1 || date_.x > 366) {
                    throw(std::runtime_error(
                        "ERROR (loadXML): Invalid Julian day of year " + std::to_string(date_.x) +
                        " given in timeseries. Julian day must be greater than zero and not greater than 366."));
                } else if (date_.y < 1000 || date_.y > 10000) {
                    throw(std::runtime_error("ERROR (loadXML): Invalid year of " + std::to_string(date_.y) +
                                             " given in timeseries. Year should be in YYYY format."));
                }
...
std::vector<std::string> Context::getLoadedXMLFiles() { return XMLfiles; }

void Context::writeDataToXMLstream(const char* data_group, const std::vector<std::string>& data_labels, void* ptr,
    ...
    for (const auto& label : data_labels) {
        ...
        if (strcmp(data_group, "primitive") == 0) {
            ...
            throw(std::runtime_error("ERROR (writeDataToXMLstream): unknown data group argument of " +
                                     std::string(data_group) + ". Must be one of primitive, object, or global."));
        }
...
void Context::writeXML(const char* filename, bool quiet) const {
    ...
    for (auto primitive : primitives) {
        ...
        outfile << "\t<color>" << color.r << " " << color.g << " " << color.b << " " << color.a << "</color>"
                << std::endl;
        if (prim->hasTexture()) {
    ...
    for (const auto& object : objects) {
        ...
        for (size_t l = 0; l < pdata_labels.size(); l++) {
            if (pdata_types.at(l) == HELIOS_TYPE_FLOAT) {
                outfile << "\t<primitive_data_float "
                        << "label=\"" << pdata_labels.at(l) << "\">" << std::endl;
                ...
                for (size_t p = 0; p < primitiveUUIDs.size(); p++) {
                    if (doesPrimitiveDataExist(primitiveUUIDs.at(p), pdata_labels.at(l).c_str())) {
                        ...
                        outfile << "\t\t<data label=\"" << p << "\"> " << std::flush;
                        for (size_t i = 0; i < data.size(); i++) {
                            outfile << data.at(i).x << " " << data.at(i).y << " " << data.at(i).z << " " << data.at(i).w
                                    << std::flush;
                        }
                        outfile << " </data>" << std::endl;
                    }
        ...
        if (obj->getObjectType() == OBJECT_TYPE_TILE) {
            ...
            outfile << "\t<subdivisions> " << subdiv.x << " " << subdiv.y << " " << subdiv.z << " </subdivisions>"
                    << std::endl;
    ...
    for (const auto& iter : globaldata) {
        ...
        if (type == HELIOS_TYPE_UINT) {
            ...
            for (size_t i = 0; i < data.size; i++) {
                outfile << data.global_data_vec3.at(i).x << " " << data.global_data_vec3.at(i).y << " "
                        << data.global_data_vec3.at(i).z << std::flush;
                if (i != data.size - 1) {
                    outfile << " " << std::flush;
                }
            }
            ...
            for (size_t i = 0; i < data.size; i++) {
                outfile << data.global_data_vec4.at(i).x << " " << data.global_data_vec4.at(i).y << " "
                        << data.global_data_vec4.at(i).z << " " << data.global_data_vec4.at(i).w << std::flush;
                if (i != data.size - 1) {
                    outfile << " " << std::flush;
                }
            }
            ...
            for (size_t i = 0; i < data.size; i++) {
                outfile << data.global_data_int3.at(i).x << " " << data.global_data_int3.at(i).y
                        << data.global_data_int3.at(i).z << std::flush;
                if (i != data.size - 1) {
                    outfile << " " << std::flush;
                }
            }
            ...
            for (size_t i = 0; i < data.size; i++) {
                outfile << data.global_data_int4.at(i).x << " " << data.global_data_int4.at(i).y
                        << data.global_data_int4.at(i).z << data.global_data_int4.at(i).w << std::flush;
                if (i != data.size - 1) {
                    outfile << " " << std::flush;
                }
            }
    ...
    for (const auto& iter : timeseries_data) {
        ...
        for (size_t i = 0; i < data.size(); i++) {
            ...
            outfile << "\t   <date>" << a_date.day << " " << a_date.month << " " << a_date.year << "</date>"
                    << std::endl;

            outfile << "\t   <time>" << a_time.hour << " " << a_time.minute << " " << a_time.second << "</time>"
                    << std::endl;
...
std::vector<uint> Context::loadPLY(const char* filename, const vec3& origin, float height,
                                   const SphericalCoord& rotation, const RGBcolor& default_color) {
    ...
    for (uint row = 0; row < vertexCount; row++) {
        for (auto& propertie : properties) {
            if (strcmp(propertie.c_str(), "x") ==
                0) {  // Note: permuting x,y,z to match our coordinate system (z-vertical instead of y-vertical)
...
void Context::writePLY(const char* filename) const {
    ...
    for (size_t v = 0; v < verts.size(); v++) {
        PLYfile << verts.at(v).x << " " << verts.at(v).y << " " << verts.at(v).z << " " << round(colors.at(v).r) << " "
                << round(colors.at(v).g) << " " << round(colors.at(v).b) << std::endl;
    }
...
std::vector<uint> Context::loadOBJ(const char* filename, const vec3& origin, float height,
                                   const SphericalCoord& rotation, const RGBcolor& default_color) {
    return loadOBJ(filename, origin, height, rotation, default_color, "ZUP");
}

std::vector<uint> Context::loadOBJ(const char* filename, const vec3& origin, float height,
                                   const SphericalCoord& rotation, const RGBcolor& default_color, const char* upaxis) {
    ...
    if (strcmp(upaxis, "XUP") != 0 && strcmp(upaxis, "YUP") != 0 && strcmp(upaxis, "ZUP") != 0) {
        throw(std::runtime_error("ERROR (loadOBJ): Up axis of " + std::string(upaxis) +
    ...
    for (std::map<std::string, std::vector<std::vector<int>>>::const_iterator iter = face_inds.begin();
         iter != face_inds.end(); ++iter) {
            ...
            for (uint t = 2; t < face_inds.at(material).at(i).size(); t++) {
                ...
                if (!texture.empty() && !texture_inds.at(material).at(i).empty()) {  // has texture
                    ...
                    if (t < texture_inds.at(material).at(i).size()) {
                        ...
                        ID = addTriangle(origin + v0 * scl, origin + v1 * scl, origin + v2 * scl, texture.c_str(),
                                         texture_uv.at(iuv0), texture_uv.at(iuv1), texture_uv.at(iuv2));
...
std::map<std::string, std::string> Context::loadMTL(const std::string& filebase, const std::string& material_file) {
    ...
    if (!inputMTL.is_open()) {
        ...
        inputMTL.open(file.c_str());
        if (!inputMTL.is_open()) {
            throw(std::runtime_error("ERROR (loadMTL): Material file " + std::string(file) +
                                     " given in .obj file cannot be found."));
        }
    }
            ...
            while (strcmp("newmtl", line.c_str()) != 0 && inputMTL.good()) {
                if (strcmp("map_a", line.c_str()) == 0) {
                    ...
                    while (stream.good()) {
                        stream >> tmp;
                        int e = (int)tmp.size();
                        if ((strncmp(&tmp[e - 1], "g", 1) == 0 && strncmp(&tmp[e - 2], "n", 1) == 0 &&
                             strncmp(&tmp[e - 3], "p", 1) == 0) ||
                            (strncmp(&tmp[e - 1], "g", 1) == 0 && strncmp(&tmp[e - 2], "p", 1) == 0 &&
                             strncmp(&tmp[e - 3], "j", 1) == 0) ||
                            (strncmp(&tmp[e - 1], "g", 1) == 0 && strncmp(&tmp[e - 2], "e", 1) == 0 &&
                             strncmp(&tmp[e - 3], "p", 1) == 0 && strncmp(&tmp[e - 4], "j", 1) == 0)) {
                            ...
                            if (!tfile.is_open()) {
                                ...
                                tfile.open(texturefile.c_str());
                                if (!tfile.is_open()) {
                                    std::cerr << "WARNING (loadOBJ): Texture file " << texturefile
                                              << " given in .mtl file cannot be found." << std::endl;
                                }
                            }
                            tfile.close();
...
void Context::writeOBJ(const char* filename) const {
    ...
    //  - image files need to be copied to the location where the .obj file is being written otherwise they won't be
    //  found.
    //  - should parse "filename" to check that extension is .obj, and remove .obj extension when appending .mtl for
    //  material file.
    ...
    if (uv_inds.at(0).x < 0) {
        ...
        file << "f " << faces.at(0).x << "/" << uv_inds.at(0).x << " " << faces.at(0).y << "/" << uv_inds.at(0).y << " "
             << faces.at(0).z << "/" << uv_inds.at(0).z << std::endl;
    }
    ...
    for (size_t f = 1; f < faces.size(); f++) {
        if (current_texture != texture_list.at(f)) {
            ...
            if (mtl_exist_flag) {
                ...
                file << "usemtl material" << (mtl_index + 1)
                     << std::endl;  // we plus 1 here as we index mtl from 1 instead of 0 in the file.
            } else {
        ...
        if (uv_inds.at(f).x < 0) {
            ...
            // assert( uv_inds.at(f).x <= uv.size() && uv_inds.at(f).y <= uv.size() && uv_inds.at(f).z <= uv.size() );
            file << "f " << faces.at(f).x << "/" << uv_inds.at(f).x << " " << faces.at(f).y << "/" << uv_inds.at(f).y
                 << " " << faces.at(f).z << "/" << uv_inds.at(f).z << std::endl;
        }
    }
    file.close();
    ...
    for (size_t f = 1; f < faces.size(); f++) {
        ...
        if (!mtl_exist_flag) {
            if (texture_list.at(f).empty()) {
                if (current_color.r != colors.at(f).r && current_color.g != colors.at(f).g &&
...
void Context::writePrimitiveData(std::string filename, const std::vector<std::string>& column_format,
                                 bool print_header) const {
    writePrimitiveData(filename, column_format, getAllUUIDs(), print_header);
}

void Context::writePrimitiveData(std::string filename, const std::vector<std::string>& column_format,
                                 const std::vector<uint>& UUIDs, bool print_header) const {
    ...
    if (uuidexistswarning) {
        std::cerr << "WARNING (Context::writePrimitiveData): Vector of UUIDs passed to writePrimitiveData() function "
                     "contained UUIDs that do not exist, which were skipped."
                  << std::endl;
    }
    if (dataexistswarning) {
        std::cerr << "WARNING (Context::writePrimitiveData): Primitive data requested did not exist for one or more "
                     "primitives. A default value of 0 was written in these cases."
                  << std::endl;
    }
    if (datatypewarning) {
        std::cerr << "WARNING (Context::writePrimitiveData): Only scalar primitive data types (uint, int, float, and "
                     "double) are supported for this function. A column of 0's was written in these cases."
                  << std::endl;
    }
...
PrimitiveType Context::getPrimitiveType(uint UUID) const { return getPrimitivePointer_private(UUID)->getType(); }
...
float Context::getPrimitiveArea(uint UUID) const { return getPrimitivePointer_private(UUID)->getArea(); }
...
void Context::getPrimitiveBoundingBox(const std::vector<uint>& UUIDs, vec3& min_corner, vec3& max_corner) const {
    ...
    for (uint UUID : UUIDs) {
        if (!doesPrimitiveExist(UUID)) {
            throw(std::runtime_error("ERROR (getPrimitiveBoundingBox): Primitive with UUID of " + std::to_string(UUID) +
                                     " does not exist in the Context."));
        }
...
const std::vector<std::vector<bool>>* Context::getPrimitiveTextureTransparencyData(uint UUID) const {
    if (primitiveTextureHasTransparencyChannel(UUID)) {
        const std::vector<std::vector<bool>>* data =
            textures.at(getPrimitivePointer_private(UUID)->getTextureFile()).getTransparencyData();
        return data;
    } else {
        throw(std::runtime_error(
            "ERROR (getPrimitiveTransparencyData): Texture transparency data does not exist for primitive " +
            std::to_string(UUID) + "."));
    }
}

void Context::overridePrimitiveTextureColor(uint UUID) { getPrimitivePointer_private(UUID)->overrideTextureColor(); };

void Context::usePrimitiveTextureColor(uint UUID) { getPrimitivePointer_private(UUID)->useTextureColor(); };
...
void Context::printPrimitiveInfo(uint UUID) const {
    ...
    for (uint i = 0; i < pd.size(); i++) {
        ...
        std::cout << "   "
                  << "[name: " << pd.at(i) << ", type: " << dstype << ", size: " << dsize << "]:" << std::endl;
...
RGBAcolor Context::getObjectColorRGBA(uint ObjID) const { return getObjectPointer_private(ObjID)->getColorRGBA(); }
...
void Context::setObjectColor(uint ObjID, const RGBcolor& color) { getObjectPointer_private(ObjID)->setColor(color); }
...
void Context::setObjectColor(uint ObjID, const RGBAcolor& color) { getObjectPointer_private(ObjID)->setColor(color); }
...
void Context::overrideObjectTextureColor(uint ObjID) { getObjectPointer_private(ObjID)->overrideTextureColor(); }

void Context::useObjectTextureColor(uint ObjID) { getObjectPointer_private(ObjID)->useTextureColor(); }
...
void Context::getObjectBoundingBox(const std::vector<uint>& ObjIDs, vec3& min_corner, vec3& max_corner) const {
    ...
    for (uint ObjID : ObjIDs) {
        if (objects.find(ObjID) == objects.end()) {
            throw(std::runtime_error("ERROR (getObjectBoundingBox): ObjectID of " + std::to_string(ObjID) +
                                     " does not exist in the Context."));
        }
...
float Context::getSphereObjectRadius(uint& ObjID) const { return getSphereObjectPointer_private(ObjID)->getRadius(); }
