...
uint Context::copyPrimitive( uint UUID ){
    ...
    if( type==PRIMITIVE_TYPE_PATCH ){
        ...
        if( !p->hasTexture() ){
            ...
            if( uv.size()==4 ){
                patch_new = (new Patch( texture_file.c_str(), uv, textures, parentID, currentUUID ));
                patch_new->setSolidFraction(solid_fraction);
            }else{
...
void Context::writeXML( const char* filename, bool quiet ) const{
    ...
    auto file_extension = std::string(std::__fs::filesystem::path(filename).extension());
...
void Context::writeOBJ( const std::string &filename, const std::vector<uint> &UUIDs, const std::vector<std::string> &primitive_dat_fields ) const{
  ...
  auto file_extension = std::string(std::__fs::filesystem::path(filename).extension());
  auto file_stem = std::string(std::__fs::filesystem::path(filename).stem());
  auto file_path = std::string(std::__fs::filesystem::path(filename).parent_path());
...
//  assert(verts.size() == faces.size());
  ...
  auto ret = std::__fs::filesystem::create_directory( texture_dir );
  for (int mat = 0; mat < materials.size(); mat++) {
    std::string texture = materials.at(mat).texture;
    if( !texture.empty() && std::__fs::filesystem::exists(texture) ) {
      auto file = std::__fs::filesystem::path(texture).filename();
      std::__fs::filesystem::copy_file( texture, texture_dir + std::string(file), std::__fs::filesystem::copy_options::overwrite_existing );
