...
void Context::addVariable( const char* variable_name ){

  if( variables.find(variable_name) != variables.end() ){
  
    std::vector<float> variable_values;
    variable_values.resize(getPrimitiveCount(),0.f);
    
    variables[variable_name] = variable_values;

  }
    
}

void Context::setVariableValue( uint UUID, const char* variable_name, float value ){

  if( primitives.find(UUID)==primitives.end() ){
    std::cerr << "ERROR (setVariableValue): Primitive #" << UUID << " does not exist in the context." << std::endl;
    exit(EXIT_FAILURE);
  }

  setPrimitiveData(UUID,variable_name,HELIOS_TYPE_FLOAT,1,&value);

}

void Context::setVariableValue( std::vector<uint> UUID, const char* variable_name, float value ){

  for( size_t p=0; p<UUID.size(); p++ ){

    size_t U = UUID.at(p);
    
    if( primitives.find(U)==primitives.end() ){
      std::cerr << "ERROR (setVariableValue): Primitive #" << U << " does not exist in the context." << std::endl;
      exit(EXIT_FAILURE);
    }

    setPrimitiveData(U,variable_name,HELIOS_TYPE_FLOAT,1,&value);

  }
    
}

float Context::getVariableValue( uint UUID, const char* variable_name ) const{

  float data;
  if( doesPrimitiveDataExist(UUID,variable_name) ){
    getPrimitiveData(UUID,variable_name,data);
  }else{
    data = 0.f;
  }
...
std::vector<float> Context::getVariableValue( std::vector<uint> UUIDs, const char* variable_name ) const{

  std::vector<float> data;
  data.resize(UUIDs.size());
  for( size_t p=0; p<UUIDs.size(); p++ ){
    if( doesPrimitiveDataExist(UUIDs.at(p),variable_name) ){
       getPrimitiveData(UUIDs.at(p),variable_name,data.at(p));
    }else{
      data.at(p) = 0.f;
    }
  }
...
bool Context::doesVariableExist( const char* variable_name ) const{
