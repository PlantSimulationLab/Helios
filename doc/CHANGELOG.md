...
- Internode girth in parameter-based growth now follows the pipe model with disused pipes retained: cross-sectional area is proportional to the cumulative leaf area ever carried above a node, without the previous per-phytomer `365/age` scaling of `girth_area_factor`. The old scaling shrank old basal wood at every annual leaf flush and produced stepwise, nearly cylindrical limbs on perennial trees. The girth factors of the perennial library models were refit so their base diameters are held, and tree models that borrowed another model's factor now set their own.
- Added shade-driven branch shedding to parameter-based growth. At each dormancy boundary, fine branches that stood in deep shade through the season (from a shadow-propagation voxel grid captured at the season's peak leaf area, after Palubicki et al. 2009), and low-crown fine branches, are pruned. The shade at a point can be queried with `PlantArchitecture::getShadowLightExposureAtPoint()`.
- Added growth priority to parameter-based growth. At each dormancy boundary the best-lit laterals of each parent gain node budget, internode length and bud-break probability over successive seasons, while the rest are demoted, so dominant laterals develop into limbs.
- Branch shedding and growth priority act only on shoots of type "proleptic" or "sylleptic"; all other shoot types are treated as structural wood. Models whose fine shoots use other type names are neither shed nor reprioritized. Neither process runs when the carbohydrate model is enabled. Together they reduce the leaf area of perennial tree models to roughly 55-60% of previous versions; annuals, eastern redbud and grapevines are unchanged.
- Shoot tortuosity is now an Ornstein-Uhlenbeck process on curvature over a physical correlation length, set by the new `ShootParameters::tortuosity_persistence_length` (default 0.5 m), with an azimuthal restoring rotation toward the shoot's initial heading, so long shoots no longer wander far off their original bearing. `ShootParameters::tortuosity` is now the standard deviation of that curvature in degrees per metre, rather than an angle applied to every internode segment, so shoot shape no longer depends on `PhytomerParameters::internode.length_segments`; previously each extra subdivision added another full-size random turn, and a shoot with five segments per internode could wander enough to overwhelm its gravitropic response. Library values were converted to preserve each library plant's previous amount of wiggle; convert values in your own shoot types as tortuosity × `internode.length_segments` / `internode_length_max`.
- Recalibrated the `almond` (Nonpareil) and `almond_aldrich` library models against Phytograph QSM reconstructions of seven-year-old orchard trees, and added a new `almond_independence` model calibrated the same way. Almond scaffolds now break bud acrotonically, the vegetative bud at the trunk/scaffold crotch is removed as orchard training would, and almond `max_age` is raised from 1825 to 3000 days.
- The trunks of the `almond`, `almond_aldrich`, `almond_independence` and `almond_wood_colony` models now lean (about 1.5-11 degrees) and bow gently along their length, as measured on orchard trunks, instead of being straight tubes with a small uniform tilt.
- Copying or assigning a `ShootParameters` or `PhytomerParameters` no longer consumes random draws. Previously the nested phytomer-parameter assignment resampled every distributed field, so re-defining an existing shoot type with `defineShootType()` or `updateCurrentShootParameters()`, even with unchanged values, shifted the random stream and produced a different plant. Per-phytomer variation now comes from the new `PhytomerParameters::resample()`, which each phytomer calls once when it is created, so it no longer depends on whether a sample had already been cached on the shoot type. Seeded plants therefore differ in realization from previous versions, though not in distribution.
- Fixed `PlantArchitecture::readPlantStructureXML()` restoring leaves of flexible species (e.g. `sorghum`, `maize`) with a different droop than the plant that was written. The leaf compliance drawn when each phytomer was created was not saved, so the reader drew a new one; `writePlantStructureXML()` now records it as `<leaf_flexibility>`. Files written before the tag existed still load.
- Fixed `PlantArchitecture::writePlantStructureXML()` dropping the flowers and fruit of any phytomer whose leaves had been removed (by `removeShootLeaves()`, `removePlantLeaves()`, `pruneGroundCollisions()` or leaf senescence), so a reloaded plant was missing them; for example, pods on bean or cowpea nodes whose leaves had been clipped at the ground. The retained petiole direction of such a phytomer, which orients its peduncles and the shoots growing from it, was also not saved, so it was redrawn on every load. Both are now written in a `<shed_petiole>` node. Files written before the tag existed still load.
- Fixed `PlantArchitecture::writePlantStructureXML()` dropping terminal floral buds, and the axillary floral buds of phytomers with no petioles, so a reloaded plant was missing their flowers and fruit; for example, a reloaded `sorghum` or `wheat` plant had lost its head. These are now written in an `<internode_floral_buds>` node. Also fixed the peduncle parameters recorded for a terminal bud overwriting those of an axillary bud on the same phytomer, which gave one of the two peduncles a different shape after reloading.
- Leaflet petiolules are now built parametrically: a cylinder of length `LeafPrototype::petiolule_length` (a new parameter, per compound-leaf index, as a fraction of blade length) ending where the blade begins, then a short join that flattens and tapers onto the blade base. The radius is taken from the petiole where the leaflet attaches, so the stalk continues the petiole without a step. This replaces a fixed OBJ stalk whose size was unrelated to the petiole. The `soybean`, `bean` and `cowpea` petiolules are now about 3, 2 and 3.5 mm long respectively, the same on every leaflet, within published ranges.
- The `tomato` and `cherrytomato` models now build petiolules and use a new blade-only texture, `TomatoLeaf_blade.png`, in place of textures with a stalk painted onto the blade image. Blade size is unchanged. Their petiole taper is raised from 0.25 to 0.7, so the rachis thins toward the tip as the real one does, and the leaflet stalks that follow it are about 1.2-2.3 mm thick on tomato and 1.5-4.2 mm on cherry tomato (whose lowest leaflets attach near the thick petiole base), rather than 3-4.7 mm.
- Petiolules are now drawn with their petiole's material instead of being coloured primitive by primitive. Colouring them individually gave every stalk primitive its own anonymous copy of the material, several thousand per plant for `soybean`, `bean`, `cowpea` and the tomatoes.
- Fixed `PlantArchitecture::readPlantStructureXML()` relabelling every petiolule as leaf. Restoring a leaf's saved primitive data copied the blade's `object_label` onto every primitive of the leaf, so a reloaded `soybean`, `bean` or `cowpea` plant had no primitives labelled "petiolule", and anything keyed on the label (leaf area, leaf optical properties) treated the stalks as blade.
- Leaf area now excludes petiolules. `PlantArchitecture::sumPlantLeafArea()`, `getPlantLeafAreas()`, `Phytomer::getLeafArea()`, `Shoot::sumShootLeafArea()` and the per-area leaf costs of the carbohydrate and nitrogen models counted each leaflet's stalk as leaf, over-stating leaf area by about 2% for `bean`, `soybean` and `cowpea`. The new `PlantArchitecture::getLeafBladeArea()` reports the blade area of a single leaf object.
...
- The `grapevine_VSP` library model's primary shoots now grow to 20 nodes instead of 17, so the canopy reaches the dimensions of commercial VSP: shoot tips sit on average about 0.8 m above the cordon and the tallest reach 1.9-2.1 m, where previously most shoots stopped below the top catch wire. Leaf area per vine rises by about 13%.

## Collision Detection

- Fixed batched ray casts (`CollisionDetection::castRays()`, `castRaysSoA()` and the GPU path) missing triangles smaller than a few millimetres, and `castRay()` missing those smaller than about 0.1 mm. The test for a ray parallel to a triangle used a fixed tolerance on a quantity that scales with the triangle's area, so small triangles were rejected at every ray angle. LiDAR synthetic scans were the most visible casualty: leaflet petiolules received about 1% of the hits they should, and thin stems and other finely meshed organs were under-sampled.
...
- Fixed the canopy airspace model not conserving energy or water at the soil surface. The airspace network exchanged heat with the soil through a fixed Kustas-Norman conductance while each ground primitive's own energy balance used the flat-plate conductance of its patch size, and the network ignored soil evaporation that the ground primitives computed. The heat and water vapor the soil released did not match what the canopy air received, by more than 100 W/m2 in a test case, biasing canopy air temperature and humidity, and soil temperature depended on how finely the ground was tiled. The network now uses the ground primitives' own conductances and includes soil evaporation from their `moisture_conductance` and `surface_humidity`. Soil fluxes are averaged over the ground primitives' area.
- Ground primitives passed to `EnergyBalanceModel::enableCanopyAirspaceModel()` that have no `boundarylayer_conductance` data now use the Kustas and Norman (1999) bare-soil conductance, the same relation as the boundary-layer conductance plug-in's "Ground" model, instead of the flat-plate relation. A conductance set by the user or by that plug-in is still used unchanged.
- Fixed the surface energy balance solving with a saturation vapor pressure offset by 0.15 K from the one used to report `latent_flux`, so the reported fluxes of a transpiring surface did not close its energy balance (by about 1% of the latent flux). Surface temperatures of transpiring primitives change slightly.
...
- Fixed SIF emission being assigned to the wrong leaf faces. The top face always emitted the Fluspect-B forward spectrum and the bottom face the backward spectrum, whichever face was lit, but the backward spectrum belongs on the face that received the excitation. For the usual leaf lit from above, the top face's red SIF (680-700 nm) was under-stated by about half and the bottom face's over-stated by about double, with smaller errors in the far-red. Each face now emits the backward spectrum of the excitation it receives plus the forward spectrum of the excitation received by the opposite face.
- Fixed solar-induced fluorescence ignoring changes to leaf biochemistry after the first SIF dispatch. The Fluspect-B kernel cache was keyed on the `fluspect_spectrum` label alone, and `LeafOptics::run()` in nitrogen-auto mode reuses its bin labels with new pigment values each time it rebuilds its bins, so over a simulation every leaf kept the kernel computed from its label's first values. Kernels are now recomputed whenever the biochemistry behind a label changes, and SIF follows leaves that are moved to a different label.
...
- Fixed the OptiX 6.5 backend applying other bands' direct source flux when any camera is registered and `RadiationModel::runBand()` is called on a subset of the model's bands. The i-th launched band received the source flux of the model's i-th band in alphabetical order rather than its own, so absorbed flux could be wrong by the ratio between the two bands' fluxes (a 10 nm band received the flux of a 20 nm band, for example). SIF excitation bands were affected whenever a SIF camera was registered. The OptiX 8 and Vulkan backends were not affected.
- Fixed the OptiX 8 backend reading the transparency mask of textured triangles upside down. The cut-out was mirrored top to bottom, so a triangle whose mask is not symmetric top to bottom blocked and absorbed radiation where its image is transparent and let it pass where the image is opaque, and emitted and scattered rays were launched from the wrong part of the triangle. Patches, and the OptiX 6.5 and Vulkan backends, were not affected.

## Core

- Added the "algae" colormap (the sequential light-to-dark green colormap from the Python cmocean package) and the "lines" colormap (the seven MATLAB-style distinct colors previously available only in the visualizer) to `Context::generateColormap()` and `Context::colorPrimitiveByDataPseudocolor()`.
- Added `Context::getColormapNames()` and `Context::getColormapControlPoints()`, which return the predefined colormap names and the colors and positions that define each one. `Context::generateColormap()` is now a static method, and an unknown colormap name produces an error listing the valid names.
- `Context::generateColormap()` now throws a clear error when fewer than 2 colors are requested. Previously 1 color threw an uninformative `std::out_of_range` and 0 colors returned an empty table, which `Context::colorPrimitiveByDataPseudocolor()` then indexed out of range. The warning claiming that tables of more than 9999 colors were truncated was removed, since they never were.
- Fixed `Context::colorPrimitiveByDataPseudocolor()` dividing by zero when all primitives have the same data value (or `data_min` equals `data_max`). All such primitives are now explicitly given the first color of the colormap.
- Fixed tube objects that were rotated, translated or scaled and then edited with `Context::setTubeNodes()`, `setTubeRadii()`, `scaleTubeGirth()`, `scaleTubeLength()` or `appendTubeSegment()`. After such an edit, `Context::getTubeObjectNodes()` and `getTubeObjectNodeRadii()` applied the old transformation a second time. The errors were several centimeters on PlantArchitecture petioles, and XML output was wrong in the same way. Radius edits also moved the tube back to its untransformed position. `Context::copyObject()` on a transformed tube likewise produced a copy whose reported nodes had the transformation applied twice.
- Fixed `Context::copyObject()` on a cone that had been rotated, translated or scaled. The copy is drawn in the right place, but it reported doubly-transformed geometry: `getConeObjectNodes()`, `getConeObjectNodeRadii()`, `getConeObjectLength()` and `getConeObjectVolume()` were wrong. Its analytic vertex normals were also computed from the wrong axis, so a copied cone could be shaded with normals nearly opposite to its surface.
- Fixed `Calendar2Julian()` returning a Julian day one too high for every August date in a leap year, which made `Date::incrementDay()` skip from August 1 to August 3 and caused `Context::loadTabularTimeseriesData()` to misdate hour-2400 records in August of leap years. `Calendar2Julian()` and `Julian2Calendar()` now also apply the Gregorian century rule, so years such as 1900 and 2100 are no longer treated as leap years, and both functions now throw a clear error for an out-of-range month or Julian day.

## Visualizer

- Added the `Visualizer::COLORMAP_ALGAE` and `Visualizer::COLORMAP_GREEN` colormaps. The predefined visualizer colormaps are now taken from the Context's definitions, so each is identical to the Context colormap of the same name.

## LiDAR

- The error from `LiDARcloud::loadTreeQSMColormap()` for an invalid colormap name now lists every available colormap. It previously listed a fixed subset that omitted some valid names.
- Fixed `LiDARcloud::calculateLeafArea()` accepting scans whose timestamps are shared by several pulses when a miss without `target_count` sorted first in the scan, which is always the case after `LiDARcloud::gapfillMisses()`. The check that a timestamp group holds no more returns than its pulse's `target_count` was then skipped for the whole scan, and rounded timestamps were inverted as enormous beams with meaningless transmittance. The check now judges every beam on its real returns. It also rejects a beam in which two returns carry the same `target_index`. The error names the scan and how many beams are affected.
- Fixed `LiDARcloud::triangulateHitPoints()` producing no triangles for multi-return data whose `target_index` counts from 1, as in every LAS/LAZ export. The first-return filter kept only `target_index == 0`. It now decides per scan whether indices start at 0 or 1, as the other per-pulse filters do.
- `LiDARcloud::gapfillMisses()` now throws when a scan's timestamps were rounded more coarsely than the scanner fires (a float32 round-trip, for example). It detects this when returns sharing a timestamp point in different directions. Such timestamps were previously taken as the pulse clock, and the synthesized misses landed in the wrong scan-grid cells without error. The error states the timestamp resolution and the pulse period the scan implies.
- Added `LiDARcloud::getNominalScanGridCell()`, which returns the (row, column) cell of a static raster scan that a point lies in, taking into account the scan's tilt and azimuth offset. Returns given these cells as `row`/`column` data can be gap-filled correctly even when their timestamps are rounded.
- Added `LiDARcloud::deleteHitData()`, which removes a per-hit data column from every hit, including from misses synthesized by `gapfillMisses()` that report it.
- `LiDARcloud::gapfillMisses()` now gap-fills a static raster scan whose returns carry neither `timestamp` nor `row`/`column` data, placing each return on the declared raster by its direction (taking into account the scan's tilt and azimuth offset). Previously it threw. It first checks that the returns lie on the declared raster, and throws if their positions show a wrong size, angular range, tilt or offset. Moving, spinning multibeam and Risley-prism scans still need timestamps or row/column data. A static scan with rounded timestamps can therefore be processed by removing them with `deleteHitData("timestamp")` (and `deleteHitData("target_index")` for multi-return data) before gap-filling, and the rounded-timestamp errors now say so.
- Fixed `LiDARcloud::calculateLeafArea()` accepting rounded timestamps on returns without `target_count` or `target_index`, such as a single-return import: the returns of several pulses sharing one rounded timestamp were inverted as one beam without error. The inversion now rejects a scan whose returns sharing a timestamp point in different directions. When their directions agree but `target_index` or `target_count` contradicts the grouping, the error instead names those columns and says to remove them and keep `timestamp`.

# [1.3.88] 2026-09-23

## Core

- Helios is now licensed under the GNU Lesser General Public License, version 2.1 or (at your option) any later version (`LGPL-2.1-or-later`), replacing GPL-2.0-only. Applications that link against Helios as a library no longer need to be released under the GPL, provided users can replace the linked Helios library with a modified version as LGPL requires; modifications to Helios itself must still be shared under the LGPL.
- `CompoundObject::translate()` is substantially faster for objects with many primitives. The shift is now added directly to the translation column of each primitive's transformation matrix instead of being applied as a general 4x4 matrix product reached through `doesPrimitiveExist()`, `getPrimitiveTransformationMatrix()` and `setPrimitiveTransformationMatrix()`. Primitives whose matrix is not affine still go through the full product, so results are unchanged.
- `Context::copyPrimitiveData()` and `Context::copyPrimitive()` are faster. The former resolves the source and destination primitives once instead of looking each up by UUID for every data type copied, and the latter no longer derives a triangle's vertices and rebuilds a transformation matrix from them only to overwrite that matrix with the source's. Both are on the path that builds plants by copying prototype primitives.

## Plant Architecture

- Plant models in the library can now declare the empirical leaf inclination distribution measured for their species, as Beta distribution parameters given to `registerPlantModel()`, and `PlantArchitecture::buildPlantInstanceFromLibrary()` steers plants of such a model toward that distribution as they grow without the caller asking for it. The declaration is read, replaced or cleared with `getPlantModelLeafInclinationDistribution()`, `setPlantModelLeafInclinationDistribution()` and `doesPlantModelDeclareLeafInclinationDistribution()`; an explicit call to `enablePlantLeafElevationAngleDistributionTracking()` or `disablePlantLeafAngleDistributionTracking()` on a built plant overrides it. A model that declares no distribution consumes no additional random draws and grows exactly the plant it did before.
- The `cowpea` library model now declares the leaf inclination distribution measured for the species (Beta parameters mu=1.398, nu=1.574), so cowpea plants built with `PlantArchitecture::buildPlantInstanceFromLibrary()` are steered toward that distribution as they grow. Cowpea leaf angles therefore differ from previous versions; calling `setPlantModelLeafInclinationDistribution("cowpea", 0, 0)` before building restores the previous behavior.
- The `easternredbud` library model now declares a measured leaf inclination distribution (Beta parameters mu=1.00, nu=2.20, i.e. a mean inclination of about 62 degrees), so redbud plants built with `PlantArchitecture::buildPlantInstanceFromLibrary()` are steered toward that distribution as they grow. Redbud leaf angles therefore differ from previous versions; calling `setPlantModelLeafInclinationDistribution("easternredbud", 0, 0)` before building restores the previous behavior.
- `PlantArchitecture::writePlantStructureXML()` now records the leaf inclination distribution a plant is being steered toward and `readPlantStructureXML()` restores it. Previously a reloaded plant kept the leaf angles it had but silently stopped tracking, so any growth after the read drifted off the distribution with no indication that it had. Azimuth tracking is still not recorded, so a plant tracking both axes returns tracking inclination only, and the steering strength is restored to the default rather than a value the caller chose. Files written before the tag existed still load.
- `PlantArchitecture::registerPlantModel()` takes an additional optional `leaf_inclination_Beta` argument. Existing calls are unaffected, since it defaults to (0,0) meaning the model declares no measured leaf inclination distribution.
- Fixed leaf angle distribution tracking steering the wrong quantity, so that the distribution reported by `PlantArchitecture::getPlantLeafInclinationAngleDistribution()` moved away from the requested Beta distribution on species with non-planar blades. The tracker scored its progress using one mean normal per leaf while the reported distribution bins every blade facet, and on a curved or folded blade these differ: cowpea's facets spread about 13 degrees about the direction the blade faces. Leaf area therefore piled into the steepest bins and the flattest bin never filled at any `lambda_degrees`. Tracking now scores itself over blade facets, so the reported distribution converges on the requested one. Steered leaf angles differ from previous versions for species with curved leaves, including `cowpea`; per-step cost is unchanged, since the facets were already being read to find each blade's direction.
- Leaf angle distribution tracking is substantially faster per growth step. `chooseLeafAngleTarget()` now builds the sine and cosine of each angle bin once per leaf instead of evaluating a trigonometric pair for all 432 bins, and `Phytomer::setLeafNormal()` accepts an optional `current_normal` argument so the tracker can pass the blade normal it has already measured rather than making `setLeafNormal()` re-derive it by re-filtering the leaf object's primitives. Leaf angles are unchanged.
- Fixed leaves placed by `setPetioleLeafGeometry()` being moved off their measured positions as soon as they started expanding. `Phytomer::setPetioleAndLeafScaleFraction()` recomputed every leaf base from `LeafParameters::leaflet_offset` and the petiole parameter length, which is not where a measured leaf sits, so a reconstruction grown forward had its leaflets snap to procedural positions on the first timestep while a leaf held at full size was unaffected. A prescribed leaf now keeps its position along the petiole and its offset from it as the petiole elongates, so it stays attached the way a procedural leaf does.
- Fixed the nitrogen model creating nitrogen as leaves expanded. A leaf's nitrogen per area was carried unchanged onto the area it gained each step, so an expanding leaf appeared to accumulate nitrogen with no supply and a starved canopy could green up on its own. A leaf's nitrogen is now diluted by its growth, and only uptake or remobilization adds to it.
- Fixed the nitrogen model giving a limited supply to the oldest leaves on a plant and none to new ones. The available pool was handed out to leaves in storage order until it ran dry; newly acquired nitrogen now goes first to expanding leaves and then to mature leaves below target, each group sharing what is available in proportion to its demand, so a nitrogen shortage yellows the oldest leaves first.
- Nitrogen remobilization in `PlantArchitecture` now responds to a shortfall rather than to leaf age. When the available pool cannot meet the demand of expanding leaves or growing fruit, mature leaves (oldest first) release nitrogen above `minimum_leaf_N_area` at the first-order rate `NitrogenParameters::leaf_remobilization_rate` (default 0.03/day). Previously remobilization only began once a leaf passed a fraction of `max_leaf_lifespan`, which never happened for library plants whose lifespan is effectively infinite, and it moved an amount per call that depended on the timestep rather than elapsed time. `NitrogenParameters::remobilization_age_threshold` has been removed, so code that sets it must be updated.
- Added leaf senescence to the nitrogen model. Over the final `NitrogenParameters::leaf_senescence_duration_fraction` of `max_leaf_lifespan` a leaf gradually returns `leaf_remobilization_efficiency` of its nitrogen above the minimum to the plant's available pool and yellows before being shed, and nitrogen stress brings the onset forward by up to `stress_senescence_advance_fraction` of the lifespan. Leaves reaching `max_leaf_lifespan` previously took all their nitrogen with them when shed; plants with the default, effectively infinite lifespan are unaffected.
- Added `PlantArchitecture::getPlantAvailableNitrogen()`, which returns the nitrogen in a plant's available pool awaiting allocation to its organs.
- Corrected the nitrogen model documentation, which stated that the Photosynthesis plugin reads `nitrogen_stress_factor` automatically. It does not; calling code must read the stress factor and scale photosynthetic parameters itself.

## Leaf Optics

- Fixed `LeafOptics::run()` in nitrogen mode (`LeafOpticsProperties_Nauto`) holding leaves at a fixed spectrum as their nitrogen changed. Bins were placed only on the first call, so leaves whose nitrogen later moved beyond that range were clamped to the outermost bin for the rest of the run, and a leaf placed in a bin centered at zero nitrogen (as every newly emerged leaf is) could never be reassigned; a time series begun with uniform leaf nitrogen rendered one spectrum throughout. The bins are now rebuilt and all leaves reassigned whenever nitrogen moves outside the range they span.
- `LeafOptics::run()` in nitrogen mode now spaces its `num_bins` spectrum bins evenly across the leaves' nitrogen range instead of at quantiles of the leaf population. Quantile bins piled up wherever many leaves shared a value, leaving the rest of the range to one or two spectra so that leaves of quite different nitrogen rendered identically and the canopy showed hard colour steps.
- Changed the `LeafOpticsProperties_Nauto` reassignment defaults: `reassignment_threshold` is now 0 (relative test disabled, previously 0.30) and `min_reassignment_change` is now 0.05 g N/m² (previously 0.3). The old defaults held a leaf on a bin several bins away from its current nitrogen, e.g. a leaf that filled from 2.2 to 2.5 g N/m² kept the 2.2 spectrum for the rest of a simulation.

## LiDAR

- Fixed `ScanMetadata::rc2direction()` returning directions that did not match the scan grid the synthetic scan actually samples. Its azimuth ran *backwards* from `phiMin` instead of sweeping toward `phiMax`, and both its azimuth and zenith used an exclusive step (dividing by `Nphi`/`Ntheta` rather than `Nphi-1`/`Ntheta-1`), so a (row,column) cell was mapped to the wrong direction: on a 60-degree azimuth window the last column was off by 64 degrees, and a zenith row by several degrees. Users converting scan-table indices to directions, or calling the `addHitPoint(scanID, xyz, row_column, color, data)` overload, received incorrect beam directions. The mapping now matches the documented grid (inclusive endpoints, azimuth increasing with column); the periodic azimuth step of a spinning multibeam scan is unchanged.
- Fixed `ScanMetadata::direction2rc()` binning directions into the wrong scan-grid column and row. It scaled by `Nphi`/`Ntheta` where the grid steps by `Nphi-1`/`Ntheta-1`, stretching the index by `N/(N-1)` and mis-binning roughly half of all columns at any grid size, with the last two columns collapsing into the final cell. It also took the absolute value of the offset from `phiMin`, which folded the sweep back on itself for a scan whose azimuth range crosses 360 degrees; such directions, which `LiDARcloud::getHitRaydir()` reports wrapped into [0,2pi), now unwrap correctly. This affected `LiDARcloud::exportPointCloudPTX()`, which used the mapping to place each return into the PTX grid, so exported PTX files had points written to the wrong grid cells.
- `LiDARcloud::validateRayDirections()` now performs a check again. It compares every hit carrying `row`/`column` hit data against the direction its scan-grid cell maps to and raises an error if they disagree by substantially more than one cell. It had become a no-op after the per-scan hit table it previously walked was removed, and silently validated nothing.
- Removed the bundled libLAS and laszip libraries from `plugins/lidar/lib`. They were never compiled or linked (their CMake integration had been commented out), so LiDAR plug-in functionality is unchanged.

# [1.3.87] 2026-09-19

## Core

- Added `helios::evaluate_Beta_distribution_CDF()` and `helios::invert_Beta_distribution_CDF()`, giving the cumulative distribution function of the leaf inclination distribution sampled by `helios::sample_Beta_distribution()` and its inverse. Added `helios::evaluate_ellipsoidal_azimuth_CDF()` and `helios::invert_ellipsoidal_azimuth_CDF()` for the azimuth distribution sampled by `helios::sample_ellipsoidal_azimuth()`.
- Fixed `Context::setObjectAverageNormal()` giving a different final orientation depending on where the object sat in the scene. After aligning the normal it applies a twist about that normal to bring the object's local x-axis as near to world +x as possible, and the axis it twisted from was computed as a transformed point rather than a transformed direction, so it carried the object's translation. Two identical objects given the same normal a short distance apart ended up rotated by tens of degrees relative to one another.
- Fixed `Context::addMaterial()` leaving primitives without a material when called with a label that already exists. The existing material was erased while primitives assigned to it kept its ID, so the next call to `getPrimitiveColor()`, `setPrimitiveColor()`, `getPrimitiveTextureFile()` or similar on such a primitive threw an uninformative `std::out_of_range`. The material is now reset in place and its primitives stay assigned to it.

## Plant Architecture

- Added `PlantArchitecture::enablePlantLeafAngleDistributionTracking()`, together with elevation-only and azimuth-only forms and `disablePlantLeafAngleDistributionTracking()`, which steer a plant's leaf angles toward a prescribed Beta inclination and ellipsoidal azimuth distribution as the plant grows rather than re-aiming a finished plant. Each leaf is given a target angle as it emerges and turns onto it as it expands, so a fully grown leaf is never moved again and the canopy does not shift between timesteps. A `lambda` argument trades matching the distribution against keeping the leaf angles the model generated; zero leaves the plant unchanged.
- Leaf angle distribution tracking caches the blade primitives of each leaf between growth steps rather than re-selecting them by primitive data every step, roughly halving what tracking adds to `PlantArchitecture::advanceTime()`. Enabling tracking now costs about 50% more growth time than leaving it off, measured from a seedling to 45 days on one cowpea plant and on canopies up to sixteen; the overhead is proportional to leaf count.
- Fixed `PlantArchitecture::duplicatePlantInstance()` not carrying leaf angle distribution tracking onto the copy, so a duplicate of a tracked plant grew untracked with no indication that the setting had been dropped. The tracking configuration is now copied; the duplicate starts with its own empty primitive cache and its own random stream, so the two plants do not draw identical targets.
- Fixed `PlantArchitecture::setPlantLeafElevationAngleDistribution()` and `setPlantLeafAzimuthAngleDistribution()` assigning randomly drawn angles to leaves in traversal order rather than matching them to the leaves as documented. A leaf held more steeply than its neighbour had an even chance of ending up shallower than it, so the plant's leaf arrangement was scrambled while its histogram matched; the matching described in the documentation ran only when an elevation and an azimuth distribution were requested together.
- Fixed `PlantArchitecture::setPlantLeafAngleDistribution()` and its variants rotating every leaf about its own normal so that blades faced world +x regardless of the petiole bearing them, collapsing leaf azimuth across the canopy.
- Fixed leaf angles imposed by `PlantArchitecture::setPlantLeafAngleDistribution()` and its variants being lost when the plant was written with `writePlantStructureXML()` and read back with `readPlantStructureXML()`, which restored the leaf orientations the plant had been grown with.
- `PlantArchitecture::setPlantLeafAngleDistribution()` and its variants now weight the distribution by leaf area rather than by leaf count, matching how `getPlantLeafInclinationAngleDistribution()` reports it, and no longer build an N-by-N matrix over every leaf in the canopy -- imposing a distribution on a large canopy is substantially faster and no longer allocates memory quadratic in the leaf count.
- Added `Phytomer::setLeafNormal()`, which aims a single leaf's blade at a given direction about its own base and records the resulting orientation so that it survives an XML round trip.
- Fixed `PlantArchitecture::getPlantLeafInclinationAngleDistribution()` and `getPlantLeafAzimuthAngleDistribution()` (all overloads) counting petiolule facets as leaf lamina. The petiolule is part of the same Context object as the blade it carries, and its facets wrap a roughly cylindrical stalk, so they contributed area at inclinations where the plant has no leaf surface; on a bean this was about 2% of the reported leaf area and, because the stalk stands closer to vertical than the blade, nearly nine times heavier in the steepest inclination bin than in the flattest. Both distributions now accumulate blade facets only.
- Fixed `PlantArchitecture::readPlantStructureXML()` freezing the stem of a plant that was still growing when `writePlantStructureXML()` saved it. Every phytomer was rebuilt at full elongation and only then given back its saved target length, so an internode that had not finished elongating came back believing it was already full-grown and never elongated again, while the plant's leaves carried on expanding. Affects every species, not only plants reconstructed from measured geometry.
- Fixed the shoot type's phytomer creation function resizing measured internodes built by `PlantArchitecture::addShootFromNodePositions()`. Library creation functions scale an internode's target length by node position or plant age, which shrank reconstructed wood below the geometry it was measured from, or left it elongating on later timesteps. The creation function still runs on measured phytomers, so leaf scale and everything else it sets still apply; only its effect on the measured internode is undone.
- Fixed petiole-free phytomers losing their phyllotactic spiral when `internode.phyllotactic_angle` is given a random distribution. Such a phytomer's direction around the stem was computed as its node index times its own freshly drawn angle rather than as the running sum of the angles below it, so each draw's error was multiplied by the node index and branches seated on a trunk or other leafless axis were scattered around it instead of following the spiral. Constant phyllotactic angles, and phytomers that carry petioles, were unaffected.
- Fixed lateral leaflets of compound leaves being rolled out of the plane of their leaf whenever the petiole rises. The roll that compensates for the petiole's elevation was applied with the wrong sign, so each lateral blade was tilted by twice the elevation instead of lying flat with the rachis, standing fully edge-on at 45 degrees of elevation; terminal leaflets were correct. Affects every compound-leaved species, including tomato, bean, cowpea, soybean and strawberry. Leaves on horizontal petioles, and the sign of a user-set `leaf.roll`, are unchanged.
- Fixed `PlantArchitecture::readPlantStructureXML()` making petioles of still-growing leaves far too thin. The saved base radius is already the petiole's current radius, but the reader multiplied every node beyond the base by the leaf's growth fraction again, so a reloaded young petiole tapered to a fraction of its saved thickness; fully grown leaves were unaffected.
- Fixed `Phytomer::getPetioleLength()` returning 0 for every phytomer, which it had always done. It now reports the mean current arclength of the phytomer's petiole centerlines, and a new `Phytomer::getPetioleLength(uint petiole_index)` overload reports one petiole. Anything deriving a quantity from it -- a rachis length taken as petiole length subtracted from a leaf's reach, for instance -- was silently wrong.
- Fixed a crash when a plant carries petioles with different numbers of leaflets drawn from the same leaf prototype. The cache of unique leaf blades was keyed only on `LeafPrototype::unique_prototype_identifier` and built for the leaflet count of the first phytomer that needed it, so a later phytomer with more leaflets -- a shoot type whose `leaf.prototype` was copied from another type with a different `leaf.leaves_per_petiole` (a one-leaflet cotyledon type copied from a compound type), or a single shoot type whose `leaves_per_petiole` is a random parameter spanning a range -- threw an uncaught `std::out_of_range` from the `Phytomer` constructor with no error message in a release build. Blades are now cached per leaflet count, so each petiole is built from blades made for its own leaflet positions, and `readPlantStructureXML()` restores such a plant instead of rejecting it; species with a single leaflet count per prototype are unchanged.
- Added `ShootParameters::leaf_expansion_rate_max`, a relative expansion rate for the leaves and petioles of a shoot that is separate from `elongation_rate_max`, so a species can expand its leaves faster or slower than it elongates its internodes. It defaults to a negative sentinel meaning "not set", in which case leaves expand at `elongation_rate_max` exactly as before, so every existing shoot type and every model in the plant library is unchanged.
- Petioles now elongate at the shoot's `ShootParameters::elongation_rate_max` rather than at the rate its leaf blades expand, so a petiole keeps extending after the blade it carries has reached full size instead of stopping with it. `Phytomer::setPetioleScaleFraction()` and `Phytomer::setPetioleAndLeafScaleFraction()` set a petiole's elongation independently of its leaves, and `Phytomer::setLeafScaleFraction()` continues to move both together. A shoot type that leaves `ShootParameters::leaf_expansion_rate_max` unset -- the default, and the case for every model in the plant library -- has one rate for both and is unchanged.
- Added `Phytomer::scalePetioleMaxLength()`, the petiole counterpart of `Phytomer::scaleInternodeMaxLength()`. It scales the fully-elongated length every petiole on the phytomer is growing toward while leaving their present length unchanged, so a phytomer creation function can give a leaf born on a young plant a shorter final petiole. `Phytomer::scalePetioleGeometry()` could not be used for this, since it sets the length the petiole has now and the petiole then goes on elongating past it.
- Added `Phytomer::scaleLeafSizeMax()`, the leaf counterpart of `Phytomer::scaleInternodeMaxLength()` and `Phytomer::scalePetioleMaxLength()`. It scales the size every leaf on the phytomer is expanding toward while leaving the blades at their present size, so a leaf built from measured geometry can be handed back to the growth model still the size it was measured and go on expanding. `Phytomer::scaleLeafPrototypeScale()` could not be used for this, since it rescales the blade itself and leaves the expansion fraction unchanged.
- Added `LeafParameters::intercalary_leaflet_scale`, which builds interruptedly pinnate compound leaves -- the form of tomato, potato and much of the Rosaceae, in which small intercalary leaflets sit between the major leaflet pairs. Pairs are counted outward from the tip; the odd-numbered pairs are major and step by `leaflet_scale` once per major pair, and the even-numbered pairs are intercalary at this fraction of the major pair just distal to them, so leaflet size no longer has to fall monotonically from the tip. The total leaflet count is still `leaves_per_petiole`. The default of zero gives the previous simply pinnate behavior and leaves every existing species unchanged.
- Added petiole bending under leaf weight through `PhytomerParameters::petiole.flexibility` (dimensionless bending compliance) and `petiole.flexibility_aging` (timescale over which that compliance grows linearly with age), the petiole counterpart of the leaf prototype `flexibility` parameters. A petiole now arches downward along its length as a tapered cantilever loaded by the leaflets attached to it, bending further as its leaf grows and continuing to lower as it ages, carrying its leaflets with the local curve of the centerline and never passing hanging straight down; petioles set by `PlantArchitecture::setPetioleNodePositions()` or carrying leaves posed by `setPetioleLeafGeometry()` are not bent. The petiole is clamped where it meets the stem, so it goes on leaving the stem at the pitch it was generated with and the droop appears beyond the insertion as curvature accumulating along the length, as measured petioles do. The default of zero leaves every existing species unchanged.
- Fixed the `grapevine_VSP` and `grapevine_Wye` plant library models carrying several times the leaf area of a real vine. Leaf blades were built at roughly 250 cm^2 against the 121-127 cm^2 measured on vertically shoot-positioned vines, and on `grapevine_VSP` the lateral shoots had no shoot type of their own, so each inherited its parent's full-length, full-size-leaf type; since laterals hold over half the leaves on a mature vine, `sumPlantLeafArea()` returned about 19 m^2 per vine, a leaf area index near 4 where VSP canopies run 1.0-1.5. A `grapevine_lateral` shoot type now carries shorter shoots with ~39 cm^2 blades, and both models report a leaf area index of about 1.0-1.5 at maturity.
- The trunk and canes of the `grapevine_VSP` plant library model are now shaped like trained wood and differ from vine to vine, where every vine previously had an identical constant-radius vertical trunk and perfectly straight horizontal canes. The trunk leans and bends slightly, flares at the ground, and ends in a swollen head a little below the fruiting wire; the canes climb from the head onto the wire and then follow it loosely, tapering toward the tip. The variation is bounded, so the trunk stays upright and the canes stay within a few centimeters of the wire. The `trunk_height` build parameter is now the height of the fruiting wire, and the nominal trunk diameter was reduced from 10 cm to 8 cm.
- Corrected the documentation of `PlantArchitecture::setPlantLeafAngleDistribution()`, `setPlantLeafElevationAngleDistribution()` and `setPlantLeafAzimuthAngleDistribution()` (all overloads), which described the leaf angles as being assigned by the Hungarian algorithm. The algorithm has not been used since the assignment was rewritten in v1.3.87; angles are matched to the leaves by rank, weighted by leaf area. The multi-plant `setPlantLeafAzimuthAngleDistribution()` also described its azimuth angles as following a Beta rather than an ellipsoidal distribution.
- Fixed `PlantArchitecture::enableSoftCollisionAvoidance()` and `enableSolidObstacleAvoidance()` switching collision-detection messages on regardless of the plant's own setting, so a plant that had called `disableMessages()` began printing collision and BVH output as soon as avoidance was enabled, with no public call able to silence it. The new `CollisionDetection` instance now inherits the `PlantArchitecture` instance's message setting.

## Radiation

- Fixed `RadiationModel::runBand()` discarding scattered sky radiation whenever a band combined a radiation source, diffuse sky radiation and scattering with emission disabled -- the usual configuration of a shortwave band (present since v1.3.64). Sky radiation was never scattered beyond its first interception while the first bounce of the direct beam was counted twice, so absorbed fluxes and camera images were too low wherever multiply-scattered sky radiation matters, such as NIR deep in a canopy. Bands run with no source, or together with an emitting band, were unaffected.
- Fixed `RadiationModel::runBand()` giving a band a different result when it was run together with bands that have a larger `setScatteringDepth()`. A band whose scattering depth was exhausted kept scattering to the largest depth in the call using another band's reflectivity and transmissivity (Vulkan backend), or stopped the scattering of the remaining bands and had its slot overwritten by theirs (OptiX backends), and a band with a scattering depth of zero had its scattered direct energy launched anyway. Each band now gives the same fluxes as when it is run alone.
- Camera `white_balance` "auto" now balances against the light that actually reaches the surfaces in view. It previously divided each channel by the area under its spectral response, which assumes a spectrally flat light, so under a non-flat source such as a cool-white LED white surfaces rendered with a strong colour cast, and cameras without spectral responses were never balanced; `RadiationModel::runBand()` now scales the red, green and blue bands so that a white surface lit by the sources and sky reaching the view renders neutral, independent of the colours in the scene. A camera that sees no lit surface in one of its bands now prints a warning and is left unbalanced.
- Fixed the primitive data "specular_scale" not scaling specular reflection as documented. The scale was used only if some primitive in the scene had a value above 0: with every primitive at 0, cameras rendered full-strength highlights (a scale of 1) instead of none, and otherwise a primitive with "specular_exponent" but no "specular_scale" rendered no highlight instead of a scale of 1. The OptiX 6.5 backend ignored "specular_scale" entirely. Each primitive's specular reflection is now multiplied by its own "specular_scale", or by 1 when it has none, on all backends.
- Reduced the memory `RadiationModel::runBand()` uses in scenes with cameras but no specular reflection. The buffer of incident radiation read for specular highlights, holding a value for every source, camera, primitive and band on the OptiX backends, is now allocated only when some primitive has the primitive data "specular_exponent".

## Aerial LiDAR

- Fixed `AerialLiDARcloud::gridijk2index()` using the number of cells in y as its row stride where its inverse `gridindex2ijk()` uses the number in x, so the two disagreed on any grid whose x- and y-resolutions differ. Cell (0,1,0) of a 4x8x3 grid resolved to index 8 instead of 4, and 84 of that grid's 96 cells failed the round trip, silently mixing up leaf area and transmission probability between cells.
- The `aeriallidar` plug-in now loads the `visualizer` plug-in it links against when that plug-in is not already in the project's `PLUGINS` list, as the `lidar` plug-in already did. Building a project or test that listed `aeriallidar` alone failed at the CMake configure step with `The dependency target "visualizer" of target "aeriallidar" does not exist`.

## Boundary Layer Conductance

- Fixed the "InclinedPlate" model of `BLConductanceModel::run()` returning NaN for a surface cooler than the air in light wind (e.g., a 20 cm leaf 10 K below air temperature at 0.1 m/s), which the energy balance model then silently replaced with its default temperature. The model switched to the buoyancy-opposing form of the mixed-convection correlation for cool surfaces, although the wind is always transverse to gravity and the documentation specifies the buoyancy-assisting form. The assisting form is now always used, so the conductance of a surface cooler than the air equals that of one warmer by the same amount, and is higher than before.

## Canopy Generator

- Fixed `CanopyGenerator::buildCanopy()` for `WhiteSpruceCanopyParameters` swapping the two values of `canopy_configuration`: "uniform" placed trees at random within their grid cells and "random", the default, placed them on a regular grid. White spruce canopies built with the default parameters are now randomly arranged, as documented; set `canopy_configuration` to "uniform" to keep the regular grid.

## Photosynthesis

- Fixed `PhotosynthesisModel::run()` returning a meaningless `net_photosynthesis` for a primitive whose `boundarylayer_conductance` is zero (or negative, which is clipped to zero) while its `moisture_conductance` is not. Combining the two conductances evaluated 0/0 for hypostomatous leaves, the default, and the Farquhar solver then reported a positive assimilation rate of several umol/m^2/s despite there being no CO2 supply. Such a primitive now gives the same result as one with both conductances zero.

# [1.3.86] 2026-09-10

## Core

- Added `Context::calculateAreaIndex()`, which computes the one-sided leaf or plant area index of a group of primitives on a ground-area basis -- the quantity \f$L\f$ appearing in Beer's law. Four overloads are provided: leaves alone or leaves plus woody elements, each either deriving the ground area from the horizontal footprint of the domain bounding box or taking it explicitly. Woody primitives are counted as one half of their summed one-sided area, since a tube or cone encloses the branch it represents and so sums to the full cylinder surface rather than the projected area. Voxel primitives are rejected, because a voxel's area is its total enclosing surface area rather than a one-sided area.
- Fixed `Context::loadXML()` writing past the end of a stack array when a `<transform>` tag held more than 16 values, which could corrupt memory or crash before the "does not contain exactly 16 data values" error was raised. Such a file is now rejected with that error without touching memory beyond the matrix.
- Fixed `Context::loadPLY()` hanging forever on a PLY file whose header is truncated before the `end_header` line; it now raises an error naming the file. Also fixed `loadPLY()` treating scalar properties of the face element (for example `property int flags`, as some mesh tools export) as vertex properties, which shifted every vertex coordinate by one token and made the load fail with an out-of-range error or, if the tokens happened to parse, silently produce garbled geometry. Face rows are now read property by property so only the vertex index list is used.
- Improved `Context::cropDomainX()`, `cropDomainY()`, `cropDomainZ()` and `cropDomain()` from quadratic to linear time in the number of primitives removed. They deleted rejected primitives one at a time, and each deletion of an object member searched and re-packed the parent object's member list (and rebuilt a Polymesh's face table), so cropping half of a tile or mesh with N primitives cost O(N^2); at 64k primitives the crop is now about 50x faster and the gap grows with scene size.
- Improved `Context::cleanDeletedUUIDs()` and `Context::cleanDeletedObjectIDs()` (all overloads) and the batch form of `Context::deletePrimitive()` from O(N*k) to O(N) for a list of N entries with k stale ones; a list of 320k UUIDs with half deleted now cleans in about a millisecond instead of over a second.

## Energy Balance

- Added `EnergyBalanceModel::enableCanopyAirspaceModel()`, which resolves the within-canopy air temperature and humidity surrounding leaves from a vertically layered resistance network instead of holding them at a prescribed value, so that the canopy feeds back on the air driving its own transpiration. The airspace is divided into vertical layers of equal leaf area index, each exchanging sensible heat and water vapor with its leaves, its neighboring layers, and -- for the outermost layers -- the soil surface and the above-canopy reference air, which is imposed as a fixed boundary condition read from global data `air_temperature_reference`, `air_humidity_reference` and `wind_speed_reference`. Unlike `EnergyBalanceModel::enableAirEnergyBalance()`, which evolves a prognostic boundary layer and thereby assumes a horizontally infinite canopy, this model suits canopies of limited extent subject to advection such as an orchard block; the two determine the same air state and may not both be enabled. `run()` iterates the surface energy balance and the airspace solution to convergence, sets primitive data `air_temperature`, `air_humidity` and `wind_speed` on the canopy primitives, and reports `canopy_air_temperature`, `canopy_air_humidity`, their per-layer counterparts, `aerodynamic_resistance` and `canopy_airspace_iterations` as global data. Because it solves for a steady state it cannot be combined with the form of `run()` that takes a timestep.
- Added `EnergyBalanceModel::disableCanopyAirspaceModel()` and `EnergyBalanceModel::setCanopyAirspaceConvergence()`, the latter setting the temperature tolerance and iteration cap of the canopy airspace solution (default 0.01 K and 50 iterations).

## Plant Architecture

- Fixed `PlantArchitecture::writePlantStructureXML()` recording a shoot built by `PlantArchitecture::addShootFromNodePositions()` with one node per internode segment rather than one node per phytomer, so that `PlantArchitecture::readPlantStructureXML()` rebuilt it with `PhytomerParameters::internode.length_segments` times as many phytomers, each that fraction of its measured length. A bean stem built as three 0.1 m phytomers of a shoot type with two length segments came back as six phytomers of 0.05 m. The shape, endpoints and radii were reproduced exactly, so the discrepancy showed up only in the phytomer count and in per-phytomer quantities derived from it; shoot types with a single length segment were unaffected.
- Fixed the carbohydrate model being applied once per plant in the model on every sub-step of `PlantArchitecture::advanceTime()`. Each carbon-balance step (maintenance respiration, growth cost, starch sequestration and transfer, phyllochron adjustment, organ abortion) iterated over every plant rather than only the plant being advanced, so with N plants every shoot's pool was debited N times per sub-step and advancing a single plant also ran the carbon balance on all others. Multi-plant simulations with the carbon model enabled starved and pruned shoots far too early; single-plant runs were unaffected.
- Fixed `PlantArchitecture::advanceTime()` producing different plants depending on how the time span was split across calls. The sub-step was chosen from the phyllochrons of shoots existing when the call began, and each shoot added at most one phytomer per sub-step, so a shoot type with a shorter phyllochron than its parent that emerged during the call (bindweed primary shoots, grapevine main shoots) grew at the parent's slower rate when a season was advanced in one call but at its own rate when advanced a day at a time. Epicormic shoot emergence was also resampled over the full call span on every sub-step, inflating emergence by the number of sub-steps. Plants still depend somewhat on how the span is split, because bud break is resolved to the sub-step. Results change from previous versions for such plants and for any plant using `enableEpicormicChildShoots()`.
- Added `PlantArchitecture::setPetioleLeafCount()`, which changes the number of leaves (leaflets) on one petiole of an existing phytomer by rebuilding them procedurally. Previously the count was fixed by the shoot type's `leaf.leaves_per_petiole` for every phytomer, so a measured compound leaf with a different number of leaflets could not be prescribed with `setPetioleLeafGeometry()`.
- **Breaking change:** `PlantArchitecture::setPetioleLeafGeometry()` now applies a prescribed leaf's roll, pitch and yaw as intrinsic rotations in the leaf's rest frame on its petiole, the same way for every leaf. Previously they went through the procedural placement chain, so a single leaf's roll flipped sign on alternate nodes, a lateral leaflet's roll was mirrored by side and offset by the petiole's elevation, a terminal leaflet ignored its roll and yaw, and yaw turned about the world vertical; the same angles therefore gave differently posed blades depending on where the leaf sat. Leaves prescribed with given angles in v1.3.85 are posed differently by this version, so angles fitted or tuned against v1.3.85 must be re-derived.
- Fixed `PlantArchitecture::setPetioleLeafGeometry()` leaving the blade primitives of a re-prescribed leaf without the `object_label` primitive data of "leaf". The first prescription copied a labelled cached prototype, but every later one rebuilt the leaf from the prototype function and dropped the label, so anything selecting leaves by label (per-organ area sums, optical properties, the sorting of blades from petioles) silently missed those leaves.
- Fixed `PlantArchitecture::readPlantStructureXML()` rejecting any file written before `<leaf_prototype>` was recorded, and any file containing a leaf rebuilt by `setPetioleLeafGeometry()` or `setPetioleLeafCount()`, with an error that the prototype index exceeds the number of unique prototypes.
- Fixed `PlantArchitecture::writePlantStructureXML()` and `readPlantStructureXML()` losing prescribed organ geometry: petiole paths set by `setPetioleNodePositions()` and the poses and base positions of leaves set by `setPetioleLeafGeometry()` were not saved, and the petioles and leaves of a shoot built by `addShootFromNodePositions()` came back as the shoot type would have generated them. All of these are now written and restored.
- Added `PlantArchitecture::setShootInternodeLengthMax()`, which sets the target length of internodes grown at the apex of an existing shoot. A shoot built by `addShootFromNodePositions()` grows toward the mean of its prescribed internode lengths, and there was previously no way to change that, so a measured seedling (whose measured stem is mostly hypocotyl) could not be grown forward with realistic internodes. The value is not saved by `writePlantStructureXML()`, so it must be set again after `readPlantStructureXML()`.

## Visualizer

- Fixed the headless EGL build (`HELIOS_VISUALIZER_EGL`) failing to compile on Linux, where `<EGL/egl.h>` defaulted to the X11 platform and pulled in `Xlib.h`, whose `Colormap` typedef collides with the Helios `Colormap` struct declared in `Visualizer.h`. The EGL context is created through `EGL_PLATFORM_DEVICE_EXT` with pbuffer surfaces and never uses an X11 display, so the X11 platform headers are now suppressed.
- Fixed `Visualizer::plotUpdate()` and `Visualizer::printWindow()` never showing changes made to a primitive after it was first displayed. Since v1.3.83 the Visualizer skipped re-uploading any primitive it had already sent to the GPU, so translating, rotating, scaling or recoloring an existing primitive, changing its texture through the per-primitive texture setters, changing the data it is colored by, or calling `colorContextPrimitivesRandomly()`, `colorContextObjectsRandomly()`, `setColorbarRange()` or `setColormap()` after the first frame had no visible effect until the geometry was cleared and rebuilt; a growth time-lapse driven by `PlantArchitecture::advanceTime()` showed leaves frozen at their first-frame size. Each uploaded primitive is now tracked with a fingerprint of its transform, color, texture and color-by data so that only primitives that changed are re-uploaded, a change to the color mapping recolors the displayed geometry in a single pass, and the automatic colorbar range spans every displayed data-colored primitive rather than only those modified since `Context::markGeometryClean()`. Object data changed with `Context::setObjectData()` after `Context::markGeometryClean()`, hiding or showing a primitive, and changes made only through material setters are still not shown until the geometry is rebuilt.
- Fixed textured Context voxels being drawn by `Visualizer::plotUpdate()` and `printWindow()` with the texture and color override reversed: a voxel whose texture color was not overridden showed its flat primitive color instead of its texture, and a voxel whose texture color was overridden showed its texture instead of its color. Voxels now follow the same override rule as every other primitive type.
- Fixed point culling in `Visualizer::plotUpdate()` and `printWindow()` scaling quadratically with the number of points added with `addPoint()`, which made a point cloud of a few hundred thousand points take minutes to render and a cloud of two million points effectively never finish. The frustum and distance passes also assigned each point's visibility to an unrelated point (the lookup ran in hash order rather than insertion order), so culling hid an arbitrary subset of the cloud, and the rendered-point count reported by `getPointRenderingMetrics()` never decreased. Both passes are now linear, act on the correct point, and the distance pass no longer re-shows points the frustum pass had culled.
- Fixed `Visualizer::plotUpdate()` and `printWindow()` intermittently throwing "UUID of N does not exist in the visualizer geometry" when Context geometry was drawn together with geometry added directly to the Visualizer (`addPoint()`, `addLine()`, text). Such geometry carries a random 64-bit ID, and material assignment truncated it to a 32-bit UUID and mistook it for whatever Context primitive had that UUID; when that primitive was hidden (for example a plant model's cached leaf prototypes) the render aborted. With a large point cloud overlaid on a plant model this happened on a large fraction of renders. Recoloring the scene, for example after `colorContextPrimitivesByData()`, likewise painted such geometry with the colormap color of the Context primitive its truncated ID matched.

## LiDAR

- Fixed `LiDARcloud::lastHitFilter()` doing the opposite of what it documents: it deleted the last return of every pulse and kept all earlier returns, so a multi-return cloud filtered to "last hits only" contained everything except the last hits. It now keeps exactly the final return of each pulse, mirroring `firstHitFilter()`.
- Fixed the leaf-area inversion counting beams that were intercepted in front of a voxel as beams that entered it. `getCellBeamCount()`, the sampling variance from `getCellLADVariance()`, the intervals from `getCellLeafAreaConfidenceInterval()` and `getGroupLADConfidenceInterval()`, `exportLeafAreaUncertainty()`, and the `min_voxel_hits` threshold of `calculateLeafArea()` used the number of beams whose ray line crossed the voxel rather than the number that reached it, so for occluded voxels deep in a canopy the beam count was inflated by every beam blocked in front of them and the reported confidence intervals were far too narrow. The beam count, mean path length and variance are now computed over the same population of beams as the transmission probability.
- Per-hit scalar data columns are now typed and stored narrow while their values allow it, with one presence bit per hit instead of a byte: `target_index`, `target_count`, `is_miss`, `nRaysHit`, `channel`, `gapfillMisses_code`, `row` and `column` start as 32-bit integers, `intensity`, `distance`, `deviation`, `echo_width`, `reflectance` and `reflectivity_lidar` as 32-bit floats, and `timestamp`, `pulse_id`, `origin_x/y/z` and every unrecognized label as 64-bit doubles. A narrow column widens to double on the first value it does not hold exactly, so every value reads back exactly as stored; a label costs 4.125 bytes per hit instead of 9 while it stays narrow, which `estimateHitPointMemory()` reflects. `LiDARcloud::createHitDataColumn()` fixes a label's type explicitly (an explicit integer column rejects a non-integral value with an error; an explicit float column stores at float precision), `getHitDataType()` reports the current type, and `getHitDataColumn()`/`getScanHitDataColumn()` gained `float` and `int32_t` overloads, the latter raising an error for a value that is not a 32-bit integer. A NaN passed to `addHitPoint()` now leaves the label absent on that hit, as in `addHitPoints()`, rather than storing a NaN.
- Added `LiDARcloud::addHitPoints()`, a bulk ingest that appends `n` hits from contiguous double-precision position and value arrays with a single capacity check and a single growth of the cloud, so a loop over row chunks read from disk costs amortised constant time per point; with `reserveHitPoints()` first there is no growth transient at all. `addHitPoint()` now also honours the `setMaxHitPoints()` cap, and a rejected `addHitPoint()` (an unknown scan, a value an explicitly typed column cannot take, the cap) no longer collapses the cloud's virtualized misses into stored points or creates columns before raising its error.
- `triangulateHitPoints()` and `calculateLeafArea()` no longer walk the whole cloud once per scan: each scan's hits are located through a per-scan index built once for the cloud, and the inversion's working arrays are sized to the scan being processed rather than to `getHitCount()` (previously a `getHitCount()`-sized map was allocated per scan). Per-hit string-keyed data lookups in those paths were replaced by column reads. On a regular `addGrid()` lattice, hits are now binned to voxels arithmetically instead of by testing every cell per hit, removing an O(hits x cells) pass from both operations. Results are unchanged. The index is rebuilt at most once after each change even when const readers are called from several threads at once.
- Added per-scan bulk readers `LiDARcloud::getScanHitCount()`, `getScanHitXYZColumn()`, `getScanHitDataColumn()` and `getScanHitIndices()`, which cost O(hits in the scan) rather than O(hits in the cloud).
- Added block-tiled leaf-area inversion: `calculateLeafArea()` overloads taking an inclusive lattice range `(ijk_min, ijk_max)` invert only those cells, leave the rest untouched, and size their scratch (including the parallel walk's per-thread copies) to the block, so a grid can be inverted block by block from one culled cloud per block. `getCellGlobalIJK()` and `getGridGlobalCount()` expose the lattice coordinates.
- Added `LiDARcloud::setTriangulationSink()`, which hands each scan's triangles to a callback instead of retaining the mesh while keeping the per-voxel sums `calculateLeafArea()` needs, and `setSyntheticScanHitSink()`, which reports each traced chunk of `syntheticScan()` as soon as it is stored so the caller can write it out and release it with the new order-preserving `deleteHitPoints(first, count)`. A synthetic scan's chunk size now also accounts for the per-return staging cost, never below the existing GPU batch floor, and a seeded `syntheticScan()` no longer depends on how it is chunked: the beam-footprint random numbers are drawn in single-chunk order however the scan is divided, so a scan that previously ran in several chunks (a small `setSyntheticScanMemoryBudget()`, or a very large scan) now reproduces the single-chunk realization for its seed.
- The timestamp path of `LiDARcloud::gapfillMisses()` (scans with per-hit timestamps but no row/column indices) now synthesizes the same miss population as the row/column path, through the same code: each return is attached to its scan-grid cell from the pulse clock (the declared clock when it reproduces the timestamps, otherwise a period and anchor recovered from the data), the cells are recorded as `row`/`column` hit data, and one miss is synthesized for every empty cell of the declared grid along the direction of the fitted scan-grid model, held in virtualized form. Previously this path interpolated misses only along sweeps that contained returns, placed them by interpolating between neighbouring returns, and stored every one as a hit point (a 30-million-return scan on a production raster cost gigabytes and its misses carried `gapfillMisses_code` 2 and 3 for edge extrapolation, which no longer exist: every synthesized miss is code 1). `gapfill_grid_only` now restricts the fill to rows whose beams reach the voxel grid. Moving-platform scans are supported on this path with each miss emitted from the platform pose at its pulse's time, reported through `getHitOrigin()` and `origin_x`/`origin_y`/`origin_z`. `gapfillMissesCount()` no longer collects the filled positions it does not return.
- The scan-grid model that places synthesized misses is now fitted in the scanner head's own frame (the head axis is estimated from the returns, so a scanner that was not level is modelled exactly instead of leaving a tilt-sized error in every miss), gained a row term in the azimuth (the continuous head rotation during a column sweep, which Helios' own synthetic scanner models), and gained per-row zenith and azimuth corrections where the sweep measurably bows, as a real mirror does. Both gap-filling paths use it. For a static scan whose returns carry timestamps, the row/column path now stamps its misses on the clock recovered from those timestamps rather than the scan's declared default of one unit per pulse from zero.
- The timestamp path of `gapfillMisses()` now measures a static scan's raster from the timestamps and the beam angles whenever the declared clock and size do not describe the returns, so a declared size that only approximates the instrument's (for example one estimated from the point count) still yields the instrument's misses: columns are cut where the beam's zenith jumps back, whatever the mirror's dead time between columns; rows are counted in pulses from each column's first return and anchored to the declared zenith range; columns are numbered by the column period and anchored to the declared azimuth range (from the first column with a return on a full turn), so empty leading columns are filled; the raster holds as many rows and columns of the measured spacing as the declared angular extent spans, and that measured size replaces the declared one (`getScanSizeTheta()`/`getScanSizePhi()`). Previously such a scan was filled only in part: a 150 x 200 raster declared as 142 x 211 or 158 x 190 received about 93% of its misses. `gapfill_grid_only` selects rows by the grid's zenith range measured about the fitted head axis, exactly. Validated against a RIEGL VZ-series scan exported with its 7.5 million exact misses, declared as 2,600 x 7,000 over the zenith extent of its returns: gap-filling the returns alone reproduced 99.8% of the recorded misses within 1.5 row steps, at a median direction error of 0.016 degrees.
- Fixed `gapfillMisses()` placing synthesized misses degrees off the scanner's beams whenever returns near the zenith or nadir were present, which includes any full-sphere raster: the azimuth fit unwrapped its samples in a single chain, so one pole return (whose azimuth is meaningless) could put every later column on the wrong branch. On the synthetic scanner's full-sphere raster the misses were on average about 2 degrees (up to 8 degrees) off the misses the scanner records itself; they are now within 0.03 degrees. Returns are weighted by how well their direction determines the azimuth, and the fitted head axis is used only when it fits better than the world frame.
- `gapfillMisses()` no longer collapses the virtualized misses of other scans into stored points when it writes `is_miss`, `gapfillMisses_code` or `row`/`column` data onto a scan's returns, so gap-filling the scans of a cloud one after another keeps every scan's misses at about a bit per scan-grid cell.
- `triangulateHitPoints()` now starts each run from an empty mesh; previously a second call appended its triangles to those of the first.
- Removed the per-scan (row, column) hit table that `addScan()` allocated at 4 bytes per scan-grid cell (hundreds of megabytes for a production raster) but that no ingest path ever wrote, so `getHitIndex()` returned -1 for every cell. `getHitIndex()` now finds the hit whose `row`/`column` hit data names the cell -- a stored return, or the virtualized miss gap-filling synthesized there -- and returns -1 for an empty cell; it raises an error for a scan none of whose hits carry `row`/`column` data. `validateRayDirections()` is unchanged: it rejects moving-platform scans and performs no per-hit check for static scans.
- `LiDARcloud::calculateLeafArea()` now accounts for returns that were removed from a multi-return pulse, for example by cropping the cloud to the voxel grid or by a ground filter, when the surviving returns still carry `target_index` and `target_count`. Previously a pulse whose later returns had been deleted counted its surviving in-voxel return as the whole beam, so a mostly transmitted beam read as fully intercepted and leaf area density came out too high. Removed returns beyond the last surviving return are now counted as transmitted, and returns removed from between two surviving returns cannot be placed and are left out. Added `LiDARcloud::getCroppedReturnStats()`, which reports how many removed returns the last inversion placed before, beyond or between surviving returns.

## Radiation

- Fixed `RadiationModel::runBand()` rebuilding surface radiative properties through a different code path on every call after the first. Unless a source or band had been added or a spectrum changed in between, the second and later calls re-integrated every `reflectivity_spectrum`/`transmissivity_spectrum` per primitive with no caching (minutes of host time per call on large canopies), let a spectrum override a per-band `reflectivity_<band>`/`transmissivity_<band>` value so absorbed fluxes differed from the first call, and threw "Band has no wavelength bounds" for a camera-only band that had rendered fine on the first call. All calls now use the single cached builder with the documented per-band-overrides-spectrum precedence.
- Fixed every radiation camera after the first rendering scattered radiance weighted by the first camera's spectral response. The direct, diffuse and scattering ray launches accumulated camera-weighted scatter for a single camera and `RadiationModel::runBand()` handed that one result to every camera, so with two or more cameras whose `setCameraSpectralResponse()` curves differed, only the first camera's image was correct and the others reproduced its weighting. Each camera now accumulates and integrates scatter weighted by its own response, on all three ray-tracing backends.
- Fixed `RadiationModel::runBand()` failing on a band whose direct ray count is zero, as an emission-only longwave band set up with `setDirectRayCount(band, 0)` is. The direct ray launch was issued with no rays: on the OptiX 8 backend it aborted the run with "OPTIX_ERROR_INVALID_VALUE", and on the OptiX 6 backend it divided by zero while sizing its launch batches and then skipped the launch without reporting anything. Both backends now skip a launch that has no rays, matching the diffuse launch, which already did.

## Collision Detection

- Fixed `CollisionDetection` never noticing that a primitive already in the BVH had been translated, rotated or scaled in place. `ensureBVHCurrent()` compared only UUID sets, so `castRay()`, `castRays()`, `castRaysSoA()` and `findCollisions()` kept using the old bounding boxes until `rebuildBVH()` was called by hand, and even after that call the batched ray casters intersected against a stale vertex cache, so rays through the primitive's new position missed it and rays through its old position could still hit. While automatic BVH rebuilds are enabled, a move is now detected and triggers a rebuild on the next query, and a rebuild refreshes the cached vertices of exactly the primitives that moved. A move is not detected once the geometry has been marked clean with `Context::markGeometryClean()`, or by a target-restricted query whose target set is unchanged.
- Fixed the target-restricted `CollisionDetection::findCollisions()` overload leaving the BVH narrowed to its targets after it returned, so every later query that meant "all geometry" (the unrestricted `findCollisions()` overloads, `castRay()` with no targets and the batched ray casters) silently ignored all primitives outside those targets until `rebuildBVH()` was called by hand. While automatic BVH rebuilds are enabled, a query-scoped restriction is now widened automatically by the next all-geometry query; a BVH explicitly built over a subset with `buildBVH()` keeps its restriction, including across a target-restricted query, which no longer discards it.

## Plant Hydraulics

- Fixed `PlantHydraulicsModel::run()` computing stem and root water potentials from the whole-plant sum of `latent_flux` (W) instead of the leaf-area-weighted mean (W/m^2) that the per-unit-area hydraulic conductances require. Root and stem water potential drops below the soil water potential were scaled by the plant's total leaf area in m^2, so a 50 m^2 tree was 50x too dry and a 0.05 m^2 seedling 20x too wet; leaf water potentials inherited the error through the stem potential. Plants with exactly 1 m^2 of leaf area were unaffected.

## Boundary Layer Conductance

- Fixed the "InclinedPlate" model of `BLConductanceModel::run()` returning a boundary-layer conductance in m/s instead of mol/m^2/s when the wind speed is zero. The pure free-convection branch omitted the molar density of air, so still-air conditions (greenhouses, night, chambers) gave a conductance about 41 times too small, with a corresponding jump when the wind speed crossed from zero to any positive value; the energy balance then saw a nearly insulated leaf and the stomatal models a boundary-layer conductance far too low.

# [1.3.85] 2026-09-08

## Core

- Fixed `helios::randu()` and `helios::randu(int,int)` drawing from vastly fewer distinct values on Windows than on Linux or macOS. They were built on `std::rand()` scaled by `RAND_MAX`, which is 32767 under MSVC against 2147483647 under glibc, so on Windows `randu(0,N-1)` could only ever return one of about 32768 evenly-spaced values -- for any collection larger than that, most elements could never be selected. Both now use `std::mt19937`, which behaves identically on every platform, backed by a thread-local engine so the functions remain safe to call from parallel regions.
- Fixed `helios::randu(int,int)` and `Context::randu(int,int)` returning the two endpoints of the range only about half as often as the interior values, because a continuous draw over a half-open interval was rounded to nearest. Both now use `std::uniform_int_distribution` and are uniform over the inclusive range; `LiDARcloud` leaf-group and triangle index draws were the main callers affected.
- Added `helios::seedRandomGenerator()`, which seeds the generator behind the free-function `helios::randu()` so that a run using it can be reproduced. The generator is otherwise seeded non-deterministically from `std::random_device`, replacing the previous dependence on whether some plug-in had called `std::srand()`. The generator is shared across threads and access to it is synchronized, so a seed set from any thread applies to every subsequent `randu()` call; note that seeding fixes the sequence of values drawn rather than which thread draws which value, so it does not by itself make a parallel loop reproducible.
- Meshes loaded by `Context::loadOBJ()` and `Context::loadPLY()` from a file that supplies no vertex normals now have normals generated from the mesh connectivity, and report `NORMAL_SOURCE_COMPUTED` rather than `NORMAL_SOURCE_NONE`. Most exporters omit normals unless explicitly asked, so such a mesh previously fell back to per-face normals and rendered faceted however the shading model was configured -- a curved imported surface could not be shaded smoothly without the caller calling `Context::computePolymeshObjectVertexNormals()` itself. Normals are blended across every edge so that the mesh's shared-vertex topology is left intact and queries such as `Context::isPolymeshObjectClosed()` and `Context::getPolymeshObjectVolume()` are unaffected; normals authored in the file are never overwritten.
- Fixed `helios::validateOutputPath()` appending a redundant separator to a path that already ended in one, producing `"dir//"` or, on Windows, `"dir\/"`. The trailing-separator check was applied to `std::filesystem::path::parent_path()`, which never retains a trailing separator, so the condition was always true.
- `Context::loadOBJ()` now returns the same UUIDs in the same order on every run regardless of thread count. Triangles were appended to a shared list from inside an OpenMP critical section taken once per triangle, so the ordering followed thread scheduling; each face now fills its own bucket and the buckets are concatenated in face order, which also removes the lock contention that made the loop slower with more threads.
- Fixed `Context::addTile()` producing untextured black geometry when given a texture file. The sub-patches were created with the texture but then assigned the default material, and because the renderer takes the texture from the material rather than from the primitive, the tile drew as the default material's flat color; `Context::getPrimitiveTextureFile()` also reported no texture for them. A texture-carrying material is now created and shared across the tile's sub-patches, matching `Context::addPatch()` and `Context::addTileObject()`, which were unaffected. This is what made the leaves of the "olive" plant-architecture library model render as black rectangles.
- Fixed the textured overloads of `Context::addTile()`, `Context::addTileObject()` and `Context::addAdaptiveTileObject()` not invalidating the `Context::getAllUUIDs()` cache. These build their sub-patches directly rather than through `Context::addPatch()`, so a textured tile added after anything had already called `getAllUUIDs()` was absent from every subsequent call until some other operation happened to invalidate the cache, leaving the primitives invisible to any caller that enumerates the Context. The untextured overloads delegate to `addPatch()` and were never affected.
- The CMake project template now warns when no build type is specified and the build defaults to Debug, and prints a reminder on multi-config generators that `cmake --build . --config Release` is required. An accidentally-Debug Helios build is dramatically slower, most severely on Windows where MSVC's Debug defaults add `/RTC1` instrumentation around every local, with nothing in the output to explain the slowdown.

## Collision Detection

- Fixed cone-based queries returning a different answer on each run against unchanged geometry. `CollisionDetection::findNearestSolidObstacleInCone()` and `CollisionDetection::detectGapsInCone()` spread sample directions across the cone, and those directions were drawn from a `std::mt19937` seeded from `std::random_device` on every call, so the reported obstacle distance and direction varied between runs and an obstacle covering only part of the cone was found on some runs and missed on others -- with no way for a caller to seed it and reproduce a result. The samples are now placed on a deterministic equal-area spiral over the cone, which is both repeatable and more evenly distributed than the random draws, so small obstacles that random clumping could hide in are now resolved.
- `CollisionDetection::calculateVoxelRayPathLengths()` now returns hits in a fixed, ray-ordered sequence rather than one that depends on thread scheduling, and no longer takes a lock for every ray-voxel intersection. The loop is parallelized over voxels instead of rays so each iteration is the sole writer of its own result.

## Plant Architecture

- Fixed plant organs built from more than one material keeping an internal auto-generated label on all but the first material, which left them out of `Context::listMaterials()` and so unreachable for the user to edit. Only the material of the object's first primitive was renamed to the descriptive `<plant>_<shoot type>_<organ>` form. The olive leaf, which is built from separate upper- and lower-surface textures, was the case in the library: its lower surface never appeared in the material listing.
- Cowpea pods in the "cowpea" library model now sit above the foliage rather than inside it. Floral buds were allowed on every node of a second-order shoot and on every node above the fifth of the main stem and its branches, so most peduncles started at about half the canopy height, and the pendant pod hung back down into the leaves even though the peduncle itself reached the canopy surface. Floral buds are now confined to the upper half of every shoot, and the plant has fewer, longer internodes (13 nodes of up to 4 cm, in place of 20 of 2.5 cm) and slightly smaller leaflets, which halves its leaf area to about 0.8 m2 per mature plant in line with field measurements. `ShootParameters::flower_bud_break_probability` was raised to keep the number of peduncles per plant unchanged. Pods hidden from above by the plant's own leaves fell from about 16% to 5% of pod area on an isolated plant.
- Leaves generated by `GenericLeafPrototype()` now carry vertex normals, so they shade smoothly rather than as flat facets. The blade is assembled from individual triangles, which supply only a face normal, and Helios does not synthesize vertex normals implicitly -- so a generated leaf rendered faceted no matter how the renderer was configured, and the midrib fold and lateral curl described by the leaf lattice read as flat panels. Normals are computed across every edge of the blade, which leaves the vertex ordering of the underlying lattice unchanged. Affects every species whose leaves are generated rather than loaded from a mesh file.
- Fixed a child shoot losing its outward offset from the parent as soon as the plant grew. `Shoot::updateShootNodes()`, which runs whenever a parent's nodes are rebuilt and therefore on every timestep, re-derived a child's base as the parent's node position alone, discarding the offset that `addChildShoot()` had applied to seat the branch on the parent's surface. Every branch dropped onto the axis of the shoot it came from, and because the correction cascades to descendants the displacement compounded down each branch order; a manually-built skeleton was pulled visibly out of shape by the first call to `advanceTime()`. The offset is now re-applied on each update, cached per shoot and re-derived only when the parent internode turns or thickens.
- Fixed `PlantArchitecture::addChildShoot()` placing the base of a child shoot at the wrong point on its parent when the parent phytomer has no petioles. The base is offset by the parent's radius so the branch emerges from the parent's surface, but with no petiole present to give a direction the offset was applied along the parent's own internode axis, which points along the parent rather than away from it. On a petiole-free plant -- a leafless woody skeleton, or any shoot type defined with `petioles_per_internode = 0` -- every child was therefore shifted up its parent by 0.9 of the parent radius instead of outward. The direction is now taken from the child's own base rotation, so the base is displaced radially and toward the side of the parent that child actually grows out of; two shoots leaving one node in opposite directions no longer emerge on the same side. Plants whose phytomers carry petioles are unaffected.
- Added `PlantArchitecture::addShootFromNodePositions()`, which builds a shoot from caller-supplied internode node positions and radii rather than generating its path from the shoot type's curvature and tortuosity parameters. This is intended for reconstructing a plant from measured geometry such as a QSM, a digitized skeleton or photogrammetry: a shoot built through `addBaseStemShoot()`, `appendShoot()` or `addChildShoot()` is extrapolated from its base rotation and so cannot follow a measured curve, and approximating one by chaining many short shoots produced a separate capped tube object per link, leaving visible gaps wherever the path bent. The whole branch is now one shoot rendered as one continuous tube, with the usual phytomer, bud, petiole and leaf structure, and it can be grown normally with `advanceTime()`.
- Added `PlantArchitecture::setPetioleNodePositions()` and `PlantArchitecture::setPetioleLeafGeometry()`, which prescribe the path of a petiole and the base position, orientation and size of the leaves on it from measured geometry. These are the organ-level counterpart of `addShootFromNodePositions()`, intended for reconstructing a plant from per-organ-labelled measurements such as a segmented point cloud, where petiole path and leaflet placement were previously generated from the shoot type's parameters with no way to set them. The prescribed geometry is not re-scaled, re-placed or re-seated by `advanceTime()`, and prescribed leaves are additionally exempt from the self-weight droop. Petiole node counts are free, but the number of leaves on a petiole is fixed when the phytomer is created and a mismatch is rejected; leaf rotations are given in the petiole and internode frame rather than the world frame.
- Fixed the random placement of berries within a grape cluster using `std::rand()` scaled by `RAND_MAX`, which gave Windows roughly 65000 times fewer distinct positions than Linux and so made clusters visibly less varied there. Berry positions now draw from `helios::randu()`, which behaves identically on every platform.
- Fixed the pipe model that sets internode girth counting the area of every inflorescence, not just a terminal one, so that a woody plant's trunk thickened in response to its own bloom. Because the radius only ever increases, the thickening was permanent: an eastern redbud, which is cauliflorous and flowers heavily on old wood, kept a trunk sized by flowers it had long since dropped -- on a QSM-based reconstruction the bloom was 99% of the trunk's supported area for about two weeks and left the trunk 87% thicker for the rest of its life. Only an inflorescence borne on the shoot apex of a herbaceous stem now counts, via the new `Phytomer::inflorescenceLoadsStem()`, which is what the term was introduced for in v1.3.84; the sorghum and maize culm taper is unchanged. `Phytomer::getInflorescenceArea()` still reports the area of every inflorescence.
- Added an overload of `PlantArchitecture::addShootFromNodePositions()` taking a separate growth shoot type, so that the parameters used to build measured geometry and the parameters governing how that shoot grows afterwards no longer have to be the same. Reconstructing measured wood requires zeroing curvature and tortuosity and raising `ShootParameters::max_nodes` above the longest measured branch, and until now those build-time values also governed new growth: shoots grown from a reconstruction came out perfectly straight and never reached their node cap, with no error raised. The growth type now supplies the node caps, the gravitropic curvature of phytomers added at the apex, and the type of the shoots the vegetative buds on measured wood produce; `ShootParameters::girth_area_factor` and bud-break probability are deliberately still taken from the shoot the wood belongs to. The existing single-type form is unchanged. `PlantArchitecture::duplicatePlantInstance()` and `PlantArchitecture::readPlantStructureXML()` both carry the growth type across, so a copied or reloaded reconstruction grows the same way the original did.
- Fixed `PlantArchitecture::readPlantStructureXML()` restoring petioles at slightly the wrong angle, which tilted every leaf hanging from them. The axes a petiole was built from were not saved, so reading re-derived them by replaying the rotation chain against the previous phytomer's petiole axis; because each phytomer was reached through the one below it, small discrepancies accumulated along a shoot and were worst at the growing tip, where a phytomer a day or two old came back with its petiole up to 6 degrees out. `writePlantStructureXML()` now records the axes and reading uses them directly. Over a twenty-seed sweep the worst cowpea leaf moved from 12.7 degrees off the grown plant's orientation to 0.18. Files written before this change are still read with the previous re-derivation.
- Fixed a child shoot being lifted off the node it attaches to instead of being pushed out of its parent's surface, leaving a visible gap at the junction with the parent's petiole exposed through it. The base of a child shoot is displaced one parent radius "outward", but the direction used was the parent's raw petiole axis, and a petiole is not perpendicular to the stem -- a soybean petiole leaves at roughly 22 degrees from the stem, so most of the displacement went up the stem rather than away from it. Only the component perpendicular to the parent's axis is now used, so the offset is purely radial and the shoot meets its parent at the attachment node. Species whose petioles leave the stem near perpendicular are largely unaffected.
- Fixed a shoot created by `PlantArchitecture::appendShoot()` being displaced sideways off the axis of the shoot it continues. An appended shoot carries its parent's axis onward from the tip, but `Shoot::updateShootNodes()` re-seated it using the rule meant for a lateral branch, offsetting the base by 0.9 of the parent's radius away from the parent's axis; because the offset is proportional to the radius it grew as the stem thickened, and it was re-applied on every timestep. In the soybean, bean and cowpea models this appeared as the trifoliate shoot floating off to the side of the unifoliate shoot below it instead of continuing it, with the break widening as the plant matured. Appended shoots are now seated on the parent's axis, and `appendShoot()` places its base on the parent's node tip so that the first timestep of growth no longer shifts the shoot. Lateral branches from `addChildShoot()` keep their outward offset and are unaffected.
- Fixed `PlantArchitecture::readPlantStructureXML()` failing outright on any plant whose shoot type sets `petiole.petioles_per_internode = 0`, which is the usual configuration for a leafless woody skeleton. Reading such a file raised `ERROR (Phytomer::getPetioleAxisVector): Petiole index out of range` because the parent petiole axis was read directly rather than substituting the notional "ghost petiole" that shoot construction uses for a petiole-free phytomer, so a plant of this kind could be written but never read back. The substitution is now available to any caller as the new `Phytomer::getPetioleAxisVectorOrGhost()`.
- Fixed `PlantArchitecture::writePlantStructureXML()` discarding the shape of a shoot built by `PlantArchitecture::addShootFromNodePositions()`. Only per-phytomer lengths and angles were written, and the reader rebuilt the path by integrating forward from the shoot's base rotation -- which is identically zero for a prescribed shoot, since its direction comes from the measured node positions rather than from a rotation. A reconstruction saved and reloaded therefore came back standing vertical with its internode lengths intact and its measured shape lost. The node positions and radii are now written and the shoot is rebuilt from them, reproducing the path exactly; files for shoots that were not built this way are unchanged.
- Fixed `Shoot::sampleVegetativeBudBreak()` and `Shoot::sampleVegetativeBudBreak_carb()` reading vegetative bud-break probability from the plant's stored shoot type rather than from the shoot itself. A shoot holds its own copy of its type's parameters, taken when it was created, so setting `ShootParameters::vegetative_bud_break_probability_min`, `_max` or `_decay_rate` on an individual shoot obtained from `PlantArchitecture::getPlantShoot()` had no effect: the shoot went on breaking buds at its type's rate, and nothing indicated the setting had been ignored. This contradicted the documented behavior that bud-break probability is taken from the shoot bearing the bud, and it mattered most for reconstructions from measured geometry, where per-shoot parameters are the point. Both methods now read the shoot's own parameters, matching `ShootParameters::growth_requires_dormancy`, which the same calculation already read from the shoot; neither method is `const` any longer. Every library species assigns these probabilities as constants that a shoot inherits unchanged, so no library plant breaks buds at a different rate than before; the sequence in which buds are sampled does change, however, so a plant built from a given seed may differ in detail from one built with the same seed previously.
- Fixed `PlantArchitecture::appendShoot()` crashing when the parent shoot had no phytomers. The guard for this case had an empty body, so rather than raising an error the call went on to read the parent's last phytomer from an empty list and segfaulted. A shoot is left in this state by `PlantArchitecture::pruneBranch()` called with a node index of 0, which removes all of its phytomers while keeping it in the plant's shoot list.
- Added `PlantArchitecture::getPlantLeafAreas()`, `PlantArchitecture::getPlantInternodeLengths()` and `PlantArchitecture::getPlantLeafInclinations()`, which report the leaf area, internode length and leaf inclination the model actually produced, measured from the built geometry and returned one entry per organ. `getCurrentShootParameters()` reports a shoot type -- the distributions a parameter is drawn from -- and there was previously no way to ask what the plant was delivered with; the existing introspection is per-plant aggregates such as `getPlantHeight()` and `getPlantLeafCount()`. The distinction matters for calibration because a `RandomParameter` caches its first draw and a shoot holds a copy of its type's parameters, so a plant can be built with no variation at all while its parameters describe a wide spread -- a difference that previously required exporting the geometry and computing statistics outside Helios.
- Fixed `PlantArchitecture::advanceTime()` and `PlantArchitecture::duplicatePlantInstance()` building new phytomers from the plant's stored shoot type rather than from the shoot they belong to. A shoot holds its own copy of its type's parameters, and `advanceTime()` already read the internode radius from that copy in the same call, so every other field -- petiole, leaf and inflorescence -- ignored anything set on an individual shoot: a shoot given a longer petiole through `PlantArchitecture::getPlantShoot()` grew new phytomers at its type's petiole length instead, with nothing to indicate the setting had been dropped. `advanceTime()` now reads the shoot's parameters, and `duplicatePlantInstance()` reads each phytomer's own, so a copy reproduces the plant it was made from even where that plant's parameters were changed part-way through its life.

- Fixed the maize library model's flag leaf running into the tassel. `Phytomer::createPetiole()` pins the last phytomer on a shoot to a near-zero 5 degree petiole pitch, and the terminal floral bud carrying the tassel is anchored to the same node tip with maize's peduncle pitch of 0, so the flag leaf left the culm on very nearly the peduncle's own axis and the blade passed through the tassel. `MaizePhytomerCreationFunction()` now adds 18 degrees back as a solid-body petiole rotation, lifting the blade clear of the peduncle while leaving it the most erect blade in the canopy -- roughly 34 degrees from vertical against about 51 for the lower canopy, which is the posture maize's erectophile upper canopy calls for. The correction stays below the 25 degrees sorghum uses for the same problem, since the sorghum flag leaf is the most horizontal leaf on the plant and maize's is the most erect.
- Corrected the maize library model's upper-canopy leaf sizes, which were far too large. Maize leaf area peaks at or just below the ear and falls away sharply over the last few ranks, but the taper in `MaizePhytomerCreationFunction()` left the top three leaves at full size and the flag leaf at 0.59 of the largest leaf's area, against the 0.19-0.26 that fitted vertical profiles give for commercial hybrids. The taper is now a least-squares fit to those profiles and reaches full size about six ranks below the tassel, which puts the flag leaf near a quarter of the largest leaf's area. Total plant leaf area falls by roughly 12%, entirely in the top six ranks; plants of other species are unaffected.
- Fixed the maize library model's tassel spending its whole life undersized because it grew on the ear's clock. Inflorescence expansion is driven by the plant-level fruit-maturity threshold, which `buildMaizePlant()` sets to the 58-day R1-to-R6 grain-fill period of the ear, but a maize tassel is fully elongated at VT and does not grow afterward. The tassel was therefore at roughly 40% of full size at silking and still expanding at physiological maturity -- ten days after tasseling its primary branches measured 5.9 cm against a field value near 20 cm. The new `InflorescenceParameters::inflorescence_maturity_period` lets an inflorescence expand on its own schedule; maize sets it to 6 days and every other species is unaffected, since a non-positive value defers to the plant-level threshold as before.
- Enlarged the maize library model's tassel to field dimensions. `flowers_per_peduncle`, which sets the number of primary tassel branches, went from 7 to 11 (modern commercial hybrids carry 10-12), and `fruit_prototype_scale` from 0.15 to 0.195, giving a branch length near the 20 cm modal value measured on scanned tassels rather than 15.4 cm. `flower_offset` went from 0.1 to 0.095 to spread the larger number of branches over the upper 90% of the peduncle.
- Widened the insertion angle of the bindweed library model's first-rank lateral shoots (`primary_bindweed`) from 50-80 degrees to 70-80 degrees, so they leave the base stem closer to perpendicular without lying flat on the ground. Second-rank shoots (`secondary_bindweed`) keep the previous 50-80 degree range.
- Reduced the gravitropic curvature of the bindweed library model's primary and secondary shoots from 40 to 30, so the sprawling shoots bend toward the ground less sharply.
- Fixed the bindweed library model building every leaf on its edge, with the blade normals pointing along the shoot instead of upward. The model's `leaf.roll` of 90 degrees had been cancelling a stem-inclination term that v1.3.72 removed from the leaf roll, so from that version on the same value rolled each blade a quarter turn about its midrib on the ground-hugging shoots. The roll is now 0 and bindweed leaves face the sky again.
- Fixed the "grapevine_VSP" library model building a 10 cm trunk, leaving the cordons resting on the ground instead of on a fruiting wire. The `trunk_height` build parameter defaulted to 0.1 m against an internode length of 0.1 m, so the trunk came out one node tall; it now defaults to 0.8 m, restoring the eight-node trunk the model had before `trunk_height` was introduced. Callers that pass their own `trunk_height` are unaffected.
- Fixed a lateral shoot growing out of a thick woody stem starting at the diameter of that stem rather than its own. `PlantArchitecture::advanceTime()` seeded a shoot from a breaking vegetative bud with the parent phytomer's initial radius, so on the grapevine VSP trunk (`internode.radius_initial` 0.05 m) a new shoot began as a 5 cm-radius cone that tapered to 3 mm over a single internode, and because the pipe model only ever increases a radius the bulge was permanent. The new shoot's own `internode.radius_initial` is now used, clamped to the parent internode radius so a branch is never born thicker than the stem carrying it. Species whose shoot types all share a similar initial radius are essentially unaffected.
- Fixed flowers and fruit being aimed at the mirror image of their peduncle's azimuth, so that `InflorescenceParameters::pitch` no longer held the angle it is defined to hold. The rotation that turns an inflorescence prototype into the plane of the peduncle carrying it was applied with the wrong sign, which merely mirrors the fruit while the peduncle lies in the xz-plane but tips it out of that plane once the peduncle has a y-component -- so the peduncle's own elevation leaked into an angle that is measured relative to the peduncle. A rice panicle showed it most clearly: grains stood at roughly 70-80 degrees to a horizontal stretch of rachis, nearly perpendicular to it, and relaxed toward the configured 20-25 degrees only where the rachis drooped steeply. Species that set a pitch of zero are repositioned but keep the correct angle; those with a non-zero pitch (rice, maize, olive, bean, soybean, cowpea, tomato) had the angle itself wrong and are visibly changed. This is the same defect fixed for leaf blades in v1.3.84.

## Radiation

- Added `CameraProperties::exposure_target`, the target median scene luminance for `"auto"` exposure. It was previously hardcoded to 0.18, the middle-grey convention, which assumes a scene of grey-card mean reflectance; a canopy imaged from above is darker, so reproducing a real camera's output required a different value and there was no way to ask for one. Defaults to 0.18, so existing behavior is unchanged. A non-positive target is now rejected rather than blackening or inverting the image.

## LiDAR

- Fixed `LiDARcloud::getHitDataColumn()` and `LiDARcloud::getHitScanIDColumn()` computing iterator offsets through `long`, which is 32 bits on Windows against 64 bits on Linux and macOS. A point cloud with more than roughly 2.1 billion entries produced an out-of-range fill on Windows only; the offsets now use `std::ptrdiff_t`.
- `LiDARcloud::isMultiReturnData()` is now public. It reports whether the cloud contains multi-return data (any hit with `target_count` greater than 1), which selects which branch `LiDARcloud::triangulateHitPoints()` takes: multi-return data is triangulated from first returns only with an adaptive separation filter, while single-return data treats every return independently, and the two can differ substantially in reconstructed surface area. Callers assembling a cloud themselves previously had no way to query this and could only infer it from the fields they supplied.
- Added a `LiDARcloud::calculateLeafArea()` overload taking one G(theta) per grid cell, in grid-cell order, instead of a single value applied to the whole grid. This supports inverting leaf area for a canopy whose leaf-angle distribution varies in space -- typically with height -- which previously could only be done by triangulating the cloud; like the single-value overload it does not require `LiDARcloud::triangulateHitPoints()`. The vector must have one entry per grid cell and every value must be in (0,1], both of which are checked.
- Fixed `LiDARcloud::calculateLeafArea()` returning wrong leaf area for a terrain-following voxel grid, one built through the `LiDARcloud::addGrid()` overload taking per-column z offsets. Such a grid is not a regular lattice, but it could still be accepted as one by the fast voxel-traversal path, which reconstructs each cell's bounds from the grid origin and the cell's index alone and so ignored the per-column offset entirely. Beams were then attributed to the wrong voxels, with the vertically displaced ones receiving none at all and reporting empty or NaN leaf-area density. Grids carrying a column offset now take the per-cell path, which reads each cell's stored bounds; axis-regular grids are unaffected and keep the fast path.

## Project Builder

- Fixed the Windows file-open and save-as dialogs failing with "No file selected." for any file whose path exceeded 260 characters -- a OneDrive-synced Documents folder is enough to reach that -- because `GetOpenFileName` fails outright when the selection does not fit the caller's buffer. The dialogs now use a 32768-character buffer.
- Fixed the Windows file dialogs mangling paths containing non-ASCII characters, by switching from the ANSI entry points, which transcode through the process's active code page, to the wide-character ones with UTF-8 conversion.
- Fixed the Windows file dialogs leaving the process working directory wherever the user last browsed, which broke every subsequent relative asset path. `OFN_NOCHANGEDIR` is now set and the working directory is restored whether the dialog is accepted or cancelled; previously it was restored only on acceptance.

## Visualizer

- Fixed deep shadows taking on a colour cast -- typically maroon or purple -- instead of going neutrally dark. The shadow visibility term subtracted 0.35 for each of four occluded shadow-map samples from a starting value of 1.0 and was never clamped, so a fully-occluded fragment reached -0.4 and then *subtracted* the diffuse and specular terms from the ambient one. Because that subtraction is per-channel against colours that differ per channel, the channels crossed zero at different points and the darkest pixels shifted hue. Shadow visibility is now clamped so that full occlusion removes direct light rather than adding negative light.
- The `Visualizer` can now create a headless OpenGL context through EGL on Linux, binding directly to the GPU with no display server of any kind. Headless mode previously opened a hidden 1x1 GLFW window, which still requires X11 or Wayland, so `Visualizer` construction failed outright on a batch compute node and every plug-in built on it -- including `SyntheticAnnotation` -- was unusable there. EGL is tried first in headless mode and the GLFW path is used when it is unavailable, so behavior on machines that already worked is unchanged. Requires EGL at build time; without it the plug-in builds and behaves exactly as before.
- Fixed the `Visualizer` rendering an empty frame for a camera looking straight down (or straight up). The view matrix was built against a fixed +z world up vector, which is parallel to the view direction in that case, so `glm::lookAt` normalized a zero vector and every element of the matrix became NaN -- nothing rasterized, with no error reported. A nadir camera over a canopy is the standard overhead view, and it produced a blank image. Affects `Visualizer::plotUpdate()`, `Visualizer::printWindow()`, `Visualizer::getViewMatrix()` and everything built on them, including `SyntheticAnnotation`.
- `Visualizer::plotInteractive()` now raises an error when the `Visualizer` was constructed in headless mode, instead of driving a window that either was never shown or does not exist.
- Fixed rendering becoming progressively slower the longer a `Visualizer` was used, to the point that a large scene stopped completing at all. Text is drawn one texture per character, and `Visualizer::registerTextureGlyph()` did not check whether an identical character was already registered, so each rebuild of the colorbar -- which happens on every `Visualizer::plotUpdate()` -- added a texture for every letter of every tick label. The GPU texture array is sized to the number of textures, with every layer as large as the biggest texture in the scene, so each frame destroyed the array, reallocated it larger and re-uploaded every layer. On a plant scene with high-resolution leaf textures this moved hundreds of megabytes per frame and grew without bound. Textures are now deduplicated by content, so repeated rendering of an unchanged scene registers nothing new.
- `Visualizer::printWindow()` no longer re-renders the scene when the frame already on the GPU is the one it would capture. Combined with the fix above, calling `Visualizer::plotUpdate()` immediately before `printWindow()` -- a common pattern -- is now merely redundant rather than dramatically slower than calling `printWindow()` alone.
- `Visualizer::plotUpdate()` no longer rebuilds the colorbar and watermark when nothing that determines their appearance has changed, and no longer re-uploads the whole scene to the GPU on any frame that changed a single primitive. Previously the unconditional overlay rebuild dirtied geometry on every frame, which forced a full re-upload of every vertex, color and flag buffer in addition to the per-primitive update of the primitives that actually changed, so an unchanged scene uploaded itself twice per frame.
- Fixed the `Visualizer` reading past the end of its list of primitives to update when primitives had been deleted from the `Context` since the last render, and taking time quadratic in scene size to process those deletions.
- Fixed geometry rendering from stale GPU buffers after the `Visualizer` compacted its internal storage, which occurs once more than 250,000 primitives have been deleted or when `Visualizer::clearContextGeometry()` is called. Compaction renumbers every primitive's position in the buffers but did not flag them for re-upload, so primitives that had not otherwise changed were drawn using another primitive's data.
- Fixed the ACES filmic tone curve brightening the image instead of tone-mapping it, making rendered scenes washed out and lighter than the albedo they were given: a mid-grey surface rendered lighter than mid-grey. The published Narkowicz coefficients fit a pre-exposed version of the ACES response, and applying them to raw linear radiance lifts every value below roughly 0.73 rather than compressing the highlights. The documented input scaling that matches the unmodified ACES response is now applied. **Rendered images are darker and closer to their intended tones than in the previous version**; images produced with `Visualizer::disableLinearPipeline()` or `Visualizer::enableExactColorMode()` are unaffected.
- Fixed the `Visualizer` leaving texture 0 bound to the `shadowMap` and `phongMaterialTable` shader samplers, which made them incomplete on every draw call. The shadow map is allocated only on the first shadowed render and the Phong material table only while building geometry from a `Context`, so any scene that used neither -- the common case of an unshadowed render, or of geometry added directly to the visualizer -- drew with two samplers pointing at no texture. Rendering was correct, because neither sampler is read on that path, but macOS OpenGL drivers reported the invalid state once per process as `UNSUPPORTED (log once): POSSIBLE ISSUE: unit N ... is unloadable and bound to sampler type (Float)`. Both samplers are now given valid, inert contents when the visualizer is initialized: a 1x1 depth texture holding the far plane, which can never darken a fragment, and an empty material table.

## Synthetic Annotation

- `SyntheticAnnotation::render()` now computes every object's bounding box in a single pass over the rendered ID image, rather than rescanning the whole image once per object. Annotation time no longer grows with the product of object count and resolution: a 2592x2048 frame containing 15,800 labeled leaves went from over four minutes to sixteen seconds.
- Added `SyntheticAnnotation::setCameraFieldOfView()`, which sets the vertical field of view of the rasterized camera. The projection was previously pinned at the `Visualizer`'s 45 degree default, so annotations could not be made geometrically comparable to a camera specified elsewhere, such as a `RadiationModel` camera given by its horizontal field of view.
- Added `SyntheticAnnotation::enableRGBRendering()` and `SyntheticAnnotation::disableRGBRendering()`. The shaded RGB preview rasterizes the whole scene a second time and dominates the cost of `render()` on a large scene; it can now be skipped when only annotations are wanted.
- `SyntheticAnnotation` now records its internal per-primitive label ID under `syntheticannotation_labelID` rather than `object_label`. `object_label` is set as a *string* naming the organ or material by `PlantArchitecture`, `ProjectBuilder` and `Context::loadOBJ`, so annotating any scene built by those aborted with "Primitive data 'object_label' already exists with type 'string'", and the labels a scene carried were deleted when `render()` returned.
- `SyntheticAnnotation::render()` now draws its RGB pass headless, as the label pass already did. It previously required a display even when writing to a file, and took the windowed pixel readback path, which guesses whether the front or back buffer holds the current frame.

# [1.3.84] 2026-08-02

## Core

- Fixed the core build putting only the vendored zlib source directory on the include path, so that `zlib.h` resolved its `zconf.h` from the system rather than from the copy CMake generates to match the vendored library. On machines with system zlib headers installed, `global.cpp` was compiled against vendored zlib 1.3.1 declarations using a different version's configuration header and then linked against the vendored library; on machines without them (minimal containers, typical Windows setups), the build failed outright with `zconf.h: No such file or directory`. The build directory holding the generated header is now listed first, matching what the libjpeg and libpng lines already did.
- Fixed the vendored `zlib.h` including a stale checked-in `zconf.h.included` snapshot on Windows instead of the configuration header CMake generates. Windows builds bypassed the generated header entirely and so were unaffected by the include-path correction above; the vendored header now includes `zconf.h` on every platform, as upstream zlib does.
- Fixed first-party code compiling against a checked-in copy of libpng's `pnglibconf.h` while libpng's own sources used the copy generated by the build. The two configuration headers happened to agree, so the effect was latent rather than observable, but any change to libpng's generated configuration would have silently produced a single link mixing two different configurations. The generated header's directory is now on the include path and the redundant checked-in copy has been removed.
- Compound objects can now report which of their member primitives meet at each mesh vertex, via the new `Context::doesObjectHaveSharedVertexTopology()`, `Context::getObjectSharedVertexCount()`, `Context::getObjectPrimitiveSharedVertexIndices()` and `Context::getPrimitiveSharedVertexIndices()`. Two primitives meeting at a corner report the same index there, which lets a consumer average a per-face quantity onto the vertices they have in common. `Tube`, `Sphere`, `Cone`, `Tile` and `AdaptiveTile` derive this from their own lattice and `Polymesh` reports its face table, while `Box` and `Disk` report none because their faces are flat. The new `helios::VertexWeldMode` argument selects whether coincident vertices are welded everywhere (`WELD_FULL`) or only within a cross-section (`WELD_CROSS_SECTION_ONLY`), which for a swept surface keeps variation along the axis from being averaged away.
- `Context::getPolymeshObjectVolume()` now separates a mesh carrying a face table into its connected pieces and sums the volume of those that are closed, rather than refusing to compute a volume whenever any part of the mesh is open. A solid modelled alongside open decoration -- a fruit with its sepals or stalk -- now reports the volume of the solid; an error is raised only when no piece of the mesh is closed. A piece whose edges are shared by more than two facets is treated as not closed, since the divergence theorem returns a meaningless number for a self-intersecting surface.
- `Context::getPolymeshObjectVolume()` now also checks closure for a mesh that carries no face table, by matching up facets according to which of their corners coincide, and raises an error naming the number of unmatched edges. Such a mesh previously had the divergence theorem applied to it unconditionally, so an open surface returned a number that was not only meaningless but not even fixed, changing whenever the object was moved.
- Added the `HELIOS_PUSH_IGNORE_DEPRECATED` and `HELIOS_POP_IGNORE_DEPRECATED` macros to `global.h`, which suppress deprecation warnings across a region of code. These let a structure holding a deprecated member still copy it in its own assignment and duplication code without warning, so the warning stays pointed at the calling code the deprecation is aimed at.
- Added `Context::setPolymeshObjectVertices()`, which moves every shared vertex of a polymesh in a single call and pushes the new positions out to the member primitives, so that a mesh can be deformed without tearing. Transforming member primitives one at a time leaves each shared vertex wherever the last facet processed put it, and re-inverts the object transformation matrix for every facet. Texture coordinates are untouched and so is each primitive's solid fraction, which depends on the (u,v) coordinates rather than the vertex positions, so deforming a textured mesh does not re-rasterize its alpha mask. Vertex normals are not recomputed and no longer describe the deformed surface; call `Context::computePolymeshObjectVertexNormals()` if exact normals are needed.

## Radiation

- Added `RadiationModel::enableCameraFluxSmoothing()`, `RadiationModel::disableCameraFluxSmoothing()`, `RadiationModel::isCameraFluxSmoothingEnabled()` and `RadiationModel::getCameraFluxSmoothingCreaseAngle()`, which reconstruct camera images by interpolating the outgoing flux across each facet instead of holding it constant. The flux is first averaged onto the vertices neighbouring facets share, weighted by facet area, so a coarsely tessellated curved surface -- a `Tube` trunk, a low-subdivision `Sphere`, an imported mesh -- no longer renders as flat panels with a step at every facet edge. Smoothing is off by default and changes what the camera reports: a pixel carries a blend of the primitive's flux with those of its neighbours rather than the `radiation_flux_*` value of the primitive behind it. The radiation solve, the `radiation_flux_*` values themselves, and the pixel-label and depth images are unchanged. It applies to `Tube`, `Sphere`, `Cone`, `Tile`, `AdaptiveTile` and `Polymesh` objects carrying a face table; `Box`, `Disk` and primitives with no parent object are left alone. The crease angle, in degrees, is the angle between adjacent facet normals above which a `Polymesh` edge is treated as a hard edge and flux is not averaged across it.

## Plant Architecture

- Fixed the peduncle of a terminal inflorescence being sized independently of the culm it attaches to, so that the two tubes met in a visible step. The peduncle radius came from `PeduncleParameters::radius` while the internode radius came from the pipe model, which sizes each internode from the leaf area above it -- and the terminal internode carries the inflorescence rather than leaves, so it was left far thinner than the structure it supports. Sorghum rendered a 16 mm peduncle on a 6 mm culm tip and maize an 8 mm peduncle on a 15 mm culm tip, the error running in opposite directions. The peduncle is now derived from the culm-tip radius, and re-derived as the culm thickens rather than being fixed at the moment the floral bud appears. Affects terminal inflorescences on herbaceous stems (sorghum, maize, wheat, rice); a lateral peduncle, or one on a woody stem, keeps its configured radius.
- Fixed leaf expansion being measured against the shoot type's `prototype_scale` rather than against each leaf's own maximum size. `current_leaf_scale_factor` records how far a leaf has grown toward its own full size, so a leaf whose target is smaller than the species maximum -- which a phytomer creation function sets to vary leaf size by rank -- could never reach a fraction of 1 and had the shortfall applied to its size a second time: a blade set to 62% of the maximum rendered at 62% of 62%. Species whose leaves are a uniform size along the shoot are unaffected, since for them the two divisors are equal; maize's upper leaves, which do taper, were undersized and are now slightly larger.
- Sorghum leaf size now peaks a few ranks below the flag leaf and declines toward it, instead of saturating at the maximum from the fourth node upward and leaving the flag leaf fractionally the largest blade on the plant. The rank of the largest leaf follows the relation of Demarco et al. (2026), placing it three to four ranks below the flag leaf, and the flag leaf is now the smallest blade at roughly 40% of the largest by area.
- The pipe model that sets internode girth now counts the area of a terminal inflorescence alongside the leaves it supports, via the new `Phytomer::getInflorescenceArea()` and `Shoot::sumDownstreamInflorescenceArea()`. A sorghum panicle is borne above every node on the culm but contributed nothing to the calculation, so the upper culm tapered toward the leaf area of the topmost phytomer alone and finished at 6 mm below a head of roughly 1670 cm2 -- a 3.5:1 base-to-tip taper against the roughly 1.4:1 measured in the field. The culm tip is now 11.6 mm and the taper 2.0:1. Under the carbohydrate model only the rendered radius changes; the shoot's structural carbon demand stays tied to leaf area.
- Sorghum now builds the elongated internode that separates the flag leaf from the last normal leaf below it. The internode scale ramp saturated well below the top of the plant, so this internode was the same length as the eleven beneath it and the flag leaf sat among the upper canopy instead of standing clear of it. Its length is set equal to the peduncle length, matching the flag-leaf-to-panicle-base distance, which lengthens a mature culm by roughly 0.22 m.
- Fixed leaf blades being aimed at the mirror image of their petiole's azimuth. The rotation that turns a leaf prototype to face the same way as the petiole it hangs from was applied with the wrong sign, so a petiole pointing at +45 degrees received a blade pointing at -45 degrees. On an upright stem every leaf was mirrored by the same amount and the arrangement stayed symmetric, so the error was not visible; on a leaning or curving stem the blades of alternate phytomers were folded back across the stem instead of extending away from it, which made a distichous grass such as maize or sorghum read as a wedge of leaves rather than a flat fan. Affects every species, most visibly grasses, whose petiole is a short stub and whose blade is therefore the only visible part of the leaf.
- Leaves generated by `GenericLeafPrototype()` now carry an indexed face set describing their own connectivity, built from the lattice the blade is generated on and from the connectivity `Context::loadOBJ()` retained for the petiolule. Consumers that reconstruct a field across a mesh -- camera flux smoothing in the radiation model, `Context::computePolymeshObjectVertexNormals()`, shared-vertex OBJ and PLY export -- previously saw a generated leaf as a bag of loose triangles.
- Fixed generated leaves losing their petiolule. Since v1.3.83 `Context::loadOBJ()` groups what it loads into a `Polymesh` object, so the petiolule primitives already had a parent when `Context::addPolymeshObject()` assembled the leaf and were silently dropped from it. Every leaf copied to a phytomer was built without its petiolule, and the prototype's own petiolule was left behind at the origin at prototype scale as stray geometry belonging to no plant.
- Fixed the `object_label` primitive data of a petiolule being overwritten with "leaf". The label arrives from the group name in the petiolule's OBJ file, and labelling the whole leaf object afterwards erased it, so the petiolule was given the blade's colour and, downstream, the blade's optical properties rather than the petiole's.
- The `CowpeaPod` asset is now watertight: the open stem end is capped and the coincident vertices at the pod tip are welded into one. The pod body previously enclosed no volume, so `Context::getPolymeshObjectVolume()` -- which the carbohydrate model calls for each fruit -- had no volume to report for it. The sepal remains open by design.
- Leaves can now droop under their own weight as they grow, via the new `LeafPrototype::flexibility` parameter. A blade is treated as a cantilever loaded by its own weight, so it points along the shoot's growing direction while it is small and bends over as it expands, which is the behaviour of maize, sorghum and wheat blades and of most leaves generally. The parameter is dimensionless and describes how far the leaf bends once fully grown; it defaults to zero, which gives the previous rigid leaf that keeps its prototype shape at every size, so species that do not set it are unchanged. Because droop follows from the leaf's size, there is no age or time term: a leaf prevented from growing to full size also stays comparatively straight.
- Deprecated `LeafPrototype::leaf_buckle_length` and `LeafPrototype::leaf_buckle_angle`, which bent a leaf by a fixed angle at a fixed station along its length to approximate the same self-weight droop now modelled directly. Existing code continues to compile and still produces a drooping leaf: when `LeafPrototype::flexibility` is left at zero and a buckle angle was set, the pair is converted to the flexibility that reproduces the same tip droop on the fully-grown leaf, so the mature leaf is shaped much as before but now reaches that shape gradually as it expands rather than being born with it. An explicitly-set `LeafPrototype::flexibility` always takes precedence, so code that has already migrated is unaffected. Setting either parameter now raises a compiler deprecation warning naming the replacement; both will be removed in a future version. The maize, sorghum and wheat models now set `LeafPrototype::flexibility` directly.
- Added the `LeafPrototype::longitudinal_curvature_exponent` parameter, which sets how the longitudinal curvature of a generated leaf blade is distributed along its length: the deflection at a fraction `x` along the blade is `longitudinal_curvature * x^longitudinal_curvature_exponent`, so a larger exponent keeps the inner blade straight and turns the leaf over near the tip while a smaller one spreads the bending evenly. It defaults to 4, the value the blade was previously hard-coded to, so existing species are unchanged; the maize, sorghum and wheat models now use 3. The exponent also now drives the derivative that orients the wave displacement along the leaf surface, which was previously fixed at the quartic and so described the wrong surface for any other exponent.
- The sorghum model has been recalibrated against side-view imagery of young potted plants. Leaf blades now emerge directly from the culm instead of being held roughly six stem radii away on a zero-radius petiole, which made them look detached; blades leave the culm more steeply, taking their orientation from the arc of the blade rather than from a shallow attachment; the base stem is built upright, since a tilted base made the two ranks of leaves converge rather than sit opposite one another; and the blades arc over along their length rather than staying straight and turning down only at the tip, so the proportion that arc past their own apex matches the imagery. Plants are visibly different from previous versions: more upright, with leaves attached at the stem and the lower leaves arching over.
- The maize, sorghum and wheat models no longer apply a basal `petiole_roll`. The roll term decays as `exp(-70x)` along the blade, so at the value these models used it acted only on the innermost few percent of the leaf, kinking it upward just as it left the stem. Grass leaves from these three models now leave the stem smoothly.
- Added `ShootParameters::getChildShootTypeLabels()` and `ShootParameters::getChildShootTypeProbabilities()`, which read back the child shoot types set by `ShootParameters::defineChildShootTypes()`. These values were previously held in protected members readable only from within the plug-in, so a caller that reconstructs a `ShootParameters` from its individual values, such as a scripting-language binding, could not inspect a shoot type's branching. `defineChildShootTypes()` remains the only way to set them, so its validation is unchanged. Note that child shoot types referring to a shoot type that was never defined are dropped from the stored parameters when a plant is built, without renormalizing the remaining probabilities, so the values read back are not necessarily those originally supplied.
- Added the `LeafPrototype::flexibility_taper` parameter, which sets how much more compliant the leaf blade is at its tip than at its base. A blade of uniform stiffness bends hardest at its clamped base and stays nearly straight over its outer half, whereas a real grass leaf stays straight near the base and curves hardest toward the tip, because the midrib carrying the load thins several-fold along the blade. The parameter is the ratio of tip compliance to base compliance and defaults to 1, the uniform blade the deflection previously assumed, so existing species are unchanged; the maize, sorghum and wheat models now use 150. A tapered blade bends much further for a given `LeafPrototype::flexibility`, so the two must be calibrated together.
- Added the `LeafPrototype::flexibility_aging` and `LeafPrototype::flexibility_aging_max` parameters, which let a blade go on bending after it has stopped growing. Droop previously followed from the leaf's size alone, so the oldest leaves on a plant were also its straightest; the blade's compliance now follows a sigmoid in leaf age, holding its emergence value for roughly twice `flexibility_aging` days, softening over a window of similar length, and levelling off at `flexibility_aging_max` times its starting value. `flexibility_aging` is in days and defaults to zero, which disables ageing entirely and reproduces the previous size-only droop, so existing species are unchanged; the maize, sorghum and wheat models now use 11 days with a ceiling of five to six times.
- The maize, sorghum and wheat leaf shapes have been recalibrated around the new compliance taper and ageing. `LeafPrototype::flexibility` falls to 1.5-2.3 for maize, 1.2-1.8 for sorghum and 0.8-1.2 for wheat, because the taper multiplies it many times over along the blade; the longitudinal curvature exponent is 3 for all three and the baked-in longitudinal curvature narrows to -0.07 through -0.04, so a blade is now built almost straight and acquires its arc by bending rather than emerging with it; and the surface wave amplitude drops from 0.1 to 0.06 for maize and sorghum and 0.04 for wheat. Plants of all three species are visibly different from v1.3.83: a blade stays straight where it leaves the culm and turns over toward its tip, and how far it arcs depends on its age, so the oldest leaves arch right over while those still in the whorl stay nearly straight.
- Fixed `PlantArchitecture::readPlantStructureXML()` not restoring the mature leaf size, so that saving and reloading a plant repeatedly inflated it without bound. The reader left `leaf_size_max` at the species prototype default -- around 1 m against a real leaf of around 0.1 m -- and `PlantArchitecture::writePlantStructureXML()` writes each leaf's scale as the product of that value and the current growth fraction, so the second file described leaves an order of magnitude too large. The error was invisible after one save/load cycle and compounded on each one after that: a cowpea reloaded twice reported roughly eighty times its true leaf area and three times its true height. Reported with a reproducer by Heesup Yun (@heesup, GitHub #101), who also contributed the original patch.
- Fixed `PlantArchitecture::readPlantStructureXML()` not restoring the scalar peduncle parameters -- length, radius, pitch, curvature and roll -- that it had just rebuilt each peduncle's geometry from. Only the peduncle vertices were restored, so `PlantArchitecture::writePlantStructureXML()` fell back to drawing fresh random values for them and every re-export of a saved plant gave its fruit and flowers a different set of stalks.
- Fixed fruit growing by the square of their growth fraction on each save/load cycle. `PlantArchitecture::readPlantStructureXML()` created fruit geometry at the mature size already multiplied by the saved growth fraction, which recorded that product as the fruit's mature size, so the next file's `<flower_base_scale>` was too small by the fraction and reloading it applied the fraction a second time.
- Fixed `PlantArchitecture::readPlantStructureXML()` discarding the internode curvature and yaw perturbations it parsed. The perturbations were used to rebuild the internode vertices but never stored on the phytomer, so `PlantArchitecture::writePlantStructureXML()` had none to write and a saved plant's branch tortuosity was lost on the first re-export, leaving straight internodes.
- Fixed `PlantArchitecture::readPlantStructureXML()` re-drawing the number of leaflets per petiole from the shoot type's random parameter when positioning leaflets, rather than using the count in the file. A trifoliate leaf could be positioned as though it were unifoliate and vice versa, so compound leaves on a reloaded plant were placed inconsistently with the leaves that had just been built for them.
- `PlantArchitecture::readPlantStructureXML()` now clears the floral buds a phytomer inherited from its shoot type when the file records none for it, and raises an error for a `<current_fruit_scale_factor>` outside the range zero to one instead of silently collapsing the fruit. A reloaded plant previously carried prototype buds that were never in the saved plant, and re-exported them.
- `PlantArchitecture::writePlantStructureXML()` no longer writes out shoots that have been pruned. A pruned shoot keeps its slot in the plant's shoot tree so that shoot IDs stay stable, but it has no phytomers and no geometry; writing it produced an empty `<shoot>` element that the reader rebuilt as a live shoot.
- Fixed `PlantArchitecture::writePlantStructureXML()` throwing while writing a petiole carrying exactly two leaflets. The index of the lateral leaflet was computed as one less than the tip index, which is zero for two leaflets, so the leaf size array was read at index -1.
- Fixed `PlantArchitecture::pruneGroundCollisions()` leaving a plant's geometry and its structure out of step. A shoot whose stem crossed the ground plane had its internode tube deleted from the `Context` but kept all of its phytomers in the shoot tree, so leaf area, carbohydrate transfer and `PlantArchitecture::writePlantStructureXML()` went on counting organs that no longer existed; the whole shoot and its descendants are now removed together.
- The peduncle roll written to XML by `PlantArchitecture::writePlantStructureXML()` is now the value that was actually used to orient the inflorescence, rather than a fresh draw from the roll parameter taken at the moment the file was written.
- Fixed the leaves of a plant restored by `PlantArchitecture::readPlantStructureXML()` being pointed in the wrong directions. The reader carried its own copy of the rotation chain that orients a leaf on its petiole, and the copy had drifted from the one the `Phytomer` constructor uses: the azimuth of the petiole was applied with the opposite sign, mirroring every blade; the roll of a unifoliate leaf used a different formula; the curvature-aware blade-up correction was missing entirely; and the leaflets of a species whose leaflets all attach at the petiole tip were arranged as though they were spaced along it. A reloaded cowpea's blades faced an average of 56 degrees away from the grown plant's, and a reloaded sorghum's up to 141 degrees. Both paths now share `Phytomer::orientLeaf()` and `Phytomer::compoundLeafRotation()`.
- Fixed `PlantArchitecture::readPlantStructureXML()` integrating petiole curvature over the segment length of the petiole at its current size rather than at its mature size, which is what the `Phytomer` constructor uses. Every petiole that had not finished expanding was rebuilt slightly straighter than it was grown, and the leaves hanging from it inherited the error.
- `PlantArchitecture::writePlantStructureXML()` now records which of a species' interchangeable leaf prototypes each blade was copied from, in a new `<leaf_prototype>` tag, and `PlantArchitecture::readPlantStructureXML()` uses it. The prototypes are different shapes, so drawing one at random on read - which is what happened before - rebuilt the plant from a different set of blades than the one saved. Files written without the tag still load and are assigned a prototype at random, since the information is not in them.
- `PlantArchitecture::writePlantStructureXML()` now records the age of each phytomer in a new `<phytomer_age>` tag, and `PlantArchitecture::readPlantStructureXML()` restores it and re-applies the self-weight deflection to every leaf it rebuilds. The compliance of a drooping blade follows its age, and neither the age nor the deflection survived a save and reload, so a restored grass came back with rigid, undrooped leaves however flexible its species is. Files written without the tag load with phytomers at age zero, as they did before.
- Fixed `Phytomer::scaleLeafPrototypeScale()` and `Phytomer::setLeafPrototypeScale()` moving the leaf growth fraction as well as the mature leaf size. A leaf's rendered size is `leaf_size_max` times `current_leaf_scale_factor`; these functions set the first of those, but also divided or multiplied the second by the same factor, so the geometry was scaled once while the product of the two was left unchanged or scaled twice. Everything reading that product was wrong by the scale factor: `PlantArchitecture::writePlantStructureXML()` wrote a leaf scale no geometry ever had, so reloading a saved plant came back with leaves the wrong size - individual young leaves by as much as a factor of four and whole-plant leaf area by five to twelve percent - and the self-weight leaf deflection bent each blade as though it were longer than it is. Affects every species whose phytomer creation function rescales the leaf prototype, which includes maize, sorghum, wheat, rice, tomato, cherry tomato, bean and strawberry; their leaves are slightly larger than in v1.3.83, by about two to nine percent of total leaf area, and the leaf prototype scale setters no longer alter petiole curvature.
- Fixed a very flexible leaf being drawn as a sharp hinge near its base followed by a dead-straight remainder. The self-weight deflection applied through `LeafPrototype::flexibility` clamped each station's tilt just short of straight down but let the underlying bend accumulator run on past it, so every station beyond the point where the clamp first bit was pinned at the same angle and no amount of stiffness taper could relieve it. The accumulator is now clamped along with the angle, and a blade this flexible curves smoothly along its whole length.
- Midrib folding of a leaf generated by `GenericLeafPrototype()` now tapers along the blade instead of being applied uniformly, so the leaf is channelled where the midrib is a prominent keel near the base and flattens out toward the tip; a uniform fold left the blade V-shaped right to its tip, which read as a stiff gutter rather than a leaf. The taper is normalised to average one over the blade, so `LeafPrototype::midrib_fold_fraction` still means the same average fold and existing species are not flattened -- only its distribution along the leaf changes.
- Maize leaves above the ear are now stiffer than those below it, so they hold the erect attitude their insertion angle gives them instead of arcing over to the horizontal. This is the erectophile canopy commercial maize has been bred toward, roughly 20-50 degrees from vertical above the ear against 60-80 below it. `MaizePhytomerCreationFunction()` scales leaf flexibility by node rank, fading the stiffening out below the ear so the lower leaves keep the arc they should have, and the maize petiole pitch narrows from -40..-20 to -30..-16 degrees.
- The rice, sorghum and wheat models now scale leaf size and internode length along the shoot independently, and their internodes lengthen with height rather than shortening. A single ramp served both before, which either stunted the lowest leaves -- already close to full length on a real plant while the internodes carrying them are the shortest on it -- or spread the base of the stem out too far, and it bunched the upper leaves together at the apex.
- The wheat model now produces a plant of realistic height and habit. `PlantArchitecture::buildWheatPlant()` passed a literal internode length that overrode the shoot type's `internode_length_max`, leaving the plant knee-high; the culm is now built from the shoot type's value over eight nodes rather than twenty, the leaves are larger (`prototype_scale` 0.22 to 0.36) with the flag leaf shorter than those below it, the peduncle is straight rather than curved at -100 degrees/m so the ear stands upright instead of drooping, and the flag leaf is tilted 28 degrees off the culm so it swings clear of the ear rather than passing through the spike.
- Fixed `PlantArchitecture::readPlantStructureXML()` rebuilding a species' interchangeable leaf prototypes as different shapes than the plant that was saved was built from. The prototypes carry random shape parameters -- cowpea's trifoliate blade draws its longitudinal curvature from a uniform distribution over -0.3 to -0.1 -- and both the reader and the `Phytomer` constructor build the set lazily, at the first phytomer that needs it, by which point the two have drawn different amounts of randomness from the `Context`. The `<leaf_prototype>` tag added in v1.3.84 therefore restored each blade to the right index of the wrong set of shapes: a reloaded cowpea's blades faced an average of 3 degrees away from the grown plant's, and individual leaves up to 12 degrees, from the moment of germination rather than accumulating with age. The set is now built from a private random stream keyed on `LeafPrototype::unique_prototype_identifier`, which does not depend on how much randomness has been drawn before it, and the mean discrepancy falls to 0.4 degrees. Leaf shapes differ from v1.3.84 for every species that defines more than one unique leaf prototype, because they are now drawn from a different stream.
- `AsparagusLeafPrototype()` now draws a cladode's curvature and roll from the leaf prototype's own random number generator rather than directly from the `Context`, so that it follows the private prototype stream above and a reloaded asparagus is rebuilt from the cladodes it was saved with. Asparagus cladode shapes differ from v1.3.84.
- Added `LeafPrototype::setRandomGenerator()` and `LeafPrototype::getRandomGenerator()`, which redirect every random shape parameter of a leaf prototype at a given generator and report the one in use, and `RandomParameter_float::setRandomGenerator()`, which does the same for a single parameter. A custom leaf prototype function should draw from `LeafPrototype::getRandomGenerator()` rather than from the `Context` if its blades are to survive a save and reload.

## LiDAR

- Fixed `LiDARcloud::gapfillMisses()` never returning for a scan acquired top-down (zenith angle decreasing over the course of a sweep). The edge-extrapolation step used the signed average change in zenith angle, whose sign only records the sweep direction, so on such a scan the extrapolation walked away from the angular bound it tests against and, once the step fell below the floating-point resolution of the accumulated angle, looped forever. The magnitude is now used, and a zero step is guarded in both the upward and downward extrapolation.
- Misses synthesized by the row/column gap-filling path now carry a `timestamp` reconstructed from their scan-grid cell. Multi-return returns are grouped into per-pulse beams by shared timestamp, and that grouping falls back to treating every return as its own beam if any point in the scan lacks the field, so gap-filling a multi-return cloud through this path previously collapsed the beam structure and made `LiDARcloud::calculateLeafArea()` compute transmission per return rather than per pulse, reporting an incorrect leaf area with no error raised.
- `LiDARcloud::gapfillMisses()` is substantially faster and uses far less memory when gap-filling a scan whose declared grid is much finer than its returns populate. Synthesized misses are now written directly into pre-sized columnar storage instead of being appended one at a time through a per-point `std::map`, grid occupancy is tracked in a bitset rather than a `std::set` of index pairs, and the fill loop runs in parallel over scan rows. On a 600 x 4000 raster filling 2.35 million misses this reduced the resident cost from 321 to about 168 bytes per synthesized miss and the call from 1.11 s to roughly 0.16 s. Filled points, their hit data, and the order they are added are unchanged.
- `LiDARcloud::calculateHitGridCell()` now rejects a hit lying outside the bounding box of every grid cell before testing the cells individually. Points far outside the voxel grid -- in particular the misses `gapfillMisses()` places at 20 km -- previously ran the full per-cell containment loop only to be assigned no cell. On a gap-filled cloud with a small grid this was measured about 19x faster, and the saving grows with the number of grid cells.
- The scan-grid row and column indices are no longer stored on each hit point. They were written by every hit-creation path and read by none, costing eight bytes on every point in the cloud plus a direction-to-row/column conversion per hit. The `row`/`column` hit data written by the ASCII importer and by `LiDARcloud::gapfillMisses()` is unaffected, as is the `LiDARcloud::addHitPoint()` overload that takes a row/column pair.
- Misses synthesized by the row/column gap-filling path are no longer stored as individual hit points. Because such a miss is fully determined by its scan-grid cell, the per-row angular model fitted during gap-filling and the scan origin, the whole population is now held as one bit per grid cell plus that model and reconstructed on demand. This is a persistent saving rather than a transient one -- it is the size of the cloud after `LiDARcloud::gapfillMisses()` returns -- and it scales with the declared grid rather than with the number of misses. On a 600 x 4000 raster filling 2.35 million misses the resident cost fell from 321 to about 14 bytes per miss and the call from 1.11 s to about 0.06 s. The misses remain visible through `LiDARcloud::getHitCount()` and every index-based accessor, and are collapsed into ordinary hit points automatically before any operation that renumbers the hit index space (adding or deleting a hit point, writing hit data or a grid cell), or on demand via the new `LiDARcloud::materializeMisses()`. `LiDARcloud::getVirtualMissCount()` and `LiDARcloud::hasVirtualMisses()` report how many are currently held this way. The timestamp-based gap-filling path is unchanged.
- Added `LiDARcloud::gapfillMissesCount()`, which gap-fills a scan (or every scan) and returns only the number of points added. The existing overloads return the position of every filled point, which for a fine scan grid is a large allocation a caller that only needs the count does not want.
- Added `LiDARcloud::reserveHitPoints()`, and the ASCII scan loader now counts a file's data lines and reserves before reading it. Growing the hit-point array and its scalar-data columns by repeated insertion reallocates each of them geometrically, and both the old and new buffers are live during every reallocation; for a cloud of tens of millions of returns that transient is gigabytes on top of the steady-state cost. On Windows it is charged against the system commit limit at allocation time, so a scan that fits comfortably once loaded could still fail while loading. Measured on 1.2 million returns, reserving cut allocated memory from 190 MB to 72 MB.
- Hit points now store their beam direction as three floats rather than a `helios::SphericalCoord`. That type carries its elevation and zenith as const-reference members aliasing its own private fields, so it occupies 40 bytes to hold four floats, 16 of which are pointers into the object itself. The stored value and everything derived from it are unchanged; `HitPoint` shrinks from 80 to 44 bytes, which for a 45-million-return scan is about 1.6 GB.
- A point cloud that would grow past a configurable limit now fails with a message naming the projected point count, the limit, and both in megabytes, rather than throwing from inside the allocator where neither the scan responsible nor the memory it needed is visible. The limit defaults to 100 million stored points (`LiDARcloud::DEFAULT_MAX_HIT_POINTS`) and is changed with `LiDARcloud::setMaxHitPoints()`; it exists to catch a mis-specified scan grid, not to constrain a machine that has the memory. Gap-filled misses held in virtualized form are exempt, since they cost about a bit per grid cell rather than a point each, but materializing them is checked. Added `LiDARcloud::estimateHitPointMemory()`, which reports the resident cost of a given number of points for the cloud's current set of scalar-data labels.
- The leaf-area inversion no longer keeps every beam path length through every voxel. It previously collected one value per beam-voxel intersection, which grows without bound with scan size -- about 0.5 GB for a 45-million-return scan. Because the inversion solves a mean of exponentials over those path lengths, which depends only on their distribution, a voxel's samples are now collapsed into a fixed-size histogram once it accumulates more than a thousand of them; a voxel crossed by a hundred thousand beams costs the same as one crossed by a few thousand. Measured against exact accumulation on a multi-return scan at four grid resolutions, recovered leaf-area density differs by at most 2e-6 relative, roughly four orders of magnitude inside the solver's own convergence tolerance, and G(theta) is unchanged. `LiDARcloud::setExactPathLengths()` restores exact accumulation.
- Grouping returns into per-pulse beams no longer sorts a scan that is already in acquisition order. Every return of a pulse carries that pulse's time, so a cloud straight from a scanner or a LAS/LAZ file already has its pulses contiguous and grouping is a single pass over it; the sort is still used for a cloud whose order has been disturbed, and is now stable so both paths order returns identically within a beam. Note that a gap-filled cloud is not in acquisition order, because `LiDARcloud::gapfillMisses()` appends synthesized misses grouped by sweep column.
- `LiDARcloud::calculateLeafArea()` now raises an error when more returns share a timestamp than the pulse's `target_count` says it produced, instead of treating them as one beam. Returns are grouped into beams by shared timestamp, so timestamps that were rounded or truncated on export -- or synthesized per sweep rather than per pulse -- merge unrelated pulses into a single beam whose transmittance, and therefore the leaf area derived from it, is meaningless. The message names the timestamp, the group size, and the declared return count.
- `LiDARcloud::gapfillMisses()` now reconstructs the directions of missing scan-grid cells from a scan-wide angular model rather than a per-row one. It previously fitted a median zenith and a Theil-Sen azimuth line for each row -- some 7,500 coefficients for a 2,500-row scan -- and interpolated the parameters of any row too sparse to fit from its neighbours. Measured against the scanner's own row/column indices on four scans from two instruments (RIEGL VZ-600i at 7200x2501 and VZ-2000i at 4500x2512, 36 million returns in total), the four-coefficient model reconstructs beam directions to 0.014-0.020 degrees mean error against 0.029-0.090 for the per-row fit, and its worst case is 0.07 degrees against 1.80. The per-row fit's tail came from rows with too few returns of their own -- 12% of rows on one scan -- which a model fitted across the whole grid does not have. Added `LiDARcloud::getScanGridDirection()`, which returns the reconstructed direction of a scan-grid cell.
- `LiDARcloud::gapfillMisses()` now raises an error when a scan's returns do not form a consistent angular raster about the stated scan origin, instead of silently reconstructing miss directions from a wrong one. Beam directions are computed as position minus origin, so an origin that is off by d displaces a return at range R by about d/R radians -- an error that grows for near returns and that no angular model can absorb. The check is the residual of the scan-grid fit and costs nothing extra; on a real scan it was 0.0002 rad with the correct origin against 0.10 rad with the origin one metre out.
- The `gapfillMisses_code` value 4 ("extrapolated row") is retired. Every gap-filled cell is now reconstructed from the same scan-wide model, so a row with no returns of its own is no longer a distinct case; those points are reported as 1 (gapfilled) like any other.
- Fixed `LiDARcloud::coordinateShift()` and `LiDARcloud::coordinateRotation()` leaving gap-filled miss points behind. A miss synthesized by `LiDARcloud::gapfillMisses()` has its position reconstructed from the scan origin recorded when it was created, and that record was not transformed along with the scan and its stored points, so the misses stayed where they were while the rest of the cloud moved. Registering scans into a common frame after gap-filling therefore placed every synthesized miss incorrectly, and because misses are what the leaf-area inversion counts as transmitted beams, the resulting leaf area was wrong with no error raised.
- Reading a whole gap-filled point cloud through the per-hit accessors is much faster, and `LiDARcloud::calculateLeafArea()` no longer does it the slow way. A gap-filled miss is stored implicitly, and recovering one by index requires locating it within its scan row, so a loop calling `LiDARcloud::getHitXYZ()` over every point paid that cost tens of millions of times. Added `LiDARcloud::getHitXYZColumn()` and `LiDARcloud::getHitScanIDColumn()`, which read the whole cloud in one pass, and the leaf-area inversion now uses them: measured over 576,000 gap-filled misses, its gather step went from 0.32 s to 0.008 s.

## Synthetic Annotation

- `SyntheticAnnotation::addSkyDome()` now accepts an empty filename, which removes the sky dome and leaves the flat colour set by `SyntheticAnnotation::setBackgroundColor()` showing. The sky dome is drawn over the background colour, so a caller that set a colour in order to match a photographed backdrop previously never saw it.

## Parameter Optimization

- Removed two include directories for the bundled NLopt that pointed at a path which is never created: `nlopt.hpp` is generated into the root of NLopt's build directory, not into `src/api` beneath it. The header was already being found through the `nlopt` target's own interface, so builds were unaffected, but the stale paths would have masked the real lookup had the plugin ever been switched to link NLopt by file path.
