...
- Fixed the nitrogen model giving a limited supply to the oldest leaves on a plant and none to the newest. The available pool was handed out leaf by leaf in storage order until it ran dry, so under nitrogen shortage the first leaves ever grown filled to target while every new leaf stayed at zero — deficiency appeared on the youngest leaves, the reverse of what plants show. Newly acquired nitrogen now goes to expanding leaves first, shared in proportion to demand, and mature leaves below target are topped up only from what is left over.
- Fixed nitrogen remobilization, which never ran for library plants and ran too fast when it did. Leaves donated only once they reached 70 % of `max_leaf_lifespan`, which most library plants leave at its effectively infinite default, so nitrogen was never moved; where a lifespan was set, a donor gave up to 70 % of its remobilizable nitrogen per call regardless of the time step, and leaves between 50 % and 70 % of their lifespan neither gave nor received. Remobilization now answers a shortfall: whenever uptake cannot meet the demand of expanding leaves and fruit, mature leaves release nitrogen oldest-first at a first-order rate set by the new `NitrogenParameters::leaf_remobilization_rate` (default 0.03 per day) times the time step. Leaf-to-fruit translocation draws on the same supply. `NitrogenParameters::remobilization_age_threshold` has been removed.
- Fixed the nitrogen model creating nitrogen whenever a leaf grew. Leaf nitrogen is tracked per unit area, and a leaf that expanded kept its nitrogen per area over the new area, so its nitrogen content rose with no supply; a plant whose soil supply had run out still gained nitrogen and its expanding leaves greened up. A leaf's nitrogen per area is now diluted as it expands, so its content changes only through uptake, remobilization and resorption.
- Added leaf senescence to the nitrogen model. Over the last `NitrogenParameters::leaf_senescence_duration_fraction` of `max_leaf_lifespan` (default 0.25) a leaf returns `leaf_remobilization_efficiency` of its nitrogen above `minimum_leaf_N_area` to the plant, yellowing as it does, and nitrogen stress begins senescence earlier by up to `stress_senescence_advance_fraction` of the lifespan (default 0.30). Previously a leaf kept all of its nitrogen until it was shed and that nitrogen was discarded with it, so the oldest leaves stayed the greenest on the plant. Has no effect for plants whose `max_leaf_lifespan` keeps its default.
- Added `PlantArchitecture::getPlantAvailableNitrogen()`, returning the nitrogen in a plant's available pool.
- Corrected the nitrogen model documentation, which stated that the Photosynthesis plugin reads `nitrogen_stress_factor`. No plugin reads it; calling code must apply it.

## Leaf Optics

- Nitrogen-based automatic mode in `LeafOptics::run()` now spaces its spectrum bins evenly across the leaves' nitrogen range instead of at quantiles of the leaf population. In a growing canopy most leaves share one value (every newly emerged leaf is at zero), so the quantile bins collapsed onto it: a request for 32 bins could yield six spectra, and leaves of quite different nitrogen rendered identically with hard colour steps between groups.
- Changed the default `LeafOpticsProperties_Nauto::reassignment_threshold` from 0.30 to 0 (disabled) and `min_reassignment_change` from 0.3 to 0.05 g N/m². With the old defaults a leaf whose nitrogen changed by less than 30 % of its bin centre kept its spectrum, so a leaf that filled from 2.2 to 2.5 g N/m² rendered at the 2.2 spectrum for the rest of a simulation and a canopy at uniform nitrogen showed two shades.
- Fixed leaves in nitrogen-based automatic mode never leaving a spectrum bin centered at zero nitrogen. The relative-change test governing reassignment divided by the bin center and substituted a change of zero when that center was zero, so the test could never pass. Since a leaf emerges holding no nitrogen, and a growing canopy therefore always places a bin at zero, every leaf created during a simulation kept the minimum-chlorophyll spectrum for the rest of the run however much nitrogen it went on to accumulate — a canopy whose leaf nitrogen varied widely rendered almost uniformly yellow.
- Fixed `LeafOptics::run()` in nitrogen-based automatic mode keeping the spectrum bins it created on its first call for the remainder of the simulation. Leaves whose nitrogen later moved outside the range those bins spanned were clamped to the outermost bin and held its spectrum indefinitely, so a canopy simulated over a season stopped tracking its own nitrogen; a run whose first call was made while every leaf still held its initialization value collapsed to a single bin and rendered one fixed leaf spectrum throughout. Bins are now rebuilt over the current nitrogen distribution, and every tracked leaf object reassigned, whenever a nitrogen value falls outside the established range.
