...
int main() {
    ...
    // note that since we have used the `helios' namespace above, we do not need to declare the context as:
    // helios::Context
    Context context;
    ...
    for (int j = 0; j < size.y; j++) {
        for (int i = 0; i < size.x; i++) {
            ...
            if (fabs(position.x) < dx.x * 5 && fabs(position.y) < dx.y * 5) {
                if ((j * size.x + i) % 2 == 0) {
                    context.addPatch(position, dx, make_SphericalCoord(0.f, rot),
                                     "plugins/visualizer/textures/marble_white.jpg");
                } else {
                    context.addPatch(position, dx, make_SphericalCoord(0.f, rot),
                                     "plugins/visualizer/textures/marble_black.jpg");
                }
            } else if (position.magnitude() < 0.5 * D) {
                context.addPatch(position - make_vec3(0, 0, 0.01), dx, make_SphericalCoord(0.f, rot),
                                 "plugins/visualizer/textures/grass3.jpg");
            }
        }
    }

    context.addPatch(make_vec3(0, 0, -0.005), make_vec2(9.1, 9.1), make_SphericalCoord(0.f, 0),
                     make_RGBcolor(0.2, 0.2, 0.2));
