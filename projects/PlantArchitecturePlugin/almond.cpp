...
void PlantArchitecture::addAlmondShoot() {
    ...
    uint uID_trunk = addBaseStemShoot(plant0, 5, make_AxisRotation(0, 0, 0. * M_PI), "trunk");
...
void PlantArchitecture::addAlmondTree() {
    ...
    shoot_parameters_trunk.phytomer_parameters = phytomer_parameters_trunk;
    shoot_parameters_trunk.internode_radius_initial = 0.004;
    shoot_parameters_trunk.bud_break_probability = 1;
    shoot_parameters_trunk.bud_time = 0;
    ...
    uint uID_trunk = addBaseStemShoot(plant0, 3, make_AxisRotation(context_ptr->randu(0.f, 0.05f * M_PI), context_ptr->randu(0.f, 2.f * M_PI), 0.f * M_PI), "trunk");
    ...
    for( int i=0; i<Nscaffolds; i++ ) {
        uint uID_shoot = appendShoot(plant0, uID_trunk, context_ptr->randu(6,8), make_AxisRotation(context_ptr->randu(deg2rad(35), deg2rad(45)), (float(i) + context_ptr->randu(-0.1f, 0.1f)) / float(Nscaffolds) * 2 * M_PI, 0), "scaffold");
    ...
    setPlantPhenologicalThresholds(plant0, 1, 10, 3, 5, 12);
