...
int StomatalConductanceModel::selfTest() { return 0; }
...
void StomatalConductanceModel::run() { run(context->getAllUUIDs()); }
...
void StomatalConductanceModel::run(const std::vector<uint> &UUIDs) {
    ...
    for (uint UUID : UUIDs) {
        if (!context->doesPrimitiveExist(UUID)) {
            std::cout << "WARNING (StomatalConductance::run): primitive " << UUID << " does not exist in the Context."
                      << std::endl;
            continue;
        }
