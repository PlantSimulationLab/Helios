...
SolarPosition::SolarPosition(int __UTC, float __latitude, float __longitude, helios::Context* __context) {
    ...
    if (__latitude < -90 || __latitude > 90) {
        std::cout << "WARNING (SolarPosition): Latitude must be between -90 and +90 deg (a latitude of " << latitude
    ...
    if (longitude < -180 || longitude > 180) {
        std::cout << "WARNING (SolarPosition): Longitude must be between -180 and +180 deg (a longitude of "
                  << longitude << " was given). Default longitude is being used." << std::endl;
        longitude = 121.76;
    } else {
        longitude = __longitude;
    }
...
int SolarPosition::selfTest(void) const {
    ...
    if (fabs(theta_s - theta_actual) > 10 || fabs(phi_s - phi_actual) > 5) {
        error_count++;
        std::cout << "failed: verification test for known solar position does not agree with calculated result."
                  << std::endl;
    }
    ...
    if (fabs(LW - 310.03192f) > errtol) {
        error_count++;
        std::cout << "failed: verification test for ambient longwave model does not agree with known result."
                  << std::endl;
    }
...
SphericalCoord SolarPosition::calculateSunDirection(helios::Time time, helios::Date date) const {
    ...
    delta = 0.006918f - 0.399912f * cos(Gamma) + 0.070257f * sin(Gamma) - 0.006758f * cos(2.f * Gamma) +
            0.000907f * sin(2.f * Gamma) - 0.002697f * cos(3.f * Gamma) + 0.00148f * sin(3.f * Gamma);

    // equation of time (Iqbal Eq. 1.4.1 after Spencer)
    EoT = 229.18f * (0.000075f + 0.001868f * cos(Gamma) - 0.032077f * sin(Gamma) - 0.014615f * cos(2.f * Gamma) -
                     0.04089f * sin(2.f * Gamma));
...
Time SolarPosition::getSunriseTime(void) const {
    // This is a lazy way to find the sunrise/sunset time.  If anyone wants to do the math and solve for it
    // algebraically, go for it.
...
float SolarPosition::getSolarFlux(const float pressure, const float temperature, const float humidity,
...
float SolarPosition::getSolarFluxPAR(const float pressure, const float temperature, const float humidity,
                                     const float turbidity) {
    float Eb_PAR, Eb_NIR, fdiff;
    GueymardSolarModel(pressure, temperature, humidity, turbidity, Eb_PAR, Eb_NIR, fdiff);
    return Eb_PAR;
...
float SolarPosition::getSolarFluxNIR(const float pressure, const float temperature, const float humidity,
                                     const float turbidity) {
    float Eb_PAR, Eb_NIR, fdiff;
    GueymardSolarModel(pressure, temperature, humidity, turbidity, Eb_PAR, Eb_NIR, fdiff);
    return Eb_NIR;
...
float SolarPosition::getDiffuseFraction(const float pressure, const float temperature, const float humidity,
                                        const float turbidity) {
    float Eb_PAR, Eb_NIR, fdiff;
    GueymardSolarModel(pressure, temperature, humidity, turbidity, Eb_PAR, Eb_NIR, fdiff);
    return fdiff;
...
void SolarPosition::GueymardSolarModel(const float pressure, const float temperature, const float humidity,
                                       const float turbidity, float& Eb_PAR, float& Eb_NIR, float& fdiff) {
    ...
    //---- Rayleigh ----//
    // NOTE: Rayleigh scattering dominates the atmospheric attenuation, and thus variations in the model predictions are
    // almost entirely due to pressure (and theta)
    float mR =
        1.f / (cos(theta) + 0.48353 * pow(theta * 180 / M_PI, 0.095846) / pow(96.741 - theta * 180 / M_PI, 1.754));
    ...
    //---- Ozone -----//
    float uo =
        (235 + (150 + 40 * sin(0.9856 * (DOY - 30) * M_PI / 180.f) + 20 * sin(3 * (longitude * M_PI / 180.f + 20))) *
                   pow(sin(1.28 * latitude * M_PI / 180.f), 2)) *
        0.001f;  // O3 atm-cm
    ...
    //---- Aerosol ----//
    float ma =
        1.f / (cos(theta) + 0.16851 * pow(theta * 180 / M_PI, 0.18198) / pow(95.318 - theta * 180 / M_PI, 1.9542));
    ...
    float d3 = alpha.x * (0.15232 - 0.08721 * alpha.x + 0.012664 * alpha.x * alpha.x) /
               (1.f - 0.90454 * alpha.x + 0.26167 * alpha.x * alpha.x);
    ...
    // diffuse irradiation
    float Edp_PAR = To_PAR * Tg_PAR * Tn_PAR_p * Tw_PAR_p *
                    (BR_PAR * (1.f - TR_PAR) * pow(Ta_PAR, 0.25) + Ba * TR_PAR * (1.f - pow(Tas_PAR, 0.25))) * E0_PAR;
    float Edp_NIR = To_NIR * Tg_NIR * Tn_NIR_p * Tw_NIR_p *
                    (BR_NIR * (1.f - TR_NIR) * pow(Ta_NIR, 0.25) + Ba * TR_NIR * (1.f - pow(Tas_NIR, 0.25))) * E0_NIR;
