...
void PhotosynthesisModel::setModelCoefficients(std::string species) {
    std::string s = species;
    FarquharModelCoefficients fmc;
    fmc = getModelCoefficients(s);
    farquharmodelcoeffs = fmc;
    farquharmodel_coefficients.clear();
    model = "farquhar";
}

void PhotosynthesisModel::setModelCoefficients(std::string species, const std::vector<uint> &UUIDs) {
    std::string s = species;
    FarquharModelCoefficients fmc;
    fmc = getModelCoefficients(s);
    for (uint UUID: UUIDs) {
...
FarquharModelCoefficients PhotosynthesisModel::getModelCoefficients(std::string species) {
...
float PhotosynthesisModel::evaluateCi_Farquhar(float Ci, std::vector<float> &variables, const void *parameters) {
    ...
    float s = clamp(0.5 + 0.5 * (Wc - Wj) / smooth_factor, 0.0, 1.0);
    float smooth_min = Wc * (1.f - s) + Wj * s - smooth_factor * s * (1.f - s);

    if (TPUflag == 1) {
        smooth_factor = 0.99f;
        s = clamp(0.5 + 0.5 * (smooth_min - Wp) / smooth_factor, 0.0, 1.0);
