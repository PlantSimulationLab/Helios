...
void PhotosynthesisModel::setModelType_Empirical() { model_flag = 1; }

void PhotosynthesisModel::setModelType_Farquhar() { model_flag = 2; }
...
void PhotosynthesisModel::run() { run(context->getAllUUIDs()); }
...
void PhotosynthesisModel::run(const std::vector<uint>& lUUIDs) {
    for (uint UUID : lUUIDs) {
        ...
        if (context->doesPrimitiveDataExist(UUID, "temperature") &&
            ...
            if (TL < 0) {
                TL = 0;
                std::cout << "WARNING (runPhotosynthesis): Temperature value provided was negative. Clipping to zero. "
                             "Are you using absolute temperature units?"
                          << std::endl;
            }
        } else {
            TL = TL_default;
        ...
        if (context->doesPrimitiveDataExist(UUID, "air_CO2") &&
            ...
            if (CO2 < 0) {
                CO2 = 0;
                std::cout
                    << "WARNING (runPhotosynthesis): CO2 concentration value provided was negative. Clipping to zero."
                    << std::endl;
            }
        } else {
            CO2 = CO2_default;
        ...
        if (context->doesPrimitiveDataExist(UUID, "moisture_conductance") &&
            ...
            if (gM < 0) {
                gM = 0;
                std::cout << "WARNING (runPhotosynthesis): Moisture conductance value provided was negative. Clipping "
                             "to zero."
                          << std::endl;
            }
        } else {
        ...
        if (A == 0) {
            std::cout << "WARNING (PhotosynthesisModel): Solution did not converge for primitive " << UUID << "."
                      << std::endl;
        }
...
// float PhotosynthesisModel::evaluateCi_Farquhar( const float Ci, const float CO2, const float i_PAR, const float TL,
// const float gM, float& A, int& limitation_state ) const{
...
float PhotosynthesisModel::evaluateFarquharModel(float i_PAR, float TL, float CO2, float gM, float& Ci,
                                                 int& limitation_state) {
    float A = 0;
    Ci = 100;
...
EmpiricalModelCoefficients PhotosynthesisModel::getEmpiricalModelCoefficients() { return empiricalmodelcoeffs; }

FarquharModelCoefficients PhotosynthesisModel::getFarquharModelCoefficients() { return farquharmodelcoeffs; }
