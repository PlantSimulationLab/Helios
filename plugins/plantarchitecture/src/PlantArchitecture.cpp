...
bool Shoot::sampleVegetativeBudBreak( uint node_index ) const {
    ...
    if ( !shoot_parameters.growth_requires_dormancy && probability_decay<0 ) {
        ...
        bud_break_probability = std::max( probability_min, 1.f - probability_decay * float(this->current_node_number - node_index - 1) );
    } else if ( probability_decay<0 ) { //probability maximum at base
        bud_break_probability = std::max( probability_min, 1.f - fabs(probability_decay) * float(node_index) );
    } else {
