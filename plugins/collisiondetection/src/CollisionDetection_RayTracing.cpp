...
std::vector<std::vector<CollisionDetection::HitResult>> CollisionDetection::calculateVoxelPathLengths(const vec3 &scan_origin, const std::vector<vec3> &ray_directions, const std::vector<vec3> &voxel_centers, const std::vector<vec3> &voxel_sizes) {
    ...
    if (printmessages) {
        std::cout << "Calculating voxel path lengths for " << num_rays << " rays through " 
                  << num_voxels << " voxels..." << std::endl;
    }
    ...
    if (printmessages) {
        ...
        std::cout << "Completed voxel path length calculations. Total ray-voxel intersections: " 
...
bool CollisionDetection::rayAABBIntersect(const vec3& ray_origin, const vec3& ray_direction, 
    ...
    if (std::abs(ray_direction.x) > EPSILON) {
        ...
        if (tmin > tmax) return false;  // Early exit if no intersection
    } else {
...
bool CollisionDetection::patchIntersect(const vec3 &origin, const vec3 &direction,
    ...
    if (std::abs(denom) > EPSILON) { // Not parallel to plane
        ...
        if (t > EPSILON && t < 1e8f) { // Valid intersection distance
            ...
            if (ddota >= -EPSILON && ddota <= (a * a) + EPSILON && 
                ddotb >= -EPSILON && ddotb <= (b * b) + EPSILON) {
                distance = t;
                return true;
            }
        }
