...
int test_linearRand() {
...

    {
        ...
        glm::u8vec2 AMax(std::numeric_limits<glm::u8>::min());
        {
            for (std::size_t i = 0; i < 100000; ++i) {
                ...
                if (!glm::all(glm::lessThanEqual(A, glm::u8vec2(Max)))) ++Error;
                if (!glm::all(glm::greaterThanEqual(A, glm::u8vec2(Min)))) ++Error;
                assert(!Error);
            }
        ...
        glm::u16vec2 BMax(std::numeric_limits<glm::u16>::min());
        {
            for (std::size_t i = 0; i < 100000; ++i) {
                ...
                if (!glm::all(glm::lessThanEqual(B, glm::u16vec2(Max)))) ++Error;
                if (!glm::all(glm::greaterThanEqual(B, glm::u16vec2(Min)))) ++Error;
                assert(!Error);
            }
        ...
        glm::u32vec2 CMax(std::numeric_limits<glm::u32>::min());
        {
            for (std::size_t i = 0; i < 100000; ++i) {
                ...
                if (!glm::all(glm::lessThanEqual(C, glm::u32vec2(Max)))) ++Error;
                if (!glm::all(glm::greaterThanEqual(C, glm::u32vec2(Min)))) ++Error;
                assert(!Error);
            }
        ...
        glm::u64vec2 DMax(std::numeric_limits<glm::u64>::min());
        {
            for (std::size_t i = 0; i < 100000; ++i) {
                ...
                if (!glm::all(glm::lessThanEqual(D, glm::u64vec2(Max)))) ++Error;
                if (!glm::all(glm::greaterThanEqual(D, glm::u64vec2(Min)))) ++Error;
                assert(!Error);
            }
...

    {
        ...
        glm::i8vec2 AMax(std::numeric_limits<glm::i8>::min());
        {
            for (std::size_t i = 0; i < 100000; ++i) {
                ...
                if (!glm::all(glm::lessThanEqual(A, glm::i8vec2(Max)))) ++Error;
                if (!glm::all(glm::greaterThanEqual(A, glm::i8vec2(Min)))) ++Error;
                assert(!Error);
            }
        ...
        glm::i16vec2 BMax(std::numeric_limits<glm::i16>::min());
        {
            for (std::size_t i = 0; i < 100000; ++i) {
                ...
                if (!glm::all(glm::lessThanEqual(B, glm::i16vec2(Max)))) ++Error;
                if (!glm::all(glm::greaterThanEqual(B, glm::i16vec2(Min)))) ++Error;
                assert(!Error);
            }
        ...
        glm::i32vec2 CMax(std::numeric_limits<glm::i32>::min());
        {
            for (std::size_t i = 0; i < 100000; ++i) {
                ...
                if (!glm::all(glm::lessThanEqual(C, glm::i32vec2(Max)))) ++Error;
                if (!glm::all(glm::greaterThanEqual(C, glm::i32vec2(Min)))) ++Error;
                assert(!Error);
            }
        ...
        glm::i64vec2 DMax(std::numeric_limits<glm::i64>::min());
        {
            for (std::size_t i = 0; i < 100000; ++i) {
                ...
                if (!glm::all(glm::lessThanEqual(D, glm::i64vec2(Max)))) ++Error;
                if (!glm::all(glm::greaterThanEqual(D, glm::i64vec2(Min)))) ++Error;
                assert(!Error);
            }
    ...
    for (std::size_t i = 0; i < 100000; ++i) {
        ...
        if (!glm::all(glm::lessThanEqual(A, glm::f32vec2(static_cast<float>(Max))))) ++Error;
        if (!glm::all(glm::greaterThanEqual(A, glm::f32vec2(static_cast<float>(Min))))) ++Error;

        glm::f64vec2 const B(glm::linearRand(glm::f64vec2(Min), glm::f64vec2(Max)));
        if (!glm::all(glm::lessThanEqual(B, glm::f64vec2(Max)))) ++Error;
        if (!glm::all(glm::greaterThanEqual(B, glm::f64vec2(Min)))) ++Error;
        assert(!Error);
    }
