...
int perf(bool NextFloat) {
    ...
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::fastCos(i);

    const std::clock_t timestamp2 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::cos(i);
...
int perf(bool NextFloat) {
    ...
    const std::clock_t timestamp1 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::fastSin(i);

    const std::clock_t timestamp2 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::sin(i);
...
int perf(bool NextFloat) {
    ...
    const std::clock_t timestamp1 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::fastTan(i);

    const std::clock_t timestamp2 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::tan(i);
...
int perf(bool NextFloat) {
    ...
    const std::clock_t timestamp1 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::fastAcos(i);

    const std::clock_t timestamp2 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::acos(i);
...
int perf(bool NextFloat) {
    ...
    float result = 0.f;
    const std::clock_t timestamp1 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::fastAsin(i);
    const std::clock_t timestamp2 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::asin(i);
    const std::clock_t timestamp3 = std::clock();
    const std::clock_t time_fast = timestamp2 - timestamp1;
    const std::clock_t time_default = timestamp3 - timestamp2;
...
int perf(bool NextFloat) {
    ...
    float result = 0.f;
    const std::clock_t timestamp1 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::fastAtan(i);
    const std::clock_t timestamp2 = std::clock();
    for (float i = begin; i < end; i = NextFloat ? glm::next_float(i) : i += 0.1f) result = glm::atan(i);
    const std::clock_t timestamp3 = std::clock();
    const std::clock_t time_fast = timestamp2 - timestamp1;
    const std::clock_t time_default = timestamp3 - timestamp2;
...
GLM_FUNC_QUALIFIER vecType<T, P> taylorSeriesNewCos(vecType<T, P> const& x) {
    ...
    return static_cast<T>(1) - Powed2 * static_cast<T>(0.5) +
...
int perf_fastCosNew(float Begin, float End, std::size_t Samples) {
    ...
    for (std::size_t i = 0; i < Samples; ++i) Results[i] = fastCosNew(AngleShift + glm::vec4(Begin + Steps * i));
    ...
    int Error = 0;
    for (std::size_t i = 0; i < Samples; ++i) Error += Results[i].x >= -1.0f && Results[i].x <= 1.0f ? 0 : 1;
    return Error;
}
...
GLM_FUNC_QUALIFIER vecType<T, P> taylorSeriesRefCos(vecType<T, P> const& x) {
    return static_cast<T>(1) - (x * x) / glm::factorial(static_cast<T>(2)) +
...
int perf_fastCosOld(float Begin, float End, std::size_t Samples) {
    ...
    for (std::size_t i = 0; i < Samples; ++i) Results[i] = glm::fastCos(AngleShift + glm::vec4(Begin + Steps * i));
...
int perf_cos(float Begin, float End, std::size_t Samples) {
    ...
    for (std::size_t i = 0; i < Samples; ++i) Results[i] = glm::cos(AngleShift + glm::vec4(Begin + Steps * i));
