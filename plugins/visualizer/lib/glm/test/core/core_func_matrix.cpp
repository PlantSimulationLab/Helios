...
int test_inverse() {
    ...
    glm::mat4x4 A4x4(glm::vec4(1, 0, 1, 0), glm::vec4(0, 1, 0, 0), glm::vec4(0, 0, 1, 0), glm::vec4(0, 0, 0, 1));
    glm::mat4x4 B4x4 = inverse(A4x4);
    glm::mat4x4 I4x4 = A4x4 * B4x4;
    Failed += I4x4 == glm::mat4x4(1) ? 0 : 1;

    glm::mat3x3 A3x3(glm::vec3(1, 0, 1), glm::vec3(0, 1, 0), glm::vec3(0, 0, 1));
    glm::mat3x3 B3x3 = glm::inverse(A3x3);
    glm::mat3x3 I3x3 = A3x3 * B3x3;
    Failed += I3x3 == glm::mat3x3(1) ? 0 : 1;

    glm::mat2x2 A2x2(glm::vec2(1, 1), glm::vec2(0, 1));
...
int test_inverse_simd() {
    ...
    glm::mat4x4 const A4x4(glm::vec4(1, 0, 1, 0), glm::vec4(0, 1, 0, 0), glm::vec4(0, 0, 1, 0), glm::vec4(0, 0, 0, 1));
...
int test_inverse_perf(std::size_t Count, std::size_t Instance, char const* Message) {
    ...
    for (std::size_t i = 0; i < TestInputs.size(); ++i) TestOutputs[i] = glm::inverse(TestInputs[i]);

    std::clock_t EndTime = std::clock();

    for (std::size_t i = 0; i < TestInputs.size(); ++i) TestOutputs[i] = TestOutputs[i] * TestInputs[i];
    ...
    for (std::size_t Entry = 0; Entry < TestOutputs.size(); ++Entry) {
        ...
        for (glm::length_t y = 0; y < m.length(); ++y)
            for (glm::length_t x = 0; x < m[y].length(); ++x) Diff = glm::max(m[y][x], i[y][x]);
    }
