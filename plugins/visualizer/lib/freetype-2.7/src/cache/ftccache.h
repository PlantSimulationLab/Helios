...
#define FTC_NODE_TOP_FOR_HASH(cache, hash) \
    ((cache)->buckets +                    \
     ((((hash) & (cache)->mask) < (cache)->p) ? ((hash) & ((cache)->mask * 2 + 1)) : ((hash) & (cache)->mask)))
#else
FT_LOCAL(FTC_Node*)
ftc_get_top_node_for_hash(FTC_Cache cache, FT_Offset hash);
#define FTC_NODE_TOP_FOR_HASH(cache, hash) ftc_get_top_node_for_hash((cache), (hash))
#endif
...
#ifndef FTC_INLINE
FT_LOCAL(FT_Error)
FTC_Cache_Lookup(FTC_Cache cache, FT_Offset hash, FT_Pointer query, FTC_Node* anode);
#endif
...
#define FTC_CACHE_LOOKUP_CMP(cache, nodecmp, hash, query, node, error)                            \
    ...
    for (;;) {                                                                                    \
        _node = *_pnode;                                                                          \
        if (_node == NULL) goto NewNode_;                                                         \
                                                                                                  \
        if (_node->hash == _hash && _nodcomp(_node, query, _cache, &_list_changed)) break;        \
                                                                                                  \
        _pnode = &_node->link;                                                                    \
    }                                                                                             \
                                                                                                  \
    ...
    {                                                                                             \
        ...
        void* _nl = &_manager->nodes_list;                                                        \
                                                                                                  \
        if (_node != _manager->nodes_list) FTC_MruNode_Up((FTC_MruNode*)_nl, (FTC_MruNode)_node); \
    }                                                                                             \
    goto Ok_;                                                                                     \
                                                                                                  \
    NewNode_:                                                                                     \
...
#define FTC_CACHE_LOOKUP_CMP(cache, nodecmp, hash, query, node, error)           \
    FT_BEGIN_STMNT                                                               \
    error = FTC_Cache_Lookup(FTC_CACHE(cache), hash, query, (FTC_Node*)&(node)); \
    FT_END_STMNT
...
#define FTC_CACHE_TRYLOOP_END(list_changed)                                                                       \
    if (!error || FT_ERR_NEQ(error, Out_Of_Memory)) break;                                                        \
                                                                                                                  \
    _try_done = FTC_Manager_FlushN(_try_manager, _try_count);                                                     \
    if (_try_done > 0 && (list_changed != NULL)) *(FT_Bool*)(list_changed) = TRUE;                                \
                                                                                                                  \
    if (_try_done == 0) break;                                                                                    \
                                                                                                                  \
    if (_try_done == _try_count) {                                                                                \
        _try_count *= 2;                                                                                          \
        if (_try_count < _try_done || _try_count > _try_manager->num_nodes) _try_count = _try_manager->num_nodes; \
    }                                                                                                             \
    }                                                                                                             \
    }
