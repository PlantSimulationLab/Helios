...
FT_LOCAL(void)
FTC_GNode_Init(FTC_GNode node, FT_UInt gindex, /* glyph index for node */
               FTC_Family family);
...
#ifndef FTC_INLINE
FT_LOCAL(FT_Error)
FTC_GCache_Lookup(FTC_GCache cache, FT_Offset hash, FT_UInt gindex, FTC_GQuery query, FTC_Node* anode);
#endif

/* */

#define FTC_FAMILY_FREE(family, cache) FTC_MruList_Remove(&FTC_GCACHE((cache))->families, (FTC_MruNode)(family))
...
#define FTC_GCACHE_LOOKUP_CMP(cache, famcmp, nodecmp, hash, gindex, query, node, error) \
    ...
    _gquery->gindex = (gindex);                                                         \
                                                                                        \
    FTC_MRULIST_LOOKUP_CMP(&_gcache->families, _gquery, _fcompare, _mrunode, error);    \
    ...
    if (!error) {                                                                       \
        ...
        FTC_CACHE_LOOKUP_CMP(cache, nodecmp, hash, query, node, error);                 \
                                                                                        \
        if (--_gqfamily->num_nodes == 0) FTC_FAMILY_FREE(_gqfamily, _gcache);           \
    }                                                                                   \
    FT_END_STMNT
/* */
...
#define FTC_GCACHE_LOOKUP_CMP(cache, famcmp, nodecmp, hash, gindex, query, node, error)   \
    FT_BEGIN_STMNT                                                                        \
                                                                                          \
    error = FTC_GCache_Lookup(FTC_GCACHE(cache), hash, gindex, FTC_GQUERY(query), &node); \
                                                                                          \
    FT_END_STMNT
