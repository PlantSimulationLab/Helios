    ...
    if (_first) {                                                           \
        ...
        do {                                                                \
            if (_compare(_node, (key))) {                                   \
                if (_node != _first) FTC_MruNode_Up(_pfirst, _node);        \
                                                                            \
                node = _node;                                               \
                goto MruOk_;                                                \
            }                                                               \
...
FTC_MruList_Find(FTC_MruList list, FT_Pointer key);

FT_LOCAL(FT_Error)
FTC_MruList_Lookup(FTC_MruList list, FT_Pointer key, FTC_MruNode *pnode);

#define FTC_MRULIST_LOOKUP(list, key, node, error) error = FTC_MruList_Lookup((list), (key), (FTC_MruNode *)&(node))
