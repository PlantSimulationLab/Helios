...
#define AF_ANGLE_DIFF(result, angle1, angle2)              \
    ...
    AF_Angle _delta = (angle2) - (angle1);                 \
                                                           \
    while (_delta <= -AF_ANGLE_PI) _delta += AF_ANGLE_2PI; \
                                                           \
    while (_delta > AF_ANGLE_PI) _delta -= AF_ANGLE_2PI;   \
                                                           \
    result = _delta;                                       \
...
#define AF_SCALER_EQUAL_SCALES(a, b)                                                                 \
    ((a)->x_scale == (b)->x_scale && (a)->y_scale == (b)->y_scale && (a)->x_delta == (b)->x_delta && \
     (a)->y_delta == (b)->y_delta)
...
typedef void (*AF_WritingSystem_ApplyHintsFunc)(FT_UInt glyph_index, AF_GlyphHints hints, FT_Outline* outline,
...
#define AF_DECLARE_WRITING_SYSTEM_CLASS(writing_system_class) \
    FT_CALLBACK_TABLE const AF_WritingSystemClassRec writing_system_class;

#define AF_DEFINE_WRITING_SYSTEM_CLASS(writing_system_class, system, m_size, m_init, m_scale, m_done, m_stdw, h_init, \
                                       h_apply)                                                                       \
    FT_CALLBACK_TABLE_DEF                                                                                             \
    const AF_WritingSystemClassRec writing_system_class = {system,                                                    \
                                                                                                                      \
                                                           m_size,                                                    \
                                                                                                                      \
                                                           m_init, m_scale, m_done, m_stdw,                           \
                                                                                                                      \
                                                           h_init, h_apply};

#define AF_DECLARE_SCRIPT_CLASS(script_class) FT_CALLBACK_TABLE const AF_ScriptClassRec script_class;

#define AF_DEFINE_SCRIPT_CLASS(script_class, script, ranges, nonbase_ranges, top_to_bottom, std_charstring) \
    FT_CALLBACK_TABLE_DEF                                                                                   \
    const AF_ScriptClassRec script_class = {                                                                \
        script, ranges, nonbase_ranges, top_to_bottom, std_charstring,                                      \
    };

#define AF_DECLARE_STYLE_CLASS(style_class) FT_CALLBACK_TABLE const AF_StyleClassRec style_class;

#define AF_DEFINE_STYLE_CLASS(style_class, style, writing_system, script, blue_stringset, coverage) \
    FT_CALLBACK_TABLE_DEF                                                                           \
    const AF_StyleClassRec style_class = {style, writing_system, script, blue_stringset, coverage};
...
#define AF_DEFINE_WRITING_SYSTEM_CLASS(writing_system_class, system, m_size, m_init, m_scale, m_done, h_init, h_apply) \
    FT_LOCAL_DEF(void)                                                                                                 \
    FT_Init_Class_##writing_system_class(AF_WritingSystemClassRec* ac) {                                               \
