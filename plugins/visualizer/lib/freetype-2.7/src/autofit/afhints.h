...
#define AF_HINTS_DO_VERTICAL(h) (!_af_debug_disable_vert_hints && !AF_HINTS_TEST_SCALER(h, AF_SCALER_FLAG_NO_VERTICAL))
...
FT_LOCAL(FT_Error)
af_axis_hints_new_edge(AF_AxisHints axis, FT_Int fpos, AF_Direction dir, FT_Bool top_to_bottom_hinting,
                       FT_Memory memory, AF_Edge* edge);
...
FT_LOCAL(void)
af_glyph_hints_scale_dim(AF_GlyphHints hints, AF_Dimension dim, FT_Fixed scale, FT_Pos delta);
#endif
...
#define AF_SEGMENT_DIST(seg1, seg2) \
    (((seg1)->pos > (seg2)->pos) ? (seg1)->pos - (seg2)->pos : (seg2)->pos - (seg1)->pos)
