...
typedef FT_UInt (*FT_CMap_CharVarIndexFunc)(FT_CMap cmap, FT_CMap unicode_cmap, FT_UInt32 char_code,
...
#define FT_DEFINE_CMAP_CLASS(class_, size_, init_, done_, char_index_, char_next_, char_var_index_, char_var_default_, \
                             variant_list_, charvariant_list_, variantchar_list_)                                      \
...
#define FT_DECLARE_CMAP_CLASS(class_) void FT_Init_Class_##class_(FT_Library library, FT_CMap_ClassRec* clazz);
...
FT_BASE(FT_Error)
ft_property_string_set(FT_Library library, const FT_String* module_name, const FT_String* property_name,
                       FT_String* value);
#endif
...
#define FT_REQUEST_WIDTH(req) \
    ((req)->horiResolution ? ((req)->width * (FT_Pos)(req)->horiResolution + 36) / 72 : (req)->width)

#define FT_REQUEST_HEIGHT(req) \
    ((req)->vertResolution ? ((req)->height * (FT_Pos)(req)->vertResolution + 36) / 72 : (req)->height)
...
#define FT_DEFINE_OUTLINE_FUNCS(class_, move_to_, line_to_, conic_to_, cubic_to_, shift_, delta_) \
    static const FT_Outline_Funcs class_ = {move_to_, line_to_, conic_to_, cubic_to_, shift_, delta_};

#else /* FT_CONFIG_OPTION_PIC */

#define FT_DEFINE_OUTLINE_FUNCS(class_, move_to_, line_to_, conic_to_, cubic_to_, shift_, delta_) \
    static FT_Error Init_Class_##class_(FT_Outline_Funcs* clazz) {                                \
...
#define FT_DEFINE_RASTER_FUNCS(class_, glyph_format_, raster_new_, raster_reset_, raster_set_mode_, raster_render_, \
                               raster_done_)                                                                        \
    const FT_Raster_Funcs class_ = {glyph_format_,    raster_new_,    raster_reset_,                                \
...
#define FT_DEFINE_RASTER_FUNCS(class_, glyph_format_, raster_new_, raster_reset_, raster_set_mode_, raster_render_, \
                               raster_done_)                                                                        \
    void FT_Init_Class_##class_(FT_Raster_Funcs* clazz) {                                                           \
...
#define FT_DEFINE_GLYPH(class_, size_, format_, init_, done_, copy_, transform_, bbox_, prepare_) \
    FT_CALLBACK_TABLE_DEF                                                                         \
    const FT_Glyph_Class class_ = {size_, format_, init_, done_, copy_, transform_, bbox_, prepare_};

#else /* FT_CONFIG_OPTION_PIC */

#define FT_DEFINE_GLYPH(class_, size_, format_, init_, done_, copy_, transform_, bbox_, prepare_) \
    void FT_Init_Class_##class_(FT_Glyph_Class* clazz) {                                          \
...
#define FT_DEFINE_RENDERER(class_, flags_, size_, name_, version_, requires_, interface_, init_, done_,                \
                           get_interface_, glyph_format_, render_glyph_, transform_glyph_, get_glyph_cbox_, set_mode_, \
                           raster_class_)                                                                              \
    FT_CALLBACK_TABLE_DEF                                                                                              \
    const FT_Renderer_Class class_ = {FT_DEFINE_ROOT_MODULE(flags_, size_, name_, version_, requires_, interface_,     \
                                                            init_, done_, get_interface_) glyph_format_,               \
                                                                                                                       \
                                      render_glyph_,                                                                   \
                                      transform_glyph_,                                                                \
                                      get_glyph_cbox_,                                                                 \
...
#define FT_DEFINE_RENDERER(class_, flags_, size_, name_, version_, requires_, interface_, init_, done_,                \
                           get_interface_, glyph_format_, render_glyph_, transform_glyph_, get_glyph_cbox_, set_mode_, \
                           raster_class_)                                                                              \
    void FT_Destroy_Class_##class_(FT_Library library, FT_Module_Class* clazz) {                                       \
        FT_Renderer_Class* rclazz = (FT_Renderer_Class*)clazz;                                                         \
        FT_Memory memory = library->memory;                                                                            \
                                                                                                                       \
        class_##_pic_free(library);                                                                                    \
        if (rclazz) FT_FREE(rclazz);                                                                                   \
    }                                                                                                                  \
                                                                                                                       \
    FT_Error FT_Create_Class_##class_(FT_Library library, FT_Module_Class** output_class) {                            \
        FT_Renderer_Class* clazz = NULL;                                                                               \
        FT_Error error;                                                                                                \
        FT_Memory memory = library->memory;                                                                            \
                                                                                                                       \
        if (FT_ALLOC(clazz, sizeof(*clazz))) return error;                                                             \
                                                                                                                       \
        ...
        if (error) {                                                                                                   \
            ...
            return error;                                                                                              \
                                                                                                                       ...
                                                                                                                       \
        FT_DEFINE_ROOT_MODULE(flags_, size_, name_, version_, requires_, interface_, init_, done_, get_interface_)     \
                                                                                                                       \
        clazz->glyph_format = glyph_format_;                                                                           \
                                                                                                                       \
