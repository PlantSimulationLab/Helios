   ...
   public:
    ...
    void set(std::vector<helios::RGBcolor> ctable, std::vector<float> clocs, int size, float minval_, float maxval_) {
        ...
        if (clocs.size() != Ncolors) {
            ...
            throw(
                std::runtime_error("ERROR(Colormap::set): Cannot initialize colormap because minimum colormap value "
        ...
        for (int c = 0; c < Ncolors - 1; c++) {
            ...
            for (int i = 0; i < cmapsize; i++) {
                if (i >= cmin && i <= cmax) {
                    cmap.at(i).r =
                        ctable.at(c).r + (float(i) - cmin) / (cmax - cmin) * (ctable.at(c + 1).r - ctable.at(c).r);
                    cmap.at(i).g =
                        ctable.at(c).g + (float(i) - cmin) / (cmax - cmin) * (ctable.at(c + 1).g - ctable.at(c).g);
                    cmap.at(i).b =
                        ctable.at(c).b + (float(i) - cmin) / (cmax - cmin) * (ctable.at(c + 1).b - ctable.at(c).b);
                }
            }
        }
    }
    ...
    helios::vec2 getRange(void) const { return helios::make_vec2(minval, maxval); }

    float getLowerLimit(void) const { return minval; }

    float getUpperLimit(void) const { return maxval; }
...
class Visualizer {
    ...
    */
    ...
    //! Visualizer constructior with option to remove window decorationos (e.g., header bar, trim). This is a workaround
    //! for an error that occurs on Linux systems when printing the window to a JPEG image (printWindow). Once a fix is
    //! found, this function will likely be removed
    ...
    /*   //! Do not apply any coordinate transformation to positions. In this case, all positions must be between -1
     * and 1. */
    /*   NO_TRANSFORM=0,  */

    /*   //! Apply a perspective transformation to positions according to the current view. To set the view, use \ref
     * setCameraPosition(). */
    ...
    enum CoordinateSystem {
        ...
        //! Coordinates are normalized to unity and are window-aligned.  The point (x,y)=(0,0) is in the bottom left
        //! corner of the window, and (x,y)=(1,1) is in the upper right corner of the window.  The z-coordinate
        //! specifies the depth in the screen-normal direction, with values ranging from -1 to 1.  For example, an
        //! object at z=0.5 would be in front of an object at z=0.
    ...
    //! Set camera position
    /** \param[in] "cameraPosition" (x,y,z) position of the camera, i.e., this is where the actual camera or `eye' is
       positioned. \param[in] "lookAt" (x,y,z) position of where the camera is looking at.
    */
    ...
    */
    ...
    /** \param[in] "direction" Vector pointing in the direction of the light source (vector starts at light source and
     * points toward scene.) */
    ...
    */
    ...
    /** \param[in] "center" (x,y,z) location of the rectangle center
        ...
        \param[in] "coordFlag" If flag value is false, no transform is applied to the rectangle.  In this case `size'
       and `center' values are normalized window coordinates, where the size of the window is 1x1 and (x=0,y=0) is the
       bottom left of the window. If flag value is true, a perspective transform is applied to the rectangle, and
       physical dimensions are used.
    */
    void addRectangleByCenter(const helios::vec3 &center, const helios::vec2 &size,
    ...
    */
    ...
    //! Add a rectangle by giving the coordinates of its center - rectangle is colored by and RGB color value but is
    //! masked by the alpha channel of a PNG image file
    /** \param[in] "center" (x,y,z) location of the rectangle center
        \param[in] "size" Size in the x- and y-directions
        \param[in] "rotation" spherical rotation angle (elevation,azimuth)
        \param[in] "color" R-G-B color of the rectangle
    ...
    */
    void addRectangleByCenter(const helios::vec3 &center, const helios::vec2 &size,
                              const helios::SphericalCoord &rotation, const helios::RGBcolor &color, const Glyph *glyph,
                              CoordinateSystem coordFlag);

    //! Add a rectangle by giving the coordinates of its four vertices
    /** \param[in] "vertices" (x,y,z) coordinates of four vertices
        \param[in] "color" R-G-B color of the rectangle
        \param[in] "coordFlag" If flag value is false, no transform is applied to the rectangle.  In this case
       `vertices' values are normalized window coordinates, where the size of the window is 1x1 and (x=0,y=0) is the
       bottom left of the window. If flag value is true, a perspective transform is applied to the rectangle, and
       physical dimensions are used.
    */
    void addRectangleByVertices(const std::vector<helios::vec3> &vertices, const helios::RGBcolor &color,
                                CoordinateSystem coordFlag);
    ...
    /** \param[in] "vertices" (x,y,z) coordinates of four vertices
       ...
       physical dimensions are used.
    */
    void addRectangleByVertices(const std::vector<helios::vec3> &vertices, const helios::RGBAcolor &color,
                                CoordinateSystem coordFlag);
    ...
    /** \param[in] "vertices" (x,y,z) coordinates of four vertices
       ...
       physical dimensions are used.
    */
    void addRectangleByVertices(const std::vector<helios::vec3> &vertices, const char *texture_file,
                                CoordinateSystem coordFlag);
    ...
    */
    void addRectangleByVertices(const std::vector<helios::vec3> &vertices, const char *texture_file,
                                const std::vector<helios::vec2> &uvs, CoordinateSystem coordFlag);

    //! Add a rectangle by giving the coordinates of its four vertices and mask by texture map transparency channel, but
    //! color by R-G-B value
    /** \param[in] "vertices" (x,y,z) coordinates of four vertices
        \param[in] "texture_file" File corresponding to the JPEG image to be used as a texture map
        \param[in] "uvs" u-v coordinates for rectangle vertices
        \param[in] "color" R-G-B color of the rectangle
    ...
    */
    void addRectangleByVertices(const std::vector<helios::vec3> &vertices, const helios::RGBcolor &color,
                                const char *texture_file, const std::vector<helios::vec2> &uvs,
                                CoordinateSystem coordFlag);

    //! Add a rectangle by giving the coordinates of its four vertices - rectangle is colored by an RGB color value but
    //! is masked by the alpha channel of a PNG image file
    /** \param[in] "vertices" (x,y,z) coordinates of four vertices
        \param[in] "color" R-G-B color of the rectangle
        \param[in] "texture_file" File corresponding to the JPEG image to be used as a texture map
        \param[in] "coordFlag" If flag value is false, no transform is applied to the rectangle.  In this case
    ...
    */
    void addRectangleByVertices(const std::vector<helios::vec3> &vertices, const helios::RGBcolor &color,
                                const Glyph *glyph, CoordinateSystem coordFlag);
    ...
    */
    ...
    /** \param[in] "vertex0" (x,y,z) location of first vertex
        ...
        \param[in] "coordFlag" If flag value is false, no transform is applied to the rectangle.  In this case `size'
       and `center' values are normalized window coordinates, where the size of the window is 1x1 and (x=0,y=0) is the
       bottom left of the window. If flag value is true, a perspective transform is applied to the triangle, and
       physical dimensions are used.
    */
    void addTriangle(const helios::vec3 &vertex0, const helios::vec3 &vertex1, const helios::vec3 &vertex2,
                     const helios::RGBcolor &color, CoordinateSystem coordFlag);
    ...
    */
    void addTriangle(const helios::vec3 &vertex0, const helios::vec3 &vertex1, const helios::vec3 &vertex2,
                     const helios::RGBAcolor &color, CoordinateSystem coordFlag);
    ...
    */
    void addTriangle(const helios::vec3 &vertex0, const helios::vec3 &vertex1, const helios::vec3 &vertex2,
                     ...
                     const helios::vec2 &uv2, CoordinateSystem coordFlag);

    //! Add a triangle by giving the coordinates of its three vertices and color by a constant color, but mask using
    //! transparency channel of texture map
    /** \param[in] "vertex0" (x,y,z) location of first vertex
        \param[in] "vertex1" (x,y,z) location of first vertex
        \param[in] "vertex2" (x,y,z) location of first vertex
        \param[in] "texture_file" File corresponding to the image to be used as a texture map
    ...
    */
    void addTriangle(const helios::vec3 &vertex0, const helios::vec3 &vertex1, const helios::vec3 &vertex2,
                     const char *texture_file, const helios::vec2 &uv0, const helios::vec2 &uv1,
                     const helios::vec2 &uv2, const helios::RGBAcolor &color, CoordinateSystem coordFlag);
    ...
    */
    void addVoxelByCenter(const helios::vec3 &center, const helios::vec3 &size, const helios::SphericalCoord &rotation,
                          const helios::RGBcolor &color, CoordinateSystem coordFlag);
    ...
    /** \param[in] "size" Size in the x-, y- and z-directions
        ...
        \param[in] "coordFlag" If flag value is false, no transform is applied to the voxel.  In this case `size' and
       `center' values are normalized window coordinates, where the size of the window is 1x1 and (x=0,y=0) is the
       bottom left of the window. If flag value is true, a perspective transform is applied to the voxel, and physical
       dimensions are used.
    */
    void addVoxelByCenter(const helios::vec3 &center, const helios::vec3 &size, const helios::SphericalCoord &rotation,
                          const helios::RGBAcolor &color, CoordinateSystem coordFlag);
    ...
    /** \param[in] "center" (x,y,z) location of the disk center
        ...
        \param[in] "Ndivisions" Number of discrete divisions in making disk. (e.g., Ndivisions=4 makes a square,
       Ndivisions=5 makes a pentagon, etc.) \param[in] "color" R-G-B color of the disk \param[in] "coordFlag" If flag
       value is false, no transform is applied to the disk.  In this case `size' and `center' values are normalized
       window coordinates, where the size of the window is 1x1 and (x=0,y=0) is the bottom left of the window. If flag
       value is true, a perspective transform is applied to the disk, and physical dimensions are used.
    */
    void addDiskByCenter(const helios::vec3 &center, const helios::vec2 &size, const helios::SphericalCoord &rotation,
                         uint Ndivisions, const helios::RGBcolor &color, CoordinateSystem coordFlag);

    //! Add a disk by giving the coordinates of its center
    /** \param[in] "center" (x,y,z) location of the disk center
        \param[in] "size" length of disk semi-major and semi-minor axes
        \param[in] "Ndivisions" Number of discrete divisions in making disk. (e.g., Ndivisions=4 makes a square,
       Ndivisions=5 makes a pentagon, etc.) \param[in] "color" R-G-B-A color of the disk \param[in] "coordFlag" If flag
       value is false, no transform is applied to the disk.  In this case `size' and `center' values are normalized
       window coordinates, where the size of the window is 1x1 and (x=0,y=0) is the bottom left of the window. If flag
       value is true, a perspective transform is applied to the disk, and physical dimensions are used.
    */
    void addDiskByCenter(const helios::vec3 &center, const helios::vec2 &size, const helios::SphericalCoord &rotation,
                         uint Ndivisions, const helios::RGBAcolor &color, CoordinateSystem coordFlag);

    //! Add a texture mapped disk by giving the coordinates of its center
    /** \param[in] "center" (x,y,z) location of the disk center
        \param[in] "size" length of disk semi-major and semi-minor axes
        \param[in] "Ndivisions" Number of discrete divisions in making disk. (e.g., Ndivisions=4 makes a square,
       Ndivisions=5 makes a pentagon, etc.) \param[in] "texture_file" File corresponding to the JPEG image to be used as
       a texture map \param[in] "coordFlag" If flag value is false, no transform is applied to the disk.  In this case
       `size' and `center' values are normalized window coordinates, where the size of the window is 1x1 and (x=0,y=0)
       is the bottom left of the window. If flag value is true, a perspective transform is applied to the disk, and
       physical dimensions are used.
    */
    void addDiskByCenter(const helios::vec3 &center, const helios::vec2 &size, const helios::SphericalCoord &rotation,
                         uint Ndivisions, const char *texture_file, CoordinateSystem coordFlag);
    ...
    /**
        ...
        \param[in] "coordFlag" If flag value is false, no transform is applied to the rectangle.  In this case `size'
       and `center' values are normalized window coordinates, where the size of the window is 1x1 and (x=0,y=0) is the
       bottom left of the window. If flag value is true, a perspective transform is applied to the lines, and physical
       dimensions are used.
    */
    void addLine(const helios::vec3 &start, const helios::vec3 &end, const helios::RGBcolor &color, uint linewidth,
                 CoordinateSystem coordFlag);
    ...
    /**
       ...
       dimensions are used.
    */
    void addLine(const helios::vec3 &start, const helios::vec3 &end, const helios::RGBAcolor &color, uint linewidth,
                 CoordinateSystem coordFlag);
    ...
    /**
       ...
       dimensions are used.
    */
    void addPoint(const helios::vec3 &position, const helios::RGBcolor &color, uint pointsize,
                  CoordinateSystem coordFlag);
    ...
    /**
       ...
       dimensions are used.
    */
    void addPoint(const helios::vec3 &position, const helios::RGBAcolor &color, uint pointsize,
                  CoordinateSystem coordFlag);
    ...
    /**
        ...
        \param[in] "coordFlag" If flag value is false, no transform is applied to the rectangle.  In this case `size'
       and `center' values are normalized window coordinates, where the size of the window is 1x1 and (x=0,y=0) is the
       bottom left of the window. If flag value is true, a perspective transform is applied to the sphere, and physical
       dimensions are used.
    */
    void addSphereByCenter(float radius, const helios::vec3 &center, uint Ndivisions, const helios::RGBcolor &color,
                           CoordinateSystem coordFlag);
    ...
    /**
       ...
       dimensions are used.
    */
    void addSphereByCenter(float radius, const helios::vec3 &center, uint Ndivisions, const helios::RGBAcolor &color,
                           CoordinateSystem coordFlag);
    ...
    */
    ...
    DEPRECATED(void addSkyDomeByCenter(float radius, const helios::vec3 &center, uint Ndivisions,
                                       const char *texture_file, int layer));
    ...
    /** \param[in] "textstring" String of text to display
       ...
       physical dimensions are used.
    */
    void addTextboxByCenter(const char *textstring, const helios::vec3 &center, const helios::SphericalCoord &rotation,
                            const helios::RGBcolor &fontcolor, uint fontsize, const char *fontname,
                            CoordinateSystem coordFlag);
    ...
    /**
     ...
     \param[in] "sign" either "both" or "positive" should the axes be drawn in both positive and negative directions or
     just positive
     */
    ...
    */
    ...
    /** \param[in] "ticks" Vector of values corresponding to ticks
        \note If tick values are outside of the colorbar range (see \ref setColorBarRange()), the colorbar will be
       automatically expanded to fit the tick values.
    */
    ...
    */
    ...
    //! Update the graphics window based on current geometry, then continue the program, with the option not to display
    //! the graphic window
    ...
    //! Print the current graphics window to a JPEG image file. File will be given a default filename and saved to the
    //! current directory from which the executable was run.
    ...
    */
    ...
    /** \param[out] "buffer" Pixel data. The data is stored as r-g-b * column * row. So indices (0,1,2) would be the RGB
     * values for row 0 and column 0, indices (3,4,5) would be RGB values for row 0 and column 1, and so on. Thus,
     * buffer is of size 3*width*height.
     */
   ...
   private:
    ...
    void addColorbarByCenter(const char *title, const helios::vec2 size, const helios::vec3 center,
                             const helios::RGBcolor font_color, const Colormap colormap);
    ...
    /** \note This will be recast to have type GLFWwindow*.  This has to be done in order to keep library-dependent
     * variables out of the header. */
    void *window;
    ...
    /** colorbar_flag=0 means the colorbar is off and no enable/disable functions have been called, colorbar_flag=1
     * means the colorbar is off and disableColorbar() was explicitly called and thus the colorbar should remain off,
     * colorbar_flag=2 means the colorbar is on. */
    uint colorbar_flag;
