/** \file "VoxelIntersection.cpp" Primary source file for Primitive-Voxel Intersection plug-in.
    ...
    Copyright (C) 2018-2022  Brian Bailey, Eric Kent
...
int VoxelIntersection::selfTest(void) {
    ...
    std::vector<uint> s1_1 =
        vi_1.slicePrimitivesUsingGrid(allUUIDs_1, make_vec3(0.0, 0, 0), make_vec3(1, 1, 1), make_int3(1, 1, 1));
    if (s1_1.size() == 1) {
    ...
    std::vector<uint> s1_2 =
        vi_2.slicePrimitivesUsingGrid(allUUIDs_2, make_vec3(0.5, 0, 0), make_vec3(1, 1, 1), make_int3(1, 1, 1));

    if (s1_2.size() == 4) {
        std::cout << "passed" << std::endl;
    } else {
        error_count++;
        std::cerr << "failed: single patch that should be sliced into four triangles was sliced into " << s1_2.size()
                  << std::endl;
    }
    ...
    std::vector<uint> s1_3 =
        vi_3.slicePrimitivesUsingGrid(allUUIDs_3, make_vec3(0.5, 0.5, 0), make_vec3(1, 1, 1), make_int3(1, 1, 1));

    if (s1_3.size() == 12) {
        std::cout << "passed" << std::endl;
    } else {
        error_count++;
        std::cerr << "failed: single patch that should be sliced into 12 triangles was sliced into " << s1_3.size()
                  << std::endl;
    }
    ...
    if (count == 1) {
        ...
        error_count++;
        std::cerr << "failed: a single triangle should have been classified as inside the voxel but " << count
                  << " were inside." << std::endl;
    }
    ...
    for (uint i = 0; i < 100; i++) {
        ...
        context_6.addPatch(make_vec3(randu(), randu(), randu()), make_vec2(0.05, 0.05), rotation_6,
                           "../../../plugins/visualizer/textures/GrapeLeaf.png", make_vec2(0.5, 0.5), make_vec2(1, 1));
    }
    ...
    context_7.cropDomain(make_vec2(grid_center_7.x - grid_size_7.x * 0.5, grid_center_7.x + grid_size_7.x * 0.5),
                         make_vec2(grid_center_7.y - grid_size_7.y * 0.5, grid_center_7.y + grid_size_7.y * 0.5),
                         make_vec2(grid_center_7.z - grid_size_7.z * 0.5, grid_center_7.z + grid_size_7.z * 0.5));
    ...
    if (UUIDs_after_crop_7.size() == UUIDs_in_cells_7) {
        ...
        error_count++;
        std::cerr << "failed: number of primitives after cropping doesn't match number in grid cells: "
                  << UUIDs_after_crop_7.size() << " vs. " << UUIDs_in_cells_7 << std::endl;
    }
    ...
    for (uint i = 0; i < 100; i++) {
        ...
        context_8.addPatch(make_vec3(randu(), randu(), randu()), make_vec2(0.05, 0.05), rotation_8,
                           "../../../plugins/visualizer/textures/GrapeLeaf.png", make_vec2(0.5, 0.5), make_vec2(1, 1));
    }
    ...
    context_8.cropDomain(make_vec2(grid_center_8.x - grid_size_8.x * 0.5, grid_center_8.x + grid_size_8.x * 0.5),
                         make_vec2(grid_center_8.y - grid_size_8.y * 0.5, grid_center_8.y + grid_size_8.y * 0.5),
                         make_vec2(grid_center_8.z - grid_size_8.z * 0.5, grid_center_8.z + grid_size_8.z * 0.5));
    ...
    if (UUIDs_after_crop_8.size() == UUIDs_in_cells_8) {
        ...
        error_count++;
        std::cerr << "failed: number of primitives after cropping doesn't match number in grid cells: "
                  << UUIDs_after_crop_8.size() << " vs. " << UUIDs_in_cells_8 << std::endl;
    }
    ...
    for (int k = 0; k < Ninside.z; k++) {
        for (int j = 0; j < Ninside.y; j++) {
            for (int i = 0; i < Ninside.x; i++) {
                vec3 position =
                    vorigin + make_vec3(-0.5 * vsize.x + 2 * psize.x + i * (vsize.x - 4 * psize.x) / float(Ninside.x),
                                        -0.5 * vsize.y + 2 * psize.y + j * (vsize.y - 4 * psize.y) / float(Ninside.y),
                                        -0.5 * vsize.z + 2 * psize.x + k * (vsize.z - 4 * psize.x) / float(Ninside.z));
...
helios::vec3 VoxelIntersection::linesIntersection(helios::vec3 line1_point, helios::vec3 line1_direction,
                                                  helios::vec3 line2_point, helios::vec3 line2_direction) {
    ...
    if (((h.x >= 0 && k.x >= 0) || (h.x < 0 && k.x < 0)) && ((h.y >= 0 && k.y >= 0) || (h.y < 0 && k.y < 0)) &&
...
std::vector<uint> VoxelIntersection::slicePrimitive(uint UUID, std::vector<helios::vec3> voxel_face_vertices) {
    ...
    helios::vec3 face_normal = cross(voxel_face_vertices.at(1) - voxel_face_vertices.at(0),
                                     voxel_face_vertices.at(2) - voxel_face_vertices.at(1));
    ...
    helios::vec3 d1a = helios::make_vec3(-1 * face_normal.x * voxel_face_vertices.at(0).x,
                                         -1 * face_normal.y * voxel_face_vertices.at(0).y,
                                         -1 * face_normal.z * voxel_face_vertices.at(0).z);
    helios::vec3 d2a =
        helios::make_vec3(-1 * patch_normal.x * patch_vertices.at(1).x, -1 * patch_normal.y * patch_vertices.at(1).y,
                          -1 * patch_normal.z * patch_vertices.at(1).z);
    ...
    // go through the different edges of the patch and calculate intersection points with line along edge of patch and
    // intersection line
    if (patch_vertices.size() == 4) {
        possible_points.resize(4);
        possible_points.at(0) = linesIntersection(patch_vertices.at(1), patch_vertices.at(1) - patch_vertices.at(0),
                                                  ipoint, direction_vector);
        possible_points.at(1) = linesIntersection(patch_vertices.at(2), patch_vertices.at(2) - patch_vertices.at(1),
                                                  ipoint, direction_vector);
        possible_points.at(2) = linesIntersection(patch_vertices.at(3), patch_vertices.at(3) - patch_vertices.at(2),
                                                  ipoint, direction_vector);
        possible_points.at(3) = linesIntersection(patch_vertices.at(0), patch_vertices.at(0) - patch_vertices.at(3),
                                                  ipoint, direction_vector);
        ...
        for (uint i = 0; i < 4; i++) {
            ...
            bool test_x = ((possible_points.at(i).x >= vi1.x && possible_points.at(i).x <= vi0.x) ||
                           (possible_points.at(i).x >= vi0.x && possible_points.at(i).x <= vi1.x));
            bool test_y = ((possible_points.at(i).y >= vi1.y && possible_points.at(i).y <= vi0.y) ||
                           (possible_points.at(i).y >= vi0.y && possible_points.at(i).y <= vi1.y));
            bool test_z = ((possible_points.at(i).z >= vi1.z && possible_points.at(i).z <= vi0.z) ||
                           (possible_points.at(i).z >= vi0.z && possible_points.at(i).z <= vi1.z));
        ...
        possible_points.at(1) = linesIntersection(patch_vertices.at(2), patch_vertices.at(2) - patch_vertices.at(1),
                                                  ipoint, direction_vector);
        possible_points.at(2) = linesIntersection(patch_vertices.at(0), patch_vertices.at(0) - patch_vertices.at(2),
                                                  ipoint, direction_vector);
    ...
    if (actual_points.size() == 1) {
        resulting_UUIDs.push_back(UUID);
        if (printmessages) {
            std::cout << "the primitive intersected with the face at a single point (a corner) - no slicing needed"
                      << std::endl;
        }
        return resulting_UUIDs;
    }
    ...
    if (patch_vertices.size() == 4) {
        if ((approxSame(actual_points.at(0), patch_vertices.at(0), absTol) &&
             approxSame(actual_points.at(1), patch_vertices.at(1), absTol)) ||
            (approxSame(actual_points.at(0), patch_vertices.at(1), absTol) &&
             approxSame(actual_points.at(1), patch_vertices.at(0), absTol)) ||
            (approxSame(actual_points.at(0), patch_vertices.at(1), absTol) &&
             approxSame(actual_points.at(1), patch_vertices.at(2), absTol)) ||
            (approxSame(actual_points.at(0), patch_vertices.at(2), absTol) &&
             approxSame(actual_points.at(1), patch_vertices.at(1), absTol)) ||
            (approxSame(actual_points.at(0), patch_vertices.at(2), absTol) &&
             approxSame(actual_points.at(1), patch_vertices.at(3), absTol)) ||
            (approxSame(actual_points.at(0), patch_vertices.at(3), absTol) &&
             approxSame(actual_points.at(1), patch_vertices.at(2), absTol)) ||
            (approxSame(actual_points.at(0), patch_vertices.at(3), absTol) &&
             approxSame(actual_points.at(1), patch_vertices.at(0), absTol)) ||
            (approxSame(actual_points.at(0), patch_vertices.at(0), absTol) &&
             approxSame(actual_points.at(1), patch_vertices.at(3), absTol))) {
            if (printmessages) {
                std::cout << "the primitive intersected with the face along an edge - no need to slice" << std::endl;
            }
    ...
    // if the number of 'actual intersection points' is greater than 2, choose the points that are farthest apart from
    // each other
    if (actual_points.size() == 3) {
        std::cout << "WARNING: 3 intersection points between primitive edge and voxel face for UUID " << UUID
                  << ". Using points farthest apart." << std::endl;
    ...
    // determine which side of the plane vertex 0 is on and use that to determine the sign of the buffer to add to the
    // face coordinate
    //  the buffer ensures that the vertices will be categorized into grid cells correctly
    //  note that some of these checks are based on the assumption of a axis aligned grid - would need to be re-worked
    //  if implementing rotated grid

    helios::vec3 face_coordinate =
        make_vec3(fabs(face_normal.x) * voxel_face_vertices.at(0).x, fabs(face_normal.y) * voxel_face_vertices.at(0).y,
                  fabs(face_normal.z) * voxel_face_vertices.at(0).z);
    ...
    if (patchHasTexture == false) {
        if (patch_vertices.size() == 4) {
            ...
            if ((actual_points_edge_ID.at(0) == 0 && actual_points_edge_ID.at(1) == 2)) {
                t0 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(0) + buffer, patch_color);
                t1 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, patch_color);
                t2 = context->addTriangle(actual_points.at(1) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(3) + buffer, patch_color);
                t3 = context->addTriangle(actual_points.at(0) - buffer, patch_vertices.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, patch_color);
                ...
                t1 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, patch_color);
                t2 = context->addTriangle(actual_points.at(0) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(3) + buffer, patch_color);
                t3 = context->addTriangle(actual_points.at(1) - buffer, patch_vertices.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, patch_color);
            } else if ((actual_points_edge_ID.at(0) == 1 && actual_points_edge_ID.at(1) == 3)) {
                t0 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(1) + buffer, patch_color);
                t1 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(3) - buffer, patch_color);
                t2 = context->addTriangle(actual_points.at(1) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(1) + buffer, patch_color);
                t3 = context->addTriangle(actual_points.at(0) - buffer, patch_vertices.at(2) - buffer,
                                          patch_vertices.at(3) - buffer, patch_color);
                ...
                t2 = context->addTriangle(actual_points.at(0) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(1) + buffer, patch_color);
                t3 = context->addTriangle(actual_points.at(1) - buffer, patch_vertices.at(2) - buffer,
                                          patch_vertices.at(3) - buffer, patch_color);
                ...
                t1 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, patch_color);
                t2 = context->addTriangle(actual_points.at(0) - buffer, patch_vertices.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, patch_color);
                t3 = context->addTriangle(actual_points.at(1) - buffer, patch_vertices.at(2) - buffer,
                                          patch_vertices.at(3) - buffer, patch_color);
                ...
                t1 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, patch_color);
                t2 = context->addTriangle(actual_points.at(1) - buffer, patch_vertices.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, patch_color);
                t3 = context->addTriangle(actual_points.at(0) - buffer, patch_vertices.at(2) - buffer,
                                          patch_vertices.at(3) - buffer, patch_color);
                ...
                t1 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(3) + buffer, patch_color);
                t2 = context->addTriangle(actual_points.at(1) + buffer, patch_vertices.at(2) + buffer,
                                          patch_vertices.at(3) + buffer, patch_color);
                t3 = context->addTriangle(actual_points.at(0) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(3) + buffer, patch_color);
                ...
                t1 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(3) + buffer, patch_color);
                t2 = context->addTriangle(actual_points.at(0) + buffer, patch_vertices.at(2) + buffer,
                                          patch_vertices.at(3) + buffer, patch_color);
                t3 = context->addTriangle(actual_points.at(1) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(3) + buffer, patch_color);
        ...
        if (line_direction == make_vec3(0, 0, 0)) {  // they are already in the xy plane
        } else {
            // rotate them
            float theta = acos_safe((patch_normal.z * xy.z) / (patch_normal.magnitude()) * xy.magnitude());
            for (uint i = 0; i < actual_points.size(); i++) {
                rotated_actual_points_t.at(i) =
                    rotatePointAboutLine(actual_points.at(i), patch_vertices.at(0), line_direction, theta);
            }

            for (uint i = 0; i < patch_vertices.size(); i++) {
                rotated_patch_vertices_t.at(i) =
                    rotatePointAboutLine(patch_vertices.at(i), patch_vertices.at(0), line_direction, theta);
            }
        }
        ...
        if (patch_vertices.size() == 4) {
            ...
            for (uint i = 0; i < rotated_actual_points_t.size(); i++) {
                ...
                if (approxSame(point_1.x, point_0.x, absTol, relTol)) {
                    ...
                    point_uv.x = point_0uv.x + (rotated_actual_points_t.at(i).x - point_0.x) / (point_1.x - point_0.x) *
                                                   (point_1uv.x - point_0uv.x);
                }

                if (approxSame(point_1.y, point_0.y, absTol, relTol)) {
                    point_uv.y = point_0uv.y;
                } else {
                    point_uv.y = point_0uv.y + (rotated_actual_points_t.at(i).y - point_0.y) / (point_1.y - point_0.y) *
                                                   (point_1uv.y - point_0uv.y);
                }
        ...
        if (patch_vertices.size() == 4) {
            ...
            if ((actual_points_edge_ID.at(0) == 0 && actual_points_edge_ID.at(1) == 2)) {
                t0 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(0) + buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(0));
                t1 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(2));
                t2 = context->addTriangle(actual_points.at(1) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(3) + buffer, tex, ip_uv.at(1), v_uv.at(0), v_uv.at(3));
                t3 = context->addTriangle(actual_points.at(0) - buffer, patch_vertices.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, tex, ip_uv.at(0), v_uv.at(1), v_uv.at(2));
            } else if ((actual_points_edge_ID.at(0) == 2 && actual_points_edge_ID.at(1) == 0)) {
                t0 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(0) + buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(0));
                t1 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(2));
                t2 = context->addTriangle(actual_points.at(0) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(3) + buffer, tex, ip_uv.at(0), v_uv.at(0), v_uv.at(3));
                t3 = context->addTriangle(actual_points.at(1) - buffer, patch_vertices.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, tex, ip_uv.at(1), v_uv.at(1), v_uv.at(2));
            } else if ((actual_points_edge_ID.at(0) == 1 && actual_points_edge_ID.at(1) == 3)) {
                t0 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(1) + buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(1));
                t1 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(3) - buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(3));
                t2 = context->addTriangle(actual_points.at(1) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(1) + buffer, tex, ip_uv.at(1), v_uv.at(0), v_uv.at(1));
                t3 = context->addTriangle(actual_points.at(0) - buffer, patch_vertices.at(2) - buffer,
                                          patch_vertices.at(3) - buffer, tex, ip_uv.at(0), v_uv.at(2), v_uv.at(3));
            } else if ((actual_points_edge_ID.at(0) == 3 && actual_points_edge_ID.at(1) == 1)) {
                t0 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(1) + buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(1));
                t1 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(3) - buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(3));
                t2 = context->addTriangle(actual_points.at(0) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(1) + buffer, tex, ip_uv.at(0), v_uv.at(0), v_uv.at(1));
                t3 = context->addTriangle(actual_points.at(1) - buffer, patch_vertices.at(2) - buffer,
                                          patch_vertices.at(3) - buffer, tex, ip_uv.at(1), v_uv.at(2), v_uv.at(3));
                // cases where intersection points are on adjacent sides
            } else if ((actual_points_edge_ID.at(0) == 0 && actual_points_edge_ID.at(1) == 3)) {
                t0 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(0) + buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(0));
            ...
            if ((actual_points_edge_ID.at(0) == 0 && actual_points_edge_ID.at(1) == 1)) {
                ...
                t1 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(2) + buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(2));
                t2 = context->addTriangle(actual_points.at(0) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(2) + buffer, tex, ip_uv.at(0), v_uv.at(0), v_uv.at(2));
            } else if ((actual_points_edge_ID.at(0) == 1 && actual_points_edge_ID.at(1) == 0)) {
                t0 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(1) - buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(1));
                t1 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(2) + buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(2));
                t2 = context->addTriangle(actual_points.at(1) + buffer, patch_vertices.at(0) + buffer,
                                          patch_vertices.at(2) + buffer, tex, ip_uv.at(1), v_uv.at(0), v_uv.at(2));
            } else if ((actual_points_edge_ID.at(0) == 0 && actual_points_edge_ID.at(1) == 2)) {
                t0 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(0) + buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(0));
                t1 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                ...
                t0 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(0));
                t1 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(0) + buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(2));
                t2 = context->addTriangle(actual_points.at(0) + buffer, patch_vertices.at(1) + buffer,
                                          patch_vertices.at(0) + buffer, tex, ip_uv.at(0), v_uv.at(1), v_uv.at(0));
            } else if ((actual_points_edge_ID.at(0) == 2 && actual_points_edge_ID.at(1) == 1)) {
                t0 = context->addTriangle(actual_points.at(0) - buffer, actual_points.at(1) - buffer,
                                          patch_vertices.at(2) - buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(0));
                t1 = context->addTriangle(actual_points.at(0) + buffer, actual_points.at(1) + buffer,
                                          patch_vertices.at(0) + buffer, tex, ip_uv.at(0), ip_uv.at(1), v_uv.at(2));
                t2 = context->addTriangle(actual_points.at(1) + buffer, patch_vertices.at(1) + buffer,
                                          patch_vertices.at(0) + buffer, tex, ip_uv.at(1), v_uv.at(1), v_uv.at(0));
            }
...
std::vector<uint> VoxelIntersection::slicePrimitivesUsingGrid(std::vector<uint> UUIDs, helios::vec3 grid_center,
                                                              helios::vec3 grid_size, helios::int3 grid_divisions) {
    // set up the grid
    std::vector<std::vector<helios::vec3>> grid_face_vertices;
    helios::vec3 grid_min = make_vec3(grid_center.x - grid_size.x * 0.5, grid_center.y - grid_size.y * 0.5,
                                      grid_center.z - grid_size.z * 0.5);
    helios::vec3 grid_max = make_vec3(grid_center.x + grid_size.x * 0.5, grid_center.y + grid_size.y * 0.5,
                                      grid_center.z + grid_size.z * 0.5);
    helios::vec3 grid_spacing =
        make_vec3(grid_size.x / grid_divisions.x, grid_size.y / grid_divisions.y, grid_size.z / grid_divisions.z);
        ...
        for (uint k = 0; k < (grid_divisions.z); k++) {
            for (uint j = 0; j < (grid_divisions.y); j++) {
                for (uint i = 0; i < (grid_divisions.x); i++) {
                    helios::vec3 cell_min =
                        make_vec3(grid_min.x + float(i) * grid_spacing.x, grid_min.y + float(j) * grid_spacing.y,
                                  grid_min.z + float(k) * grid_spacing.z);
                    helios::vec3 cell_max = make_vec3(grid_min.x + float(i) * grid_spacing.x + grid_spacing.x,
                                                      grid_min.y + float(j) * grid_spacing.y + grid_spacing.y,
                                                      grid_min.z + float(k) * grid_spacing.z + grid_spacing.z);
    ...
    if (printmessages) {
        ...
        std::cout << UUIDs_no_slice.size() << " input primitives ("
                  << float(UUIDs_no_slice.size()) / float(UUIDs.size()) * 100 << "%) not sliced" << std::endl;
        std::cout << UUIDs_to_slice.size() << " input primitives ("
                  << float(UUIDs_to_slice.size()) / float(UUIDs.size()) * 100 << "%) being sliced" << std::endl;
    }
    ...
    for (uint p = 0; p < UUIDs_to_slice.size(); p++) {
        ...
        for (uint k = 0; k < (grid_divisions.z); k++) {
            for (uint j = 0; j < (grid_divisions.y); j++) {
                for (uint i = 0; i < (grid_divisions.x); i++) {
                    helios::vec3 cell_min = make_vec3(grid_min.x + i * grid_spacing.x, grid_min.y + j * grid_spacing.y,
                                                      grid_min.z + k * grid_spacing.z);
                    helios::vec3 cell_max = make_vec3(grid_min.x + i * grid_spacing.x + grid_spacing.x,
                                                      grid_min.y + j * grid_spacing.y + grid_spacing.y,
                                                      grid_min.z + k * grid_spacing.z + grid_spacing.z);
                    ...
                    for (uint v = 0; v < verts.size(); v++) {
                        ...
                        float relTol = pow(10, -20);
                        bool test2_x =
                            (verts.at(v).x > cell_min.x || approxSame(verts.at(v).x, cell_min.x, absTol, relTol)) &&
                            (verts.at(v).x < cell_max.x || approxSame(verts.at(v).x, cell_max.x, absTol, relTol));
                        bool test2_y =
                            (verts.at(v).y > cell_min.y || approxSame(verts.at(v).y, cell_min.y, absTol, relTol)) &&
                            (verts.at(v).y < cell_max.y || approxSame(verts.at(v).y, cell_max.y, absTol, relTol));
                        bool test2_z =
                            (verts.at(v).z > cell_min.z || approxSame(verts.at(v).z, cell_min.z, absTol, relTol)) &&
                            (verts.at(v).z < cell_max.z || approxSame(verts.at(v).z, cell_max.z, absTol, relTol));
...
std::vector<std::vector<uint>> VoxelIntersection::getCellPrimitives(void) { return cell_primitives; }
