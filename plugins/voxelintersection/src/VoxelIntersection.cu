...
__global__ void insideVolume_vi(const uint Nhits, const float3* d_hit_xyz, const uint Ngridcells,
                                const float3* d_grid_size, const float3* d_grid_center, const float* d_grid_rotation,
                                int* d_hit_vol) {
...
void VoxelIntersection::disableMessages(void) { printmessages = false; }

void VoxelIntersection::enableMessages(void) { printmessages = true; }
...
void VoxelIntersection::calculatePrimitiveVoxelIntersection(std::vector<uint> UUIDs) {
    ...
    insideVolume_vi<<<dimGrid, dimBlock>>>(N, d_hit_xyz, Ncells, d_grid_size, d_grid_center, d_grid_rotation,
                                           d_hit_vol);
