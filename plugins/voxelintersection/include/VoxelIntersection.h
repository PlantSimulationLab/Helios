...
class VoxelIntersection {
   public:
    ...
    /** \param[in] "UUIDs" Unique universal identifiers for primitives (non-voxels) to include in intersection
     * calculation
     */
     ...
     */
    helios::vec3 linesIntersection(helios::vec3 line1_point, helios::vec3 line1_direction, helios::vec3 line2_point,
     ...
     */
    std::vector<uint> slicePrimitivesUsingGrid(std::vector<uint> UUIDs, helios::vec3 grid_center,
    ...
    /**
     ...
     \note vector is one element longer than the number of grid cells, with the last element storing UUIDs that didn't
     fall into any grid cell
     */
