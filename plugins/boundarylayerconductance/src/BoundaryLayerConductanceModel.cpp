...
int BLConductanceModel::selfTest(void) {
    ...
    if (fabs(gH.at(0) - 0.104571f) / gH.at(0) > err_tol || fabs(gH.at(1) - 0.133763f) / gH.at(1) > err_tol ||
        fabs(gH.at(2) - 0.087149f) / gH.at(2) > err_tol || fabs(gH.at(3) - 0.465472f) / gH.at(3) > err_tol) {
...
void BLConductanceModel::enableMessages(void) { message_flag = true; }

void BLConductanceModel::disableMessages(void) { message_flag = false; }
...
void BLConductanceModel::setBoundaryLayerModel(const std::vector<uint>& UUIDs, const char* gH_model) {
    ...
    if (strcmp(gH_model, "Polhausen") == 0) {
        ...
        std::cout << "WARNING (EnergyBalanceModel::setBoundaryLayerModel): Boundary-layer conductance model "
                     ""
                  << gH_model
                  << ""
                     " is unknown. Skipping this function call.."
                  << std::endl;
        return;
    }
...
void BLConductanceModel::run() { run(context->getAllUUIDs()); }
...
float BLConductanceModel::calculateBoundaryLayerConductance(const uint gH_model, const float U, const float L,
                                                            const float inclination, float TL, float Ta) {
    assert(gH_model < 4);
    ...
    if (gH_model == 0) {  // Polhausen equation
        // This comes from the correlation by Polhausen - see Eq. XX of Campbell and Norman (1998). It assumes a flat
        // plate parallel to the direction of the flow, which extends infinitely in the cross-stream direction and "L"
        // in the streamwise direction. It also assumes that the air is at standard temperature and pressure, and flow
        // is laminar, forced convection.
        ...
        if (inclination < 75.f * M_PI / 180.f || inclination > 105.f * M_PI / 180.f) {
            gH = 41.4 * alpha / L * 2.f * F1 * sqrtf(Re) *
                 pow(1.f + free_direction *
                               pow(2.f * F2 * pow(Gr * fabs(cos(inclination)) / (Re * Re), 0.25) / (3.f * F1), 3),
                     1.f / 3.f);
            ...
            gH = 41.4 * alpha / L * 2.f * F1 * sqrtf(Re) *
                 pow(1.f + free_direction * pow((F3 * pow(Gr * pow(Re, -5.f / 2.f), 1.f / 5.f) * pow(Gr, C)) /
                                                    (6.f * (1.f / 5.f + C) * F1),
                                                3),
                     1.f / 3.f);
        }
        ...
        // assuming standard temperature and pressure, (m^3 air)*(1.2041 kg/m^3)/(0.02897 kg/mol) = 41.56 (mol
        // air)/(m^2-sec.)
