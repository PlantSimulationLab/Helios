...
uint WeberPennTree::buildTree(const char* treename, helios::vec3 origin) { return buildTree(treename, origin, 1); }
...
uint WeberPennTree::buildTree(const char* treename, helios::vec3 origin, float scale) {
    if (trees_library.find(treename) == trees_library.end()) {
        std::cerr << "ERROR (WeberPennTree::buildTree): Tree " << treename << " does not exist in the tree library."
                  << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    if (file.substr(file.find_last_of(".") + 1) != "png") {
        std::cerr << "ERROR (WeberPennTree::buildTree): Leaf image file for tree " << treename << " is not a PNG image."
                  << std::endl;
        exit(EXIT_FAILURE);
#ifndef _WIN32
    } else if (access(file.c_str(), F_OK) == -1) {
        std::cerr << "ERROR (WeberPennTree::buildTree): Leaf image file " << treename << " does not exist."
                  << std::endl;
        exit(EXIT_FAILURE);
#endif
    }

    uint ID_leaf_template = context->addTileObject(
        make_vec3(0, 0, 0),
        make_vec2(parameters.LeafScale * scale, parameters.LeafScale * parameters.LeafScaleX * scale),
        make_SphericalCoord(0, M_PI), leaf_segs, parameters.LeafFile.c_str());
    ...
    // Length of trunk
    float length0 = (parameters.Scale + getVariation(parameters.ScaleV)) *
                    (parameters.nLength[0] + getVariation(parameters.nLengthV[0])) * scale;
    ...
    if (parameters.BaseSplits > 0) {  // trunk splits at base
        ...
        for (uint i = 1; i < base_nodes; i++) {
            // nodes.at(i) = nodes.at(i-1) + rotatePoint( make_vec3( 0, 0,
            // length0*(1-parameters.BaseSize)/parameters.nCurveRes.at(0) ), theta, phi );
            ...
            radius.at(i) =
                length0 * parameters.Ratio * (1 - parameters.nTaper.at(0) * nodes.at(i).z / length0) * flarez;
        ...
        for (uint j = 0; j < parameters.BaseSplits + 1; j++) {  // looping over clones (splits)
            ...
            float angle_split =
                (parameters.nSplitAngle.at(0) - getVariation(parameters.nSplitAngleV.at(0))) * M_PI / 180.f;

            vec3 current_normal = make_vec3(0, 0, 1);

            helios::SphericalCoord child_rotation = make_SphericalCoord(angle_split, phi_split);

            recursiveBranch(parameters, 0, 0, base_position, current_normal, child_rotation, length0 - offset_child,
                            radius.at(base_nodes - 1), offset_child, origin, scale, ID_leaf_template);
        ...
        for (uint i = 1; i < parameters.nCurveRes.at(0); i++) {
            ...
            if (parameters.Levels > 0 && Zplus >= parameters.BaseSize) {
                for (uint s = 0; s < stems_per_segment; s++) {
                    vec3 base_position =
                        nodes.at(i - 1) + (nodes.at(i) - nodes.at(i - 1)) * float(s + 0.5) / float(stems_per_segment);

                    float downangle =
                        (parameters.nDownAngle.at(1) + getVariation(parameters.nDownAngleV.at(1))) * M_PI / 180.f;
                    helios::SphericalCoord child_rotation = make_SphericalCoord(downangle, phi_child);

                    float radius_parent =
                        radius.at(i - 1) + base_position.z / length0 * (radius.at(i) - radius.at(i - 1));

                    // float offset_child = parameters.BaseSize*length0+(i-2)*dlength0 +
                    // dlength0*float(s)/float(stems_per_segment-1);
                    float base_nodes = floor(parameters.BaseSize * parameters.nCurveRes.at(0));
                    float offset_child = (Zminus - parameters.BaseSize) * length0 +
                                         float(s + 0.5) / float(stems_per_segment - 1) * dlength0;

                    phi_child += (parameters.nRotate.at(1) + getVariation(parameters.nRotateV.at(1))) * M_PI / 180.f;

                    if (offset_child > 0) {
                        recursiveBranch(parameters, 1, 0, base_position, current_normal, child_rotation, length0,
                                        radius_parent, offset_child, origin, scale, ID_leaf_template);
                    }
                }
            }

            theta += (-parameters.nCurve.at(0) + getVariation(parameters.nCurveV.at(0))) /
                     float(parameters.nCurveRes.at(0)) * M_PI / 180.f;
        }
    }
...
void WeberPennTree::recursiveBranch(WeberPennTreeParameters parameters, uint n, uint seg_start,
                                    helios::vec3 base_position, helios::vec3 parent_normal,
                                    helios::SphericalCoord child_rotation, float length_parent, float radius_parent,
                                    float offset_child, helios::vec3 origin, float scale, const uint leaf_template) {
    if (n < parameters.Levels) {  // Branches
        ...
        float ratio;
        if (n <= 1) {
            ratio = (length_parent * (1 - parameters.BaseSize) - offset_child) /
                    (length_parent * (1 - parameters.BaseSize));
        } else {
        ...
        if (n == 0) {
            ...
            radius.at(0) = radius_parent * pow(length_child / length_parent, parameters.RatioPower) *
                           (1 - parameters.nTaper.at(n) * float(seg_start) / float(parameters.nCurveRes.at(n)));
        }
        ...
        for (uint i = seg_start + 1; i < parameters.nCurveRes.at(n) + 1; i++) {
            ...
            vec3 child_normal = sphere2cart(
                make_SphericalCoord(parent_rotation.elevation - child_rotation.elevation, parent_rotation.azimuth));
            child_normal.normalize();

            child_normal =
                rotatePointAboutLine(child_normal, make_vec3(0, 0, 0), parent_normal, child_rotation.azimuth);
            ...
            for (uint s = 0; s < stems_per_segment; s++) {
                child_rotation.elevation += (parameters.nCurve.at(n) + getVariation(parameters.nCurveV.at(n))) /
                                            float(parameters.nCurveRes.at(n)) / float(stems_per_segment) * M_PI / 180.f;
                ...
                vec3 base_position =
                    nodes.at(i - seg_start - 1) + (nodes.at(i - seg_start) - nodes.at(i - seg_start - 1)) * f;
                ...
                float offset_child =
                    dlength_child * (i - seg_start - 1) + dlength_child * float(s) / float(stems_per_segment);

                float downangle =
                    (parameters.nDownAngle.at(n + 1) + getVariation(parameters.nDownAngleV.at(n + 1))) * M_PI / 180.f;
                SphericalCoord rotation = make_SphericalCoord(downangle, phi_child);

                recursiveBranch(parameters, n + 1, 0, base_position, normal, rotation, length_child, radius_p,
                                offset_child, origin, scale, leaf_template);

                if (n == parameters.Levels - 1) {  // leaves
                    phi_child +=
                        (parameters.nRotate.at(n + 1) + getVariation(parameters.nRotateV.at(n + 1))) * M_PI / 180.f;
                } else {  // branches
                    phi_child +=
                        (parameters.nRotate.at(n + 1) + getVariation(parameters.nRotateV.at(n + 1))) * M_PI / 180.f;
                }
            ...
            if (parameters.nSegSplits.at(n) > 0 && i < parameters.nCurveRes.at(n)) {
                float angle_split =
                    (parameters.nSplitAngle.at(n + 1) - getVariation(parameters.nSplitAngleV.at(n + 1))) * M_PI / 180.f;
                ...
                for (uint j = 0; j < parameters.nSegSplits.at(n) + 1; j++) {  // looping over clones (splits)
                    ...
                    recursiveBranch(parameters, n, i, base_position, normal, rotation, length_parent, radius_p,
                                    offset_child, origin, scale, leaf_template);
        ...
        if (parameters.leafAngleCDF.size() > 0) {
            ...
            context->getObjectPointer(ID)->rotate(
                M_PI, "x");  // flip leaf (this is so lighting looks right based on leaf normal)
...
void WeberPennTree::setBranchRecursionLevel(const uint level) { branchLevels = level; }

void WeberPennTree::setTrunkSegmentResolution(const uint segs) {
    if (segs < 3) {
        std::cout << "WARNING (WeberPennTree::setTrunkSegmentResolution): cannot set trunk segment resolution less "
                     "than 3. Ignoring this command."
                  << std::endl;
    } else {
        trunk_segs = segs;
    }
...
void WeberPennTree::setBranchSegmentResolution(const uint segs) {
    if (segs < 3) {
        std::cout << "WARNING (WeberPennTree::setBranchSegmentResolution): cannot set branch segment resolution less "
                     "than 3. Ignoring this command."
                  << std::endl;
    } else {
        branch_segs = segs;
...
void WeberPennTree::setLeafSubdivisions(const helios::int2 segs) {
    if (segs.x < 1 || segs.y < 1) {
        std::cout << "WARNING (WeberPennTree::setLeafSubdivisions): cannot set number of leaf subdivisions to less "
                     "than 1. Ignoring this command."
                  << std::endl;
    } else {
        leaf_segs = segs;
    }
...
WeberPennTreeParameters WeberPennTree::getTreeParameters(const char* treename) {
    if (trees_library.find(treename) == trees_library.end()) {
        std::cerr << "ERROR (WeberPennTree::getTreeParameters): Tree " << treename
                  << " does not exist in the tree library." << std::endl;
        exit(EXIT_FAILURE);
    }
...
void WeberPennTree::setTreeParameters(const char* treename, const WeberPennTreeParameters parameters) {
    if (trees_library.find(treename) == trees_library.end()) {
        std::cerr << "ERROR (WeberPennTree::setTreeParameters): Tree " << treename
                  << " does not exist in the tree library." << std::endl;
        exit(EXIT_FAILURE);
    }
...
void WeberPennTree::seedRandomGenerator(const uint seed) { generator.seed(seed); }
...
void WeberPennTree::loadXML(const char* filename) {
    ...
    if (!result) {
        ...
        std::cerr << "XML [" << filename << "] parsed with errors, attr value: ["
    ...
    if (helios.empty()) {
        std::cout << "failed." << std::endl;
        std::cerr << "ERROR (loadXML): XML file must have tag '<helios> ... </helios>' bounding all other tags."
                  << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    for (pugi::xml_node p = helios.child("WeberPennTree"); p; p = p.next_sibling("WeberPennTree")) {
        ...
        if (strlen(basesize_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): BaseSize was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        } else {
        ...
        if (strlen(basesplits_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): BaseSplits was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        } else {
        ...
        if (strlen(basesplitsize_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): BaseSplitSize was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        } else {
        ...
        if (strlen(ratiopower_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): RatioPower was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        } else {
        ...
        if (strlen(lobedepth_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): LobeDepth was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        } else {
        ...
        if (strlen(nsegsplits_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): nSegSplits was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < endLevel; i++) {
            if (nsegsplits_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nSegSplits was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        if (strlen(nsplitangle_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): nSplitAngle was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < endLevel; i++) {
            if (nsplitangle_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nSplitAngle was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        if (strlen(nsplitanglev_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): nSplitAngleV was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < endLevel; i++) {
            if (nsplitanglev_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nSplitAngleV was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        if (strlen(ncurveres_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): nCurveRes was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < endLevel; i++) {
            if (ncurveres_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nCurveRes was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        for (int i = 0; i < endLevel; i++) {
            if (ncurve_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nCurve was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        for (int i = 0; i < endLevel; i++) {
            if (ncurvev_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nCurveV was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        if (strlen(ncurveback_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): nCurveBack was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < endLevel; i++) {
            if (ncurveback_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nCurveBack was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        for (int i = 0; i < endLevel; i++) {
            if (nlength_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nLength was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        if (strlen(nlengthv_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): nLengthV was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < endLevel; i++) {
            if (nlengthv_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nLengthV was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        for (int i = 0; i < endLevel; i++) {
            if (ntaper_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nTaper was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        if (strlen(ndownangle_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): nDownAngle was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < endLevel; i++) {
            if (ndownangle_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nDownAngle was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        if (strlen(ndownanglev_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): nDownAngleV was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < endLevel; i++) {
            if (ndownanglev_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nDownAngleV was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        for (int i = 0; i < endLevel; i++) {
            if (nrotate_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nRotate was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        if (strlen(nrotatev_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): nRotateV was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < endLevel; i++) {
            if (nrotatev_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nRotateV was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        if (strlen(nbranches_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): nBranches was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < endLevel; i++) {
            if (nbranches_stream.peek() == EOF) {
                std::cerr << "ERROR (WeberPennTree::loadXML): nBranches was not given for level " << i << " of tree "
                          << label << "." << std::endl;
                exit(EXIT_FAILURE);
            } else {
        ...
        if (leaffile_str.empty()) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): LeafFile was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        } else {
        ...
        if (strlen(leafscale_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): LeafScale was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        } else {
        ...
        if (strlen(leafscalex_str) == 0) {
            std::cout << "failed." << std::endl;
            std::cerr << "ERROR (WeberPennTree::loadXML): LeafScaleX was not given for tree " << label << "."
                      << std::endl;
            exit(EXIT_FAILURE);
        } else {
        ...
        if (leafAngleDist.size() > 0) {
            ...
            if (fabs(tsum - 1.f) > 0.001) {
                std::cout << "WARNING (WeberPennTree::loadXML): Leaf angle distribution for tree " << label
                          << " does not sum to 1. Assuming isotropic." << std::endl;
    ...
    if (tree_count == 0) {
        std::cout << "failed." << std::endl;
        std::cerr << "ERROR (WeberPennTree::loadXML): XML file " << filename << " did not contain any tree definitions."
                  << std::endl;
    } else {
        std::cout << "done." << std::endl;
...
void WeberPennTree::optionalOutputPrimitiveData(const char* label) {
    if (strcmp(label, "element_type") == 0 || strcmp(label, "branch_radius") == 0) {
        ...
        std::cout << "WARNING (WeberPennTree::optionalOutputPrimitiveData): unknown optional output primitive data "
                  << label << std::endl;
    }
}
