...
void WeberPennTree::recursiveBranch(WeberPennTreeParameters parameters, uint n, uint seg_start, helios::vec3 base_position, helios::vec3 parent_normal, helios::SphericalCoord child_rotation, float length_parent, float radius_parent,
                                    float offset_child, helios::vec3 origin, float scale, const uint leaf_template, float base_size, uint base_splits) {
    ...
    if (n < parameters.Levels) { // Branches
        ...
        for (uint i = seg_start + 1; i < parameters.nCurveRes.at(n) + 1; i++) {
            ...
            for (uint s = 0; s < stems_per_segment; s++) {
                ...
                child_rotation = make_SphericalCoord(child_rotation.radius, child_rotation.elevation + (parameters.nCurve.at(n) + getVariation(parameters.nCurveV.at(n))) / float(parameters.nCurveRes.at(n)) / float(stems_per_segment) * M_PI / 180.f,
                                                     child_rotation.azimuth);
