...
static ParameterOptimization::Result evaluatePopulation(
        const std::vector<ParametersToOptimize>& pop,
        const std::function<float(const ParametersToOptimize&)>& simulation){
...
ParameterOptimization::Result ParameterOptimization::run(
        std::function<float(const ParametersToOptimize&)> simulation,
        ...
        const OptimizationSettings& settings)
{
    ...
    for (size_t g = 0; g < settings.generations; g++) {
        ...
        std::partial_sort(population.begin(), population.begin() + elitism_count, population.end(),
