...
int EnergyBalanceModel::selfTest() {
    ...
    //---- Equilibrium test -----//
    // If we set the absorbed radiation flux to 5.67e-8*T^4 and have a zero moisture conductance, we should get a
    // surface temperature of exactly T.
...
void EnergyBalanceModel::enableMessages() { message_flag = true; }

void EnergyBalanceModel::disableMessages() { message_flag = false; }

void EnergyBalanceModel::addRadiationBand(const char* band) { radiation_bands.push_back(band); }
...
void EnergyBalanceModel::optionalOutputPrimitiveData(const char* label) {
    if (strcmp(label, "boundarylayer_conductance_out") == 0 || strcmp(label, "vapor_pressure_deficit") == 0) {
        ...
        std::cout << "WARNING (EnergyBalanceModel::optionalOutputPrimitiveData): unknown output primitive data "
                  << label << std::endl;
    }
}
