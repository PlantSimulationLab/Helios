...
__device__ float evaluateEnergyBalance(float T, float R, float Qother, float eps, float Ta, float ea, float pressure,
                                       float gH, float gS, uint Nsides, float heatcapacity, float dt, float Tprev) {
    ...
    float es =
        611.f *
        exp(17.502f * (T - 273.f) /
            ((T - 273.f) + 240.97f));  // This is Clausius-Clapeyron equation (See Campbell and Norman pp. 41 Eq. 3.8).
                                       // Note that temperature must be in Kelvin, and result is in Pascals
...
__global__ void solveEnergyBalance(uint Nprimitives, float* To, float* R, float* Qother, float* eps, float* Ta,
                                   float* ea, float* pressure, float* gH, float* gS, uint* Nsides, float* TL,
                                   float* heatcapacity, float dt) {
    ...
    float resid_old = evaluateEnergyBalance(T_old, R[p], Qother[p], eps[p], Ta[p], ea[p], pressure[p], gH[p], gS[p],
                                            Nsides[p], heatcapacity[p], dt, To[p]);
    float resid_old_old = evaluateEnergyBalance(T_old_old, R[p], Qother[p], eps[p], Ta[p], ea[p], pressure[p], gH[p],
                                                gS[p], Nsides[p], heatcapacity[p], dt, To[p]);
...
void EnergyBalanceModel::run() { run(context->getAllUUIDs()); }

void EnergyBalanceModel::run(float dt) { run(context->getAllUUIDs(), dt); }

void EnergyBalanceModel::run(const std::vector<uint>& UUIDs) { run(UUIDs, 0.f); }
...
void EnergyBalanceModel::run(const std::vector<uint>& UUIDs, float dt) {
    ...
    if (Nprimitives == 0) {
        std::cerr << "ERROR (EnergyBalanceModel): No primitives have been added to the context.  There is nothing to "
                     "simulate. Exiting..."
                  << std::endl;
        return;
    }
    ...
    // Look through all flux primitive data in the context and sum them up in vector Rn.  Each element of Rn corresponds
    // to a primitive.
        ...
        for (size_t u = 0; u < Nprimitives; u++) {
            ...
            if (!context->doesPrimitiveDataExist(p, str)) {
                throw(std::runtime_error(
                    "ERROR (EnergyBalanceModel::run): No radiation was found in the context for band " +
                    std::string(radiation_bands.at(b)) + ". Did you run the radiation model for this band?"));
            } else if (context->getPrimitiveDataType(p, str) != HELIOS_TYPE_FLOAT) {
                throw(std::runtime_error("ERROR (EnergyBalanceModel::run): Radiation primitive data for band " +
    ...
    for (uint u = 0; u < Nprimitives; u++) {
        ...
        if (context->doesPrimitiveDataExist(p, "temperature") &&
            context->getPrimitiveDataType(p, "temperature") == HELIOS_TYPE_FLOAT) {
        ...
        if (context->doesPrimitiveDataExist(p, "air_temperature") &&
            context->getPrimitiveDataType(p, "air_temperature") == HELIOS_TYPE_FLOAT) {
        ...
        if (context->doesPrimitiveDataExist(p, "air_humidity") &&
            context->getPrimitiveDataType(p, "air_humidity") == HELIOS_TYPE_FLOAT) {
        ...
        // Air vapor pressure
        float esat =
            611.f * exp(17.502f * (Ta[u] - 273.f) /
                        ((Ta[u] - 273.f) +
                         240.97f));  // This is Clausius-Clapeyron equation (See Campbell and Norman pp. 41 Eq. 3.8).
                                     // Note that temperature must be in degC, and result is in Pascals
        ...
        if (context->doesPrimitiveDataExist(p, "air_pressure") &&
            context->getPrimitiveDataType(p, "air_pressure") == HELIOS_TYPE_FLOAT) {
        ...
        if (context->doesPrimitiveDataExist(p, "boundarylayer_conductance") &&
            context->getPrimitiveDataType(p, "boundarylayer_conductance") == HELIOS_TYPE_FLOAT) {
            context->getPrimitiveData(p, "boundarylayer_conductance", gH[u]);
        } else {
            // Wind speed
            float U;
            if (context->doesPrimitiveDataExist(p, "wind_speed") &&
                context->getPrimitiveDataType(p, "wind_speed") == HELIOS_TYPE_FLOAT) {
            ...
            if (context->doesPrimitiveDataExist(p, "object_length") &&
                context->getPrimitiveDataType(p, "object_length") == HELIOS_TYPE_FLOAT) {
        ...
        if (context->doesPrimitiveDataExist(p, "moisture_conductance") &&
            context->getPrimitiveDataType(p, "moisture_conductance") == HELIOS_TYPE_FLOAT) {
        ...
        if (context->doesPrimitiveDataExist(p, "other_surface_flux") &&
            context->getPrimitiveDataType(p, "other_surface_flux") == HELIOS_TYPE_FLOAT) {
        ...
        if (context->doesPrimitiveDataExist(p, "twosided_flag") &&
            context->getPrimitiveDataType(p, "twosided_flag") == HELIOS_TYPE_UINT) {
            uint flag;
        ...
        if (context->doesPrimitiveDataExist(p, "heat_capacity") &&
            context->getPrimitiveDataType(p, "heat_capacity") == HELIOS_TYPE_FLOAT) {
    ...
    solveEnergyBalance<<<dimGrid, dimBlock>>>(Nprimitives, d_To, d_R, d_Qother, d_eps, d_Ta, d_ea, d_pressure, d_gH,
                                              d_gS, d_Nsides, d_T, d_heatcapacity, dt);
