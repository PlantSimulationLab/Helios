...
class EnergyBalanceModel {
   public:
    ...
    /** \param[in] "UUIDs" Unique universal identifiers (UUIDs) for primitives that should be included in energy balance
     * calculations. All other primitives will be skipped by the model.*/
    ...
    /** \param[in] "UUIDs"  Unique universal identifiers (UUIDs) for primitives that should be included in energy
       balance calculations. All other primitives will be skipped by the model. \param[in] "dt" Time step in seconds.
    */
    void run(const std::vector<uint>& UUIDs, float dt);

    //! Add the label of a radiation band in the RadiationModel plug-in that should be used in calculation of the
    //! absorbed all-wave radiation flux
