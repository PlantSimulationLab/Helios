...
#define RT_CHECK_ERROR_NOEXIT(func)                                            \
    do {                                                                        \
        ...
        if (code != RT_SUCCESS) {                                              \
            ...
            std::cerr << "WARNING (OptiX cleanup): " << message                 \
...
private:
    ...
    void addBuffer(const char* name, RTbuffer& buffer, RTvariable& variable,
    ...
    void diffuseToBuffers(const std::vector<float>& flux,
                         const std::vector<float>& extinction,
                         const std::vector<helios::vec3>& peak_dir,
                         const std::vector<float>& dist_norm,
                         const std::vector<float>& sky_energy);
    void skyModelToBuffers(const std::vector<helios::vec4>& sky_radiance_params,
                          const std::vector<float>& camera_sky_radiance,
                          const helios::vec3& sun_direction,
                          const std::vector<float>& solar_disk_radiance,
