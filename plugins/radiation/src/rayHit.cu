...
RT_PROGRAM void closest_hit_diffuse() {
    ...
    if ((periodic_flag.x == 1 || periodic_flag.y == 1) && primitive_type[objID] == 5) {  // periodic boundary condition
        ...
        // Note: UUID corresponds to the object that the ray hit (i.e., where energy is coming from), and UUID_origin is
        // the object the ray originated from (i.e., where the energy is being recieved)
        ...
        //   optix::Ray ray_transmit = optix::make_Ray(ray.origin+(t_hit+prd.area+1e-5)*ray.direction, ray.direction,
        //   ray.ray_type, 1e-4, RT_DEFAULT_MAX); PerRayData prd_transmit = prd; float beta = rho[UUID]+tau[UUID];
...
RT_PROGRAM void miss_diffuse() {
    ...
    if (primitive_type[objectID[prd.origin_UUID]] == 4) {  // ray was launched from voxel
        ...
        if (t_rho > 0 || t_tau > 0) {
            if (prd.face) {  // reflection from top, transmission from bottom
                atomicFloatAdd(&scatter_buff_top[prd.origin_UUID],
                               fd * diffuseFlux * t_rho * prd.strength);  // reflection
                atomicFloatAdd(&scatter_buff_bottom[prd.origin_UUID],
                               fd * diffuseFlux * t_tau * prd.strength);  // transmission
            } else {  // reflection from bottom, transmission from top
                atomicFloatAdd(&scatter_buff_bottom[prd.origin_UUID],
                               fd * diffuseFlux * t_rho * prd.strength);  // reflection
                atomicFloatAdd(&scatter_buff_top[prd.origin_UUID],
