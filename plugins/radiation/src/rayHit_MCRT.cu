...
RT_PROGRAM void closest_hit_MCRT(){
  ...
  //     rtPrintf("Launching transmission ray from prim #%d with strength %f, position (%f,%f,%f), direction (%f,%f,%f)\n",UUID,prd.strength,ray_origin.x,ray_origin.y,ray_origin.z,ray_direction.x,ray_direction.y,ray_direction.z);
...
RT_PROGRAM void miss_MCRT(){
  atomicAdd( &Rsky[prd.origin_UUID], prd.strength );
}
