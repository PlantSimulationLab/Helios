...
RT_PROGRAM void rectangle_intersect(int objID /**< [in] index of primitive in geometric object.*/) {
    if (prd.origin_UUID == patch_UUID[objID]) {  // the ray should not intersect the primitive from which it was
                                                 // launched
        return;
            ...
            if (ddotb > 0.0 && ddotb < dot(b, b)) {
                ...
                if (maskID[ID] == -1) {  // no texture transparency
                    ...
                    if (uvID[ID] == -1) {  // does not have custom (u,v) coordinates
                        ind = make_uint3(roundf(float(sz.x - 1) * fabs(uv.x)), roundf(float(sz.y - 1) * fabs(uv.y)),
                                         maskID[ID]);
                    } else {
                        ...
                        ind = make_uint3(roundf(float(sz.x - 1) * (uvmin.x + fabs(uv.x) * duv.x)),
                                         roundf(float(sz.y - 1) * (uvmin.y + fabs(uv.y) * duv.y)), maskID[ID]);
...
RT_PROGRAM void rectangle_bounds(int objID, float result[6]) {
    ...
    float3 v2 = patch_vertices[make_uint2(2, objID)];
    float3 v3 = patch_vertices[make_uint2(3, objID)];
    float3 min = make_float3(fmin(fmin(v0.x, v1.x), fmin(v2.x, v3.x)), fmin(fmin(v0.y, v1.y), fmin(v2.y, v3.y)),
                             fmin(fmin(v0.z, v1.z), fmin(v2.z, v3.z)));
    float3 max = make_float3(fmax(fmax(v0.x, v1.x), fmax(v2.x, v3.x)), fmax(fmax(v0.y, v1.y), fmax(v2.y, v3.y)),
                             fmax(fmax(v0.z, v1.z), fmax(v2.z, v3.z)));
    aabb->set(min, max);
}
...
RT_PROGRAM void triangle_intersect(int objID /**< [in] index of primitive in geometric object.*/) {
    if (prd.origin_UUID ==
        triangle_UUID[objID]) {  // the ray should not intersect the primitive from which it was launched
        return;
    } else if (twosided_flag[objID] == 2) {  // if twosided_flag=2, ignore intersection (transparent)
        return;
    }
        ...
        if (gamma > 0.0 && beta + gamma < 1.0) {
            ...
            if (t > 1e-8) {
                ...
                if (maskID[ID] == -1) {  // no texture transparency
                    ...
                    uint3 ind = make_uint3(roundf(float(sz.x - 1) * fabs(uv.x)), roundf(float(sz.y - 1) * fabs(uv.y)),
                                           maskID[ID]);
                    if (ind.x >= sz.x || ind.y >= sz.y) {
                        rtPrintf("ERROR: texture out of bounds.\n");
                    }
...
RT_PROGRAM void voxel_intersect(int objID /**< [in] index of primitive in geometric object.*/) {
    if (prd.origin_UUID == voxel_UUID[objID]) {  // the ray should not intersect the primitive from which it was
                                                 // launched
        return;
    } else if (twosided_flag[objID] == 2) {  // if twosided_flag=2, ignore intersection (transparent)
        return;
        ...
        if (ddota > 0.0 && ddota < dot(a, a)) {
            ...
            if (ddotb > 0.0 && ddotb < dot(b, b)) {
                ...
                if (maskID[ID] == -1) {  // no texture transparency
                    if (rtPotentialIntersection(t)) {
                        // UUID =
                        // tile_UUID[objID]+floor(uv.y*object_subdivisions[objID].y)*object_subdivisions[objID].x+floor((1.f-uv.x)*object_subdivisions[objID].x);
                        UUID = U + floor(uv.y * object_subdivisions[ID].y) * object_subdivisions[ID].x +
                               floor((1.f - uv.x) * object_subdivisions[ID].x);
                        rtReportIntersection(0);
                    }
                } else {  // use transparency mask
                    ...
                    if (maskdata[ind]) {
                        if (rtPotentialIntersection(t)) {
                            UUID = tile_UUID[objID] +
                                   floor(uv.y * object_subdivisions[ID].y) * object_subdivisions[ID].x +
                                   floor((1.f - uv.x) * object_subdivisions[ID].x);
                            rtReportIntersection(0);
                        }
                    }
