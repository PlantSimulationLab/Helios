...
RT_PROGRAM void direct_raygen()
{
  ...
  if( primitive_type[UUID] == 0 || primitive_type[UUID] == 3 ){ //Patch or AlphaMask
    ...
    sp.x = -0.5f + float(launch_index.x)*dx + Rx*dx;
    sp.y = -0.5f + float(launch_index.y)*dy + Ry*dy;
    sp.z = 0.f;
  ...
  for( int rr=0; rr<Nsources; rr++ ){
    ...
    if( source_types[rr]==0 ){ //collimated source
      ...
      //ray_direction = source_positions[rr]-ray_origin;
...
RT_PROGRAM void diffuse_raygen(){
  ...
  Rt = (launch_index.x + rnd(prd.seed))*dx;
  Rp = (launch_index.y + rnd(prd.seed))*dy;
