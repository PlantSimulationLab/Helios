...
class RadiationModel {
    ...
    */
    ...
    /** The angular distribution of diffuse ambient radiation is computed according to N = Psi^-K, where Psi is the
       angle between the distribution peak (usually the sun direction) and the ambient direction, and K is the
       extinction coefficient. When K=0 the ambient distribution is uniform, which is the default setting \param[in]
       "label" Label used to reference the radiative band \param[in] "K" Extinction coefficient value \param[in]
       "peak_dir" Unit vector pointing in the direction of the peak in diffuse radiation (this is usually the sun
       direction)
    */
    ...
    /** The angular distribution of diffuse ambient radiation is computed according to N = Psi^-K, where Psi is the
       ...
       extinction coefficient. When K=0 the ambient distribution is uniform, which is the default setting \param[in]
       "label" Label used to reference the radiative band \param[in] "K" Extinction coefficient value \param[in]
       "peak_dir" Spherical direction of the peak in diffuse radiation (this is usually the sun direction)
    */
    ...
    /** Runs radiation calculations for a continuous spectral band (e.g., PAR: 400-700nm)
        \param[in] "label" Label used to reference the band
        \param[in] "run_direct" Sets whether the radiation model should run direct radiation calculations for this band
       (true=run; false=don't run) \param[in] "run_diffuse" Sets whether the radiation model should run diffuse
       radiation calculations for this band (true=run; false=don't run) \param[in] "run_emission" Sets whether the
       radiation model should run radiative emission calculations for this band (true=run; false=don't run) \return
       Unique integer identifier for the band
    */
    ...
    //! Add an external source of collimated radiation (i.e., source at infinite distance with parallel rays) assuming
    //! the default direction of (0,0,1)
    /**
    ...
    */
    ...
    /** \param[in] "ID" Identifier of radiation source
        \param[in] "position" If point source - (x,y,z) position of the radiation source. If collimated source -
       (nx,ny,nz) unit vector pointing toward the source.
     */
    ...
    */
    ...
    //! Set the energy threshold used to terminate scattering iterations. Scattering iterations are terminated when the
    //! maximum to-be-scattered energy among all primitives is less than "energy"
    /** \param[in] "label" Label used to reference the band
    ...
    */
    ...
    /** \param[in] "boundary" Lateral direction to enforce periodic boundary - choices are "x" (periodic only in
     * x-direction), "y" (periodic only in y-direction), or "xy" (periodic in both x- and y-directions).
     */
    ...
    */
    ...
    /* \param[in] "label" Label used to reference the band (e.g., ``PAR")
       \note Before running the band simulation, you must 1) add at least one radiative band to the simulation (see \ref
       addRadiationBand()), 2) update the Context geometry in the model (see \ref updateGeometry()), and 3) update
       radiative properties in the model (see \ref updateRadiativeProperties).
    */
   ...
   protected:
    ...
    enum RadiationSourceType { RADIATION_SOURCE_TYPE_COLLIMATED = 0, RADIATION_SOURCE_TYPE_SPHERE = 1 };
    ...
    //! Creates OptiX context and creates all associated variables, buffers, geometry, acceleration structures, etc.
    //! needed for radiation ray tracing.
    void initializeOptiX(void);

    //! Sets radiative properties for all primitives
    /** This function should be called anytime primitive radiative properties are modified. If radiative properties were
       not set in the Context, default radiative properties will be applied (black body). \note \ref
       updateRadiativeProperties() must be called before simulation can be run
    */
    ...
    */
    ...
    void addBuffer(const char *name, RTbuffer &buffer, RTvariable &variable, RTbuffertype type, RTformat format,
                   size_t dimension);
    ...
    */
    ...
    enum RayType {
        RAYTYPE_DIRECT = 0,
        RAYTYPE_DIFFUSE = 1,
        RAYTYPE_DIRECT_MCRT = 2,
        RAYTYPE_DIFFUSE_MCRT = 3,
        RAYTYPE_EMISSION_MCRT = 4
    };
...
#define RT_CHECK_ERROR(func)                                                               \
    do {                                                                                   \
        RTresult code = func;                                                              \
        if (code != RT_SUCCESS) sutilHandleError(OptiX_Context, code, __FILE__, __LINE__); \
    } while (0)
