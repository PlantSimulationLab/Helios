...
typedef enum {
    ...
    RT_TEXTURE_READ_ELEMENT_TYPE_SRGB = 2,    /*!< Read element type and apply sRGB to linear conversion during texture
                                                 read for 8-bit integer buffer formats     */
    RT_TEXTURE_READ_NORMALIZED_FLOAT_SRGB = 3 /*!< Read normalized float and apply sRGB to linear conversion during
...
typedef enum {
    RT_BUFFER_GPU_LOCAL =
        0x4, /*!< An @ref RT_BUFFER_INPUT_OUTPUT has separate copies on each device that are not synchronized */
    RT_BUFFER_COPY_ON_DIRTY = 0x8, /*!< A CUDA Interop buffer will only be synchronized across devices when dirtied by
                                      @ref rtBufferMap or @ref rtBufferMarkDirty */
    RT_BUFFER_DISCARD_HOST_MEMORY = 0x20, /*!< An @ref RT_BUFFER_INPUT for which a synchronize is forced on unmapping
                                             from host and the host memory is freed */
    ...
    RT_BUFFER_CUBEMAP =
        0x400000, /*!< Enables creation of cubemaps. If this flag is set, Width must be equal to Height, and Depth must
                     be six. If the @ref RT_BUFFER_LAYERED flag is also set, then Depth must be a multiple of six */
} RTbufferflag;
...
typedef enum {
    ...
    RT_EXCEPTION_USER_EXCEPTION_CODE_OUT_OF_BOUNDS =
        0x3EC, /*!< Exception code of user exception out of bounds - Added in OptiX 6.0 */
...
typedef enum {
    ...
    RT_GEOMETRY_BUILD_FLAG_RELEASE_BUFFERS =
        0x10, /*!< User buffers are released after consumption by acceleration structure build */
...
typedef enum {
    ...
    RT_GEOMETRY_FLAG_DISABLE_ANYHIT = 0x01, /*!< Disable any-hit program execution (execution will be skipped,including
                                               the no-op any-hit program used when an any-hit program is not specified).
                                                 Can be overridden by ray and instance flags, precedence: RTrayflags >
                                               RTinstanceflags > RTgeometryflags */
    RT_GEOMETRY_FLAG_NO_SPLITTING = 0x02,   /*!< Disable primitive splitting to avoid potential multiple any-hit program
                                               execution for a single intersection */
} RTgeometryflags;
...
typedef enum {
    ...
    RT_INSTANCE_FLAG_DISABLE_TRIANGLE_CULLING =
        1u << 0, /*!< Prevent triangles from getting culled due to face orientation (overrides ray culling flags). */
    RT_INSTANCE_FLAG_FLIP_TRIANGLE_FACING =
        1u << 1, /*!< Flip triangle orientation. This affects front/back face culling. */
    RT_INSTANCE_FLAG_DISABLE_ANYHIT =
        1u << 2, /*!< Disable any-hit program execution (including the no-op any-hit program
                      ...
                      If set, overrides any potentially set @ref RT_RAY_FLAG_FORCE_ANYHIT, @ref
                    RT_RAY_FLAG_DISABLE_ANYHIT, @ref RT_GEOMETRY_FLAG_DISABLE_ANYHIT. Can be overridden by ray flag @ref
                    RT_RAY_FLAG_FORCE_ANYHIT. Precedence: RTrayflags > RTinstanceflags > RTgeometryflags */
    RT_INSTANCE_FLAG_FORCE_ANYHIT =
        1u << 3 /*!< Force any-hit program execution.
                     Mutually exclusive with RT_INSTANCE_FLAG_DISABLE_ANYHIT.
                     If set, overrides any potentially set @ref RT_RAY_FLAG_FORCE_ANYHIT, @ref
                   RT_RAY_FLAG_DISABLE_ANYHIT, @ref RT_GEOMETRY_FLAG_DISABLE_ANYHIT. Can be overridden by ray flag @ref
                   RT_RAY_FLAG_DISABLE_ANYHIT. Overriding precedence: RTrayflags > RTinstanceflags > RTgeometryflags */
} RTinstanceflags;
...
typedef enum {
    ...
    RT_RAY_FLAG_DISABLE_ANYHIT =
        1u << 0, /*!< Disable any-hit program execution for the ray (execution will be skipped,including the no-op
                    any-hit program used when an any-hit program is not specified). Mutually exclusive with
                    RT_RAY_FLAG_FORCE_ANYHIT. If set, overrides any potentially set @ref RT_INSTANCE_FLAG_FORCE_ANYHIT.
                      Overriding precedence: RTrayflags > RTinstanceflags > RTgeometryflags */
    RT_RAY_FLAG_FORCE_ANYHIT = 1u << 1, /*!< Force any-hit program execution for the ray. See @ref
                                           RT_RAY_FLAG_DISABLE_ANYHIT. Mutually exclusive with
                                           RT_RAY_FLAG_DISABLE_ANYHIT. If set, overrides any potentially set @ref
                                           RT_GEOMETRY_FLAG_DISABLE_ANYHIT, @ref RT_INSTANCE_FLAG_DISABLE_ANYHIT.
                                             Overriding precedence: RTrayflags > RTinstanceflags > RTgeometryflags */
    RT_RAY_FLAG_TERMINATE_ON_FIRST_HIT =
        1u << 2, /*!< Terminate the ray after the first hit, also reports the first hit as closest hit. */
    ...
    RT_RAY_FLAG_CULL_DISABLED_ANYHIT = 1u << 6, /*!< Do not intersect geometry which disables any-hit programs (due to
                                                   any geometry, instance, or ray flag). */
    RT_RAY_FLAG_CULL_ENABLED_ANYHIT =
        1u << 7 /*!< Do not intersect geometry which executes any-hit programs (i.e., forced or not disabled any-hit
                   program execution, this includes a potential no-op any-hit program). */
} RTrayflags;
