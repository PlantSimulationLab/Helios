 ...
 * @param[in]   p         Pointer to where the value of the attribute will be copied from.  This must point to at least
 * \a size bytes of memory
 *
 * <B>Return values</B>
 *
 * Relevant return values:
 ...
 * - @ref RT_ERROR_INVALID_VALUE - Can be returned if \a size does not match the proper size of the attribute, or if \a
 * p is \a NULL
 *
 * <B>History</B>
 *
 ...
 *
 * @ref RT_GLOBAL_ATTRIBUTE_DISPLAY_DRIVER_VERSION_MAJOR is an attribute to query the major version of the display
 * driver found on the system. It's the first number in the driver version displayed as xxx.yy.
 *
 * @ref RT_GLOBAL_ATTRIBUTE_DISPLAY_DRIVER_VERSION_MINOR is an attribute to query the minor version of the display
 * driver found on the system. It's the second number in the driver version displayed as xxx.yy.
 *
 * @param[in]   attrib    Attribute to query
 * @param[in]   size      Size of the attribute being queried.  Parameter \a p must have at least this much memory
 * allocated
 * @param[out]  p         Return pointer where the value of the attribute will be copied into.  This must point to at
 * least \a size bytes of memory
 *
 * <B>Return values</B>
 *
 * Relevant return values:
 * - @ref RT_SUCCESS
 * - @ref RT_ERROR_INVALID_GLOBAL_ATTRIBUTE - Can be returned if an unknown attribute was addressed.
 * - @ref RT_ERROR_INVALID_VALUE - Can be returned if \a size does not match the proper size of the attribute, if \a p
 * is \a NULL, or if \a attribute+ordinal does not correspond to an OptiX device
 ...
 */
RTresult RTAPI rtVariableSet3ull(RTvariable v, unsigned long long ull1, unsigned long long ull2,
 ...
 */
RTresult RTAPI rtVariableSet4ull(RTvariable v, unsigned long long ull1, unsigned long long ull2,
                                 unsigned long long ull3, unsigned long long ull4);
 ...
 * @param[in]   u4         Unsigned integer value to be returned
 */
RTresult RTAPI rtVariableGet4ull(RTvariable v, unsigned long long* u1, unsigned long long* u2, unsigned long long* u3,
 ...
 *
 */
RTresult RTAPI rtContextSetUsageReportCallback(RTcontext context, RTusagereportcallback callback, int verbosity,
                                               void* cbdata);
...
/**
 * Deprecated in OptiX 4.0. Calling this function has no effect. The kernel is automatically compiled at launch if
 * needed.
 *
 */
 ...
 */
RTresult RTAPI rtContextLaunch3D(RTcontext context, unsigned int entryPointIndex, RTsize width, RTsize height,
                                 RTsize depth);
 ...
 * @param[in]   width                  Width of the computation grid
 * @param[in]   height                 Height of the computation grid
 * @param[in]   maxSubframes           The maximum number of subframes to be generated. Set to zero to generate an
 ...
 *
 */
RTresult RTAPI rtContextLaunchProgressive2D(RTcontext context, unsigned int entryIndex, RTsize width, RTsize height,
 ...
 *
 */
RTresult RTAPI rtProgramCreateFromPTXString(RTcontext context, const char* ptx, const char* programName,
                                            RTprogram* program);
 ...
 *
 */
RTresult RTAPI rtProgramCreateFromPTXStrings(RTcontext context, unsigned int n, const char** ptxStrings,
                                             const char* programName, RTprogram* program);
 ...
 *
 */
RTresult RTAPI rtProgramCreateFromPTXFile(RTcontext context, const char* filename, const char* programName,
                                          RTprogram* program);
 ...
 *
 */
RTresult RTAPI rtProgramCreateFromPTXFiles(RTcontext context, unsigned int n, const char** filenames,
                                           const char* programName, RTprogram* program);
 ...
 *
 */
RTresult RTAPI rtTransformSetMatrix(RTtransform transform, int transpose, const float* matrix,
 ...
 *
 */
RTresult RTAPI rtTransformSetMotionBorderMode(RTtransform transform, RTmotionbordermode beginMode,
 ...
 *
 */
RTresult RTAPI rtTransformGetMotionBorderMode(RTtransform transform, RTmotionbordermode* beginMode,
                                              RTmotionbordermode* endMode);
 ...
 *
 */
RTresult RTAPI rtGeometryGroupSetChild(RTgeometrygroup geometrygroup, unsigned int index,
 ...
 *
 */
RTresult RTAPI rtGeometryGroupGetChild(RTgeometrygroup geometrygroup, unsigned int index,
                                       RTgeometryinstance* geometryinstance);
 ...
 * list of primitives to intersect. This can be useful e.g. for higher level groups with only few children, where
 * managing a more complex structure introduces unnecessary overhead.
 *
 * - "Bvh": A standard bounding volume hierarchy, useful for most types of graph levels and geometry. Medium build
 * speed, good ray tracing performance.
 *
 * - "Sbvh": A high quality BVH variant for maximum ray tracing performance. Slower build speed and slightly higher
 * memory footprint than "Bvh".
 *
 * - "Trbvh": High quality similar to Sbvh but with fast build performance. The Trbvh builder uses about 2.5 times the
 * size of the final BVH for scratch space. A CPU-based Trbvh builder that does not have the memory constraints is
 * available. OptiX includes an optional automatic fallback to the CPU version when out of GPU memory. Please refer to
 * the Programming Guide for more details.  Supports motion blur.
 *
 ...
 * <B>Description</B>
 *
 * @ref rtGeometryInstanceGetGeometryTriangles sets \a geometrytriangles to the handle of the attached GeometryTriangles
 * node. If no GeometryTriangles node is attached or a Geometry node is attached, @ref RT_ERROR_INVALID_VALUE is
 * returned, else @ref RT_SUCCESS.
 *
 ...
 *
 */
RTresult RTAPI rtGeometryInstanceSetMaterial(RTgeometryinstance geometryinstance, unsigned int index,
                                             RTmaterial material);
 ...
 *
 */
RTresult RTAPI rtGeometryInstanceGetMaterial(RTgeometryinstance geometryinstance, unsigned int index,
                                             RTmaterial* material);
 ...
 *
 */
RTresult RTAPI rtGeometrySetMotionBorderMode(RTgeometry geometry, RTmotionbordermode beginMode,
                                             RTmotionbordermode endMode);
 ...
 *
 */
RTresult RTAPI rtGeometryGetMotionBorderMode(RTgeometry geometry, RTmotionbordermode* beginMode,
                                             RTmotionbordermode* endMode);
 ...
 * for each attached primitive to \a geometry. RTprogram's can be either generated with @ref rtProgramCreateFromPTXFile
 * or
 ...
 * RTprogram's can be either generated with @ref rtProgramCreateFromPTXFile or @ref rtProgramCreateFromPTXString. An
 * intersection program is mandatory for every geometry node.
 *
 ...
 * <B>Description</B>
 *
 * @ref rtGeometryTrianglesDestroy removes \a geometrytriangles from its context and deletes it.  \a geometrytriangles
 * should be a value returned by @ref rtGeometryTrianglesCreate.  After the call, \a geometrytriangles is no longer a
 * valid handle.
 *
 ...
 * The 3x4 matrix is expected to be in a row-major data layout, use the transpose option if \a matrix is in a
 * column-major data layout. Use rtGeometryTrianglesSetPreTransformMatrix(geometrytriangles, false, 0); to unset a
 ...
 *
 */
RTresult RTAPI rtGeometryTrianglesSetPreTransformMatrix(RTgeometrytriangles geometrytriangles, int transpose,
                                                        const float* matrix);
 ...
 * <B>Description</B>
 *
 * @ref rtGeometryTrianglesGetPreTransformMatrix returns a previously set 3x4 matrix or the 'identity' matrix (with ones
 * in the main diagonal of the 3x3 submatrix) if no matrix is set.
 *
 * @param[in]   geometrytriangles  Geometry node to query from the number of primitives
 ...
 *
 */
RTresult RTAPI rtGeometryTrianglesGetPreTransformMatrix(RTgeometrytriangles geometrytriangles, int transpose,
                                                        float* matrix);
 ...
 * A triangle geometry is either a triangle soup for which every three vertices stored in the vertex buffer form a
 * triangle, or indexed triangles are used for which three indices reference different vertices. In the latter case, an
 * index buffer must be set (@ref rtGeometryTrianglesSetTriangleIndices). The vertices of the triangles are specified
 * via one of the SetVertices functions.
 *
 ...
 * A previously set index buffer can be unset by passing NULL as \a indexBuffer parameter, e.g.,
 * rtGeometryTrianglesSetTriangleIndices( geometrytriangles, NULL, 0, 0, RT_FORMAT_UNSIGNED_INT3); Buffer \a indexBuffer
 * is expected to hold 3 times \a triangleCount indices (see @ref rtGeometryTrianglesSetPrimitiveCount). Parameter \a
 * indexBufferByteOffset can be used to specify a byte offset to the first index in buffer \a indexBuffer. Parameter \a
 * triIndicesByteStride sets the stride in bytes between triplets of indices. There mustn't be any spacing between
 * indices within a triplet, spacing is only supported between triplets. Parameter \a triIndicesFormat must be one of
 * the following: RT_FORMAT_UNSIGNED_INT3, RT_FORMAT_UNSIGNED_SHORT3.
 *
 * @param[in]   geometrytriangles               GeometryTriangles node to query for the primitive index offset
 ...
 *
 */
RTresult RTAPI rtGeometryTrianglesSetTriangleIndices(RTgeometrytriangles geometrytriangles, RTbuffer indexBuffer,
                                                     RTsize indexBufferByteOffset, RTsize triIndicesByteStride,
 ...
 * <B>Description</B>
 *
 * @ref rtGeometryTrianglesSetVertices interprets the buffer \a vertexBuffer as the vertices of triangles of the
 * GeometryTriangles \a geometrytriangles. The number of vertices is set as \a vertexCount. If an index buffer is set,
 * it is assumed that the geometry is given as indexed triangles. If the index buffer is not set, it is assumed that the
 * geometry is given as a triangle soup and \a vertexCount must be 3 times triangleCount (see @ref
 * rtGeometryTrianglesSetPrimitiveCount). Buffer \a vertexBuffer is expected to hold \a vertexCount vertices. Parameter
 * \a vertexBufferByteOffset can be used to specify a byte offset to the position of the first vertex in buffer \a
 * vertexBuffer. Parameter \a vertexByteStride sets the stride in bytes between vertices. Parameter \a positionFormat
 * must be one of the following: RT_FORMAT_FLOAT3, RT_FORMAT_HALF3, RT_FORMAT_FLOAT2, RT_FORMAT_HALF2. In case of
 * formats RT_FORMAT_FLOAT2 or RT_FORMAT_HALF2 the third component is assumed to be zero, which can be useful for planar
 * geometry. Calling this function overrides any previous call to any of the set(Motion)Vertices functions.
 *
 * @param[in]   geometrytriangles            GeometryTriangles node to query for the primitive index offset
 ...
 *
 */
RTresult RTAPI rtGeometryTrianglesSetVertices(RTgeometrytriangles geometrytriangles, unsigned int vertexCount,
                                              RTbuffer vertexBuffer, RTsize vertexBufferByteOffset,
                                              RTsize vertexByteStride, RTformat positionFormat);
 ...
 * <B>Description</B>
 *
 * @ref rtGeometryTrianglesSetMotionVertices interprets the buffer \a vertexBuffer as the vertices of triangles of the
 * GeometryTriangles \a geometrytriangles. The number of triangles for one motion step is set as \a vertexCount. Similar
 * to it's non-motion counterpart, \a vertexCount must be 3 times \a triangleCount if no index buffer is set. The total
 * number of vertices stored in \a vertexBuffer is \a vertexCount times \a motionStepCount (see @ref
 * rtGeometryTrianglesSetMotionSteps). Triangles are linearly interpolated between motion steps. Parameter \a
 * vertexBufferByteOffset can be used to specify a byte offset to the position of the first vertex of the first motion
 * step in buffer \a vertexBuffer. Parameter \a vertexByteStride sets the stride in bytes between vertices within a
 * motion step. Parameter \a vertexMotionStepByteStride sets the stride in bytes between motion steps for a single
 * vertex. The stride parameters allow for two types of layouts of the motion data: a) serialized: vertexByteStride =
 * sizeof(Vertex), vertexMotionStepByteStride = vertexCount * vertexByteStride b) interleaved:
 * vertexMotionStepByteStride = sizeof(Vertex), vertexByteStride = sizeof(Vertex) * motion_steps Vertex N at time step i
 * is at: vertexBuffer[N * vertexByteStride + i * vertexMotionStepByteStride + vertexBufferByteOffset] Parameter \a
 * positionFormat must be one of the following: RT_FORMAT_FLOAT3, RT_FORMAT_HALF3, RT_FORMAT_FLOAT2, RT_FORMAT_HALF2. In
 * case of formats RT_FORMAT_FLOAT2 or RT_FORMAT_HALF2 the third component is assumed to be zero, which can be useful
 * for planar geometry. Calling this function overrides any previous call to any of the set(Motion)Vertices functions.
 *
 * @param[in]   geometrytriangles               GeometryTriangles node to query for the primitive index offset
 ...
 * @param[in]   vertexBufferByteOffset          Offset in bytes to the first vertex of the first motion step in buffer
 * vertexBuffer
 * @param[in]   vertexByteStride                Stride in bytes between vertices, belonging to the same motion step
 * @param[in]   vertexMotionStepByteStride      Stride in bytes between vertices of the same triangle, but neighboring
 * motion step
 * @param[in]   positionFormat                  Format of the position attribute of a vertex
 *
 * <B>Return values</B>
 ...
 *
 */
RTresult RTAPI rtGeometryTrianglesSetMotionVertices(RTgeometrytriangles geometrytriangles, unsigned int vertexCount,
                                                    RTbuffer vertexBuffer, RTsize vertexBufferByteOffset,
                                                    RTsize vertexByteStride, RTsize vertexMotionStepByteStride,
 ...
 * <B>Description</B>
 *
 * @ref rtGeometryTrianglesSetMotionVerticesMultiBuffer can be used instead of @ref rtGeometryTrianglesSetMotionVertices
 * if the vertices for the different motion steps are stored in separate buffers. Parameter \a vertexBuffers must point
 * to an array of buffers of minimal size \a motionStepCount (see @ref rtGeometryTrianglesSetMotionSteps). All buffers
 * must, however, share the same byte offset as well as vertex stride and position format. Calling this function
 * overrides any previous call to any of the set(Motion)Vertices functions.
 *
 * @param[in]   geometrytriangles               GeometryTriangles node to query for the primitive index offset
 * @param[in]   vertexCount                     Number of vertices for one motion step
 * @param[in]   vertexBuffers                   Buffers that hold the vertices of the triangles per motion step
 * @param[in]   vertexBufferCount               Number of buffers passed, must match the number of motion steps before a
 * launch call
...
RTresult RTAPI rtGeometryTrianglesSetMotionVerticesMultiBuffer(RTgeometrytriangles geometrytriangles,
                                                               unsigned int vertexCount, RTbuffer* vertexBuffers,
                                                               unsigned int vertexBufferCount,
                                                               RTsize vertexBufferByteOffset, RTsize vertexByteStride,
                                                               RTformat positionFormat);
 ...
 *
 */
RTresult RTAPI rtGeometryTrianglesGetMotionRange(RTgeometrytriangles geometrytriangles, float* timeBegin,
                                                 float* timeEnd);
 ...
 *
 * <B>Description</B>
 * @ref rtGeometryTrianglesSetBuildFlags can be used to set object-specific flags that affect the
 * acceleration-structure-build behavior. If parameter \a buildFlags contains the RT_GEOMETRY_BUILD_FLAG_RELEASE_BUFFERS
 * flag, all buffers (including the vertex, index, and materialIndex buffer) holding information that is evaluated at
 * acceleration-structure-build time will be released after the build. OptiX does not take ownership over the buffers,
 * but simply frees the corresponding device memory. Sharing buffers with other GeometryTriangles nodes is possible if
 * all of them are built within one OptiX launch. Note that it is the users responsibility that the buffers hold data
 * for the next acceleration structure build if the acceleration structure is marked dirty. E.g., if the flag is set, an
 * OptiX launch will cause the acceleration structure build and release the memory afterwards. If the acceleration
 * structure is marked dirty before the next launch (e.g., due to refitting), the user needs to map the buffers before
 * the launch to fill them with data. Further, there are certain configurations with motion when the buffers cannot be
 * released in which case the flag is ignored and the data is not freed. The buffers can only be released if all
 * GeometryTriangles belonging to a GeometryGroup have the same number of motion steps and equal motion begin / end
 * times.
 *
 * @param[in]   geometrytriangles    GeometryTriangles node handle
 ...
 * By default there is one material slot.
 * This number must be equal to the number of materials that is set at the GeometryInstance where \a geometrytriangles
 * is attached to. Multi-material support for GeometryTriangles is limited to a fixed partition of the geometry into
 * sets of triangles. Each triangle set maps to one material slot (within range [0, numMaterials-1]). The mapping is set
 * via @ref rtGeometryTrianglesSetMaterialIndices. The actual materials are set at the GeometryInstance. The geometry
 * can be instanced when attached to multiple GeometryInstances. In that case, the materials attached to each
 * GeometryInstance can differ (effectively causing different materials per instance of the geometry). \a numMaterials
 * must be >=1 and <=2^16.
 *
 * @param[in]   geometrytriangles    GeometryTriangles node handle
 ...
 * Parameter \a materialIndexBufferByteOffset can be used to specify a byte offset to the first index in buffer \a
 * materialIndexBuffer. Parameter \a materialIndexByteStride sets the stride in bytes between indices. Parameter \a
 * materialIndexFormat must be one of the following: RT_FORMAT_UNSIGNED_INT, RT_FORMAT_UNSIGNED_SHORT,
 * RT_FORMAT_UNSIGNED_BYTE. The buffer is only used if the number of materials as set via @ref
 * rtGeometryTrianglesSetMaterialCount is larger than one.
 *
 * @param[in]   geometrytriangles                   GeometryTriangles node to query for the primitive index offset
 * @param[in]   materialIndexBuffer                 Buffer that holds the indices into the vertex buffer of the
 * triangles
...
RTresult RTAPI rtGeometryTrianglesSetMaterialIndices(RTgeometrytriangles geometrytriangles,
                                                     RTbuffer materialIndexBuffer, RTsize materialIndexBufferByteOffset,
                                                     RTsize materialIndexByteStride, RTformat materialIndexFormat);
 ...
 * If the geometry is instanced and different instances apply different materials to the geometry, the per-material
 * geometry-specific flags need to apply to the materials of all instances. Example with two instances with each having
 * two materials, node graph:
 *        G
 *       / \
 *      /   \
 *     T0    T1
 ...
 * Note that the final decision whether or not to execute the any-hit program at run time also depends on the flags set
 * on the ray as well as the geometry group that this geometry is part of.
 *
 * @param[in]   geometrytriangles    GeometryTriangles node handle
 ...
 *
 */
RTresult RTAPI rtGeometryTrianglesSetFlagsPerMaterial(RTgeometrytriangles geometrytriangles, unsigned int materialIndex,
 ...
 * @ref rtTrace
 */
RTresult RTAPI rtGeometryTrianglesGetFlagsPerMaterial(RTgeometrytriangles triangles, unsigned int materialIndex,
                                                      RTgeometryflags* flags);
 ...
 *
 */
RTresult RTAPI rtTextureSamplerSetWrapMode(RTtexturesampler texturesampler, unsigned int dimension,
                                           RTwrapmode wrapmode);
 ...
 *
 */
RTresult RTAPI rtTextureSamplerGetWrapMode(RTtexturesampler texturesampler, unsigned int dimension,
                                           RTwrapmode* wrapmode);
 ...
 * <B>Description</B>
 *
 * @ref rtTextureSamplerSetFilteringModes sets the minification, magnification and MIP mapping filter modes for \a
 * texturesampler. RTfiltermode must be one of the following values:
 *
 ...
 *
 */
RTresult RTAPI rtTextureSamplerSetFilteringModes(RTtexturesampler texturesampler, RTfiltermode minification,
 ...
 *
 */
RTresult RTAPI rtTextureSamplerGetFilteringModes(RTtexturesampler texturesampler, RTfiltermode* minification,
                                                 RTfiltermode* magnification, RTfiltermode* mipmapping);
 ...
 * value specifies the maximum anisotropy ratio to be used when doing anisotropic filtering. This value will be clamped
 * to the range [1,16]
 *
 * @param[in]   texturesampler   The texture sampler object to be changed
 ...
 * <B>Description</B>
 *
 * @ref rtTextureSamplerGetMipLevelClamp gets the minimum and the maximum MIP level access range for \a texturesampler
 * and stores it in \a *minLevel and \a maxLevel.
 *
 * @param[in]   texturesampler   The texture sampler object to be queried
 ...
 *
 */
RTresult RTAPI rtTextureSamplerSetBuffer(RTtexturesampler texturesampler, unsigned int deprecated0,
                                         unsigned int deprecated1, RTbuffer buffer);
 ...
 *
 */
RTresult RTAPI rtTextureSamplerGetBuffer(RTtexturesampler texturesampler, unsigned int deprecated0,
                                         unsigned int deprecated1, RTbuffer* buffer);
 ...
 * @ref RT_BUFFER_INPUT specifies that the host may only write to the buffer and the device may only read from the
 * buffer.
 ...
 * @ref RT_BUFFER_PROGRESSIVE_STREAM is used to receive stream updates generated by progressive launches (see @ref
 ...
 *
 * Flags can be used to optimize data transfers between the host and its devices. The flag @ref RT_BUFFER_GPU_LOCAL can
 * only be used in combination with @ref RT_BUFFER_INPUT_OUTPUT. @ref RT_BUFFER_INPUT_OUTPUT and @ref
 * RT_BUFFER_GPU_LOCAL used together specify a buffer that allows the host to \a only write, and the device to read \a
 * and write data. The written data will never be visible on the host side and will generally not be visible on other
 * devices.
 *
 * If @ref rtBufferGetDevicePointer has been called for a single device for a given buffer,
 * the user can change the buffer's content on that device through the pointer. OptiX must then synchronize the new
 * buffer contents to all devices. These synchronization copies occur at every @ref rtContextLaunch "rtContextLaunch",
 * unless the buffer is created with @ref RT_BUFFER_COPY_ON_DIRTY. In this case, @ref rtBufferMarkDirty can be used to
 * notify OptiX that the buffer has been dirtied and must be synchronized.
 *
 * The flag @ref RT_BUFFER_DISCARD_HOST_MEMORY can only be used in combination with @ref RT_BUFFER_INPUT. The data will
 * be synchronized to the devices as soon as the buffer is unmapped from the host using @ref rtBufferUnmap or
 ...
 * @ref RT_BUFFER_MAP_WRITE_DISCARD option enabled. If it is mapped using @ref rtBufferMap or the @ref
 * RT_BUFFER_MAP_WRITE option instead, the data needs to be synchronized to the host during mapping. Note that the data
 ...
 * See details in @ref rtBufferSetSize3D
 *
 * It is an error to call @ref rtBufferGetDevicePointer, @ref rtBufferMap or @ref rtBufferUnmap for a demand loaded
 * buffer.
 *
 ...
 * @param[in]   callbackData An arbitrary pointer from the application that is passed to the callback.  This may be \a
 * NULL.
 * @param[out]  buffer       The return handle for the buffer object.
 *
 * <B>Return values</B>
 ...
 *
 */
RTresult RTAPI rtBufferCreateFromCallback(RTcontext context, unsigned int bufferdesc, RTbuffercallback callback,
                                          void* callbackData, RTbuffer* buffer);
 ...
 *
 * A 1D layered mipmapped buffer is allocated if \a height is 1 and the @ref RT_BUFFER_LAYERED flag was set at buffer
 * creating. The number of layers is determined by the \a depth. A 2D layered mipmapped buffer is allocated if the @ref
 * RT_BUFFER_LAYERED flag was set at buffer creating. The number of layers is determined by the \a depth. A cubemap
 * mipmapped buffer is allocated if the @ref RT_BUFFER_CUBEMAP flag was set at buffer creating. \a width must be equal
 * to \a height and the number of cube faces is determined by the \a depth, it must be six or a multiple of six, if the
 * @ref RT_BUFFER_LAYERED flag was also set. Layered, mipmapped and cubemap buffers are supported only as texture
 * buffers.
 *
 * Fails with @ref RT_ERROR_ALREADY_MAPPED if called on a buffer that is mapped.
 *
 ...
 *
 */
RTresult RTAPI rtBufferGetMipLevelSize3D(RTbuffer buffer, unsigned int level, RTsize* width, RTsize* height,
                                         RTsize* depth);
 ...
 * <B>Description</B>
 *
 * @ref rtBufferUnmap unmaps a buffer from the host after a call to @ref rtBufferMap.  @ref rtContextLaunch
 * "rtContextLaunch" cannot be called while buffers are still mapped to the host.  A call to @ref rtBufferUnmap that
 * does not follow a matching @ref rtBufferMap call will return @ref RT_ERROR_INVALID_VALUE.
 *
 ...
 * <B>Description</B>
 *
 * @ref rtBufferMapEx makes the buffer contents available on the host, either by returning a pointer in \a *optixOwned,
 * or by copying the contents to a memory location pointed to by \a userOwned. Calling @ref rtBufferMapEx with proper
 * map flags can result in better performance than using @ref rtBufferMap, because fewer synchronization copies are
 * required in certain situations.
 * @ref rtBufferMapEx with \a mapFlags = @ref RT_BUFFER_MAP_READ_WRITE and \a level = 0 is equivalent to @ref
 * rtBufferMap.
 *
 * Note that this call does not stop a progressive render if called on a stream buffer.
 *
 ...
 *
 */
RTresult RTAPI rtBufferMapEx(RTbuffer buffer, unsigned int mapFlags, unsigned int level, void* userOwned,
                             void** optixOwned);
 ...
 * <B>Description</B>
 *
 * @ref rtBufferUnmapEx unmaps buffer level from the host after a call to @ref rtBufferMapEx.  @ref rtContextLaunch
 * "rtContextLaunch" cannot be called while buffers are still mapped to the host.  A call to @ref rtBufferUnmapEx that
 * does not follow a matching @ref rtBufferMapEx call will return @ref RT_ERROR_INVALID_VALUE. @ref rtBufferUnmap is
 * equivalent to @ref rtBufferUnmapEx with \a level = 0.
 *
 * Note that this call does not stop a progressive render if called with a stream buffer.
 *
 * @param[in]   buffer   The buffer to unmap
 ...
 * @param[out]  ready              Ready flag. Will be set to 1 if an update is available, or 0 if no update is
 * available.
 * @param[out]  subframeCount      The number of subframes accumulated in the latest result
 * @param[out]  maxSubframes       The \a maxSubframes parameter as specified in the call to @ref
 * rtContextLaunchProgressive2D
 *
 * <B>Return values</B>
 *
 ...
 *
 */
RTresult RTAPI rtBufferGetProgressiveUpdateReady(RTbuffer buffer, int* ready, unsigned int* subframeCount,
                                                 unsigned int* maxSubframes);
 ...
 * of format @ref RT_FORMAT_UNSIGNED_BYTE4, and the output buffer must be of format @ref RT_FORMAT_FLOAT3 or @ref
 * RT_FORMAT_FLOAT4. The use of @ref RT_FORMAT_FLOAT4 is recommended for performance reasons, even if the fourth
 * component is unused. The output buffer must be of type @ref RT_BUFFER_OUTPUT; it may not be of type @ref
 ...
 *
 * @ref RT_BUFFER_ATTRIBUTE_STREAM_FORMAT sets the encoding format used for streams sent over the network, specified as
 * a string. The default is "auto". Various other common stream and image formats are available (e.g. "h264", "png").
 * This attribute has no effect if the progressive API is used locally.
 *
 * @ref RT_BUFFER_ATTRIBUTE_STREAM_BITRATE sets the target bitrate for streams sent over the network, if the stream
 * format supports it. The data is specified as a 32-bit integer. The default is 5000000. This attribute has no effect
 * if the progressive API is used locally or if the stream format does not support variable bitrates.
 *
 * @ref RT_BUFFER_ATTRIBUTE_STREAM_FPS sets the target update rate per second for streams sent over the network, if the
 * stream format supports it. The data is specified as a 32-bit integer. The default is 30. This attribute has no effect
 * if the progressive API is used locally or if the stream format does not support variable framerates.
 *
 * @ref RT_BUFFER_ATTRIBUTE_STREAM_GAMMA sets the gamma value for the built-in tonemapping operator. The data is
 * specified as a 32-bit float, the default is 1.0. Tonemapping is executed before encoding the accumulated output into
 * the stream, i.e. on the server side if remote rendering is used. See the section on Buffers below for more details.
 *
 ...
 * <B>Description</B>
 *
 * @ref rtBufferGetAttribute is used to query buffer attributes. For a list of available attributes that can be set,
 * please refer to @ref rtBufferSetAttribute. The attribute \a RT_BUFFER_ATTRIBUTE_PAGE_SIZE can only be queried and
 * returns the page size of a demand loaded buffer in bytes.  The size of the data returned for this attribute is \a
 * sizeof(int).
 *
 * @param[in]   buffer             The buffer to query the attribute from
 * @param[in]   attrib             The attribute to query
 * @param[in]   size               The size of the attribute value, in bytes. For string attributes, this is the maximum
 * buffer size the returned string will use (including a terminating null character).
 * @param[out]  p                  Pointer to the attribute value to be filled in. Must point to valid memory of at
 * least \a size bytes.
 *
 * <B>Return values</B>
 *
 ...
 *
 */
RTresult RTAPI rtPostProcessingStageCreateBuiltin(RTcontext context, const char* builtinName,
 ...
 *
 */
RTresult RTAPI rtCommandListAppendPostprocessingStage(RTcommandlist list, RTpostprocessingstage stage,
 ...
 *
 */
RTresult RTAPI rtCommandListAppendLaunch2D(RTcommandlist list, unsigned int entryPointIndex, RTsize launchWidth,
                                           RTsize launchHeight);
 ...
 *
 */
RTresult RTAPI rtCommandListAppendLaunch3D(RTcommandlist list, unsigned int entryPointIndex, RTsize launchWidth,
                                           RTsize launchHeight, RTsize launchDepth);
...
RTresult RTAPI rtGeometryTrianglesDeclareVariable(RTgeometrytriangles geometrytriangles, const char* name,
                                                  RTvariable* v);
