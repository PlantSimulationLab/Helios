 ...
 * function must be executed once for \a context before any call to @ref rtBufferCreateFromD3D9Resource or @ref
 * rtTextureSamplerCreateFromD3D9Resource can take place. A context can only be bound to one device. Once \a device is
 ...
 * \a pszAdapterName is the DeviceName field in the \a D3DADAPTER_IDENTIFIER9 struct. In combination with @ref
 * rtContextSetDevices, this function can be used to restrict OptiX to use only one device. The same device the D3D9
 ...
 * pszAdapterName will be stored
 * @param[out]   pszAdapterName      The name of an adapter as can be found in the DeviceName field in the \a
 ...
 * <B>Description</B>
 *
 * @ref rtBufferCreateFromD3D9Resource allocates and returns a handle to a new buffer object in \a *buffer associated
 * with \a context. If the allocated size of the D3D resource is \a 0, @ref RT_ERROR_MEMORY_ALLOCATION_FAILED will be
 * returned. Supported D3D9 buffer types are:
 *
 ...
 *
 */
RTresult RTAPI rtBufferCreateFromD3D9Resource(RTcontext context, unsigned int bufferdesc, IDirect3DResource9* resource,
                                              RTbuffer* buffer);
 ...
 *
 */
RTresult RTAPI rtTextureSamplerCreateFromD3D9Resource(RTcontext context, IDirect3DResource9* resource,
 ...
 * @ref rtBufferCreateFromD3D9Resource.  If \a buffer was not created from a D3D9 resource \a **resource will be \a 0
 ...
 * <B>Description</B>
 *
 * An OptiX buffer in an unregistered state can be registered to OptiX again via @ref rtBufferD3D9Register. Once
 * registered, properties like the size of the original D3D9 resource cannot be modified anymore. Calls to the
 * corresponding D3D9 functions will return with an error code. However, the data of the D3D9 resource can still be read
 * and written by the appropriate D3D9 commands. When a buffer is already in a registered state @ref
 * rtBufferD3D9Register will return @ref RT_ERROR_RESOURCE_ALREADY_REGISTERED. A resource must be registered in order to
 * be used by OptiX. If a resource is not registered @ref RT_ERROR_INVALID_VALUE will be returned.
 *
 ...
 * rtTextureSamplerD3D9Register. Once registered, properties like the size of the original D3D9 resource cannot be
 * modified anymore. Calls to the corresponding D3D9 functions will return with an error code. However, the data of the
 * D3D9 resource can still be read and written by the appropriate D3D9 commands. When a texture sampler is already in a
 * registered state @ref rtTextureSamplerD3D9Register will return @ref RT_ERROR_RESOURCE_ALREADY_REGISTERED. A resource
 * must be registered in order to be used by OptiX. If a resource is not registered @ref RT_ERROR_INVALID_VALUE will be
 * returned.
 *
 ...
 * <B>Description</B>
 *
 * An OptiX texture sampler in a registered state can be unregistered via @ref rtTextureSamplerD3D9Unregister. Once
 * unregistered, properties like the size of the original D3D9 resource can be changed. As long as a resource is
