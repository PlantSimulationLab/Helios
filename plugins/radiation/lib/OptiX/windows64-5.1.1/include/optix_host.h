 ...
 *
 * @ref RT_GLOBAL_ATTRIBUTE_EXPERIMENTAL_EXECUTION_STRATEGY is an experimental attribute which sets the execution
 * strategy used by Optix for the next context to be created.  This attribute may be deprecated in a future release.
 ...
 */
RTresult RTAPI rtContextLaunch3D(RTcontext context, unsigned int entry_point_index, RTsize width, RTsize height,
                                 RTsize depth);
 ...
 * @param[in]   width                  Width of the computation grid
 * @param[in]   height                 Height of the computation grid
 * @param[in]   max_subframes          The maximum number of subframes to be generated. Set to zero to generate an
 ...
 *
 */
RTresult RTAPI rtContextLaunchProgressive2D(RTcontext context, unsigned int entry_index, RTsize width, RTsize height,
 ...
 *
 */
RTresult RTAPI rtProgramCreateFromPTXString(RTcontext context, const char* ptx, const char* program_name,
                                            RTprogram* program);
 ...
 *
 */
RTresult RTAPI rtProgramCreateFromPTXFile(RTcontext context, const char* filename, const char* program_name,
                                          RTprogram* program);
...
only be
* used in combination with @ref RT_BUFFER_INPUT_OUTPUT. @ref RT_BUFFER_INPUT_OUTPUT and @ref RT_BUFFER_GPU_LOCAL used
together specify a buffer
* that allows the host to \a only write, and the device to read \a and write data. The written data will never be
visible
...
buffer contents to all devices.
* These synchronization copies occur at every @ref rtContextLaunch "rtContextLaunch", unless the buffer is created with
@ref RT_BUFFER_COPY_ON_DIRTY.
* In this case, @ref rtBufferMarkDirty can be used to notify OptiX that the buffer has been dirtied and must be
synchronized.
 ...
 * <B>Description</B>
 *
 * @ref rtBufferMapEx makes the buffer contents available on the host, either by returning a pointer in \a *optix_owned,
 * or by copying the contents to a memory location pointed to by \a user_owned. Calling @ref rtBufferMapEx with proper
 * map flags can result in better performance than using @ref rtBufferMap, because fewer synchronization copies are
 * required in certain situations.
 * @ref rtBufferMapEx with \a map_flags = @ref RT_BUFFER_MAP_READ_WRITE and \a leve = 0 is equivalent to @ref
 * rtBufferMap.
 *
 * Note that this call does not stop a progressive render if called on a stream buffer.
 *
 ...
 *
 */
RTresult RTAPI rtBufferMapEx(RTbuffer buffer, unsigned int map_flags, unsigned int level, void* user_owned,
                             void** optix_owned);
 ...
 * @param[out]  max_subframes      The \a max_subframes parameter as specified in the call to @ref
 * rtContextLaunchProgressive2D
 *
 * <B>Return values</B>
 *
 ...
 *
 */
RTresult RTAPI rtBufferGetProgressiveUpdateReady(RTbuffer buffer, int* ready, unsigned int* subframe_count,
                                                 unsigned int* max_subframes);
 ...
 * <B>Description</B>
 *
 * @ref rtBufferGetAttribute is used to query buffer attributes. For a list of available attributes, please refer to
 * @ref rtBufferSetAttribute.
 *
 ...
 *
 */
RTresult RTAPI rtRemoteDeviceCreate(const char* url, const char* username, const char* password,
 ...
 * <B>Description</B>
 *
 * In order to gather information about a remote device, several attributes can be queried through @ref
 ...
 * The configuration description for the given index is copied into the destination buffer. A suggested size for the
 * destination buffer is 256 characters. The number of entries in the list is given by the value of @ref
 * RT_REMOTEDEVICE_ATTRIBUTE_NUM_CONFIGURATIONS. Only configurations compatible with the client version being used are
 * listed.
 *
 * @ref RT_REMOTEDEVICE_ATTRIBUTE_STATUS
 ...
 *    - @ref RT_REMOTEDEVICE_STATUS_CONNECTED      The remote device is connected to a cluster manager, but no
 * reservation exists.
 *    - @ref RT_REMOTEDEVICE_STATUS_RESERVED       The remote device has a rendering instance reserved, but it is not
 * yet ready.
 ...
 *
 */
RTresult RTAPI rtRemoteDeviceGetAttribute(RTremotedevice remote_dev, RTremotedeviceattribute attrib, RTsize size,
                                          void* p);
 ...
 * @param[in]   builtin_name The name of the built-in stage to instantiate
 ...
 *
 */
RTresult RTAPI rtPostProcessingStageCreateBuiltin(RTcontext context, const char* builtin_name,
...
RTresult RTAPI rtCommandListAppendPostprocessingStage(RTcommandlist list, RTpostprocessingstage stage,
                                                      RTsize launch_width, RTsize launch_height);
 ...
 *
 */
RTresult RTAPI rtCommandListAppendLaunch2D(RTcommandlist list, unsigned int entry_point_index, RTsize launch_width,
