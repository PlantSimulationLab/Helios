...
/// \ingroup optixprimepp
///
/// \brief Wraps the OptiX Prime C API @ref RTPcontext opaque type and its associated function set representing an OptiX
/// Prime context.
///
...
/// \ingroup optixprimepp
///
/// \brief Encapsulates an OptiX Prime buffer descriptor. The purpose of a buffer descriptor is to provide information
/// about a buffer's type, format, and location. It also describes the region of the buffer to use.
...
/// \ingroup optixprimepp
///
/// \brief Encapsulates an OptiX Prime model. The purpose of a model is to represent a set of triangles and an
...
class ModelObj : public RefCountedObj {
   public:
    ...
    /// Sets the triangle data for a model. This function creates a buffer descriptor of the specified type, populates
    /// it with the supplied data and assigns it to the model. The list of vertices is assumed to be a flat list of
    /// triangles and each three vertices form a single triangle. See @ref rtpModelSetTriangles for additional
    /// information
    void setTriangles(RTPsize triCount, RTPbuffertype type, const void* vertPtr, unsigned stride = 0);

    /// Sets the triangle data for a model. This function creates buffer descriptors of the specified types, populates
    /// them with the supplied data and assigns them to the model. The list of vertices uses the indices list to
    /// determine the triangles. See @ref rtpModelSetTriangles for additional information
    void setTriangles(RTPsize triCount, RTPbuffertype type, const void* indexPtr, RTPsize vertCount,
                      RTPbuffertype vertType, const void* vertPtr, unsigned stride = 0);

    /// Sets the triangle data for a model using the supplied buffer descriptor of vertices. The list of vertices is
    /// assumed to be a flat list of triangles and each three vertices shape a single triangle. See @ref
    /// rtpModelSetTriangles for additional information
    void setTriangles(const BufferDesc& vertices);

    /// Sets the triangle data for a model using the supplied buffer descriptor of vertices. The list of vertices uses
    /// the indices list to determine the triangles. See @ref rtpModelSetTriangles for additional information
    void setTriangles(const BufferDesc& indices, const BufferDesc& vertices);

    /// Sets the instance data for a model. This function creates buffer descriptors of the specified types and formats,
    /// populates them with the supplied data and assigns them to the model. See @ref rtpModelSetInstances for
    /// additional information
    void setInstances(RTPsize count, RTPbuffertype instanceType, const RTPmodel* instanceList,
                      RTPbufferformat transformFormat, RTPbuffertype transformType, const void* transformList);
...
/// \ingroup optixprimepp
///
/// \brief Encapsulates an OptiX Prime query. The purpose of a query is to coordinate the intersection of rays with a
/// model.
...
inline void ContextObj::setCpuThreads(unsigned numThreads) { CHK(rtpContextSetCpuThreads(m_ctx, numThreads)); }
...
inline RTPcontext ContextObj::getRTPcontext() { return m_ctx; }
...
inline Context ContextObj::getContext() { return Context(*this); }

//
// BUFFERDESC
//

inline BufferDescObj::BufferDescObj(const Context& ctx, RTPbufferformat format, RTPbuffertype type, void* buffer)
    : m_desc(0) {
    m_ctx = ctx;
...
inline Context BufferDescObj::getContext() { return m_ctx; }

inline void BufferDescObj::setRange(RTPsize begin, RTPsize end) { CHK(rtpBufferDescSetRange(m_desc, begin, end)); }

inline void BufferDescObj::setStride(unsigned strideBytes) { CHK(rtpBufferDescSetStride(m_desc, strideBytes)); }
...
inline RTPbufferdesc BufferDescObj::getRTPbufferdesc() { return m_desc; }
...
inline Context ModelObj::getContext() { return m_ctx; }
...
inline void ModelObj::setTriangles(RTPsize triCount, RTPbuffertype indexType, const void* indexPtr, RTPsize vertCount,
                                   RTPbuffertype vertType, const void* vertPtr, unsigned stride /*=0 */) {
    BufferDesc idxBufDesc(
        m_ctx->createBufferDesc(RTP_BUFFER_FORMAT_INDICES_INT3, indexType, const_cast<void*>(indexPtr)));
    BufferDesc vtxBufDesc(
        m_ctx->createBufferDesc(RTP_BUFFER_FORMAT_VERTEX_FLOAT3, vertType, const_cast<void*>(vertPtr)));
...
inline void ModelObj::setInstances(RTPsize count, RTPbuffertype instanceType, const RTPmodel* instanceList,
                                   RTPbufferformat transformFormat, RTPbuffertype transformType,
                                   const void* transformList) {
    BufferDesc instances(
        m_ctx->createBufferDesc(RTP_BUFFER_FORMAT_INSTANCE_MODEL, instanceType, const_cast<RTPmodel*>(instanceList)));
...
inline void ModelObj::update(unsigned hints) { CHK(rtpModelUpdate(m_model, hints)); }

inline void ModelObj::finish() { CHK(rtpModelFinish(m_model)); }
...
inline void ModelObj::copy(const Model& srcModel) { CHK(rtpModelCopy(m_model, srcModel->getRTPmodel())); }
...
inline Context QueryObj::getContext() { return m_model->getContext(); }

inline void QueryObj::setRays(const BufferDesc& rays) { CHK(rtpQuerySetRays(m_query, rays->getRTPbufferdesc())); }
...
inline void QueryObj::setHits(const BufferDesc& hits) { CHK(rtpQuerySetHits(m_query, hits->getRTPbufferdesc())); }
...
inline void QueryObj::execute(unsigned hint) { CHK(rtpQueryExecute(m_query, hint)); }

inline void QueryObj::finish() { CHK(rtpQueryFinish(m_query)); }
...
inline void QueryObj::setCudaStream(cudaStream_t stream) { CHK(rtpQuerySetCudaStream(m_query, stream)); }
...
inline RTPresult Exception::getErrorCode() const { return m_errorCode; }

inline const std::string& Exception::getErrorString() const { return m_errorMessage; }

inline const char* Exception::what() const throw() { return m_errorMessage.c_str(); }
