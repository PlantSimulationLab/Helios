...
OPTIXU_INLINE RT_HOSTDEVICE Aabb::Aabb() { invalidate(); }

OPTIXU_INLINE RT_HOSTDEVICE Aabb::Aabb(const float3& min, const float3& max) { set(min, max); }

OPTIXU_INLINE RT_HOSTDEVICE Aabb::Aabb(const float3& v0, const float3& v1, const float3& v2) { set(v0, v1, v2); }

OPTIXU_INLINE RT_HOSTDEVICE bool Aabb::operator==(const Aabb& other) const {
    return m_min.x == other.m_min.x && m_min.y == other.m_min.y && m_min.z == other.m_min.z &&
           m_max.x == other.m_max.x && m_max.y == other.m_max.y && m_max.z == other.m_max.z;
}
...
OPTIXU_INLINE RT_HOSTDEVICE float Aabb::area() const { return 2.0f * halfArea(); }
...
OPTIXU_INLINE RT_HOSTDEVICE float Aabb::maxExtent() const { return extent(longestAxis()); }
...
OPTIXU_INLINE RT_HOSTDEVICE float Aabb::distance(const float3& x) const { return sqrtf(distance2(x)); }

OPTIXU_INLINE RT_HOSTDEVICE float Aabb::signedDistance(const float3& x) const {
    if (m_min.x <= x.x && x.x <= m_max.x && m_min.y <= x.y && x.y <= m_max.y && m_min.z <= x.z && x.z <= m_max.z) {
