...
RT_MAT_DECL OPTIXU_INLINE RT_HOSTDEVICE typename Matrix<M, N>::floatM operator*(const Matrix<M, N>& m,
                                                                                const typename Matrix<M, N>::floatN& v);
RT_MAT_DECL OPTIXU_INLINE RT_HOSTDEVICE typename Matrix<M, N>::floatN operator*(const typename Matrix<M, N>::floatM& v,
                                                                                const Matrix<M, N>& m);
...
class Matrix {
   public:
    ...
    RT_HOSTDEVICE explicit Matrix(const float data[M * N]) {
        for (unsigned int i = 0; i < M * N; ++i) m_data[i] = data[i];
    }
...
OPTIXU_INLINE RT_HOSTDEVICE Matrix<M, N>::Matrix(const Matrix<M, N>& m) {
    for (unsigned int i = 0; i < M * N; ++i) m_data[i] = m[i];
}

template <unsigned int M, unsigned int N>
OPTIXU_INLINE RT_HOSTDEVICE Matrix<M, N>& Matrix<M, N>::operator=(const Matrix& b) {
    for (unsigned int i = 0; i < M * N; ++i) m_data[i] = b[i];
    return *this;
}
...
OPTIXU_INLINE RT_HOSTDEVICE typename Matrix<M, N>::floatN Matrix<M, N>::getRow(unsigned int m) const {
    ...
    for (unsigned int i = 0; i < N; ++i) v[i] = row[i];
...
OPTIXU_INLINE RT_HOSTDEVICE typename Matrix<M, N>::floatM Matrix<M, N>::getCol(unsigned int n) const {
    ...
    float* v = reinterpret_cast<float*>(&temp);
    for (unsigned int i = 0; i < M; ++i) v[i] = RT_MATRIX_ACCESS(m_data, i, n);
...
OPTIXU_INLINE RT_HOSTDEVICE void Matrix<M, N>::setRow(unsigned int m, const typename Matrix<M, N>::floatN& r) {
    ...
    for (unsigned int i = 0; i < N; ++i) row[i] = v[i];
}
...
OPTIXU_INLINE RT_HOSTDEVICE void Matrix<M, N>::setCol(unsigned int n, const typename Matrix<M, N>::floatM& c) {
    ...
    for (unsigned int i = 0; i < M; ++i) RT_MATRIX_ACCESS(m_data, i, n) = v[i];
}
...
RT_HOSTDEVICE Matrix<M, N>& operator-=(Matrix<M, N>& m1, const Matrix<M, N>& m2) {
    for (unsigned int i = 0; i < M * N; ++i) m1[i] -= m2[i];
    return m1;
}
...
OPTIXU_INLINE RT_HOSTDEVICE float2 operator*(const Matrix<2, N>& m, const typename Matrix<2, N>::floatN& vec) {
    ...
    int index = 0;
    for (unsigned int j = 0; j < N; ++j) temp.x += m[index++] * v[j];

    for (unsigned int j = 0; j < N; ++j) temp.y += m[index++] * v[j];
...
OPTIXU_INLINE RT_HOSTDEVICE float3 operator*(const Matrix<3, N>& m, const typename Matrix<3, N>::floatN& vec) {
    ...
    for (unsigned int j = 0; j < N; ++j) temp.z += m[index++] * v[j];
...
OPTIXU_INLINE RT_HOSTDEVICE float4 operator*(const Matrix<4, N>& m, const typename Matrix<4, N>::floatN& vec) {
    ...
    for (unsigned int j = 0; j < N; ++j) temp.w += m[index++] * v[j];
...
OPTIXU_INLINE RT_HOSTDEVICE float4 operator*(const Matrix<4, 4>& m, const float4& vec) {
    float4 temp;
    temp.x = m[0] * vec.x + m[1] * vec.y + m[2] * vec.z + m[3] * vec.w;
    temp.y = m[4] * vec.x + m[5] * vec.y + m[6] * vec.z + m[7] * vec.w;
    temp.z = m[8] * vec.x + m[9] * vec.y + m[10] * vec.z + m[11] * vec.w;
    temp.w = m[12] * vec.x + m[13] * vec.y + m[14] * vec.z + m[15] * vec.w;
...
RT_HOSTDEVICE Matrix<M, N>& operator*=(Matrix<M, N>& m, float f) {
    for (unsigned int i = 0; i < M * N; ++i) m[i] *= f;
    return m;
}
...
RT_HOSTDEVICE Matrix<M, N> operator*(float f, const Matrix<M, N>& m) {
    ...
    for (unsigned int i = 0; i < M * N; ++i) temp[i] = m[i] * f;
...
RT_HOSTDEVICE Matrix<M, N>& operator/=(Matrix<M, N>& m, float f) {
    float inv_f = 1.0f / f;
    for (unsigned int i = 0; i < M * N; ++i) m[i] *= inv_f;
    return m;
}
...
OPTIXU_INLINE RT_HOSTDEVICE Matrix<N, M> Matrix<M, N>::transpose() const {
    ...
    for (unsigned int row = 0; row < M; ++row)
        for (unsigned int col = 0; col < N; ++col) ret[col * M + row] = m_data[row * N + col];
    return ret;
}
...
OPTIXU_INLINE RT_HOSTDEVICE float Matrix<3, 3>::det() const {
    ...
    float d = m[0] * m[4] * m[8] + m[1] * m[5] * m[6] + m[2] * m[3] * m[7] - m[0] * m[5] * m[7] - m[1] * m[3] * m[8] -
              m[2] * m[4] * m[6];
    return d;
}
...
OPTIXU_INLINE RT_HOSTDEVICE float Matrix<4, 4>::det() const {
    const float* m = m_data;
    float d = m[0] * m[5] * m[10] * m[15] - m[0] * m[5] * m[11] * m[14] + m[0] * m[9] * m[14] * m[7] -
              m[0] * m[9] * m[6] * m[15] + m[0] * m[13] * m[6] * m[11] - m[0] * m[13] * m[10] * m[7] -
              m[4] * m[1] * m[10] * m[15] + m[4] * m[1] * m[11] * m[14] - m[4] * m[9] * m[14] * m[3] +
              m[4] * m[9] * m[2] * m[15] - m[4] * m[13] * m[2] * m[11] + m[4] * m[13] * m[10] * m[3] +
              m[8] * m[1] * m[6] * m[15] - m[8] * m[1] * m[14] * m[7] + m[8] * m[5] * m[14] * m[3] -
              m[8] * m[5] * m[2] * m[15] + m[8] * m[13] * m[2] * m[7] - m[8] * m[13] * m[6] * m[3] -
              m[12] * m[1] * m[6] * m[11] + m[12] * m[1] * m[10] * m[7] - m[12] * m[5] * m[10] * m[3] +
              m[12] * m[5] * m[2] * m[11] - m[12] * m[9] * m[2] * m[7] + m[12] * m[9] * m[6] * m[3];
    return d;
}
...
// This is a static member.
template <>
OPTIXU_INLINE RT_HOSTDEVICE Matrix<4, 4> Matrix<4, 4>::fromBasis(const float3& u, const float3& v, const float3& w,
                                                                 const float3& c) {
    float m[16];
    m[0] = u.x;
    m[1] = v.x;
...
OPTIXU_INLINE RT_HOSTDEVICE Matrix<N, N> Matrix<M, N>::identity() {
    float temp[N * N];
    for (unsigned int i = 0; i < N * N; ++i) temp[i] = 0;
    for (unsigned int i = 0; i < N; ++i) RT_MATRIX_ACCESS(temp, i, i) = 1.0f;
