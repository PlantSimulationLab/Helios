 ...
 * function must be executed once for \a context before any call to @ref rtBufferCreateFromD3D10Resource or @ref
 * rtTextureSamplerCreateFromD3D10Resource can take place. A context can only be bound to one device. Once \a device is
 ...
 * \a d3d10Device is a pointer returned from \a D3D10CreateDeviceAndSwapChain. In combination with @ref
 * rtContextSetDevices, this function can be used to restrict OptiX to use only one device. The same device the D3D10
 ...
 * <B>Description</B>
 *
 * @ref rtBufferCreateFromD3D10Resource allocates and returns a handle to a new buffer object in \a *buffer associated
 * with \a context. If the allocated size of the D3D resource is \a 0, @ref RT_ERROR_MEMORY_ALLOCATION_FAILED will be
 * returned. Supported D3D10 buffer types are:
 *
 * - ID3D10Buffer
 *
 * These buffers can be used to share data with D3D10; changes of the content in \a buffer, either done by D3D10 or
 * OptiX, will be reflected automatically in both APIs. If the size, or format, of a D3D10 buffer is changed,
 * appropriate OptiX calls have to be used to update \a buffer accordingly. OptiX keeps only a reference to D3D10 data,
 ...
 *
 */
RTresult RTAPI rtBufferCreateFromD3D10Resource(RTcontext context, unsigned int bufferdesc, ID3D10Resource* resource,
                                               RTbuffer* buffer);
 ...
 *
 */
RTresult RTAPI rtTextureSamplerCreateFromD3D10Resource(RTcontext context, ID3D10Resource* resource,
 ...
 * @ref rtBufferCreateFromD3D10Resource.  If \a buffer was not created from a D3D10 resource \a **resource will be \a 0
 ...
 * <B>Description</B>
 *
 * @ref rtTextureSamplerGetD3D10Resource stores the D3D10 resource pointer in \a **resource if \a sampler was created
 * with
 * @ref rtTextureSamplerGetD3D10Resource.  If \a sampler was not created from a D3D10 resource \a resource will be 0
 * after the call and @ref RT_ERROR_INVALID_VALUE is returned
 *
 ...
 * <B>Description</B>
 *
 * An OptiX buffer in an unregistered state can be registered to OptiX again via @ref rtBufferD3D10Register. Once
 * registered, properties like the size of the original D3D10 resource cannot be modified anymore. Calls to the
 * corresponding D3D10 functions will return with an error code. However, the data of the D3D10 resource can still be
 * read and written by the appropriate D3D10 commands. When a buffer is already in a registered state @ref
 * rtBufferD3D10Register will return @ref RT_ERROR_RESOURCE_ALREADY_REGISTERED. A resource must be registered in order
 ...
 * rtTextureSamplerD3D10Register. Once registered, properties like the size of the original D3D10 resource cannot be
 * modified anymore. Calls to the corresponding D3D10 functions will return with an error code. However, the data of the
 * D3D10 resource can still be read and written by the appropriate D3D10 commands. When a texture sampler is already in
 * a registered state @ref rtTextureSamplerD3D10Register will return @ref RT_ERROR_RESOURCE_ALREADY_REGISTERED. A
 ...
 * <B>Description</B>
 *
 * An OptiX texture sampler in a registered state can be unregistered via @ref rtTextureSamplerD3D10Unregister. Once
 * unregistered, properties like the size of the original D3D10 resource can be changed. As long as a resource is
