 ...
 * <B>Description</B>
 *
 * @ref rtBufferGetDevicePointer returns the pointer to the data of \a buffer on device \a optix_device_ordinal in **\a
 * device_pointer.
 *
 ...
 * <B>Description</B>
 *
 * If @ref rtBufferSetDevicePointer or @ref rtBufferGetDevicePointer have been called for a single device for a given
 * buffer, the user can change the buffer's content on that device through the pointer. OptiX must then synchronize the
 * new buffer contents to all devices. These synchronization copies occur at every @ref rtContextLaunch, unless the
 * buffer is declared with @ref RT_BUFFER_COPY_ON_DIRTY. In this case, @ref rtBufferMarkDirty can be used to notify
 * OptiX that the buffer has been dirtied and must be synchronized.
 *
 * Note that RT_BUFFER_COPY_ON_DIRTY currently only applies to CUDA interop buffers (buffers for which the application
 * has a device pointer).
 *
 ...
 * <B>Description</B>
 *
 * @ref rtBufferSetDevicePointer sets the pointer to the data of \a buffer on device \a optix_device_ordinal to \a
 * device_pointer.
 *
 * If @ref rtBufferSetDevicePointer has been called for a single device for a given buffer,
 * the user can change the buffer's content on that device through the pointer. OptiX must then synchronize the new
 * buffer contents to all devices. These synchronization copies occur at every @ref rtContextLaunch "rtContextLaunch",
 * unless the buffer is declared with @ref RT_BUFFER_COPY_ON_DIRTY. In this case, @ref rtBufferMarkDirty can be used to
 * notify OptiX that the buffer has been dirtied and must be synchronized.
 *
