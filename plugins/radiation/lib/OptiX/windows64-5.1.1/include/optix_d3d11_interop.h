 ...
 * function must be executed once for \a context before any call to @ref rtBufferCreateFromD3D11Resource or @ref
 * rtTextureSamplerCreateFromD3D11Resource can take place. A context can only be bound to one device. Once \a device is
 * bound to \a context, the binding is immutable and remains upon destruction of \a context.
 *
 ...
 * \a D3D11Device is a pointer returned from \a D3D11CreateDeviceAndSwapChain. In combination with @ref
 * rtContextSetDevices, this function can be used to restrict OptiX to use only one device. The same device the D3D11
 * commands will be sent to.
 *
 * This function is only supported on Windows platforms.
 *
 * @param[in]   device       A handle to the memory location where the OptiX device ordinal associated with \a
 * D3D11Device will be stored
 ...
 * <B>Description</B>
 *
 * @ref rtBufferCreateFromD3D11Resource allocates and returns a handle to a new buffer object in \a *buffer associated
 * with \a context. If the allocated size of the D3D resource is \a 0, @ref RT_ERROR_MEMORY_ALLOCATION_FAILED will be
 * returned. Supported D3D11 buffer types are:
 *
 * - ID3D11Buffer
 *
 * These buffers can be used to share data with D3D11; changes of the content in \a buffer, either done by D3D11 or
 * OptiX, will be reflected automatically in both APIs. If the size, or format, of a D3D11 buffer is changed,
 * appropriate OptiX calls have to be used to update \a buffer accordingly. OptiX keeps only a reference to D3D11 data,
 * when \a buffer is destroyed, the state of \a resource is unaltered.
 *
 ...
 *
 * Flags can be used to optimize data transfers between the host and it's devices. Currently no \a flags are supported
 * for interop buffers.
 *
 ...
 *
 */
RTresult RTAPI rtBufferCreateFromD3D11Resource(RTcontext context, unsigned int bufferdesc, ID3D11Resource* resource,
                                               RTbuffer* buffer);
 ...
 *
 */
RTresult RTAPI rtTextureSamplerCreateFromD3D11Resource(RTcontext context, ID3D11Resource* resource,
 ...
 * @ref rtBufferCreateFromD3D11Resource.  If \a buffer was not created from a D3D11 resource \a **resource will be \a 0
 * after the call and @ref RT_ERROR_INVALID_VALUE is returned.
 *
 ...
 * <B>Description</B>
 *
 * @ref rtTextureSamplerGetD3D11Resource stores the D3D11 resource pointer in \a **resource if \a sampler was created
 * with
 * @ref rtTextureSamplerGetD3D11Resource.  If \a sampler was not created from a D3D11 resource \a resource will be 0
 * after the call and @ref RT_ERROR_INVALID_VALUE is returned
 *
 ...
 * <B>Description</B>
 *
 * An OptiX buffer in an unregistered state can be registered to OptiX again via @ref rtBufferD3D11Register. Once
 * registered, properties like the size of the original D3D11 resource cannot be modified anymore. Calls to the
 * corresponding D3D11 functions will return with an error code. However, the data of the D3D11 resource can still be
 * read and written by the appropriate D3D11 commands. When a buffer is already in a registered state @ref
 * rtBufferD3D11Register will return @ref RT_ERROR_RESOURCE_ALREADY_REGISTERED. A resource must be registered in order
 * to be used by OptiX. If a resource is not registered @ref RT_ERROR_INVALID_VALUE will be returned.
 *
 ...
 * <B>Description</B>
 *
 * An OptiX texture sampler in an unregistered state can be registered to OptiX again via @ref
 * rtTextureSamplerD3D11Register. Once registered, properties like the size of the original D3D11 resource cannot be
 * modified anymore. Calls to the corresponding D3D11 functions will return with an error code. However, the data of the
 * D3D11 resource can still be read and written by the appropriate D3D11 commands. When a texture sampler is already in
 * a registered state @ref rtTextureSamplerD3D11Register will return @ref RT_ERROR_RESOURCE_ALREADY_REGISTERED. A
 * resource must be registered in order to be used by OptiX. If a resource is not registered @ref RT_ERROR_INVALID_VALUE
 * will be returned.
 *
 ...
 * <B>Description</B>
 *
 * An OptiX texture sampler in a registered state can be unregistered via @ref rtTextureSamplerD3D11Unregister. Once
 * unregistered, properties like the size of the original D3D11 resource can be changed. As long as a resource is
 * unregistered, OptiX will not be able to access the data and will fail with @ref RT_ERROR_INVALID_VALUE. When a buffer
 * is already in an unregistered state
