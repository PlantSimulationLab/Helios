...
struct VectorTypes<int, 1> {
    typedef int Type;
    template <class S>
    static __device__ __forceinline__ Type make(S s) {
        return make_int(s);
    }
};
template <>
...
struct VectorTypes<unsigned int, 1> {
    ...
    static __device__ __forceinline__ Type make(unsigned int s) { return s; }
    template <class S>
    static __device__ __forceinline__ Type make(S s) {
        return (unsigned int)s.x;
    }
};
template <>
 ...
 *
 * - \b rtLaunchIndex - The launch invocation index. Type must be one of \a unsigned int, \a uint2, \a uint3, \a int, \a
 * int2, \a int3 and is read-only.
 * - \b rtLaunchDim - The size of each dimension of the launch. The values range from 1 to the launch size in that
 * dimension. Type must be one of \a unsigned int, \a uint2, \a uint3, \a int, \a int2, \a int3 and is read-only.
 * - \b rtCurrentRay - The currently active ray, valid only when a call to @ref rtTrace is active.  The vector is \em
 * not guaranteed to be normalized.  Type must be \a optix::Ray and is read-only.
 * - \b rtCurrentTime - The current ray time.  Type must be \a float and is read-only.
 * - \b rtIntersectionDistance - The current closest hit distance, valid only when a call to @ref rtTrace is active.
 * Type must be \a float and is read-only.
 * - \b rtRayPayload - The struct passed into the most recent @ref rtTrace call and is read-write.
 * - \b attribute \a name - A named attribute passed from the intersection program to a closest-hit or any-hit program.
 * The types must match in both sets of programs.  This variable is read-only in the closest-hit or any-hit program and
 * is written in the intersection program.
 *
...
template <typename T>
struct rtCallableProgramSizeofWrapper {
...
struct buffer {
   ...
   protected:
    ...
    template <typename T2>
    __device__ __forceinline__ static void* create(type<T2>, void* v) {
        return v;
    }
...
template <typename T, int Dim = 1>
struct bufferId : public buffer<T, Dim> {
    ...
    __device__ __forceinline__ bufferId& operator=(RTbufferidnull nullid) {
        m_id = nullid;
        return *this;
    }
...
#define _OPTIX_TEX_FUNC_DECLARE_(FUNC, SIGNATURE, PARAMS) \
    template <>                                           \
    inline __device__ unsigned char FUNC SIGNATURE {      \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return (unsigned char)(tmp.x);                    \
    }                                                     \
    template <>                                           \
    inline __device__ char FUNC SIGNATURE {               \
        int4 tmp = FUNC<int4> PARAMS;                     \
        return (char)(tmp.x);                             \
    }                                                     \
    template <>                                           \
    inline __device__ unsigned short FUNC SIGNATURE {     \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return (unsigned short)(tmp.x);                   \
    }                                                     \
    template <>                                           \
    inline __device__ short FUNC SIGNATURE {              \
        int4 tmp = FUNC<int4> PARAMS;                     \
        return (short)(tmp.x);                            \
    }                                                     \
    template <>                                           \
    inline __device__ int FUNC SIGNATURE {                \
        int4 tmp = FUNC<int4> PARAMS;                     \
        return tmp.x;                                     \
    }                                                     \
    template <>                                           \
    inline __device__ unsigned int FUNC SIGNATURE {       \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return tmp.x;                                     \
    }                                                     \
    template <>                                           \
    inline __device__ uchar1 FUNC SIGNATURE {             \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return make_uchar1(tmp.x);                        \
    }                                                     \
    template <>                                           \
    inline __device__ char1 FUNC SIGNATURE {              \
        int4 tmp = FUNC<int4> PARAMS;                     \
        return make_char1(tmp.x);                         \
    }                                                     \
    template <>                                           \
    inline __device__ ushort1 FUNC SIGNATURE {            \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return make_ushort1(tmp.x);                       \
    }                                                     \
    template <>                                           \
    inline __device__ short1 FUNC SIGNATURE {             \
        int4 tmp = FUNC<int4> PARAMS;                     \
        return make_short1(tmp.x);                        \
    }                                                     \
    template <>                                           \
    inline __device__ uint1 FUNC SIGNATURE {              \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return make_uint1(tmp.x);                         \
    }                                                     \
    template <>                                           \
    inline __device__ int1 FUNC SIGNATURE {               \
        int4 tmp = FUNC<int4> PARAMS;                     \
        return make_int1(tmp.x);                          \
    }                                                     \
    template <>                                           \
    inline __device__ float FUNC SIGNATURE {              \
        float4 tmp = FUNC<float4> PARAMS;                 \
        return tmp.x;                                     \
    }                                                     \
    template <>                                           \
    inline __device__ uchar2 FUNC SIGNATURE {             \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return make_uchar2(tmp.x, tmp.y);                 \
    }                                                     \
    template <>                                           \
    inline __device__ char2 FUNC SIGNATURE {              \
        int4 tmp = FUNC<int4> PARAMS;                     \
        return make_char2(tmp.x, tmp.y);                  \
    }                                                     \
    template <>                                           \
    inline __device__ ushort2 FUNC SIGNATURE {            \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return make_ushort2(tmp.x, tmp.y);                \
    }                                                     \
    template <>                                           \
    inline __device__ short2 FUNC SIGNATURE {             \
        int4 tmp = FUNC<int4> PARAMS;                     \
        return make_short2(tmp.x, tmp.y);                 \
    }                                                     \
    template <>                                           \
    inline __device__ uint2 FUNC SIGNATURE {              \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return make_uint2(tmp.x, tmp.y);                  \
    }                                                     \
    template <>                                           \
    inline __device__ int2 FUNC SIGNATURE {               \
        int4 tmp = FUNC<int4> PARAMS;                     \
        return make_int2(tmp.x, tmp.y);                   \
    }                                                     \
    template <>                                           \
    inline __device__ float2 FUNC SIGNATURE {             \
        float4 tmp = FUNC<float4> PARAMS;                 \
        return ::make_float2(tmp.x, tmp.y);               \
    }                                                     \
    template <>                                           \
    inline __device__ uchar4 FUNC SIGNATURE {             \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return make_uchar4(tmp.x, tmp.y, tmp.z, tmp.w);   \
    }                                                     \
    template <>                                           \
    inline __device__ char4 FUNC SIGNATURE {              \
        int4 tmp = FUNC<int4> PARAMS;                     \
        return make_char4(tmp.x, tmp.y, tmp.z, tmp.w);    \
    }                                                     \
    template <>                                           \
    inline __device__ ushort4 FUNC SIGNATURE {            \
        uint4 tmp = FUNC<uint4> PARAMS;                   \
        return make_ushort4(tmp.x, tmp.y, tmp.z, tmp.w);  \
    }                                                     \
    template <>                                           \
    inline __device__ short4 FUNC SIGNATURE {             \
        int4 tmp = FUNC<int4> PARAMS;                     \
 ...
 * \b rtTex1DLod, \b rtTex2DLod, \b rtTex3DLod, \b rtTex1DLayeredLod, \b rtTex2DLayeredLod, \b rtTex1DLayered, \b
 * rtTex2DLayered.
 *
 ...
 * \b rtTex2DGather, \b rtTex1DGrad, \b rtTex2DGrad, \b rtTex3DGrad, \b rtTex1DLayeredGrad, \b rtTex2DLayeredGrad,
 * \b rtTex1DLod, \b rtTex2DLod, \b rtTex3DLod, \b rtTex1DLayeredLod, \b rtTex2DLayeredLod, \b rtTex1DLayered, \b
 * rtTex2DLayered, \b rtTexCubemap, \b rtTexCubemapLod, \b rtTexCubemapLayered and \b rtTexCubemapLayeredLod were
 * introduced in OptiX 3.9.
 *
 * <B>See also</B>
...
inline __device__ uint3 rtTexSize(rtTextureId id) { return optix::rt_texture_get_size_id(id); }
...
template <>
inline __device__ float4 rtTex1DFetch(rtTextureId id, int x) {
    return optix::rt_texture_get_fetch_id(id, 1, x, 0, 0, 0);
}
template <>
inline __device__ int4 rtTex1DFetch(rtTextureId id, int x) {
    return float4AsInt4(optix::rt_texture_get_fetch_id(id, 1, x, 0, 0, 0));
}
template <>
inline __device__ uint4 rtTex1DFetch(rtTextureId id, int x) {
...
template <>
inline __device__ float4 rtTex2DFetch(rtTextureId id, int x, int y) {
    return optix::rt_texture_get_fetch_id(id, 2, x, y, 0, 0);
}
template <>
inline __device__ int4 rtTex2DFetch(rtTextureId id, int x, int y) {
    return float4AsInt4(optix::rt_texture_get_fetch_id(id, 2, x, y, 0, 0));
}
template <>
inline __device__ uint4 rtTex2DFetch(rtTextureId id, int x, int y) {
...
template <>
inline __device__ float4 rtTex3DFetch(rtTextureId id, int x, int y, int z) {
    return optix::rt_texture_get_fetch_id(id, 3, x, y, z, 0);
}
template <>
inline __device__ int4 rtTex3DFetch(rtTextureId id, int x, int y, int z) {
    return float4AsInt4(optix::rt_texture_get_fetch_id(id, 3, x, y, z, 0));
}
template <>
inline __device__ uint4 rtTex3DFetch(rtTextureId id, int x, int y, int z) {
...
template <>
inline __device__ float4 rtTex2DGather(rtTextureId id, float x, float y, int comp) {
    return optix::rt_texture_get_gather_id(id, x, y, comp);
}
template <>
inline __device__ int4 rtTex2DGather(rtTextureId id, float x, float y, int comp) {
    return float4AsInt4(optix::rt_texture_get_gather_id(id, x, y, comp));
}
template <>
inline __device__ uint4 rtTex2DGather(rtTextureId id, float x, float y, int comp) {
...
template <>
inline __device__ float4 rtTex1DGrad(rtTextureId id, float x, float dPdx, float dPdy) {
    return optix::rt_texture_get_grad_id(id, TEX_LOOKUP_1D, x, 0, 0, 0, dPdx, 0, 0, dPdy, 0, 0);
}
template <>
inline __device__ int4 rtTex1DGrad(rtTextureId id, float x, float dPdx, float dPdy) {
    return float4AsInt4(optix::rt_texture_get_grad_id(id, TEX_LOOKUP_1D, x, 0, 0, 0, dPdx, 0, 0, dPdy, 0, 0));
}
template <>
inline __device__ uint4 rtTex1DGrad(rtTextureId id, float x, float dPdx, float dPdy) {
...
template <>
inline __device__ float4 rtTex2DGrad(rtTextureId id, float x, float y, float2 dPdx, float2 dPdy) {
    return optix::rt_texture_get_grad_id(id, TEX_LOOKUP_2D, x, y, 0, 0, dPdx.x, dPdx.y, 0, dPdy.x, dPdy.y, 0);
}
template <>
inline __device__ int4 rtTex2DGrad(rtTextureId id, float x, float y, float2 dPdx, float2 dPdy) {
    return float4AsInt4(
        optix::rt_texture_get_grad_id(id, TEX_LOOKUP_2D, x, y, 0, 0, dPdx.x, dPdx.y, 0, dPdy.x, dPdy.y, 0));
}
template <>
inline __device__ uint4 rtTex2DGrad(rtTextureId id, float x, float y, float2 dPdx, float2 dPdy) {
    return float4AsUInt4(
        optix::rt_texture_get_grad_id(id, TEX_LOOKUP_2D, x, y, 0, 0, dPdx.x, dPdx.y, 0, dPdy.x, dPdy.y, 0));
}
_OPTIX_TEX_FUNC_DECLARE_(rtTex2DGrad, (rtTextureId id, float x, float y, float2 dPdx, float2 dPdy),
                         (id, x, y, dPdx, dPdy))
template <typename T>
...
template <>
inline __device__ float4 rtTex3DGrad(rtTextureId id, float x, float y, float z, float4 dPdx, float4 dPdy) {
    return optix::rt_texture_get_grad_id(id, TEX_LOOKUP_3D, x, y, z, 0, dPdx.x, dPdx.y, dPdx.z, dPdy.x, dPdy.y, dPdy.z);
}
template <>
inline __device__ int4 rtTex3DGrad(rtTextureId id, float x, float y, float z, float4 dPdx, float4 dPdy) {
    return float4AsInt4(
        optix::rt_texture_get_grad_id(id, TEX_LOOKUP_3D, x, y, z, 0, dPdx.x, dPdx.y, dPdx.z, dPdy.x, dPdy.y, dPdy.z));
}
template <>
inline __device__ uint4 rtTex3DGrad(rtTextureId id, float x, float y, float z, float4 dPdx, float4 dPdy) {
    return float4AsUInt4(
        optix::rt_texture_get_grad_id(id, TEX_LOOKUP_3D, x, y, z, 0, dPdx.x, dPdx.y, dPdx.z, dPdy.x, dPdy.y, dPdy.z));
}
_OPTIX_TEX_FUNC_DECLARE_(rtTex3DGrad, (rtTextureId id, float x, float y, float z, float4 dPdx, float4 dPdy),
                         (id, x, y, z, dPdx, dPdy))
template <typename T>
...
template <>
inline __device__ float4 rtTex1DLayeredGrad(rtTextureId id, float x, int layer, float dPdx, float dPdy) {
    return optix::rt_texture_get_grad_id(id, TEX_LOOKUP_A1, x, 0, 0, layer, dPdx, 0, 0, dPdy, 0, 0);
}
template <>
inline __device__ int4 rtTex1DLayeredGrad(rtTextureId id, float x, int layer, float dPdx, float dPdy) {
    return float4AsInt4(optix::rt_texture_get_grad_id(id, TEX_LOOKUP_A1, x, 0, 0, layer, dPdx, 0, 0, dPdy, 0, 0));
}
template <>
inline __device__ uint4 rtTex1DLayeredGrad(rtTextureId id, float x, int layer, float dPdx, float dPdy) {
    return float4AsUInt4(optix::rt_texture_get_grad_id(id, TEX_LOOKUP_A1, x, 0, 0, layer, dPdx, 0, 0, dPdy, 0, 0));
}
_OPTIX_TEX_FUNC_DECLARE_(rtTex1DLayeredGrad, (rtTextureId id, float x, int layer, float dPdx, float dPdy),
...
template <>
inline __device__ float4 rtTex2DLayeredGrad(rtTextureId id, float x, float y, int layer, float2 dPdx, float2 dPdy) {
    return optix::rt_texture_get_grad_id(id, TEX_LOOKUP_A2, x, y, 0, layer, dPdx.x, dPdx.y, 0, dPdy.x, dPdy.y, 0);
}
template <>
inline __device__ int4 rtTex2DLayeredGrad(rtTextureId id, float x, float y, int layer, float2 dPdx, float2 dPdy) {
    return float4AsInt4(
        optix::rt_texture_get_grad_id(id, TEX_LOOKUP_A2, x, y, 0, layer, dPdx.x, dPdx.y, 0, dPdy.x, dPdy.y, 0));
}
template <>
inline __device__ uint4 rtTex2DLayeredGrad(rtTextureId id, float x, float y, int layer, float2 dPdx, float2 dPdy) {
    return float4AsUInt4(
        optix::rt_texture_get_grad_id(id, TEX_LOOKUP_A2, x, y, 0, layer, dPdx.x, dPdx.y, 0, dPdy.x, dPdy.y, 0));
}
_OPTIX_TEX_FUNC_DECLARE_(rtTex2DLayeredGrad, (rtTextureId id, float x, float y, int layer, float2 dPdx, float2 dPdy),
                         (id, x, y, layer, dPdx, dPdy))
template <typename T>
inline __device__ void rtTex2DLayeredGrad(T* retVal, rtTextureId id, float x, float y, int layer, float2 dPdx,
                                          float2 dPdy) {
...
template <>
inline __device__ float4 rtTex1DLod(rtTextureId id, float x, float level) {
    return optix::rt_texture_get_level_id(id, TEX_LOOKUP_1D, x, 0, 0, 0, level);
}
template <>
inline __device__ int4 rtTex1DLod(rtTextureId id, float x, float level) {
    return float4AsInt4(optix::rt_texture_get_level_id(id, TEX_LOOKUP_1D, x, 0, 0, 0, level));
}
template <>
inline __device__ uint4 rtTex1DLod(rtTextureId id, float x, float level) {
...
template <>
inline __device__ float4 rtTex2DLod(rtTextureId id, float x, float y, float level) {
    return optix::rt_texture_get_level_id(id, TEX_LOOKUP_2D, x, y, 0, 0, level);
}
template <>
inline __device__ int4 rtTex2DLod(rtTextureId id, float x, float y, float level) {
    return float4AsInt4(optix::rt_texture_get_level_id(id, TEX_LOOKUP_2D, x, y, 0, 0, level));
}
template <>
inline __device__ uint4 rtTex2DLod(rtTextureId id, float x, float y, float level) {
...
template <>
inline __device__ float4 rtTex3DLod(rtTextureId id, float x, float y, float z, float level) {
    return optix::rt_texture_get_level_id(id, TEX_LOOKUP_3D, x, y, z, 0, level);
}
template <>
inline __device__ int4 rtTex3DLod(rtTextureId id, float x, float y, float z, float level) {
    return float4AsInt4(optix::rt_texture_get_level_id(id, TEX_LOOKUP_3D, x, y, z, 0, level));
}
template <>
inline __device__ uint4 rtTex3DLod(rtTextureId id, float x, float y, float z, float level) {
...
template <>
inline __device__ float4 rtTex1DLayeredLod(rtTextureId id, float x, int layer, float level) {
    return optix::rt_texture_get_level_id(id, TEX_LOOKUP_A1, x, 0, 0, layer, level);
}
template <>
inline __device__ int4 rtTex1DLayeredLod(rtTextureId id, float x, int layer, float level) {
    return float4AsInt4(optix::rt_texture_get_level_id(id, TEX_LOOKUP_A1, x, 0, 0, layer, level));
}
template <>
inline __device__ uint4 rtTex1DLayeredLod(rtTextureId id, float x, int layer, float level) {
...
template <>
inline __device__ float4 rtTex2DLayeredLod(rtTextureId id, float x, float y, int layer, float level) {
    return optix::rt_texture_get_level_id(id, TEX_LOOKUP_A2, x, y, 0, layer, level);
}
template <>
inline __device__ int4 rtTex2DLayeredLod(rtTextureId id, float x, float y, int layer, float level) {
    return float4AsInt4(optix::rt_texture_get_level_id(id, TEX_LOOKUP_A2, x, y, 0, layer, level));
}
template <>
inline __device__ uint4 rtTex2DLayeredLod(rtTextureId id, float x, float y, int layer, float level) {
    return float4AsUInt4(optix::rt_texture_get_level_id(id, TEX_LOOKUP_A2, x, y, 0, layer, level));
}
_OPTIX_TEX_FUNC_DECLARE_(rtTex2DLayeredLod, (rtTextureId id, float x, float y, int layer, float level),
                         (id, x, y, layer, level))
template <typename T>
...
template <>
inline __device__ float4 rtTex1DLayered(rtTextureId id, float x, int layer) {
    return optix::rt_texture_get_base_id(id, TEX_LOOKUP_A1, x, 0, 0, layer);
}
template <>
inline __device__ int4 rtTex1DLayered(rtTextureId id, float x, int layer) {
    return float4AsInt4(optix::rt_texture_get_base_id(id, TEX_LOOKUP_A1, x, 0, 0, layer));
}
template <>
inline __device__ uint4 rtTex1DLayered(rtTextureId id, float x, int layer) {
...
template <>
inline __device__ float4 rtTex2DLayered(rtTextureId id, float x, float y, int layer) {
    return optix::rt_texture_get_base_id(id, TEX_LOOKUP_A2, x, y, 0, layer);
}
template <>
inline __device__ int4 rtTex2DLayered(rtTextureId id, float x, float y, int layer) {
    return float4AsInt4(optix::rt_texture_get_base_id(id, TEX_LOOKUP_A2, x, y, 0, layer));
}
template <>
inline __device__ uint4 rtTex2DLayered(rtTextureId id, float x, float y, int layer) {
...
template <>
inline __device__ float4 rtTexCubemap(rtTextureId id, float x, float y, float z) {
    return optix::rt_texture_get_base_id(id, TEX_LOOKUP_CUBE, x, y, z, 0);
}
template <>
inline __device__ int4 rtTexCubemap(rtTextureId id, float x, float y, float z) {
    return float4AsInt4(optix::rt_texture_get_base_id(id, TEX_LOOKUP_CUBE, x, y, z, 0));
}
template <>
inline __device__ uint4 rtTexCubemap(rtTextureId id, float x, float y, float z) {
...
template <>
inline __device__ float4 rtTexCubemapLayered(rtTextureId id, float x, float y, float z, int layer) {
    return optix::rt_texture_get_base_id(id, TEX_LOOKUP_ACUBE, x, y, z, layer);
}
template <>
inline __device__ int4 rtTexCubemapLayered(rtTextureId id, float x, float y, float z, int layer) {
    return float4AsInt4(optix::rt_texture_get_base_id(id, TEX_LOOKUP_ACUBE, x, y, z, layer));
}
template <>
inline __device__ uint4 rtTexCubemapLayered(rtTextureId id, float x, float y, float z, int layer) {
    return float4AsUInt4(optix::rt_texture_get_base_id(id, TEX_LOOKUP_ACUBE, x, y, z, layer));
}
_OPTIX_TEX_FUNC_DECLARE_(rtTexCubemapLayered, (rtTextureId id, float x, float y, float z, int layer),
                         (id, x, y, z, layer))
template <typename T>
...
template <>
inline __device__ float4 rtTexCubemapLod(rtTextureId id, float x, float y, float z, float level) {
    return optix::rt_texture_get_level_id(id, TEX_LOOKUP_CUBE, x, y, z, 0, level);
}
template <>
inline __device__ int4 rtTexCubemapLod(rtTextureId id, float x, float y, float z, float level) {
    return float4AsInt4(optix::rt_texture_get_level_id(id, TEX_LOOKUP_CUBE, x, y, z, 0, level));
}
template <>
inline __device__ uint4 rtTexCubemapLod(rtTextureId id, float x, float y, float z, float level) {
    return float4AsUInt4(optix::rt_texture_get_level_id(id, TEX_LOOKUP_CUBE, x, y, z, 0, level));
}
_OPTIX_TEX_FUNC_DECLARE_(rtTexCubemapLod, (rtTextureId id, float x, float y, float z, float level),
                         (id, x, y, z, level))
template <typename T>
...
template <>
inline __device__ float4 rtTexCubemapLayeredLod(rtTextureId id, float x, float y, float z, int layer, float level) {
    return optix::rt_texture_get_level_id(id, TEX_LOOKUP_ACUBE, x, y, z, layer, level);
}
template <>
inline __device__ int4 rtTexCubemapLayeredLod(rtTextureId id, float x, float y, float z, int layer, float level) {
    return float4AsInt4(optix::rt_texture_get_level_id(id, TEX_LOOKUP_ACUBE, x, y, z, layer, level));
}
template <>
inline __device__ uint4 rtTexCubemapLayeredLod(rtTextureId id, float x, float y, float z, int layer, float level) {
    return float4AsUInt4(optix::rt_texture_get_level_id(id, TEX_LOOKUP_ACUBE, x, y, z, layer, level));
}
_OPTIX_TEX_FUNC_DECLARE_(rtTexCubemapLayeredLod, (rtTextureId id, float x, float y, float z, int layer, float level),
                         (id, x, y, z, layer, level))
template <typename T>
inline __device__ void rtTexCubemapLayeredLod(T* retVal, rtTextureId id, float x, float y, float z, int layer,
                                              float level) {
...
_OPTIX_TEX_FUNC_DECLARE_(rtTex2DLoadOrRequest, (rtTextureId id, float x, float y, bool& isResident),
                         (id, x, y, isResident))
template <typename T>
...
_OPTIX_TEX_FUNC_DECLARE_(rtTex3DLoadOrRequest, (rtTextureId id, float x, float y, float z, bool& isResident),
                         (id, x, y, z, isResident))
template <typename T>
...
_OPTIX_TEX_FUNC_DECLARE_(rtTex1DLodLoadOrRequest, (rtTextureId id, float x, float level, bool& isResident),
                         (id, x, level, isResident))
template <typename T>
...
_OPTIX_TEX_FUNC_DECLARE_(rtTex2DLodLoadOrRequest, (rtTextureId id, float x, float y, float level, bool& isResident),
                         (id, x, y, level, isResident))
template <typename T>
inline __device__ void rtTex2DLodLoadOrRequest(T* retVal, rtTextureId id, float x, float y, float level,
                                               bool& isResident) {
...
template <>
inline __device__ float4 rtTex3DLodLoadOrRequest(rtTextureId id, float x, float y, float z, float level,
                                                 bool& isResident) {
    return optix::rt_texture_lod_load_or_request_f_id(id, 2, x, y, z, 0.f, level, &isResident);
}

template <>
inline __device__ uint4 rtTex3DLodLoadOrRequest(rtTextureId id, float x, float y, float z, float level,
                                                bool& isResident) {
    return optix::rt_texture_lod_load_or_request_u_id(id, 2, x, y, z, 0.f, level, &isResident);
}

template <>
inline __device__ int4 rtTex3DLodLoadOrRequest(rtTextureId id, float x, float y, float z, float level,
                                               bool& isResident) {
...
template <typename T>
inline __device__ void rtTex3DLodLoadOrRequest(T* retVal, rtTextureId id, float x, float y, float z, float level,
                                               bool& isResident) {
...
inline __device__ float4 rtTex1DGradLoadOrRequest(rtTextureId id, float x, float dPdx, float dPdy, bool& isResident) {
    return optix::rt_texture_grad_load_or_request_f_id(id, 1, x, 0.f, 0.f, 0.f, dPdx, 0.f, 0.f, dPdy, 0.f, 0.f,
                                                       &isResident);
}

template <>
inline __device__ uint4 rtTex1DGradLoadOrRequest(rtTextureId id, float x, float dPdx, float dPdy, bool& isResident) {
    return optix::rt_texture_grad_load_or_request_u_id(id, 1, x, 0.f, 0.f, 0.f, dPdx, 0.f, 0.f, dPdy, 0.f, 0.f,
                                                       &isResident);
}

template <>
inline __device__ int4 rtTex1DGradLoadOrRequest(rtTextureId id, float x, float dPdx, float dPdy, bool& isResident) {
    return optix::rt_texture_grad_load_or_request_i_id(id, 1, x, 0.f, 0.f, 0.f, dPdx, 0.f, 0.f, dPdy, 0.f, 0.f,
                                                       &isResident);
}

_OPTIX_TEX_FUNC_DECLARE_(rtTex1DGradLoadOrRequest, (rtTextureId id, float x, float dPdx, float dPdy, bool& isResident),
                         (id, x, dPdx, dPdy, isResident))
template <typename T>
inline __device__ void rtTex1DGradLoadOrRequest(T* retVal, rtTextureId id, float x, float dPdx, float dPdy,
                                                bool& isResident) {
...
template <typename T>
inline __device__ T rtTex2DGradLoadOrRequest(rtTextureId id, float x, float y, float2 dPdx, float2 dPdy,
                                             bool& isResident);

template <>
inline __device__ float4 rtTex2DGradLoadOrRequest(rtTextureId id, float x, float y, float2 dPdx, float2 dPdy,
                                                  bool& isResident) {
    return optix::rt_texture_grad_load_or_request_f_id(id, 2, x, y, 0.f, 0.f, dPdx.x, dPdx.y, 0.f, dPdy.x, dPdy.y, 0.f,
                                                       &isResident);
}

template <>
inline __device__ uint4 rtTex2DGradLoadOrRequest(rtTextureId id, float x, float y, float2 dPdx, float2 dPdy,
                                                 bool& isResident) {
    return optix::rt_texture_grad_load_or_request_u_id(id, 2, x, y, 0.f, 0.f, dPdx.x, dPdx.y, 0.f, dPdy.x, dPdy.y, 0.f,
                                                       &isResident);
}

template <>
inline __device__ int4 rtTex2DGradLoadOrRequest(rtTextureId id, float x, float y, float2 dPdx, float2 dPdy,
                                                bool& isResident) {
    return optix::rt_texture_grad_load_or_request_i_id(id, 2, x, y, 0.f, 0.f, dPdx.x, dPdx.y, 0.f, dPdy.x, dPdy.y, 0.f,
                                                       &isResident);
}
...
template <typename T>
inline __device__ void rtTex2DGradLoadOrRequest(T* retVal, rtTextureId id, float x, float y, float2 dPdx, float2 dPdy,
                                                bool& isResident) {
...
template <typename T>
inline __device__ T rtTex3DGradLoadOrRequest(rtTextureId id, float x, float y, float z, float4 dPdx, float4 dPdy,
                                             bool& isResident);

template <>
inline __device__ float4 rtTex3DGradLoadOrRequest(rtTextureId id, float x, float y, float z, float4 dPdx, float4 dPdy,
                                                  bool& isResident) {
...
template <>
inline __device__ uint4 rtTex3DGradLoadOrRequest(rtTextureId id, float x, float y, float z, float4 dPdx, float4 dPdy,
                                                 bool& isResident) {
...
template <>
inline __device__ int4 rtTex3DGradLoadOrRequest(rtTextureId id, float x, float y, float z, float4 dPdx, float4 dPdy,
                                                bool& isResident) {
...
template <typename T>
inline __device__ void rtTex3DGradLoadOrRequest(T* retVal, rtTextureId id, float x, float y, float z, float4 dPdx,
 ...
 */
template <
    typename ReturnT, typename Arg0T = rti_internal_callableprogram::CPArgVoid,
    typename Arg1T = rti_internal_callableprogram::CPArgVoid, typename Arg2T = rti_internal_callableprogram::CPArgVoid,
    typename Arg3T = rti_internal_callableprogram::CPArgVoid, typename Arg4T = rti_internal_callableprogram::CPArgVoid,
    typename Arg5T = rti_internal_callableprogram::CPArgVoid, typename Arg6T = rti_internal_callableprogram::CPArgVoid,
    typename Arg7T = rti_internal_callableprogram::CPArgVoid, typename Arg8T = rti_internal_callableprogram::CPArgVoid,
...
class callableProgramIdBase {
   public:
    ...
    __device__ __forceinline__ ReturnT operator()(Arg0T arg0, Arg1T arg1, Arg2T arg2, Arg3T arg3, Arg4T arg4,
                                                  Arg5T arg5, Arg6T arg6, Arg7T arg7) {
        typedef typename check_is_CPArgVoid<is_CPArgVoid<Arg8T>::result>::result Arg8_test;
        typedef typename check_is_CPArgVoid<is_CPArgVoid<Arg9T>::result>::result Arg9_test;
    ...
    __device__ __forceinline__ ReturnT operator()(Arg0T arg0, Arg1T arg1, Arg2T arg2, Arg3T arg3, Arg4T arg4,
                                                  Arg5T arg5, Arg6T arg6, Arg7T arg7, Arg8T arg8) {
        typedef typename check_is_CPArgVoid<is_CPArgVoid<Arg9T>::result>::result Arg9_test;
    ...
    __device__ __forceinline__ ReturnT operator()(Arg0T arg0, Arg1T arg1, Arg2T arg2, Arg3T arg3, Arg4T arg4,
                                                  Arg5T arg5, Arg6T arg6, Arg7T arg7, Arg8T arg8, Arg9T arg9) {
...
class markedCallableProgramIdBase {
   public:
    ...
    __device__ __forceinline__ explicit markedCallableProgramIdBase(int id, const char* callSiteName) : m_id(id) {
        m_callSiteName = callSiteName;
    }
   ...
   public                                                                                             \
    rti_internal_callableprogram::callableProgramIdBase<__VA_ARGS__> {                                \
       public:                                                                                        \
        ...
        __device__ __forceinline__ operator bool() const { return this->m_id != RT_PROGRAM_ID_NULL; } \
    }
...
class callableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T)>
    : RT_INTERNAL_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T, Arg3T);
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T, typename Arg3T, typename Arg4T>
class callableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T, Arg4T)>
    : RT_INTERNAL_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T, Arg3T, Arg4T);
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T, typename Arg3T, typename Arg4T,
          typename Arg5T>
class callableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T)>
    : RT_INTERNAL_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T);
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T, typename Arg3T, typename Arg4T,
          typename Arg5T, typename Arg6T>
class callableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T, Arg6T)>
...
#define RT_INTERNAL_MARKED_CALLABLE_PROGRAM_DEFS                                                  \
   public:                                                                                        \
    ...
    __device__ __forceinline__ explicit markedCallableProgramId(callableProgramIdType callable,   \
                                                                const char* callSiteName)         \
...
#define RT_INTERNAL_MARKED_CALLABLE_PROGRAM_DEF_NO_ARG(ReturnT)                              \
    class markedCallableProgramId<ReturnT()>                                                 \
        : public rti_internal_callableprogram::markedCallableProgramIdBase<ReturnT> {        \
...
#define RT_INTERNAL_MARKED_CALLABLE_PROGRAM_DEF_W_ARGS(ReturnT, ...)                                      \
    class markedCallableProgramId<ReturnT(__VA_ARGS__)>                                                   \
        : public rti_internal_callableprogram::markedCallableProgramIdBase<ReturnT, __VA_ARGS__> {        \
...
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T>
class boundCallableProgramId<ReturnT(Arg0T, Arg1T, Arg2T)>
    : RT_INTERNAL_BOUND_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T);
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T, typename Arg3T>
class boundCallableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T)>
    : RT_INTERNAL_BOUND_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T, Arg3T);
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T, typename Arg3T, typename Arg4T>
class boundCallableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T, Arg4T)>
    : RT_INTERNAL_BOUND_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T, Arg3T, Arg4T);
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T, typename Arg3T, typename Arg4T,
          typename Arg5T>
class boundCallableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T)>
    : RT_INTERNAL_BOUND_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T);
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T, typename Arg3T, typename Arg4T,
          typename Arg5T, typename Arg6T>
class boundCallableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T, Arg6T)>
    : RT_INTERNAL_BOUND_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T, Arg6T);
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T, typename Arg3T, typename Arg4T,
          typename Arg5T, typename Arg6T, typename Arg7T>
class boundCallableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T, Arg6T, Arg7T)>
    : RT_INTERNAL_BOUND_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T, Arg6T, Arg7T);
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T, typename Arg3T, typename Arg4T,
          typename Arg5T, typename Arg6T, typename Arg7T, typename Arg8T>
class boundCallableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T, Arg6T, Arg7T, Arg8T)>
    : RT_INTERNAL_BOUND_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T, Arg6T, Arg7T, Arg8T);
template <typename ReturnT, typename Arg0T, typename Arg1T, typename Arg2T, typename Arg3T, typename Arg4T,
          typename Arg5T, typename Arg6T, typename Arg7T, typename Arg8T, typename Arg9T>
class boundCallableProgramId<ReturnT(Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T, Arg6T, Arg7T, Arg8T, Arg9T)>
    : RT_INTERNAL_BOUND_CALLABLE_PROGRAM_DEFS(ReturnT, Arg0T, Arg1T, Arg2T, Arg3T, Arg4T, Arg5T, Arg6T, Arg7T, Arg8T,
                                              Arg9T);
}  // end namespace optix
...
static inline __device__ void rtTrace(rtObject topNode, optix::Ray ray, float time, T& prd,
                                      RTvisibilitymask mask = RT_VISIBILITY_ALL, RTrayflags flags = RT_RAY_FLAG_NONE) {
    optix::rt_trace_with_time(*(unsigned int*)&topNode, ray.origin, ray.direction, ray.ray_type, ray.tmin, ray.tmax,
                              time, mask, flags, &prd, sizeof(T));
}
...
static inline __device__ void rtTrace(rtObject topNode, optix::Ray ray, T& prd,
                                      RTvisibilitymask mask = RT_VISIBILITY_ALL, RTrayflags flags = RT_RAY_FLAG_NONE) {
    optix::rt_trace(*(unsigned int*)&topNode, ray.origin, ray.direction, ray.ray_type, ray.tmin, ray.tmax, mask, flags,
                    &prd, sizeof(T));
}
 ...
 *
 */
static inline __device__ unsigned int rtGetEntryPointIndex() { return optix::rt_get_entry_point_index(); }
 ...
 */
static inline __device__ bool rtPotentialIntersection(float tmin) { return optix::rt_potential_intersection(tmin); }
 ...
 * @ref rtPotentialIntersection
 */
static inline __device__ void rtIgnoreIntersection() { optix::rt_ignore_intersection(); }
 ...
 * @ref rtPotentialIntersection
 */
static inline __device__ void rtTerminateRay() { optix::rt_terminate_ray(); }
 ...
 * @ref rtTerminateRay
 */
static inline __device__ void rtIntersectChild(unsigned int index) { optix::rt_intersect_child(index); }
 ...
 *
 */
static inline __device__ unsigned int rtGetPrimitiveIndex() { return optix::rt_get_primitive_index(); }
 ...
 *
 */
static inline __device__ bool rtIsTriangleHit() { return optix::rt_is_triangle_hit(); }
 ...
 *
 */
static inline __device__ bool rtIsTriangleHitBackFace() { return optix::rt_is_triangle_hit_back_face(); }
 ...
 * rtIsTriangleHitBackFace
 */
static inline __device__ bool rtIsTriangleHitFrontFace() { return optix::rt_is_triangle_hit_front_face(); }
 ...
 *
 */
static inline __device__ unsigned int rtGetRayFlags() { return optix::rt_get_ray_flags(); }
 ...
 *
 */
static inline __device__ unsigned int rtGetRayMask() { return optix::rt_get_ray_mask(); }
 ...
 *
 */
static inline __device__ void rtThrow(unsigned int code) { optix::rt_throw(code); }
 ...
 *
 */
static inline __device__ unsigned int rtGetExceptionCode() { return optix::rt_get_exception_code(); }
...
static inline __device__ void rtPrintExceptionDetails() {
    ...
    if (code == RT_EXCEPTION_STACK_OVERFLOW) {
        rtPrintf(
            ...
            "  launch index : %d, %d, %d\n",
            rti_internal_register::reg_rayIndex_x, rti_internal_register::reg_rayIndex_y,
        ...
        rtPrintf(
            ...
            rti_internal_register::reg_rayIndex_x, rti_internal_register::reg_rayIndex_y,
            rti_internal_register::reg_rayIndex_z, dim, (const char*)rti_internal_register::reg_exception_64_detail0,
            rti_internal_register::reg_exception_detail2, rti_internal_register::reg_exception_64_detail1,
            dim > 1 ? rti_internal_register::reg_exception_64_detail2 : 1,
            dim > 2 ? rti_internal_register::reg_exception_64_detail3 : 1, rti_internal_register::reg_exception_detail1,
            rti_internal_register::reg_exception_64_detail4, rti_internal_register::reg_exception_64_detail5,
        ...
        switch (rti_internal_register::reg_exception_detail1) {
            ...
            case 2:
                rtPrintf(
                    ...
                    "  launch index   : %d, %d, %d\n"
                    "  location       : %s\n",
                    rti_internal_register::reg_exception_detail0, rti_internal_register::reg_rayIndex_x,
                    rti_internal_register::reg_rayIndex_y, rti_internal_register::reg_rayIndex_z,
                    (const char*)rti_internal_register::reg_exception_64_detail0);
                break;
            case 3:
                rtPrintf(
        ...
        rtPrintf(
            ...
            "  accessed index : %lld\n",
            rti_internal_register::reg_rayIndex_x, rti_internal_register::reg_rayIndex_y,
            rti_internal_register::reg_rayIndex_z, (const char*)rti_internal_register::reg_exception_64_detail0,
            rti_internal_register::reg_exception_64_detail1, rti_internal_register::reg_exception_64_detail2);
        ...
        rtPrintf(
            ...
            __int_as_float(rti_internal_register::reg_exception_detail5), rti_internal_register::reg_exception_detail6,
        ...
        rtPrintf(
            ...
            "  launch index : %d, %d, %d\n",
            code - RT_EXCEPTION_USER, rti_internal_register::reg_rayIndex_x, rti_internal_register::reg_rayIndex_y,
            rti_internal_register::reg_rayIndex_z);
...
static inline __device__ float2 rtGetTriangleBarycentrics() { return optix::rt_get_triangle_barycentrics(); }
...
static inline __device__ unsigned int rtGetGroupChildIndex() { return optix::rt_get_lowest_group_child_index(); }
