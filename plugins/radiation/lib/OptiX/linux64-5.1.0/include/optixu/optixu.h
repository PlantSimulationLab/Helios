 ...
 * @retval RTresult  Return code
 */
RTresult RTAPI rtuCUDACompileString(const char* source, const char** preprocessorArguments,
 ...
 * @retval RTresult  Return code
 */
RTresult RTAPI rtuCUDACompileFile(const char* filename, const char** preprocessorArguments,
                                  unsigned int numPreprocessorArguments, RTsize* resultSize, RTsize* errorSize);
...
#endif
RTU_INLINE RTresult rtuGeometryGroupAddChild(RTgeometrygroup geometrygroup, RTgeometryinstance child,
                                             unsigned int* index);
/** @} */
...
RTU_INLINE RTresult rtuGeometryGroupGetChildIndex(RTgeometrygroup geometrygroup, RTgeometryinstance child,
                                                  unsigned int* index);
/** @} */
...
#define RTU_CHECK_ERROR(func)                \
    do {                                     \
        RTresult code = func;                \
        if (code != RT_SUCCESS) return code; \
    } while (0)
 ...
 *   \param    usePTX32InHost64 Use 32bit PTX bounding box and intersection programs in 64bit application. Takes effect
 * only with 64bit host. \param    mesh           Output geometry \param    num_verts      Vertex count \param    verts
 * Vertices (num_verts*float*3) [ v1_x, v1_y, v1_z, v2.x, ... ] \param    num_tris       Triangle count \param indices
 * Vertex indices (num_tris*unsigned*3) [ tri1_index1, tr1_index2, ... ] \param    mat_indices    Indices of materials
 * (num_tris*unsigned) [ tri1_mat_index, tri2_mat_index, ... ]
 */
RTresult RTAPI rtuCreateClusteredMesh(RTcontext context, unsigned int usePTX32InHost64, RTgeometry* mesh,
                                      unsigned int num_verts, const float* verts, unsigned int num_tris,
                                      const unsigned* indices, const unsigned* mat_indices);
 ...
 * Vertices (num_verts*float*3) [ v1_x, v1_y, v1_z, v2.x, ... ] \param    num_tris       Triangle count \param indices
 * Vertex indices (num_tris*unsigned*3) [ tri1_index1, tr1_index2, ... ] \param    mat_indices    Indices of materials
 * (num_tris*unsigned) [ tri1_mat_index, tri2_mat_index, ... ] \param    norms          Normals (num_norms*float*3) [
 * v1_x, v1_y, v1_z, v2.x, ... ] \param    norm_indices   Indices of vertex normals (num_tris*unsigned*3) [
 * tri1_norm_index1, tri1_norm_index2 ... ] \param    tex_coords     Texture uv coords (num_tex_coords*float*2) [ t1_u,
 * t1_v, t2_u ... ] \param    tex_indices    Indices of texture uv (num_tris*unsigned*3) [ tri1_tex_index1,
 * tri1_tex_index2 ... ]
 */
RTresult RTAPI rtuCreateClusteredMeshExt(RTcontext context, unsigned int usePTX32InHost64, RTgeometry* mesh,
                                         unsigned int num_verts, const float* verts, unsigned int num_tris,
                                         const unsigned* indices, const unsigned* mat_indices, RTbuffer norms,
                                         const unsigned* norm_indices, RTbuffer tex_coords,
