...
#include <cuda_runtime.h> /* for __inline__ */
...
struct Ray {
    ...
    /** Create a Ray copied from an exemplar */
    __inline__ RT_HOSTDEVICE Ray(const Ray &r)
        : origin(r.origin), direction(r.direction), ray_type(r.ray_type), tmin(r.tmin), tmax(r.tmax) {}

    /** Create a ray with a specified origin, direction, ray_type, and min/max extents. When tmax is not given, it
     * defaults to RT_DEFAULT_MAX. */
    __inline__ RT_HOSTDEVICE Ray(float3 origin_, float3 direction_, unsigned int ray_type_, float tmin_,
                                 float tmax_ = RT_DEFAULT_MAX)
...
static __inline__ RT_HOSTDEVICE Ray make_Ray(float3 origin, float3 direction, unsigned int ray_type, float tmin,
                                             float tmax) {
    Ray ray;
    ray.origin = origin;
