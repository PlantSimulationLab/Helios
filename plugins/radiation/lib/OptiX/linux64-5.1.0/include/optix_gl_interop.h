 ...
 *
 * These buffers can be used to share data with OpenGL; changes of the content in \a buffer, either done by OpenGL or
 * OptiX, will be reflected automatically in both APIs. If the size, or format, of an OpenGL buffer is changed,
 * appropriate OptiX calls have to be used to update \a buffer accordingly. OptiX keeps only a reference to OpenGL data,
 * when \a buffer is destroyed, the state of the \a gl_id object is unaltered.
 *
 ...
 *
 */
RTresult RTAPI rtTextureSamplerCreateFromGLImage(RTcontext context, unsigned int glId, RTgltarget target,
 ...
 * As long as a buffer object is unregistered, OptiX will not be able to access the data and calls will fail with @ref
 * RT_ERROR_INVALID_VALUE. Returns \a RT_ERROR_RESOURCE_NOT_REGISTERED if \a buffer is already unregistered. An OptiX
 * buffer in an unregistered state can be registered to OptiX again via @ref rtBufferGLRegister.
 *
 * @param[in]   buffer          The handle for the buffer object
 *
 * <B>Return values</B>
 ...
 * <B>Description</B>
 *
 * Registers an OpenGL texture as accessible by OptiX. Once registered, properties like the size of the original GL
 * texture cannot be modified anymore. Calls to the corresponding GL functions will return with an error code. However,
 * the pixel data of the GL texture can still be read and written by the appropriate GL commands. Returns @ref
 * RT_ERROR_RESOURCE_ALREADY_REGISTERED if \a textureSampler is already registered. A texture sampler must be registered
 * in order to be used by OptiX. Otherwise, @ref RT_ERROR_INVALID_VALUE is returned. An OptiX texture sampler in a
 * registered state can be unregistered via @ref rtTextureSamplerGLUnregister.
 *
 ...
 * As long as a texture is unregistered, OptiX will not be able to access the pixel data and calls will fail with @ref
 * RT_ERROR_INVALID_VALUE. Returns \a RT_ERROR_RESOURCE_NOT_REGISTERED if \a textureSampler is already unregistered. An
 * OptiX texture sampler in an unregistered state can be registered to OptiX again via @ref rtTextureSamplerGLRegister.
 *
 * @param[in]   textureSampler          The handle for the texture object
 *
 * <B>Return values</B>
 ...
 *
 * @param[out]   device          A handle to the memory location where the OptiX device ordinal associated with \a gpu
 * will be stored
