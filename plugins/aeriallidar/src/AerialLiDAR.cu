...
__host__ __device__ float3 AERIALLIDAR_CUDA::normalize(const float3& a) { return a / magnitude(a); }
...
void AerialLiDARcloud::calculateLeafAreaGPU(const float Gtheta) { calculateLeafAreaGPU(Gtheta, 1); }
...
void AerialLiDARcloud::calculateLeafAreaGPU(const float Gtheta, const int minVoxelHits) {
    ...
    std::vector<float>
        hit_denom_agg;  // hit_denom corresponds to total number of scan points that reached a given grid cell
    ...
    for (uint s = 0; s < Nscans; s++) {
        ...
        intersectGridcell<<<dimGrid, dimBlock>>>(Nhits, d_scan_xyz, d_scan_raydir, Ncells, d_cell_center, d_cell_anchor,
                                                 d_cell_size, d_cell_rotation, d_dr, d_dr_hit, d_hit_denom,
                                                 d_hit_inside);
    ...
    //     mean(exp(-a*dr*Gtheta))]" << std::endl;
    //     //std::cout << "Vol #" << v << " Leaf Area: " <<
...
void AerialLiDARcloud::syntheticScan(helios::Context* context, const char* xml_file, const int rays_per_pulse,
    ...
    float3 bb_center = make_float3(xbounds.x + 0.5 * (xbounds.y - xbounds.x), ybounds.x + 0.5 * (ybounds.y - ybounds.x),
    ...
    for (int s = 0; s < synthscan.getScanCount(); s++) {
        ...
        if (extent.x <= 0 || extent.y <= 0) {
            std::cerr << "ERROR (syntheticScan): The scan extent specified in the metadata file " << xml_file
                      << " must be greater than zero." << std::endl;
            exit(EXIT_FAILURE);
        } else if (scandensity <= 0) {
            std::cerr << "ERROR (syntheticScan): The scan density specified in the metadata file " << xml_file
                      << " must be greater than zero." << std::endl;
            exit(EXIT_FAILURE);
        }
        ...
        //---- patch kernel ----//
        intersectPatches<<<dimGrid, dimBlock>>>(N, Npulse, d_origin, d_raydir, exit_diameter, beam_divergence, Npatches,
                                                d_patch_vertex, d_patch_textureID, Ntextures, d_masksize, masksize_max,
                                                d_maskdata, d_patch_uv, d_hit_t, d_hit_fnorm);

        cudaDeviceSynchronize();
        CUDA_CHECK_ERROR(cudaPeekAtLastError());  // if there was an error inside the kernel, it will show up here

        //---- triangle kernel ----//
        intersectTriangles<<<dimGrid, dimBlock>>>(N, Npulse, d_origin, d_raydir, exit_diameter, beam_divergence,
                                                  Ntriangles, d_tri_vertex, d_tri_textureID, Ntextures, d_masksize,
                                                  masksize_max, d_maskdata, d_tri_uv, d_hit_t, d_hit_fnorm);
...
void AerialLiDARcloud::calculateSyntheticLeafArea(helios::Context* context) {
    ...
    // insideVolume <<< dimGrid, dimBlock >>>( N, d_prim_xyz, getGridCellCount(), d_grid_size, d_grid_center,
    // d_grid_anchor, d_grid_rotation, d_prim_vol );
...
__global__ void AERIALLIDAR_CUDA::intersectBoundingBox(const size_t scanSize, float3* d_hit_origin,
                                                       const float3* d_scan_xyz, const float3 bbcenter,
                                                       const float3 bbsize, uint* d_boundingbox_hit) {
...
__global__ void AERIALLIDAR_CUDA::intersectGridcell(const size_t Nhitsbb, float3* d_scan_xyz, float3* d_scan_raydir,
                                                    const size_t Ncells, float3* center, float3* anchor, float3* size,
                                                    float* rotation, float* d_dr, float* d_dr_hit, float* hit_denom,
                                                    float* hit_inside) {
    ...
    float oy = origin.y;
    float oz = origin.z;
    float dx = direction_rot.x;
    float dy = direction_rot.y;
    float dz = direction_rot.z;

    float x0 = center[cell].x - 0.5f * size[cell].x;
    float x1 = center[cell].x + 0.5f * size[cell].x;
    float y0 = center[cell].y - 0.5f * size[cell].y;
    float y1 = center[cell].y + 0.5f * size[cell].y;
    float z0 = center[cell].z - 0.5f * size[cell].z;
    float z1 = center[cell].z + 0.5f * size[cell].z;
...
__global__ void AERIALLIDAR_CUDA::intersectPatches(const size_t Npulse, const int rays_per_pulse, float3* d_origin,
                                                   float3* d_raydir, const float exit_diameter,
                                                   const float beam_divergence, const int Npatches,
                                                   float3* d_patch_vertex, int* d_patch_textureID, const int Ntextures,
                                                   int2* d_masksize, int2 masksize_max, bool* d_maskdata,
                                                   float2* d_patch_uv, float* d_hit_t, float* d_hit_fnorm) {
...
__global__ void AERIALLIDAR_CUDA::intersectTriangles(const size_t Npulse, const int rays_per_pulse, float3* d_origin,
                                                     float3* d_raydir, const float exit_diameter,
                                                     const float beam_divergence, const int Ntriangles,
                                                     float3* d_tri_vertex, int* d_tri_textureID, const int Ntextures,
                                                     int2* d_masksize, const int2 masksize_max, bool* d_maskdata,
                                                     float2* d_tri_uv, float* d_hit_t, float* d_hit_fnorm) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
