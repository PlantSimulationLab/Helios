...
AerialScanMetadata::AerialScanMetadata(const helios::vec3 __center, const helios::vec2 __extent,
                                       const float __coneangle, const float __scandensity, const float __exitDiameter,
                                       const float __beamDivergence) {
...
void AerialLiDARcloud::disableMessages(void) { printmessages = false; }

void AerialLiDARcloud::enableMessages(void) { printmessages = true; }

uint AerialLiDARcloud::getScanCount(void) { return scans.size(); }

void AerialLiDARcloud::addScan(const AerialScanMetadata newscan) { scans.push_back(newscan); }
...
void AerialLiDARcloud::addHitPoint(const uint scanID, const helios::vec3 hit_xyz,
...
void AerialLiDARcloud::addHitPoint(const uint scanID, const helios::vec3 hit_xyz,
                                   const helios::SphericalCoord direction, const std::map<std::string, float> data) {
    // default color
    RGBcolor color = make_RGBcolor(1, 0, 0);
...
void AerialLiDARcloud::addHitPoint(const uint scanID, const helios::vec3 hit_xyz,
                                   const helios::SphericalCoord direction, const helios::RGBcolor color) {
    // empty data
    std::map<std::string, float> data;
...
void AerialLiDARcloud::deleteHitPoint(const uint index) {
    if (index >= hits.size()) {
        cerr << "ERROR (deleteHitPoint): Hit point #" << index
...
uint AerialLiDARcloud::getHitCount(void) const { return hits.size(); }

helios::vec3 AerialLiDARcloud::getScanCenter(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanCenter): Cannot get center of scan #" << scanID << " because there have only been "
             << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
helios::vec2 AerialLiDARcloud::getScanExtent(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanExtent): Cannot get extent for scan #" << scanID << " because there have only been "
             << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
float AerialLiDARcloud::getScanConeAngle(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanConeAngle): Cannot get cone angle for scan #" << scanID
...
float AerialLiDARcloud::getScanDensity(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanDensity): Cannot get point density for scan #" << scanID
             << " because there have only been " << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
helios::int3 AerialLiDARcloud::getHitGridCell(const uint index) const {
    if (index >= hits.size()) {
        ...
        cerr << "WARNING (getHitGridCell): calculateHitGridCell() must be run prior to calling this function"
             << std::endl;
...
void AerialLiDARcloud::setHitGridCell(const uint index, const helios::int3 ijk) {
    ...
    if (ijk.x >= gridresolution.x) {
        cerr << "ERROR (setHitGridCell): Grid index in x-direction was specified as " << ijk.x
             << ", which is larger than the maximum value based on the grid dimension of " << gridresolution.x - 1
             << "." << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.y >= gridresolution.y) {
        cerr << "ERROR (setHitGridCell): Grid index in y-direction was specified as " << ijk.y
             << ", which is larger than the maximum value based on the grid dimension of " << gridresolution.y - 1
             << "." << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.z >= gridresolution.z) {
        cerr << "ERROR (setHitGridCell): Grid index in x-direction was specified as " << ijk.z
             << ", which is larger than the maximum value based on the grid dimension of " << gridresolution.z - 1
             << "." << std::endl;
        exit(EXIT_FAILURE);
    }
...
void AerialLiDARcloud::addHitsToVisualizer(Visualizer* visualizer, const uint pointsize,
                                           const char* color_value) const {
...
void AerialLiDARcloud::addGridToVisualizer(Visualizer* visualizer) const {
    ...
    for (uint i = 0; i < Ngrid; i++) {
        ...
        visualizer->addVoxelByCenter(center, gridsize, make_SphericalCoord(0, gridrotation), color,
...
helios::int3 AerialLiDARcloud::gridindex2ijk(const int index) const {
    if (index < 0 || index >= gridresolution.x * gridresolution.y * gridresolution.z) {
        ...
        int j = floor((double(index) - double(k) * double(gridresolution.x) * double(gridresolution.y)) /
                      double(gridresolution.y));
...
helios::vec3 AerialLiDARcloud::getGridCenter(void) const { return gridcenter; }

helios::vec3 AerialLiDARcloud::getGridExtent(void) const { return gridextent; }

helios::int3 AerialLiDARcloud::getGridResolution(void) const { return gridresolution; }

float AerialLiDARcloud::getGridRotation(void) const { return gridrotation; }
...
vec3 AerialLiDARcloud::getCellSize(void) const {
    return make_vec3(gridextent.x / float(gridresolution.x), gridextent.y / float(gridresolution.y),
                     gridextent.z / float(gridresolution.z));
}
...
void AerialLiDARcloud::setCellLeafArea(const float area, const int3 ijk) {
    if (ijk.x < 0 || ijk.x >= gridresolution.x) {
        cerr << "ERROR (setCellLeafArea): Cell index in x-direction of " << ijk.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.y < 0 || ijk.y >= gridresolution.y) {
        cerr << "ERROR (setCellLeafArea): Cell index in y-direction of " << ijk.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.z < 0 || ijk.z >= gridresolution.z) {
        cerr << "ERROR (setCellLeafArea): Cell index in z-direction of " << ijk.z
             << " is outside the allowable range of 0 to " << gridresolution.z - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
float AerialLiDARcloud::getCellLeafArea(const int3 ijk) const {
    if (leaf_area.size() != gridresolution.z) {
        cerr << "ERROR (getCellLeafArea): Leaf area calculations have not been run yet. You must first call the "
                "function calculateLeafAreaGPU()."
             << std::endl;
        exit(EXIT_FAILURE);
    }

    if (ijk.x < 0 || ijk.x >= gridresolution.x) {
        cerr << "ERROR (getCellLeafArea): Cell index in x-direction of " << ijk.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.y < 0 || ijk.y >= gridresolution.y) {
        cerr << "ERROR (getCellLeafArea): Cell index in y-direction of " << ijk.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.z < 0 || ijk.z >= gridresolution.z) {
        cerr << "ERROR (getCellLeafArea): Cell index in z-direction of " << ijk.z
             << " is outside the allowable range of 0 to " << gridresolution.z - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
float AerialLiDARcloud::getCellLeafAreaDensity(const int3 ijk) const {
    if (leaf_area.size() != gridresolution.z) {
        cerr << "ERROR (getCellLeafAreaDensity): Leaf area calculations have not been run yet. You must first call the "
                "function calculateLeafAreaGPU()."
             << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    if (ijk.x < 0 || ijk.x >= gridresolution.x) {
        cerr << "ERROR (getCellLeafAreaDensity): Cell index in x-direction of " << ijk.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.y < 0 || ijk.y >= gridresolution.y) {
        cerr << "ERROR (getCellLeafAreaDensity): Cell index in y-direction of " << ijk.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.z < 0 || ijk.z >= gridresolution.z) {
        cerr << "ERROR (getCellLeafAreaDensity): Cell index in z-direction of " << ijk.z
             << " is outside the allowable range of 0 to " << gridresolution.z - 1 << std::endl;
        exit(EXIT_FAILURE);
    }

    vec3 gridsize(gridextent.x / float(gridresolution.x), gridextent.y / float(gridresolution.y),
                  gridextent.z / float(gridresolution.z));
...
void AerialLiDARcloud::setCellTransmissionProbability(const int P_denom, const int P_trans, const int3 ijk) {
    if (ijk.x < 0 || ijk.x >= gridresolution.x) {
        cerr << "ERROR (setCellTransmissionProbability): Cell index in x-direction of " << ijk.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.y < 0 || ijk.y >= gridresolution.y) {
        cerr << "ERROR (setCellTransmissionProbability): Cell index in y-direction of " << ijk.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.z < 0 || ijk.z >= gridresolution.z) {
        cerr << "ERROR (setCellTransmissionProbability): Cell index in z-direction of " << ijk.z
             << " is outside the allowable range of 0 to " << gridresolution.z - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
void AerialLiDARcloud::getCellTransmissionProbability(const int3 ijk, int& P_denom, int& P_trans) const {
    if (P_counts.size() != gridresolution.z) {
        cerr << "ERROR (getCellTransmissionProbability): Leaf area calculations have not been run yet. You must first "
                "call the function calculateLeafAreaGPU()."
             << std::endl;
        exit(EXIT_FAILURE);
    }

    if (ijk.x < 0 || ijk.x >= gridresolution.x) {
        cerr << "ERROR (getCellTransmissionProbabilit): Cell index in x-direction of " << ijk.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.y < 0 || ijk.y >= gridresolution.y) {
        cerr << "ERROR (getCellTransmissionProbabilit): Cell index in y-direction of " << ijk.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.z < 0 || ijk.z >= gridresolution.z) {
        cerr << "ERROR (getCellTransmissionProbabilit): Cell index in z-direction of " << ijk.z
             << " is outside the allowable range of 0 to " << gridresolution.z - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
void AerialLiDARcloud::setCellRbar(const float rbar, const int3 ijk) {
    if (ijk.x < 0 || ijk.x >= gridresolution.x) {
        cerr << "ERROR (setCellRbar): Cell index in x-direction of " << ijk.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.y < 0 || ijk.y >= gridresolution.y) {
        cerr << "ERROR (setCellRbar): Cell index in y-direction of " << ijk.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.z < 0 || ijk.z >= gridresolution.z) {
        cerr << "ERROR (setCellRbar): Cell index in z-direction of " << ijk.z
             << " is outside the allowable range of 0 to " << gridresolution.z - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
float AerialLiDARcloud::getCellRbar(const int3 ijk) const {
    if (r_bar.size() != gridresolution.z) {
        cerr << "ERROR (getCellRbar): Leaf area calculations have not been run yet. You must first call the function "
                "calculateLeafAreaGPU()."
             << std::endl;
        exit(EXIT_FAILURE);
    }

    if (ijk.x < 0 || ijk.x >= gridresolution.x) {
        cerr << "ERROR (getCellRbar): Cell index in x-direction of " << ijk.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.y < 0 || ijk.y >= gridresolution.y) {
        cerr << "ERROR (getCellRbar): Cell index in y-direction of " << ijk.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ijk.z < 0 || ijk.z >= gridresolution.z) {
        cerr << "ERROR (getCellRbar): Cell index in z-direction of " << ijk.z
             << " is outside the allowable range of 0 to " << gridresolution.z - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
helios::vec4 AerialLiDARcloud::RANSAC(const int maxIter, const float threshDist, const float inlierRatio,
                                      const std::vector<helios::vec3>& hits, std::vector<bool>& inliers) {
    int N = hits.size();
    ...
    for (int iter = 0; iter < maxIter; iter++) {
        ...
        for (int p = 0; p < N; p++) {
            if (p != i0 && p != i1 && p != i2) {
                ...
                float zerr =
                    fabs(zplane - point.z);  // NOTE: since we know the plane will be somewhat close to horizontal,
                                             // we're only using the vertical distance to calculate the error of fit.
...
void AerialLiDARcloud::generateHeightModel(const int maxIter, const float threshDist_ground,
                                           const float inlierRatio_ground, const float threshDist_vegetation,
                                           const float inlierRatio_vegetation) {
    if (printmessages) {
    ...
    std::vector<std::vector<std::vector<int> > > first_hits_ind;  // first index are hit indices, second index is grid
                                                                  // cell in x-dir, third index is grid cell in y-dir
    std::vector<std::vector<std::vector<int> > > last_hits_ind;  // first index are hit indices, second index is grid
                                                                 // cell in x-dir, third index is grid cell in y-dir
    ...
    for (int j = 0; j < gsize3.y; j++) {
        ...
        for (int i = 0; i < gsize3.x; i++) {
            ...
            if (last_hits_ind.at(j).at(i).size() > 3) {
                ...
                helios::vec4 groundModel =
                    RANSAC(maxIter, threshDist_ground, inlierRatio_ground, last_hits_xyz, ground_inliers);
            ...
            if (first_hits_ind.at(j).at(i).size() >= 3) {
                ...
                helios::vec4 canopyModel =
                    RANSAC(maxIter, threshDist_vegetation, inlierRatio_vegetation, first_hits_xyz, canopy_inliers);
    ...
    for (int j = 0; j < gsize3.y; j++) {
        ground_height_filled.at(j).insert(ground_height_filled.at(j).begin(), ground_height_xy.at(j).begin(),
                                          ground_height_xy.at(j).end());
    }
...
void AerialLiDARcloud::setCellVegetationHeight(const float height, const int2 ij) {
    if (ij.x < 0 || ij.x >= gridresolution.x) {
        cerr << "ERROR (setCellVegetationHeight): Cell index in x-direction of " << ij.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ij.y < 0 || ij.y >= gridresolution.y) {
        cerr << "ERROR (setCellVegetationHeight): Cell index in y-direction of " << ij.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
float AerialLiDARcloud::getCellVegetationHeight(const int2 ij) const {
    if (vegetation_height.size() != gridresolution.y) {
        cerr << "ERROR (getCellVegetationHeight): Height calculations have not been run yet. You must first call the "
                "function generateHeightModel()."
             << std::endl;
        exit(EXIT_FAILURE);
    }

    if (ij.x < 0 || ij.x >= gridresolution.x) {
        cerr << "ERROR (getCellVegetationHeight): Cell index in x-direction of " << ij.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ij.y < 0 || ij.y >= gridresolution.y) {
        cerr << "ERROR (getCellVegetationHeight): Cell index in y-direction of " << ij.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
void AerialLiDARcloud::setCellMaximumHitHeight(const float height, const int2 ij) {
    if (ij.x < 0 || ij.x >= gridresolution.x) {
        cerr << "ERROR (setCellMaximumHitHeight): Cell index in x-direction of " << ij.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ij.y < 0 || ij.y >= gridresolution.y) {
        cerr << "ERROR (setCellMaximumHitHeight): Cell index in y-direction of " << ij.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
float AerialLiDARcloud::getCellMaximumHitHeight(const int2 ij) const {
    if (maxhit_height.size() != gridresolution.y) {
        cerr << "ERROR (getCellMaximumHitHeight): Height calculations have not been run yet. You must first call the "
                "function generateHeightModel()."
             << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    if (ij.x < 0 || ij.x >= gridresolution.x) {
        cerr << "ERROR (getCellMaximumHitHeight): Cell index in x-direction of " << ij.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ij.y < 0 || ij.y >= gridresolution.y) {
        cerr << "ERROR (getCellMaximumHitHeight): Cell index in y-direction of " << ij.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
void AerialLiDARcloud::setCellGroundHeight(const float height, const int2 ij) {
    if (ij.x < 0 || ij.x >= gridresolution.x) {
        cerr << "ERROR (setCellGroundHeight): Cell index in x-direction of " << ij.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ij.y < 0 || ij.y >= gridresolution.y) {
        cerr << "ERROR (setCellGroundHeight): Cell index in y-direction of " << ij.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
float AerialLiDARcloud::getCellGroundHeight(const int2 ij) const {
    if (ground_height.size() != gridresolution.y) {
        cerr << "ERROR (getCellGroundHeight): Height calculations have not been run yet. You must first call the "
                "function generateHeightModel()."
             << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    if (ij.x < 0 || ij.x >= gridresolution.x) {
        cerr << "ERROR (getCellGroundHeight): Cell index in x-direction of " << ij.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ij.y < 0 || ij.y >= gridresolution.y) {
        cerr << "ERROR (getCellGroundHeight): Cell index in y-direction of " << ij.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
void AerialLiDARcloud::setCellCoverFraction(const float fcover, const int2 ij) {
    if (ij.x < 0 || ij.x >= gridresolution.x) {
        cerr << "ERROR (setCellCoverFraction): Cell index in x-direction of " << ij.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ij.y < 0 || ij.y >= gridresolution.y) {
        cerr << "ERROR (setCellCoverFraction): Cell index in y-direction of " << ij.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
...
float AerialLiDARcloud::getCellCoverFraction(const int2 ij) const {
    if (cover_fraction.size() != gridresolution.y) {
        cerr << "ERROR (getCellCoverFraction): Cover fraction calculations have not been run yet. You must first call "
                "the function calculateCoverFraction()."
             << std::endl;
        exit(EXIT_FAILURE);
    }

    if (ij.x < 0 || ij.x >= gridresolution.x) {
        cerr << "ERROR (getCellCoverFraction): Cell index in x-direction of " << ij.x
             << " is outside the allowable range of 0 to " << gridresolution.x - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
    if (ij.y < 0 || ij.y >= gridresolution.y) {
        cerr << "ERROR (getCellCoverFraction): Cell index in y-direction of " << ij.y
             << " is outside the allowable range of 0 to " << gridresolution.y - 1 << std::endl;
        exit(EXIT_FAILURE);
    }
