...
void AerialLiDARcloud::loadXML(const char* filename) {
    ...
    for (pugi::xml_node s = helios.child("scan"); s; s = s.next_sibling("scan")) {
        ...
        if (strlen(center_str) == 0) {
            ...
            center = string2vec3(
                center_str);  // note: pugi loads xml data as a character.  need to separate it into 3 floats
        }
        ...
        if (strlen(extent_str) == 0) {
            ...
            extent = string2vec2(
                extent_str);  // note: pugi loads xml data as a character.  need to separate it into 2 floats
        }
        if (extent.x < 0 || extent.y < 0) {
            cerr << "failed.\nERROR (loadXML): The scan extent must be positive (check scan #" << scan_count << ")."
                 << endl;
            exit(EXIT_FAILURE);
        }
