...
#define CUDA_CHECK_ERROR(ans)                                                                                                                                                                                                                            \
    ...
    */
    ...
    __global__ void intersectGridcell(const size_t Nhitsbb, float3 *d_scan_xyz, float3 *d_scan_raydir, const size_t Ncells, float3 *center, float3 *anchor, float3 *size, float *rotation, float *d_dr, float *d_dr_hit, float *hit_denom,
                                      float *hit_inside);

    __global__ void intersectPatches(const size_t Npulse, const int rays_per_pulse, float3 *d_origin, float3 *d_raydir, const float exit_diameter, const float beam_divergence, const int Npatches, float3 *d_patch_vertex, int *d_patch_textureID,
                                     const int Ntextures, int2 *d_masksize, const int2 masksize_max, bool *d_maskdata, float2 *d_patch_uv, float *d_hit_t, float *d_hit_fnorm);

    __global__ void intersectTriangles(const size_t Npulse, const int rays_per_pulse, float3 *d_origin, float3 *d_raydir, const float exit_diameter, const float beam_divergence, const int Ntriangles, float3 *d_tri_vertex, int *d_tri_textureID,
                                       const int Ntextures, int2 *d_masksize, const int2 masksize_max, bool *d_maskdata, float2 *d_tri_uv, float *d_hit_t, float *d_hit_fnorm);
