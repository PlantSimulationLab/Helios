...
struct AerialScanMetadata {
    ...
    */
    AerialScanMetadata(const helios::vec3 __center, const helios::vec2 __extent, const float __coneangle,
                       const float __scandensity, const float __exitDiameter, const float __beamDivergence);
...
struct AerialHitPoint {
    ...
    AerialHitPoint(int __scanID, helios::vec3 __position, helios::SphericalCoord __direction, helios::int2 __row_column,
                   helios::RGBcolor __color, std::map<std::string, float> __data) {
        scanID = __scanID;
        position = __position;
...
class AerialLiDARcloud {
   private:
    //! Use RANSAC algorithm to separate a set of hit points into outliers and inliers based on proximity to a best-fit
    //! plane
    /** \param[in] "maxIter" Maximum number of iterations to find best fit plane.
        ...
        \param[in] "inlierRatio" Minimum fraction of total points that must be inliers to consider the fitted plane
       valid. \param[in] "hits" Vector of (x,y,z) positions for hit points. \param[out] "inliers" Vector of flags
       denoting points as inliers or outliers (value=false means an outlier, value=true means an inlier). \return Best
       fit plane to the set of inliers. The vec4 contains the four coefficients of the fitted plane equation
       Ax+By+Cz+D=0. (x=A, y=B, z=C, w=D)
    */
    helios::vec4 RANSAC(const int maxIter, const float threshDist, const float inlierRatio,
    ...
    */
    void addHitPoint(const uint scanID, const helios::vec3 hit_xyz, const helios::SphericalCoord direction,
                     const helios::RGBcolor color, const std::map<std::string, float> data);
   ...
   */
    ...
    */
    ...
    //! Use a global index of a grid cell (ranging from 0 to Ncells-1) to retrieve the local index in the x-, y-, and
    //! z-directions
    /** \param[in] "index" Global grid cell index - ranges from 0 to Ncells-1 */
    helios::int3 gridindex2ijk(const int index) const;

    //! Use a grid cell's local index in the x-, y-, and z-directions to retrieve the cells' global index (ranging from
    //! 0 to Ncells-1)
    /** \param [in] "ijk" Index of a grid cell in the x-, y-, and z-directions. */
    ...
    */
    void syntheticScan(helios::Context* context, const char* xml_file, const int rays_per_pulse,
    ...
    */
    ...
    //! Calculate the leaf area for each grid volume
    /** \param[in] "Gtheta" G-function value to be assumed constant across all cells.
        \param [in] "minVoxelHits" Minimum number of allowable LiDAR hits per voxel. If the total number of hits in a
       voxel is less than minVoxelHits, the calculated leaf area will be set to zero. */
    void calculateLeafAreaGPU(const float Gtheta, const int minVoxelHits);

    // -------- HEIGHT MODEL -------- //

    //! Determine the ground and vegetation height for each x-y grid cell. Inputs to this function are parameters for
    //! applying the RANSAC algorithm.
    /** \param[in] "maxIter" Maximum number of iterations to find best fit plane.
        \param[in] "threshDist_ground" Maximum distance from fitted plane to be considered an inlier - for ground
  surface model. \param[in] "inlierRatio_ground" Minimum fraction of total points that must be inliers to consider the
  fitted plane valid - for ground surface model. \param[in] "threshDist_vegetation" Maximum distance from fitted plane
  to be considered an inlier - for vegetation height model. \param[in] "inlierRatio_vegetation" Minimum fraction of
  total points that must be inliers to consider the fitted plane valid - for vegetation height model.
    */
    void generateHeightModel(const int maxIter, const float threshDist_ground, const float inlierRatio_ground,
                             const float threshDist_vegetation, const float inlierRatio_vegetation);
