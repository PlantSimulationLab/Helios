...
/** A scan is initialized by providing 1) the origin of the scan, 2) the number of zenithal scan directions, 3) the range of zenithal scan angles, 4) the number of azimuthal scan directions, and 5) the range of azimuthal scan angles. This creates a
 * grid of Ntheta x Nphi scan points which are all initialized as misses.  Points are set as hits using the addHitPoint() function. There are various functions to query the scan data.
 */
struct AerialScanMetadata {
     ...
     */
    AerialScanMetadata(const helios::vec3 &center, const helios::vec2 &extent, float coneangle, float scandensity, float exitDiameter, float beamDivergence);
...
private:
    ...
    helios::vec2 rotatePoint2D(const helios::vec2 &point, float theta) const;
