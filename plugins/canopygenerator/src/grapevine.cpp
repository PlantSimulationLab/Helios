...
void CanopyGenerator::grapevineVSP(const VSPGrapevineParameters &params, const vec3 &origin) {
    ...
    pos_cordw.push_back(make_vec3(0.01f * 0.5f * params.plant_spacing * cost,
                                  0.01f * 0.5f * params.plant_spacing * sint, 0.95 * params.trunk_height));
    vec3 n_start = sphere2cart(make_SphericalCoord(
        params.cordon_height, 0.4f * M_PI * (1 + getVariation(0.2, generator)), getVariation(0.2f * M_PI, generator)));
    vec3 n_end = make_vec3(0.5f * params.cordon_height, 0, 0);

    for (int i = 1; i < Ncord; i++) {
        float frac = float(i) / float(Ncord - 1);
        vec3 n = spline_interp3(frac, pos_cordw.front(), n_start,
                                make_vec3(0.5f * params.plant_spacing * cost, 0.5f * params.plant_spacing * sint,
                                          params.trunk_height + diff),
                                n_end);
    ...
    pos_corde.push_back(make_vec3(-0.01f * 0.5f * params.plant_spacing * cost,
                                  -0.01f * 0.5f * params.plant_spacing * sint, 0.95f * params.trunk_height));
    n_start = sphere2cart(make_SphericalCoord(params.cordon_height, 0.4f * M_PI * (1 + getVariation(0.2, generator)),
                                              M_PI + getVariation(0.2 * M_PI, generator)));
    n_end = make_vec3(-0.5f * params.cordon_height, 0, 0);

    for (int i = 1; i < Ncord; i++) {
        float frac = float(i) / float(Ncord - 1);
        vec3 n = spline_interp3(frac, pos_corde.front(), n_start,
                                make_vec3(-0.5f * params.plant_spacing * cost, -0.5f * params.plant_spacing * sint,
                                          params.trunk_height + diff),
                                n_end);
    ...
    // uint ID0 = context->addTileObject( make_vec3(0,0,0), make_vec2(1,1), make_SphericalCoord(0,0),
    ...
    for (uint c = 0; c < 2; c++) {
        ...
        for (int j = 1; j < params.shoots_per_cordon + 1; j++) {
            ...
            vec3 n_start =
                sphere2cart(make_SphericalCoord(0.3f * height, 0.5f * M_PI * (1 - unif_distribution(generator)),
                                                2.f * M_PI * unif_distribution(generator)));
            // tangent vector for middle of shoot
            vec3 n_mid =
                sphere2cart(make_SphericalCoord(0.3f * height, 0.5f * M_PI * (1 - unif_distribution(generator) * 0.15),
                                                2.f * M_PI * unif_distribution(generator)));
            // tangent vector for end of shoot
            vec3 n_end =
                sphere2cart(make_SphericalCoord(0.3f * height, 0.5f * M_PI * (1 - unif_distribution(generator) * 0.5),
                                                2.f * M_PI * unif_distribution(generator)));
            ...
            vec3 cane_mid =
                cane_base + make_vec3(0.1f * getVariation(params.plant_spacing, generator),
                                      0.1f * getVariation(params.plant_spacing, generator), 0.75f * Nz * dz);
            // position of end of shoot
            vec3 cane_end = cane_base + make_vec3(0.15f * getVariation(params.plant_spacing, generator),
                                                  0.15f * getVariation(params.plant_spacing, generator), Nz * dz);
            ...
            if (params.grape_radius > 0 && params.cluster_radius > 0) {
                ...
                vec3 offset(sgn * (0.25f * params.cluster_radius + getVariation(0.1, generator)) * sint,
                            sgn * (0.1f * params.cluster_radius + getVariation(0.1, generator)) * cost, 0.f);
                UUID_grapes = addGrapeCluster(p_grape + offset, params.grape_radius, params.cluster_radius,
                                              params.grape_color, params.grape_subdivisions);
            }
            ...
            while (lfrac > 0.75 * params.leaf_width) {
                ...
                vec3 leaf_offset = rotatePointAboutLine(
                    make_vec3(getVariation(0.1f * lsize, generator), getVariation(0.75f * lsize, generator), 0),
                    make_vec3(0, 0, 0), parent_normal, flip * M_PI + getVariation(0.25 * M_PI, generator));
...
void CanopyGenerator::grapevineSplit(const SplitGrapevineParameters &params, const vec3 &origin) {
    ...
    pos_crownw.push_back(make_vec3(0.05f * 0.5f * params.cordon_spacing * cost,
                                   0.05f * 0.5f * params.cordon_spacing * sint, params.trunk_height));
    pos_crownw.push_back(make_vec3(0.25f * 0.5f * params.cordon_spacing * cost,
                                   0.25f * 0.5f * params.cordon_spacing * sint, params.trunk_height + 0.1f * diff));
    pos_crownw.push_back(make_vec3(0.45f * 0.5f * params.cordon_spacing * cost,
                                   0.45f * 0.5f * params.cordon_spacing * sint, params.trunk_height + 0.65f * diff));
    pos_crownw.push_back(make_vec3(0.75f * 0.5f * params.cordon_spacing * cost,
                                   0.75f * 0.5f * params.cordon_spacing * sint, params.cordon_height));
    ...
    pos_crowne.push_back(make_vec3(-0.05f * 0.5f * params.cordon_spacing * cost,
                                   -0.05f * 0.5f * params.cordon_spacing * sint, params.trunk_height));
    pos_crowne.push_back(make_vec3(-0.25f * 0.5f * params.cordon_spacing * cost,
                                   -0.25f * 0.5f * params.cordon_spacing * sint, params.trunk_height + 0.1f * diff));
    pos_crowne.push_back(make_vec3(-0.45f * 0.5f * params.cordon_spacing * cost,
                                   -0.45f * 0.5f * params.cordon_spacing * sint, params.trunk_height + 0.65f * diff));
    pos_crowne.push_back(make_vec3(-0.75f * 0.5f * params.cordon_spacing * cost,
                                   -0.75f * 0.5f * params.cordon_spacing * sint, params.cordon_height));
    ...
    pos_cordnw.push_back(make_vec3(0.7f * 0.5f * params.cordon_spacing * cost,
                                   0.7f * 0.5f * params.cordon_spacing * sint, 0.99f * params.cordon_height));
    pos_cordnw.push_back(make_vec3(0.85f * 0.5f * params.cordon_spacing * cost + 0.025 * sint,
                                   0.85f * 0.5f * params.cordon_spacing * sint + 0.025f * cost, params.cordon_height));
    pos_cordnw.push_back(make_vec3(0.95f * 0.5f * params.cordon_spacing * cost + 0.075 * sint,
                                   0.95f * 0.5f * params.cordon_spacing * sint + 0.075f * cost, params.cordon_height));
    pos_cordnw.push_back(make_vec3(0.5f * params.cordon_spacing * cost + 0.12f * sint,
                                   0.5f * params.cordon_spacing * sint + 0.12f * cost, params.cordon_height));
    pos_cordnw.push_back(make_vec3(0.5f * params.cordon_spacing * cost + 0.4f * 0.5f * params.plant_spacing * sint,
                                   0.5f * params.cordon_spacing * sint + 0.4f * 0.5f * params.plant_spacing * cost,
                                   0.94f * params.cordon_height));
    pos_cordnw.push_back(make_vec3(0.5f * params.cordon_spacing * cost + 0.8f * 0.5f * params.plant_spacing * sint,
                                   0.5f * params.cordon_spacing * sint + 0.8f * 0.5f * params.plant_spacing * cost,
                                   0.97f * params.cordon_height));
    pos_cordnw.push_back(make_vec3(0.5f * params.cordon_spacing * cost + 0.5f * params.plant_spacing * sint,
                                   0.5f * params.cordon_spacing * sint + 0.5f * params.plant_spacing * cost,
                                   params.cordon_height));
    ...
    pos_cordsw.push_back(make_vec3(0.7f * 0.5f * params.cordon_spacing * cost,
                                   0.7f * 0.5f * params.cordon_spacing * sint, 0.99f * params.cordon_height));
    pos_cordsw.push_back(make_vec3(0.85f * 0.5f * params.cordon_spacing * cost - 0.025 * sint,
                                   0.85f * 0.5f * params.cordon_spacing * sint - 0.025f * cost, params.cordon_height));
    pos_cordsw.push_back(make_vec3(0.95f * 0.5f * params.cordon_spacing * cost - 0.075 * sint,
                                   0.95f * 0.5f * params.cordon_spacing * sint - 0.075f * cost, params.cordon_height));
    pos_cordsw.push_back(make_vec3(0.5f * params.cordon_spacing * cost - 0.12f * sint,
                                   0.5f * params.cordon_spacing * sint - 0.12f * cost, params.cordon_height));
    pos_cordsw.push_back(make_vec3(0.5f * params.cordon_spacing * cost - 0.4f * 0.5f * params.plant_spacing * sint,
                                   0.5f * params.cordon_spacing * sint - 0.4f * 0.5f * params.plant_spacing * cost,
                                   0.94f * params.cordon_height));
    pos_cordsw.push_back(make_vec3(0.5f * params.cordon_spacing * cost - 0.8f * 0.5f * params.plant_spacing * sint,
                                   0.5f * params.cordon_spacing * sint - 0.8f * 0.5f * params.plant_spacing * cost,
                                   0.97f * params.cordon_height));
    pos_cordsw.push_back(make_vec3(0.5f * params.cordon_spacing * cost - 0.5f * params.plant_spacing * sint,
                                   0.5f * params.cordon_spacing * sint - 0.5f * params.plant_spacing * cost,
                                   params.cordon_height));
    ...
    pos_cordne.push_back(make_vec3(-0.7f * 0.5f * params.cordon_spacing * cost,
                                   -0.7f * 0.5f * params.cordon_spacing * sint, 0.99f * params.cordon_height));
    pos_cordne.push_back(make_vec3(-0.85f * 0.5f * params.cordon_spacing * cost + 0.025f * sint,
                                   -0.85f * 0.5f * params.cordon_spacing * sint + 0.025f * cost, params.cordon_height));
    pos_cordne.push_back(make_vec3(-0.95f * 0.5f * params.cordon_spacing * cost + 0.075f * sint,
                                   -0.95f * 0.5f * params.cordon_spacing * sint + 0.075f * cost, params.cordon_height));
    pos_cordne.push_back(make_vec3(-0.5f * params.cordon_spacing * cost + 0.12f * sint,
                                   -0.5f * params.cordon_spacing * sint + 0.12f * cost, params.cordon_height));
    pos_cordne.push_back(make_vec3(-0.5f * params.cordon_spacing * cost + 0.4f * 0.5f * params.plant_spacing * sint,
                                   -0.5f * params.cordon_spacing * sint + 0.4f * 0.5f * params.plant_spacing * cost,
                                   0.94f * params.cordon_height));
    pos_cordne.push_back(make_vec3(-0.5f * params.cordon_spacing * cost + 0.8f * 0.5f * params.plant_spacing * sint,
                                   -0.5f * params.cordon_spacing * sint + 0.8f * 0.5f * params.plant_spacing * cost,
                                   0.97f * params.cordon_height));
    pos_cordne.push_back(make_vec3(-0.5f * params.cordon_spacing * cost + 0.5f * params.plant_spacing * sint,
                                   -0.5f * params.cordon_spacing * sint + 0.5f * params.plant_spacing * cost,
                                   params.cordon_height));
    ...
    pos_cordse.push_back(make_vec3(-0.7f * 0.5f * params.cordon_spacing * cost,
                                   -0.7f * 0.5f * params.cordon_spacing * sint, 0.99f * params.cordon_height));
    pos_cordse.push_back(make_vec3(-0.85f * 0.5f * params.cordon_spacing * cost - 0.025f * sint,
                                   -0.85f * 0.5f * params.cordon_spacing * sint - 0.025f * cost, params.cordon_height));
    pos_cordse.push_back(make_vec3(-0.95f * 0.5f * params.cordon_spacing * cost - 0.075f * sint,
                                   -0.95f * 0.5f * params.cordon_spacing * sint - 0.075f * cost, params.cordon_height));
    pos_cordse.push_back(make_vec3(-0.5f * params.cordon_spacing * cost - 0.12f * sint,
                                   -0.5 * params.cordon_spacing * sint - 0.12f * cost, params.cordon_height));
    pos_cordse.push_back(make_vec3(-0.5f * params.cordon_spacing * cost - 0.4f * 0.5f * params.plant_spacing * sint,
                                   -0.5f * params.cordon_spacing * sint - 0.4f * 0.5f * params.plant_spacing * cost,
                                   0.94f * params.cordon_height));
    pos_cordse.push_back(make_vec3(0.5f * params.cordon_spacing * cost - 0.8f * 0.5f * params.plant_spacing * sint,
                                   -0.5f * params.cordon_spacing * sint - 0.8f * 0.5f * params.plant_spacing * cost,
                                   0.97f * params.cordon_height));
    pos_cordse.push_back(make_vec3(0.5f * params.cordon_spacing * cost - 0.5f * params.plant_spacing * sint,
                                   -0.5f * params.cordon_spacing * sint - 0.5f * params.plant_spacing * cost,
                                   params.cordon_height));
    ...
    uint ID0 = context->addTileObject(make_vec3(0, 0, 0), make_vec2(1, 1), make_SphericalCoord(0, M_PI),
                                      params.leaf_subdivisions, params.leaf_texture_file.c_str());
    ...
    for (uint d = 0; d < 2; d++) {      // cordons
        for (uint c = 0; c < 2; c++) {  // shoot bend direction
            ...
            for (int j = 1; j < params.shoots_per_cordon + 1; j++) {
                ...
                bool inside = false;
                float phirot =
                    0.5f * M_PI * (1 + (-0.5f + unif_distribution(generator)) * 1.0) + params.canopy_rotation;
                if (unif_distribution(generator) < 0.5) {
                    phirot += M_PI;
                    if (c == 0) {
                        inside = true;
                ...
                if (inside) {
                    theta0 =
                        0.5f * M_PI * unif_distribution(generator);  //*(1.f+(-0.5+unif_distribution(generator))*0.6);
                } else {
                ...
                float dz = ((1 + getVariation(0.1, generator)) * height - params.cordon_height) / float(Nz);
                for (uint k = 1; k < Nz; k++) {
                    vec3 n = rotatePoint(make_vec3(0, 0, dz),
                                         (theta0 + (theta_end - theta0) * float(k) / float(Nz - 1)), phirot);

                    pos_pshoot.push_back(pos_pshoot.back() + n +
                                         make_vec3(getVariation(0.02, generator), getVariation(0.01, generator), 0));
                ...
                if (params.grape_radius > 0 && params.cluster_radius > 0) {
                    ...
                    vec3 offset(sgn * (2.2 * params.cluster_radius + getVariation(0.1, generator)) * sint,
                                sgn * (2 * params.cluster_radius + getVariation(0.1, generator)) * cost, 0.f);
                ...
                while (lfrac > 0.5 * params.leaf_width) {
                    ...
                    parent_normal.normalize();
                    vec3 leaf_offset = rotatePointAboutLine(
                        make_vec3(0, lsize * (0.3 + getVariation(0.25, generator)), 0), make_vec3(0, 0, 0),
                        parent_normal, flip * M_PI + unif_distribution(generator) * 0.25 * M_PI);
...
void CanopyGenerator::grapevineUnilateral(const UnilateralGrapevineParameters &params, const vec3 &origin) {
    ...
    std::vector<vec3> pos_main;
    pos_main.push_back(make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * cost,
                                 (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * sint, 0.0));
    pos_main.push_back(make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * cost,
                                 (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * sint,
                                 0.2f * params.trunk_height));
    pos_main.push_back(make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * cost,
                                 (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * sint,
                                 0.22f * params.trunk_height));
    pos_main.push_back(make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * cost,
                                 (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * sint,
                                 0.6f * params.trunk_height));
    pos_main.push_back(make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * cost,
                                 (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * sint,
                                 0.96f * params.trunk_height));
    pos_main.push_back(make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * cost,
                                 (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius) * sint,
                                 params.trunk_height));
    ...
    std::vector<vec3> pos_cord;
    pos_cord.push_back(
        make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.01f * params.plant_spacing) * cost,
                  (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.01f * params.plant_spacing) * sint,
                  0.95 * params.trunk_height));
    pos_cord.push_back(
        make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.05f * params.plant_spacing) * cost,
                  (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.05f * params.plant_spacing) * sint,
                  params.trunk_height + 0.1f * diff));
    pos_cord.push_back(
        make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.15f * params.plant_spacing) * cost,
                  (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.15f * params.plant_spacing) * sint,
                  params.trunk_height + 0.65f * diff));
    pos_cord.push_back(
        make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.45f * params.plant_spacing) * cost,
                  (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.45f * params.plant_spacing) * sint,
                  params.trunk_height + 0.95f * diff));
    pos_cord.push_back(
        make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.6f * params.plant_spacing) * cost,
                  (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.6f * params.plant_spacing) * sint,
                  params.trunk_height + 1.05f * diff));
    pos_cord.push_back(
        make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.85f * params.plant_spacing) * cost,
                  (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 0.85f * params.plant_spacing) * sint,
                  params.trunk_height + diff));
    pos_cord.push_back(
        make_vec3((-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 1.0f * params.plant_spacing) * cost,
                  (-0.5 * params.plant_spacing + 0.5 * params.trunk_radius + 1.0f * params.plant_spacing) * sint,
                  params.trunk_height + diff));
    ...
    for (int j = 1; j < params.shoots_per_cordon + 1; j++) {
        ...
        for (uint k = 1; k < Nz; k++) {
            vec3 n = rotatePoint(make_vec3(0, 0, dz),
                                 mean_shoot_angle * M_PI / 180.f * (theta0 + 1.2 * float(k) / float(Nz - 1)), phirot);
        ...
        while (lfrac > 0.5 * params.leaf_width) {
            ...
            parent_normal.normalize();
            vec3 leaf_offset =
                rotatePointAboutLine(make_vec3(0, lsize * (0.3 + getVariation(0.25, generator)), 0), make_vec3(0, 0, 0),
                                     parent_normal, flip * M_PI + unif_distribution(generator) * 0.25 * M_PI);
...
void CanopyGenerator::grapevineGoblet(const GobletGrapevineParameters &params, const vec3 &origin) {
    ...
    for (uint c = 0; c < 2; c++) {
        for (int j = 1; j < params.shoots_per_cordon + 1; j++) {
            ...
            float dz = ((1 + getVariation(0.1, generator)) * height - params.cordon_height) / float(Nz);
            for (uint k = 1; k < Nz; k++) {
                vec3 n = rotatePoint(make_vec3(0, 0, dz), (theta0 + (theta_end - theta0) * float(k) / float(Nz - 1)),
                                     params.canopy_rotation + M_PI * float(c));

                pos_pshoot.push_back(pos_pshoot.back() + n +
                                     make_vec3(getVariation(0.02, generator), getVariation(0.01, generator), 0.f));
            ...
            while (lfrac > 0.5 * params.leaf_width) {
                ...
                vec3 parent_normal = interpolateTube(pos_pshoot, fmax(0, lfrac - 0.001)) - pos_leaf;
                parent_normal.normalize();
                vec3 leaf_offset = rotatePointAboutLine(make_vec3(0, lsize * (0.3 + getVariation(0.25, generator)), 0),
                                                        make_vec3(0, 0, 0), parent_normal,
                                                        flip * M_PI + unif_distribution(generator) * 0.25 * M_PI);
