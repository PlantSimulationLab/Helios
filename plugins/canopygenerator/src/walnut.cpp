...
std::vector<uint> nutPrototype(const WalnutCanopyParameters params, Context *context) {
    std::vector<uint> UUIDs =
        context->addSphere(params.fruit_subdivisions, make_vec3(0, 0, 0), 1, params.fruit_texture_file.c_str());
...
uint addBranch(const vec3 p_start, const vec3 tan_start, const float r_start, const vec3 p_end, const vec3 tan_end,
               const float r_end, const uint Nsegs, const WalnutCanopyParameters params, std::minstd_rand0 generator,
               Context *context) {
    uint ID;
    ...
    // vec3 tankink =
    // 0.5*(tan_start+tan_end)+0.*make_vec3(getVariation(1.f,generator),getVariation(1.f,generator),getVariation(1.f,generator));
...
void branchRecursion(const std::vector<vec3> position_parent, const std::vector<float> radius_parent, uint level,
                     uint max_levels, const std::vector<uint> leaf_prototype, const std::vector<uint> nut_prototype,
                     std::vector<uint> &UUID_branch_plant, std::vector<std::vector<uint> > &UUID_leaf_plant,
                     std::vector<std::vector<uint> > &UUID_fruit_plant, const WalnutCanopyParameters params,
    ...
    if (level > max_levels) {  // leaves and nuts
        ...
        for (int i = 0; i < Nleaves; i++) {
            ...
            context->translatePrimitive(
                UUIDs,
                r0 + 0.1 * params.leaf_length * make_vec3(cosf(-azimuth), sinf(-azimuth), -0.5 * sinf(downangle)));
            ...
            if (i == Nleaves - 1) {  // tip leaf
                ...
                context->scalePrimitive(
                    UUIDs, make_vec3(params.leaf_length, 0.5 * params.leaf_length, 0.5 * params.leaf_length));
                ...
                context->translatePrimitive(
                    UUIDs, r0 - 0. * params.leaf_length * make_vec3(sinf(-azimuth), cosf(-azimuth), 0));
            }
            ...
            if (params.fruit_radius > 0) {
                ...
                if (Rnut < 0.005) {  // triplet
                    ...
                    // context->getObjectPointer(objID)->translate( r0 - params.fruit_radius*make_vec3( sinf(-azimuth),
                    // cosf(-azimuth), -0.65) );

                    std::vector<uint> nut_UUIDs = context->copyPrimitive(nut_prototype);
                    UUID_fruit_plant.push_back(nut_UUIDs);

                    context->scalePrimitive(nut_UUIDs,
                                            make_vec3(params.fruit_radius, params.fruit_radius, params.fruit_radius));

                    context->translatePrimitive(
                        nut_UUIDs, r0 - params.fruit_radius * make_vec3(sinf(-azimuth), cosf(-azimuth), -0.65));
                }
                if (Rnut < 0.01) {  // double
                    ...
                    context->translatePrimitive(
                        nut_UUIDs,
                        r0 - params.fruit_radius * make_vec3(2.f * sinf(-azimuth), 2.f * cosf(-azimuth), -0.85));
                }
                if (Rnut < 0.05) {  // single
                    ...
                    context->translatePrimitive(
                        nut_UUIDs, r0 - params.fruit_radius * make_vec3(sinf(-azimuth), cosf(-azimuth), -0.75));
                }
            }
        }
        ...
        for (int i = 0; i < Nbranch; i++) {
            ...
            bnorm = rotatePointAboutLine(bnorm, make_vec3(0, 0, 0), pvec1,
                                         0.1 * M_PI + 0.15 * M_PI * float(i) / float(Nbranch - 1));
            ...
            bnorm1.normalize();
            vec3 bnorm2 =
                rotatePointAboutLine(bnorm, make_vec3(0, 0, 0), nbase, phi0 + M_PI + getVariation(0.5, generator));
            bnorm2.normalize();
            ...
            uint ID1 = addBranch(pbase, 0.5 * L1 * bnorm1, rbase1, pbase + pend1 * L1,
                                 L1 * make_vec3(0, 0, 1.f + getVariation(1, generator)), 0.15 * rbase1, 8, params,
                                 generator, context);
            uint ID2 = addBranch(pbase, 0.5 * L2 * bnorm2, rbase2, pbase + pend2 * L2,
                                 L2 * make_vec3(0, 0, 1.f + getVariation(1, generator)), 0.15 * rbase2, 8, params,
                                 generator, context);
            ...
            branchRecursion(pos_parent, rad_parent, level + 1, max_levels, leaf_prototype, nut_prototype,
                            UUID_branch_plant, UUID_leaf_plant, UUID_fruit_plant, params, generator, context);
...
void CanopyGenerator::walnut(const WalnutCanopyParameters &params, const vec3 &origin) {
    ...
    uint ID_trunk =
        addBranch(origin, make_vec3(0, 0, 1), params.trunk_radius,
                  origin + make_vec3(unif_distribution(generator) * 3 * params.trunk_radius,
                                     unif_distribution(generator) * 3 * params.trunk_radius, params.trunk_height),
    ...
    branchRecursion(pos_trunk, rad_trunk, 1, 3, leaf_prototype, nut_prototype, UUID_branch_plant, UUID_leaf_plant,
                    UUID_fruit_plant.front(), params, generator, context);
