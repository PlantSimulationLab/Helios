...
std::vector<std::vector<uint> > strawberryCluster(const vec3 center, const StrawberryParameters &params,
                                                  Context *context) {
    ...
    if (context->randu() < 0.5) {
        ...
        for (int i = 0; i < nodes3.size(); i++) {
            nodes3.at(i) = nodes3.at(i) =
                sphere2cart(make_SphericalCoord(1.5 * params.fruit_radius, 0, azimuth + 0.5 * M_PI));
        }
...
void strawberryShoot(const StrawberryParameters &params, const helios::vec3 base_position,
                     const helios::vec3 base_direction, const float length, const float bend_angle,
                     const std::vector<uint> &leaf_prototype, std::vector<std::vector<uint> > &leaf_UUIDs,
    ...
    if (params.fruit_radius > 0 && context->randu() < params.clusters_per_stem) {
        vec3 cluster_position =
            interpolateTube(nodes, 0.35) +
            sphere2cart(make_SphericalCoord(0.1 + 0.2 * context->randu(), 0, 2 * M_PI * context->randu()));
...
void CanopyGenerator::strawberry(const StrawberryParameters &params, const vec3 &origin) {
    ...
    for (int i = 0; i < params.stems_per_plant; i++) {
        vec3 position = origin + sphere2cart(make_SphericalCoord(0.1 * context->randu() * params.plant_height, 0,
                                                                 2 * M_PI * context->randu()));

        vec3 base_direction = sphere2cart(make_SphericalCoord(
            0.45 * M_PI - 0.15 * M_PI * float(i) / float(params.stems_per_plant - 1), 2 * M_PI * context->randu()));

        float tip_angle = 0.3 * M_PI * float(i) / float(params.stems_per_plant - 1);

        float length = (0.75 + 0.25 * float(params.stems_per_plant - 1 - i) / float(params.stems_per_plant - 1)) *
                       params.plant_height * (0.9 + 0.2 * context->randu());

        strawberryShoot(params, position, base_direction, length, tip_angle, leaf_prototype, leaf_UUIDs, branch_UUIDs,
                        fruit_UUIDs, context);
    }
