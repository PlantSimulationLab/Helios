        ...
        if (params.sorghum_stage ==1){
            ...
            context->deletePrimitive(UUID_stem_top);
                ...
                for (float i = 1; i < 4; i++){
                    ...
                    float Nx =  params.s1_leaf_subdivisions * 5; // subdivisions only taken for the x, y is split into the minimum possible subdivisions (2)

                    float x_size = leaf_length * Nx;

                    float dx = 1.f / float(Nx);
                    float width_split = leaf_width * 0.5;

                    float y_size = leaf_width;  // 1
                    ...
                    for (int i = 0; i < x_size; i++) {
                        for (float j = 0; j < y_size; j = j + width_split) {
                            ...
                            float x_i = x * M_PI / float(x_size * dx);
                            float sx_i = (x + sx) * M_PI / float(x_size * dx);

                            float z_1 = (x * M_PI) / float((x_size * dx) / float(A_3));
                            float z_2 = ((x + sx) * M_PI) / float((x_size * dx) / float(A_3));
                            ...
                            vec2 uv0(x / float(x_size * dx), y / float(leaf_width));
                            vec2 uv1((x + sx) / float(x_size * dx), y / float(leaf_width));
                            vec2 uv2((x + sx) / float(x_size * dx), (y + sy) / float(leaf_width));
                            vec2 uv3(x / float(x_size * dx), (y + sy) / float(leaf_width));
            ...
            for (float i = 1; i < 6; i++){
                ...
                std::vector<uint> UUIDs4;
                float Nx = params.s2_leaf_subdivisions * 10; // subdivisions only taken for the x, y is split into the minimum possible subdivisions (2)
...
// THE LEAVES
            ...
            for (int i = 1; i < (nodes_no + 1); i++) {
                ...
                float Nx = params.s3_leaf_subdivisions; // subdivisions only taken for the x, y is split into the minimum possible subdivisions (2)
                float x;
                ...
                float x_size = params.s3_leaf_size.x * Nx * x;
                float dx = 1.f / float(Nx);
                float width_split = params.s3_leaf_size.y * 0.5;
                float y_size = params.s3_leaf_size.y;  // 1
                ...
                for (int i = 0; i < x_size; i++) {
                    for (float j = 0; j < y_size; j = j + width_split) {
                        ...
                        if (j == 0) {
                            ...
                            leaf_wave_1 = -width_split;
                            leaf_wave_2 = -width_split;
                        }
                        ...
                        if (j == y_size - width_split) {
                            ...
                            leaf_wave_3 = -width_split;
                            leaf_wave_4 = -width_split;
                        ...
                        vec2 uv0(x / float(x_size * dx), y / float(params.s3_leaf_size.y));
                        vec2 uv1((x + sx) / float(x_size * dx), y / float(params.s3_leaf_size.y));
                        vec2 uv2((x + sx) / float(x_size * dx), (y + sy) / float(params.s3_leaf_size.y));
                        vec2 uv3(x / float(x_size * dx), (y + sy) / float(params.s3_leaf_size.y));
                ...
                vec3 translation(-params.s3_stem_radius * 2.5, -0.9 / (2 / float(params.s3_leaf_size.y)), 0);  // adjustment
        ...
        // THE FRUIT
            ...
            vector<vector<vector<uint>>> s4_UUID_fruit_plant;
            s4_UUID_fruit_plant.resize(1);
            vector<RGBcolor> Color_fruit_stalk;
            std::vector<vec3> nodes_fruit_stalk;
            std::vector<float> radius_fruit_stalk;

            //The fruit stalk (the stem of the fruit)
            float m = 0.125;
            for (float n = 0; n < m; n = n + 0.0125) {
                ...
                nodes_fruit_stalk.push_back(make_vec3(x, y, z));
                radius_fruit_stalk.push_back(params.s4_stem_radius);

                Color_fruit_stalk.push_back(make_RGBcolor(0.302, 0.4314, 0.2392));

            }

            std::vector<uint> UUID1 = context->addTube(params.s4_stem_subdivisions, nodes_fruit_stalk, radius_fruit_stalk, Color_fruit_stalk);

            s4_UUID_fruit_plant.front().push_back(context->copyPrimitive(UUID1));
            context->deletePrimitive(UUID1);

            // now the fruit
            std::vector<vec3> nodes_fruit;
            std::vector<float> radius_fruit;
            float adj = 20; // scale factor to match the length of meters
            float width_fruit = params.s4_fruit_size.y * 10 * adj;

            for (float n = 0; n < (width_fruit + 1); n++) { // Ball-shaped tubes which form the fruit
                float x = 0;
                float y = 0;
                float dz = n * (0.01);
                float z = dz;
                float angle = n * M_PI / float(width_fruit);
                float dr = 0.008 * sin(angle);

                nodes_fruit.push_back(make_vec3(x, y, z));
                radius_fruit.push_back(dr);

            }

            std::vector<uint> UUIDs2 = context->addTube(params.s4_fruit_subdivisions, nodes_fruit, radius_fruit, params.seed_texture_file.c_str());

            float z_value = 0;
            for (int i = (params.s4_fruit_size.x * adj) + 2; i > -1; i--) {
                std::vector<uint> UUIDs2_copy = context->copyPrimitive(UUIDs2);

                float rotation_angle;

                if (i > (((params.s4_fruit_size.x* adj) + 2) / float(3))) {
                    rotation_angle = 0.26;
                } else {
                ...
                vec3 base = interpolateTube(nodes_fruit_stalk, 0.05);
                ...
                s4_UUID_fruit_plant.front().push_back(UUIDs2_copy);

                for (int ii = 0; ii < 8; ii++) {
                    s4_UUID_fruit_plant.front().push_back(context->copyPrimitive(UUIDs2_copy));
                    float rot2 = ii * 45 * M_PI / float(180);
                    context->rotatePrimitive(s4_UUID_fruit_plant.front().back(), rot2, "z");
                }
            ...
            context->translatePrimitive(flatten(s4_UUID_fruit_plant), V_1);
...
// THE LEAVES
            ...
            for (int i = 1; i < (nodes_no + 1); i++) {
                ...
                float Nx = params.s4_leaf_subdivisions; // subdivisions only taken for the x, y is split into the minimum possible subdivisions (2)
                float x;
                ...
                float x_size = params.s4_leaf_size.x * Nx * x;

                float dx = 1.f / float(Nx);
                float width_split = params.s4_leaf_size.y * 0.5;

                float y_size = params.s4_leaf_size.y;  // 1
                ...
                for (int i = 0; i < x_size; i++) {
                    for (float j = 0; j < y_size; j = j + width_split) {
                        ...
                        if (j == 0) {
                            ...
                            leaf_wave_1 = -width_split*0.85;
                            leaf_wave_2 = -width_split*0.85;
                        }
                        ...
                        if (j == y_size - width_split) {
                            ...
                            leaf_wave_3 = -width_split*0.85;
                            leaf_wave_4 = -width_split*0.85;
                        ...
                        vec2 uv0(x / float(x_size * dx), y / float(params.s4_leaf_size.y));
                        vec2 uv1((x + sx) / float(x_size * dx), y / float(params.s4_leaf_size.y));
                        vec2 uv2((x + sx) / float(x_size * dx), (y + sy) / float(params.s4_leaf_size.y));
                        vec2 uv3(x / float(x_size * dx), (y + sy) / float(params.s4_leaf_size.y));
       ...
       UUID_fruit.push_back(s4_UUID_fruit_plant);
...
// THE FRUIT
//    a std::vector<std::vector<std::vector<uint> > > s5_UUID_fruit_plant;
//    a s5_UUID_fruit_plant.resize(1);

        vector<vector<vector<uint>>> s5_UUID_fruit_plant;
        s5_UUID_fruit_plant.resize(1);
        ...
        for (float n = 0; n < m; n = n + 0.0125) {
            ...
            nodes_fruit_stalk.push_back(make_vec3(x, y, z));
            radius_fruit_stalk.push_back(params.s5_stem_radius);
            Color_fruit_stalk.push_back(make_RGBcolor(0.302, 0.4314, 0.2392));

        }

        std::vector<uint> UUID1 = context->addTube(params.s5_stem_subdivisions, nodes_fruit_stalk, radius_fruit_stalk, Color_fruit_stalk);

        s5_UUID_fruit_plant.front().push_back(context->copyPrimitive(UUID1));
        context->deletePrimitive(UUID1);
        ...
        std::vector<float> radius_fruit;
        float adj = 20; // scale factor to match the length of meters
        float width_fruit = params.s5_fruit_size.y * 10 * adj;

        for (float n = 0; n < (width_fruit + 1); n++) {
            float x = 0;
            float y = 0;
            float dz = n * (0.01);
            float z = dz;
        ...
        std::vector<uint> UUIDs2 = context->addTube(params.s5_fruit_subdivisions, nodes_fruit, radius_fruit,params.seed_texture_file.c_str());


        float z_value = 0;
        for (int i = (params.s5_fruit_size.x*adj) + 2; i > -1; i--) {

            std::vector<uint> UUIDs_copy = context->copyPrimitive(UUIDs2);

            float rotation_angle;

            if (i > (((params.s5_fruit_size.x*adj) + 2) / float(3))) {
                rotation_angle = 0.26;
            } else {
                rotation_angle = i * 0.0867;
            }
            ...
            s5_UUID_fruit_plant.front().push_back(UUIDs_copy);

            for (int ii = 0; ii < 8; ii++) {
                s5_UUID_fruit_plant.front().push_back(context->copyPrimitive(UUIDs_copy));
                float rot2 = ii * 45 * M_PI / float(180);
                context -> rotatePrimitive(s5_UUID_fruit_plant.front().back(), rot2, "z");
        ...
        context->rotatePrimitive(flatten(s5_UUID_fruit_plant), Z, "y");
        context->translatePrimitive(flatten(s5_UUID_fruit_plant), V_1);
...
// THE LEAVES
        ...
        for (int i = 1; i < (nodes_no + 1); i++) {
            ...
            float Nx = params.s5_leaf_subdivisions; // subdivisions only taken for the x, y is split into the minimum possible subdivisions (2)
            float x;
            ...
            float x_size = params.s5_leaf_size.x * Nx * x;

            float dx = 1.f / float(Nx);
            float width_split = params.s5_leaf_size.y * 0.5;

            float y_size = params.s5_leaf_size.y;  // 1
            ...
            for (int i = 0; i < x_size; i++) {
                for (float j = 0; j < y_size; j = j + width_split) {
                    ...
                    vec2 uv0(x / float(x_size * dx), y / float(params.s5_leaf_size.y));
                    vec2 uv1((x + sx) / float(x_size * dx), y / float(params.s5_leaf_size.y));
                    vec2 uv2((x + sx) / float(x_size * dx), (y + sy) / float(params.s5_leaf_size.y));
                    vec2 uv3(x / float(x_size * dx), (y + sy) / float(params.s5_leaf_size.y));
        ...
        UUID_fruit.push_back(s5_UUID_fruit_plant);
