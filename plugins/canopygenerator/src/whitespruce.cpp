...
void CanopyGenerator::whitespruce(const WhiteSpruceCanopyParameters &params, const vec3 &origin) {
    ...
    for (int i = 0; i < Nlevels - 1; i++) {
        ...
        float z = fmin(
            params.trunk_height,
            params.base_height + i * params.level_spacing * (1 + getVariation(0.1 * params.level_spacing, generator)));
        ...
        for (int j = 0; j < Nbranches; j++) {
            ...
            pos_branch.push_back(
                origin + make_vec3(r * sinf(phi) * cosf(theta), r * cosf(phi) * cosf(theta), z + r * sinf(theta)));
            ...
            for (int k = 0; k < 2 * Nbranches; k++) {
                ...
                // vec3 bdir =
                // l*make_vec3(sinf(phi)*cosf(theta+bangle),cosf(phi)*cosf(theta+0.5*M_PI),sinf(theta+bangle)); bdir =
                // rotatePointAboutLine( bdir, base, interpolateTube( pos_branch, bfrac-0.01 ),
                // getVariation(M_PI,generator) );

                vec3 bdir = l * make_vec3(sinf(phi + (0.4 + getVariation(0.1, generator)) * M_PI * (-1 + 2 * side)),
                                          cosf(phi - 0.4 * M_PI + 0.8 * M_PI * side), 0);
                ...
                // UUID_branch = context->addTube(params.wood_subdivisions,pos_subbranch,rad_subbranch,
                // params.wood_texture_file.c_str() );
                // context->addTube(params.wood_subdivisions,pos_subbranch,rad_subbranch,
                // params.wood_texture_file.c_str() );
                ...
                for (int n = 0; n < Nneedles; n++) {
                    ...
                    UUID_leaf_plant.push_back(context->addTile(
                        make_vec3(0, 0, 0), make_vec2(0.1 * params.needle_length, params.needle_length),
                        make_SphericalCoord(0, 0), params.needle_subdivisions, params.needle_color));
