...
void write_Triads(std::vector<Triad> &ts, char *fname) {
    ...
    for (int r = 0; r < nr; r++) {
        out << ts[r].a + 1 << ' ' << ts[r].b + 1 << ' ' << ts[r].c + 1 << ' ' << ts[r].ab + 1 << ' ' << ts[r].ac + 1
            << ' ' << ts[r].bc + 1 << endl;  //" " << ts[r].ro <<  endl;
    }
    out.close();
...
int s_hull_pro(std::vector<Shx> &pts, std::vector<Triad> &triads) {
    ...
    for (int k = 3; k < nump; k++) {
        ...
        dr = rx - hull.at(0).r;
        dc = cx - hull.at(0).c;  // outwards pointing from hull.at(0] to pt.
...
void circle_cent4(float r1, float c1, float r2, float c2, float r3, float c3, float &r, float &c, float &ro2) {
    ...
    double v4 = 2 * (r3 - r1), v5 = 2 * (c3 - c1), v6 = r3 * r3 - r1 * r1 + c3 * c3 - c1 * c1,
...
int de_duplicate(std::vector<Shx> &pts, std::vector<int> &outx) {
    ...
    for (int k = 0; k < nump - 1; k++) {
        if (dpx[k].r == dpx[k + 1].r && dpx[k].c == dpx[k + 1].c) {
            // cerr << "duplicate-point ids " << dpx[k].id << "  " << dpx[k+1].id << "   at  ("  << pts[dpx[k+1].id].r
            // << "," << pts[dpx[k+1].id].c << ")" << endl;
            outx.push_back(dpx[k + 1].id);
        }
    }

    if (outx.size() == 0) return (0);
...
int Cline_Renka_test(float &Ax, float &Ay, float &Bx, float &By, float &Cx, float &Cy, float &Dx, float &Dy) {
    float v1x = Bx - Ax, v1y = By - Ay, v2x = Cx - Ax, v2y = Cy - Ay, v3x = Bx - Dx, v3y = By - Dy, v4x = Cx - Dx,
          v4y = Cy - Dy;
    ...
    if (cosA * sinD + sinA * cosD < 0) return (-1);
...
int de_duplicateX(std::vector<Shx> &pts, std::vector<int> &outx, std::vector<Shx> &pts2) {
    ...
    for (int k = 0; k < nump - 1; k++) {
        if (dpx[k].r == dpx[k + 1].r && dpx[k].c == dpx[k + 1].c) {
            // cerr << "duplicate-point ids " << dpx[k].id << "  " << dpx[k+1].id << "   at  ("  << pts[dpx[k+1].id].r
            // << "," << pts[dpx[k+1].id].c << ")" << endl; cerr << dpx[k+1].id << " ";
