   ...
   public:
    ...
    datatype get(const int i, const int j) const {
        if (i >= 0 && i < Ntheta && j >= 0 && j < Nphi) {
            ...
            std::cerr << "ERROR (hit_map.get): get index out of range. Attempting to get index map at (" << i << ","
                      << j << "), but size of scan is " << Ntheta << " x " << Nphi << "." << std::endl;
            exit(EXIT_FAILURE);
        }
    }
    void set(const int i, const int j, const datatype value) {
        if (i >= 0 && i < Ntheta && j >= 0 && j < Nphi) {
            ...
            std::cerr << "ERROR (hit_map.set): set index out of range. Attempting to set index map at (" << i << ","
                      << j << "), but size of scan is " << Ntheta << " x " << Nphi << "." << std::endl;
            exit(EXIT_FAILURE);
        }
    }
...
struct HitPoint {
    ...
    HitPoint(int __scanID, helios::vec3 __position, helios::SphericalCoord __direction, helios::int2 __row_column,
             helios::RGBcolor __color, std::map<std::string, double> __data) {
        scanID = __scanID;
        position = __position;
...
struct Triangulation {
    ...
    Triangulation(int __scanID, helios::vec3 __vertex0, helios::vec3 __vertex1, helios::vec3 __vertex2, int __ID0,
                  int __ID1, int __ID2, helios::RGBcolor __color, int __gridcell) {
        scanID = __scanID;
        vertex0 = __vertex0;
        vertex1 = __vertex1;
        vertex2 = __vertex2;
...
struct GridCell {
    ...
    GridCell(helios::vec3 __center, helios::vec3 __global_anchor, helios::vec3 __size, helios::vec3 __global_size,
             float __azimuthal_rotation, helios::int3 __global_ijk, helios::int3 __global_count) {
        center = __center;
...
/** A scan is initialized by providing 1) the origin of the scan (see \ref origin), 2) the number of zenithal scan
 * directions (see \ref Ntheta), 3) the range of zenithal scan angles (see \ref thetaMin, \ref thetaMax), 4) the number
 * of azimuthal scan directions (see \ref Nphi), 5) the range of azimuthal scan angles (see \ref phiMin, \ref phiMax).
 * This creates a grid of Ntheta x Nphi scan points which are all initialized as misses.  Points are set as hits using
 * the addHitPoint() function. There are various functions to query the scan data.
 */
struct ScanMetadata {
    ...
    */
    ScanMetadata(const helios::vec3 __origin, const uint __Ntheta, const float __thetaMin, const float __thetaMax,
                 const uint __Nphi, const float __phiMin, const float __phiMax, const float __exitDiameter,
                 const float __beamDivergence, const std::vector<std::string> __columnFormat);
...
class LiDARcloud {
   private:
    ...
    //! 2D map of hits, one value for each (theta,phi) combo of scan. = -1 if no hit, = index if hit - size =
    //! (Ntheta)x(Nphi)
    ...
    void backfillLeavesAlphaMask(const std::vector<float> leaf_size, const float leaf_aspect_ratio,
                                 const float solidfraction, const std::vector<bool> group_filter_flag);

    void calculateLeafAngleCDF(const uint Nbins, std::vector<std::vector<float>>& CDF_theta,
                               std::vector<std::vector<float>>& CDF_phi);

    void floodfill(size_t t, std::vector<Triangulation>& cloud_triangles, std::vector<int>& fill_flag,
   ...
   public:
    ...
    /**
        ...
        \note If only the (row,column) scan table coordinates are available, use \ref rc2direction() to convert them to
       a spherical scan direction coordinate.
    */
    ...
    */
    ...
    /**
        ...
        \param[in] "xyz" (x,y,z) coordinates of hit point.
        \param[in] "direction" Spherical coordinate cooresponding to the scanner ray direction for the hit point.
        \param[in] "data" Map data structure containing floating point data values for the hit point.  E.g.,
       "reflectance" could be mapped to a value of 965.2.
    */
    void addHitPoint(const uint scanID, const helios::vec3 xyz, const helios::SphericalCoord direction,
    ...
    */
    ...
    /**
       ...
       "reflectance" could be mapped to a value of 965.2.
    */
    void addHitPoint(const uint scanID, const helios::vec3 xyz, const helios::int2 row_column,
                     const helios::RGBcolor color, const std::map<std::string, double> data);
    ...
    */
    ...
    /**
        \param[in] "scanID" ID of scan.
        \return vec2.x is the minimum scan zenithal angle, and vec2.y is the maximum scan zenithal angle, both in
       radians
    */
    ...
    /** \param[in] "scanID" ID of scan.
        \return vec2.x is the minimum scan azimuthal angle, and vec2.y is the maximum scan azimuthal angle, both in
       radians
    */
   ...
   */
    ...
    */
    ...
    //! Read an XML file containing scan information
    /** \param[in] "filename" Path to XML file
        \param[in] "load_grid_only" if true only the voxel grid defined in the xml file will be loaded, the scans
       themselves will not be loaded.
     */
    ...
    */
    ...
    //! Export to file all points in the point cloud to an ASCII text file following the column format specified by the
    //! <ASCII_format></ASCII_format> tag in the scan XML file
    /**
        \param[in] "filename" Name of file
        \note If there are multiple scans in the point cloud, each scan will be exported to a different file with the
       scan ID appended to the filename. This is because different scans may have a different column format.
    */
    void exportPointCloud(const char* filename);

    //! Export to file all points from a given scan to an ASCII text file following the column format specified by the
    //! <ASCII_format></ASCII_format> tag in the scan XML file
    /**
        \param[in] "filename" Name of file
    ...
    */
    ...
    /** \param[in] "context" Pointer to the Helios context
        \note This function creates the following primitive data for each patch 1) ``gridCell" which indicates the index
       of the gridcell that contains the patch, 2) ``directFlag" which equals 1 if the leaf was part of the direct
       reconstruction, and 0 if the leaf was backfilled.
     */
    ...
    /** \param[in] "context" Pointer to the Helios context
        \note This function creates primitive data called ``leafGroup" which provides an identifier for each triangle
       based on the fill group it is in.
    */
  ...
  ection
    */
    ...
    /** \param[in] "minreflectance" Miniimum hit point reflectance value
        \note If `reflectance' data was not provided for a hit point when calling \ref Scan::addHitPoint(), the point
    ...
    */
    ...
    /** \param[in] "scalar_field" Name of a scalar field defined in the ASCII point cloud data (e.g., "reflectance")
        ...
        \param[in] "comparator" Points will be filtered if "scalar (comparator) threshold", where (comparator) is one of
       ">", "<", or "=" \note As an example, imagine we wanted to remove all hit points where the reflectance is less
       than -10. In this case we would call scalarFilter( "reflectance", -10, "<" );
    */
    void scalarFilter(const char* scalar_field, const float threshold, const char* comparator);

    //! Filter full-waveform data according to the maximum scalar value along each pulse. Any scalar value can be used,
    //! provided it is a column in the ASCII hit point data file. The resulting point cloud will have only one hit point
    //! per laser pulse.
    /* \param[in] "scalar" Name of hit point scalar data in the ASCII hit data file.
       \note This function is only applicable for full-waveform data and requires that the scalar field "timestamp" is
       provided in the ASCII hit point data file.
    */
    void maxPulseFilter(const char* scalar);

    //! Filter full-waveform data according to the minimum scalar value along each pulse. Any scalar value can be used,
    //! provided it is a column in the ASCII hit point data file. The resulting point cloud will have only one hit point
    //! per laser pulse.
    /* \param[in] "scalar" Name of hit point scalar data in the ASCII hit data file.
       \note This function is only applicable for full-waveform data and requires that the scalar field "timestamp" is
    ...
    */
    ...
    //! Filter full-waveform data to include only the first hit per laser pulse. The resulting point cloud will have
    //! only one hit point per laser pulse (first hits).
    /* \note This function is only applicable for full-waveform data and requires that the scalar field "target_index"
     * is provided in the ASCII hit point data file. The "target_index" values can start at 0 or 1 for first hits as
     * long as it is consistent throughout the point cloud.
     */
    void firstHitFilter(void);

    //! Filter full-waveform data to include only the last hit per laser pulse. The resulting point cloud will have only
    //! one hit point per laser pulse (last hits).
    /* \note This function is only applicable for full-waveform data and requires that the scalar fields "target_index"
     * and "target_count" are provided in the ASCII hit point data file. The "target_index" values can start at 0 or 1
     * for first hits as long as it is consistent throughout the point cloud.
     */
    ...
    */
    ...
    /** \param [in] "center" (x,y,z) coordinate of grid center
        \param [in] "global_anchor" (x,y,z) coordinate of grid global anchor, i.e., this is the 'center' coordinate
       entered in the xml file.  If grid Nx=Ny=Nz=1, global_anchor=center \param [in] "size" size of the grid cell in
       the x,y,z directions \param [in] "global_size" size of the global grid in the x,y,z directions \param [in]
       "rotation" rotation angle (in radians) of the grid cell about the z-axis \param [in] "global_ijk" index within
       the global grid in the x,y,z directions \param [in] "global_count" total number of cells in global grid in the
       x,y,z directions
    */
    void addGridCell(const helios::vec3 center, const helios::vec3 global_anchor, const helios::vec3 size,
    ...
    /** \param [in] "index" Index of a grid cell.  Note: the index of a grid cell is given by the order in which it was
     * added to the grid. E.g., the first cell's index is 0, and the last cell's index is Ncells-1. */
    ...
    /** \param [in] "index" Index of a grid cell.  Note: the index of a grid cell is given by the order in which it was
  added to the grid. E.g., the first cell's index is 0, an d the last cell's index is Ncells-1. */
    ...
    /*   /\** \note This function does not return a value, rather, it set the Scan variable `hit_vol' which is queried
     * by the function `Scan::getHitGridCell()'. *\/ */
    void calculateHitGridCellGPU(void);

    // ------- SYNTHETIC SCAN ------ //

    //! Run a discrete return synthetic LiDAR scan based on scan parameters given in an XML file (returns only one laser
    //! hit per pulse)
    /** \param[in] "context" Pointer to the Helios context
     */
    ...
    /** \param[in] "context" Pointer to the Helios context
        ...
        \param[in] "record_misses" If true, "miss" points (i.e., beam did not hit any primitives) are recorded in the
       scan. \note Calling syntheticScan() with scan_grid_only=true can save substantial memory for contexts with large
       domains.
    */
    void syntheticScan(helios::Context* context, const bool scan_grid_only, const bool record_misses);

    //! Run a full-waveform synthetic LiDAR scan based on scan parameters given in an XML file (returns multiple laser
    //! hits per pulse)
    /** \param[in] "context" Pointer to the Helios context
        ...
        \param[in] "pulse_distance_threshold" Threshold distance for determining laser hit locations. Hits within
       pulse_distance_threshold of each other will be grouped into a single hit. \note Calling syntheticScan() with
       rays_per_pulse=1 will effectively run a discrete return synthetic scan.
    */
    ...
    /** \param[in] "context" Pointer to the Helios context
        ...
        \param[in] "rays_per_pulse" Number of ray launches per laser pulse direction
        \param[in] "pulse_distance_threshold" Threshold distance for determining laser hit locations. Hits within
       pulse_distance_threshold of each other will be grouped into a single hit. \param[in] "record_misses" If true,
       "miss" points (i.e., beam did not hit any primitives) are recorded in the scan. \note Calling syntheticScan()
       with scan_grid_only=true can save substantial memory for contexts with large domains. \note Calling
       syntheticScan() with rays_per_pulse=1 will effectively run a discrete return synthetic scan.
    */
    void syntheticScan(helios::Context* context, const int rays_per_pulse, const float pulse_distance_threshold,
    ...
    //! Calculate the leaf area for each grid volume
    /** \param [in] "min_voxel_hits" Minimum number of allowable LiDAR hits per voxel. If the total number of hits in a
     voxel is less than min_voxel_hits, the calculated leaf area will be set to zero. \note Currently, this version
     assumes all data is discrete-return. The function calculateLeafAreaGPU_testing() deals with waveform data, but may
     not be working correctly. In the next version, these two functions will be combined.*/
    ...
    /** \param [in] "context" Pointer to the Helios context
        \param [in] "beamoutput" if true writes detailed data about each beam to ../beamoutput/beam_data_s_[scan
       index]_c_[grid cell index].txt \param [in] "fillAnalytic" if true, when the iterative LAD inversion fails, the
       analytic solution using mean dr will be substituted. If false LAD is set to 999. \note writes voxel level data to
       ../voxeloutput/voxeloutput.txt \note
    */
    ...
    /** \param[in] "P" Vector of floats where each element is the P value of a given grid cell
     ...
     \param[in] "dr_array"  2D Vector of floats where the first index is the grid cell and the second index is the beam
     index \param[in] "fillAnalytic" If true the analytic solution using mean dr will be used when the inversion fails.
     If false, LAD will be set as 999.
     */
    std::vector<float> LAD_inversion(std::vector<float> P, std::vector<float> Gtheta,
                                     std::vector<std::vector<float>> dr_array, bool fillAnalytic);

    // -------- RECONSTRUCTION --------- //

    //! Perform a leaf reconstruction based on texture-masked Patches within each gridcell.  The reconstruction produces
    //! Patches for each reconstructed leaf surface, with leaf size automatically estimated algorithmically.
    /** \param[in] "minimum_leaf_group_area" Minimum allowable area of leaf triangular fill groups. Leaf fill groups
       with total areas less than minimum_leaf_group_area are not considered in the reconstruction. \param[in]
       "maximum_leaf_group_area" Maximum area of leaf triangular fill groups. Leaf fill groups with total areas greater
       than maximum_leaf_group_area are not considered in the reconstruction. \param[in] "leaf_aspect_ratio" Ratio of
       length of leaf along midrib to with of leaf perpendicular to leaf midrib.  This will generally be the
       length/width of leaf mask. \param[in] "mask_file" Path to PNG image file to be used with Alpha Mask.
    */
    void leafReconstructionAlphaMask(const float minimum_leaf_group_area, const float maximum_leaf_group_area,
                                     const float leaf_aspect_ratio, const char* mask_file);

    //! Perform a leaf reconstruction based on texture-masked Patches within each gridcell.  The reconstruction produces
    //! Patches for each reconstructed leaf surface, with leaf size set to a constant value.
    /** \param[in] "minimum_leaf_group_area" Minimum allowable area of leaf triangular fill groups. Leaf fill groups
       with total areas less than minimum_leaf_group_area are not considered in the reconstruction. \param[in]
       "maximum_leaf_group_area" Maximum area of leaf triangular fill groups. Leaf fill groups with total areas greater
       than maximum_leaf_group_area are not considered in the reconstruction. \param[in] "leaf_aspect_ratio" Ratio of
       length of leaf along midrib to with of leaf perpendicular to leaf midrib.  This will generally be the
       length/width of leaf mask. \param[in] "leaf_length_constant" Constant length of all reconstructed leaves.
    ...
    */
    ...
    //! Reconstruct the trunk of the tree. In order to do this, you must specify the center and size of a rectangular
    //! box that encompasses the tree trunk. This routine will then try to find the largest continuous triangle group,
    //! which is assumed to correspond to the trunk.
    ...
    */
    void trunkReconstruction(const helios::vec3 box_center, const helios::vec3 box_size, const float Lmax,
