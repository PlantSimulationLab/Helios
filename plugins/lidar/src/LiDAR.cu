...
__host__ __device__ float LIDAR_CUDA::operator*(const float2& a, const float2& b) { return a.x * b.x + a.y * b.y; }
...
__host__ __device__ float LIDAR_CUDA::magnitude(const float3& a) { return sqrt(a.x * a.x + a.y * a.y + a.z * a.z); }

__host__ __device__ float3 LIDAR_CUDA::normalize(const float3& a) { return a / magnitude(a); }
...
__device__ float2 LIDAR_CUDA::d_sampleDisk(float radius, uint seed) {
    ...
    // the input radius is the place where the intensity decreases to 1/e^2 (~0.1353) of the intensity at the beam
    // center
...
bool LIDAR_CUDA::sortcol0(const std::vector<float>& v0, const std::vector<float>& v1) { return v0.at(0) < v1.at(0); }
...
bool LIDAR_CUDA::sortcol1(const std::vector<double>& v0, const std::vector<double>& v1) { return v0.at(1) < v1.at(1); }
...
void LiDARcloud::calculateHitGridCellGPU(void) {
    ...
    if (total_hits == 0) {
        std::cout << "WARNING (calculateHitGridcellGPU): There are no hits currently in the point cloud. Skipping grid "
                     "cell binning calculation."
                  << std::endl;
    } else {
        insideVolume<<<dimGrid, dimBlock>>>(total_hits, d_hit_xyz, getGridCellCount(), d_grid_size, d_grid_center,
                                            d_grid_anchor, d_grid_rotation, d_hit_vol);
    }
...
std::vector<helios::vec3> LiDARcloud::gapfillMisses(const int scan) {
    ...
    for (size_t r = 0; r < getHitCount(); r++) {
        if (getHitScanID(r) == scan) {
            ...
            if (!doesHitDataExist(r, "timestamp")) {
                std::cerr << "ERROR (LiDARcloud::gapfillMisses): timestamp value is missing for hit " << r
                          << ". Cannot gapfill. Skipping..." << std::endl;
                return xyz_filled;
            }
    ...
    for (size_t r = 0; r < hit_table.size() - 1; r++) {
        ...
        if (min_tindex == 1 && doesHitDataExist(hit_table.at(r).at(0), "target_index") &&
            doesHitDataExist(hit_table.at(r).at(0), "target_count")) {
    ...
    for (size_t r = 0; r < hit_table.size() - 1; r++) {
        ...
        if (doesHitDataExist(hit_table.at(r).at(0), "target_index") &&
            doesHitDataExist(hit_table.at(r).at(0), "target_count")) {
    ...
    for (size_t r = 0; r < hit_table_semiclean.size() - 1; r++) {
        // if there are still rows with duplicate timestamps, it probably means there is no "target_index" column, but
        // multiple hits per timestamp are still included proceed using this assumption, just get rid of the rows where
        // dt = 0 for simplicity (last hits probably are what remain).
    ...
    for (size_t r = 0; r < hit_table_clean.size() - 1; r++) {
        ...
        // for small scans (like the rectangle test case, this needs to change to < 0 or some smaller angle (that is
        // larger than noise))
        //  if( dtheta_clean.at(r) < 0 ){
        //  for normal scans, this threshold allows for 10 degrees drops in theta within a given sweep as noise. This
        ...
        for (int i = 0; i < hit_table2D.at(j).size() - 1; i++) {
            ...
            if (dt > 1.5f * dt_clean_min) {  // missing hit(s)
                ...
                for (int k = 1; k <= Ngap; k++) {
                    ...
                    float theta = hit_table2D.at(j).at(i).at(2) +
                                  (hit_table2D.at(j).at(i + 1).at(2) - hit_table2D.at(j).at(i).at(2)) * float(k) /
                                      float(Ngap + 1);
                    float phi = hit_table2D.at(j).at(i).at(3) +
                                (hit_table2D.at(j).at(i + 1).at(3) - hit_table2D.at(j).at(i).at(3)) * float(k) /
                                    float(Ngap + 1);
                    if (phi > 2.f * M_PI) {
                        phi = phi - 2.f * M_PI;
                    }
    ...
    for (int j = 0; j < hit_table2D.size(); j++) {
        ...
        if (hit_table2D.at(j).front().at(2) > theta_range.x) {
            ...
            // this commented out section extrapolates based on the last dphi - this can cause intersections of the
            // different sweeps float dphi = hit_table2D.at(j).at(1).at(3)-hit_table2D.at(j).at(0).at(3);
...
void LiDARcloud::calculateLeafAreaGPU(void) { calculateLeafAreaGPU(1); }
...
void LiDARcloud::calculateLeafAreaGPU(const int minVoxelHits) {
    ...
    if (!triangulationcomputed) {
        std::cerr << "ERROR (calculateLeafAreaGPU): Triangulation must be performed prior to leaf area calculation. "
                     "See triangulateHitPoints()."
                  << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    std::vector<float>
        G_agg;  // G is dot product between ray direction and triangle normal that was hit (only relevant for hits)
    G_agg.resize(Ncells, 0);
    std::vector<float> hit_before_agg;  // hit_before corresponds to scan points that hit something before encountering
                                        // a particular grid cell
    hit_before_agg.resize(Ncells, 0);
    std::vector<float> hit_after_agg;  // hit_after corresponds to scan points that hit something after encountering a
                                       // particular grid cell (including something inside that cell)
    hit_after_agg.resize(Ncells, 0);
    std::vector<float>
        hit_inside_agg;  // hit_inside corresponds to scan points that hit something within a particular grid cell.
    ...
    for (uint s = 0; s < Nscans; s++) {
        ...
        // First, we are going to perform a preliminary ray trace to filter out rays that do not intersect any grid
        // volumes.  This will speed up calculations overall. Consider all rays in scan to start.
        ...
        CUDA_CHECK_ERROR(cudaMemset(
            d_boundingbox_hit, 0,
            Nmisses *
                sizeof(uint)));  // initialize to zero, set equal to 1 if the ray is found to intersect bounding box
        ...
        CUDA_CHECK_ERROR(
            cudaMemcpy(boundingbox_hit, d_boundingbox_hit, Nmisses * sizeof(uint), cudaMemcpyDeviceToHost));
        ...
        if (Nmissesbb == 0) {
            std::cerr << "ERROR (calculateLeafAreaGPU): No scan rays passed through grid cells.  Check that scan "
                         "configuration is properly specified."
                      << std::endl;
        ...
        for (uint c = 0; c < Ncells; c++) {
            ...
            intersectGridcell<<<dimGrid, dimBlock>>>(Nmissesbb, origin, d_scan_xyz, d_scan_weight, center, anchor, size,
                                                     rotation, d_dr, d_hit_before, d_hit_after);
        ...
        for (uint c = 0; c < Ncells; c++) {
            ...
            intersectGridcell<<<dimGrid, dimBlock>>>(Nhits, origin, d_scan_xyz, d_scan_weight, center, anchor, size,
                                                     rotation, d_dr, d_hit_before, d_hit_after);
    ...
    for (uint v = 0; v < Ncells; v++) {
        if (Gtheta[v] == 0 || Gtheta[v] != Gtheta[v]) {
            ...
            if (printmessages) {
                std::cout << "Negative number of rays reaching cell " << v << ": " << hit_after_agg[v] << " "
                          << hit_before_agg[v] << std::endl;
            }
            setCellLeafArea(0, v);
            continue;
        ...
        if (iter >= maxiter - 1 || a != a || a > 100) {
            if (printmessages) {
                std::cout << "WARNING: LAD inversion failed for volume #" << v << ". Using average dr formulation."
                          << std::endl;
            }
        ...
        if (printmessages) {
            ...
            // std::cout << "Vol #" << v << " intersections: " << hit_inside_agg[v] << " " << hit_after_agg[v] << " " <<
            // hit_before_agg[v] << std::endl;

            std::cout << "Vol #" << v << " LAD: " << a << std::endl;
            // std::cout << "Vol #" << v << " LAD: " << a << std::endl;
            //    cout << "Vol #" << v << " LAD: " << (1.f-P)/(dr_bar[v]*Gthetav]) << " [point quadrat]" << endl;
            // std::cout << "Vol #" << v << " Leaf Area: " << getCellLeafArea(v)*100.f*100.f << " [P =
            // mean(exp(-a*dr*Gtheta))]" << std::endl; std::cout << "Vol #" << v << " Leaf Area: " <<
            // -log(P)/(dr_bar*Gtheta[v])*gridsize.x*gridsize.y*gridsize.z*10000.f << " [a = -ln(P)/(dr*Gtheta)]" <<
            // std::endl;
...
void LiDARcloud::calculateLeafAreaGPU_synthetic(helios::Context* context, bool beamoutput, bool fillAnalytic) {
    ...
    // unlike previous calculateLeafAreaGPU versions, this one does not set the cell leaf area variable and does not
    // filter based on minVoxelHits argument
    ...
    for (uint s = 0; s < Nscans; s++) {
        ...
        // create a vector each element of which represents a unique beam and is another vector of the hit point indices
        // of this_scan_xyz that is associated with this beam
        ...
        for (uint c = 0; c < Ncells; c++) {
            ...
            if (beamoutput) {
                // set up header of file that outputs one row for each beam in the current scan that interacts with the
                // current voxel

                file_beam.open("../beamoutput/beam_data_s_" + std::to_string(s) + "_c_" + std::to_string(c) + ".txt");
                file_beam << "scan, cell, beam, R_before, R_inside, R_after, R_miss, E_before, E_inside, E_after, "
                             "E_miss, sin_theta, dr, last_dr"
                          << std::endl;
            }
            ...
            intersectGridcell_synthetic<<<dimGrid, dimBlock>>>(Nhits, origin, d_scan_xyz, d_scan_weight, center, anchor,
                                                               size, rotation, d_dr, d_hit_before, d_hit_after,
                                                               d_hit_location, scanner_range);
            ...
            for (int k = 0; k < Nbeams; k++) {
                ...
                for (int j = 0; j < beam_array.at(k).size(); j++) {
                    // pull out the index of the current scan's current beam's current hit (which is used to access the
                    // overall hit index r through this_scan_index[i])
                    uint i = beam_array.at(k).at(j);

                    helios::vec3 direction =
                        getHitXYZ(this_scan_index[i]) - getScanOrigin(getHitScanID(this_scan_index[i]));
                    direction.normalize();
                    ...
                    if (hit_location[i] == 1) {
                        ...
                        E_after++;
                        if (getHitData(this_scan_index[i], "target_index") ==
                            0) {  // if this is the first hitpoint for this beam,
                            W = 1.0;
                        }
                    } else if (hit_location[i] == 4) {
                        R_miss = R_miss + getHitData(this_scan_index[i], "nRaysHit");
                        E_miss++;
                    }  // or this hitpoint / beam did not intersect the voxel and should not be added to the total
                       // number of beams for this voxel

                }  // end of loop through each hit in the current beam

                // calculate the average dr across all hitpoints for the beam (they should all be the same anyway... but
                // just in case)
                ...
                if (beamoutput) {
                    ...
                    file_beam << s << "," << c << "," << k << "," << R_before << "," << R_inside << "," << R_after
                              << "," << R_miss << "," << E_before << "," << E_inside << "," << E_after << "," << E_miss
                              << "," << sin_theta << "," << drrx << "," << last_drr << std::endl;
                }
                ...
                // P_exact
                if (R_inside != 0 || R_after != 0 ||
                    R_miss != 0) {  // only count this beam for P_exact if some energy made it to the voxel
                    P_exact_numerator += ((R_after + R_miss) / (R_before + R_inside + R_after + R_miss)) * sin_theta;
                    P_exact_denominator +=
                        ((R_inside + R_after + R_miss) / (R_before + R_inside + R_after + R_miss)) * sin_theta;
                    voxel_beam_count++;
                }

                // P_intensity
                if (R_inside != 0 || R_after != 0) {  // only count this beam for P_intensity if some energy hit inside
                                                      // or after the voxel (not including misses)
                ...
                } else if (R_inside == 0 && R_after == 0 && R_before == 0 &&
                           R_miss != 0) {  // also count this beam if all the energy missed (but still went throught the
                                           // voxel)
                ...
                // P_equal
                if (E_inside != 0 ||
                    E_after != 0) {  // only count for P_equal if some hit points were inside or after the voxel
                ...
                } else if (E_inside == 0 && E_after == 0 && E_before == 0 &&
                           E_miss != 0) {  // also count this beam if there is only a "hitpoint" that missed (far after
                                           // the voxel)
                ...
                // P_sequal
                if (E_inside != 0 ||
                    E_after != 0) {  // only count for P_sequal if some hit points were inside or after the voxel
    ...
    // this is the value of P calculated using the reference values of LAD and Gtheta (from calculateSyntheticLeafArea,
    // calculateSyntheticGtheta), and the path length from all beams for a given cell
    ...
    file_output << "cell, grid_center_x, grid_center_y, grid_center_z, grid_size_x, grid_size_y, grid_size_z, Nbeams, "
                   "LAD_ref, G_ref, dr_bar, dr_var, P_ref, G, P_first, P_sequal, P_equal, P_intensity, P_exact, "
                   "LADGref_refcheck, LADGref_first, LADGref_sequal, LADGref_equal, LADGref_intensity, LADGref_exact, "
                   "LAD_refcheck, LAD_first, LAD_sequal, LAD_equal, LAD_intensity, LAD_exact"
                << std::endl;
    ...
    for (uint c = 0; c < Ncells; c++) {
        ...
        file_output << c << "," << grid_center.x << "," << grid_center.y << "," << grid_center.z << "," << grid_size.x
                    << "," << grid_size.y << "," << grid_size.z << "," << voxel_beam_count_tot.at(c) << ","
                    << LA_ref[c] / (grid_size.x * grid_size.y * grid_size.z) << "," << Gtheta_ref[c] << ","
                    << dr_bar_ref[c] << "," << dr_var_ref[c] << "," << P_ref[c] << "," << Gtheta[c] << "," << P_first[c]
                    << "," << P_sequal[c] << "," << P_equal[c] << "," << P_intensity[c] << "," << P_exact[c] << ","
                    << LAD_refcheck_Gref[c] << "," << LAD_first_Gref[c] << "," << LAD_sequal_Gref[c] << ","
                    << LAD_equal_Gref[c] << "," << LAD_intensity_Gref[c] << "," << LAD_exact_Gref[c] << ","
                    << LAD_refcheck[c] << "," << LAD_first[c] << "," << LAD_sequal[c] << "," << LAD_equal[c] << ","
                    << LAD_intensity[c] << "," << LAD_exact[c] << std::endl;
    }
    file_output.close();
    ...
    for (uint v = 0; v < Ncells; v++) {
        ...
        if (iter >= maxiter - 1 || a != a || a > 100) {
            if (fillAnalytic) {
                ...
                if (printmessages) {
                    std::cout << "WARNING: LAD inversion failed for volume #" << v << ". setting LAD to 999"
                              << std::endl;
                }
...
__global__ void LIDAR_CUDA::insideVolume(const uint Nhits, const float3* d_hit_xyz, const uint Ngridcells,
                                         const float3* d_grid_size, const float3* d_grid_center,
                                         const float3* d_grid_anchor, const float* d_grid_rotation, int* d_hit_vol) {
...
__global__ void LIDAR_CUDA::intersectBoundingBox(const size_t scanSize, const float3 origin, const float3* d_scan_xyz,
                                                 const float3 bbcenter, const float3 bbsize, uint* d_boundingbox_hit) {
    ...
    float x0 = bbcenter.x - 0.5f * bbsize.x;
    float x1 = bbcenter.x + 0.5f * bbsize.x;
    float y0 = bbcenter.y - 0.5f * bbsize.y;
    float y1 = bbcenter.y + 0.5f * bbsize.y;
    float z0 = bbcenter.z - 0.5f * bbsize.z;
    float z1 = bbcenter.z + 0.5f * bbsize.z;
...
__global__ void LIDAR_CUDA::intersectGridcell(const size_t Nhitsbb, const float3 origin, float3* d_scan_xyz,
                                              float* d_scan_weight, const float3 center, const float3 anchor,
                                              const float3 size, const float rotation, float* d_dr, float* hit_before,
                                              float* hit_after) {
    ...
    float ox = origin_rot.x;
    float oy = origin_rot.y;
    float oz = origin_rot.z;
    float dx = direction.x;
    float dy = direction.y;
    float dz = direction.z;
...
__global__ void LIDAR_CUDA::intersectGridcell_synthetic(const size_t Nhitsbb, const float3 origin, float3* d_scan_xyz,
                                                        float* d_scan_weight, const float3 center, const float3 anchor,
                                                        const float3 size, const float rotation, float* d_dr,
                                                        float* hit_before, float* hit_after, uint* d_hit_location,
                                                        const float scanner_range) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void LiDARcloud::trunkReconstruction(const helios::vec3 box_center, const helios::vec3 box_size, const float Lmax,
    ...
    insideVolume<<<dimGrid, dimBlock>>>(N, d_hit_xyz, 1, d_grid_size, d_grid_center, d_grid_center, d_grid_rotation,
                                        d_hit_vol);
...
void LiDARcloud::syntheticScan(helios::Context* context) { syntheticScan(context, 1, 0, false, false); }
...
void LiDARcloud::syntheticScan(helios::Context* context, const int rays_per_pulse,
                               const float pulse_distance_threshold) {
...
void LiDARcloud::syntheticScan(helios::Context* context, const int rays_per_pulse, const float pulse_distance_threshold,
                               const bool scan_grid_only, const bool record_misses) {
    int Npulse;
    if (rays_per_pulse < 1) {
        Npulse = 1;
    } else {
    ...
    if (scan_grid_only == false) {
        ...
        bb_center = make_float3(xbounds.x + 0.5 * (xbounds.y - xbounds.x), ybounds.x + 0.5 * (ybounds.y - ybounds.x),
        ...
        bb_center = make_float3(boxmin.x + 0.5 * (boxmax.x - boxmin.x), boxmin.y + 0.5 * (boxmax.y - boxmin.y),
                                boxmin.z + 0.5 * (boxmax.z - boxmin.z));
    ...
    bool* d_maskdata;
    CUDA_CHECK_ERROR(cudaMalloc((void**)&d_maskdata,
                                Ntextures * masksize_max.x * masksize_max.y * sizeof(bool)));  // allocate device memory
    CUDA_CHECK_ERROR(cudaMemcpy(d_maskdata, maskdata, Ntextures * masksize_max.x * masksize_max.y * sizeof(bool),
    ...
    for (int s = 0; s < getScanCount(); s++) {
        ...
        CUDA_CHECK_ERROR(cudaMemset(
            d_bb_hit, 0,
            N * sizeof(uint)));  // initialize to zero, set equal to 1 if the ray is found to intersect bounding box
        ...
        //---- patch kernel ----//
        intersectPatches<<<dimGrid, dimBlock>>>(N, Npulse, scan_origin, d_raydir, exit_diameter, beam_divergence,
                                                Npatches, d_patch_vertex, d_patch_textureID, Ntextures, d_masksize,
                                                masksize_max, d_maskdata, d_patch_uv, d_hit_t, d_hit_fnorm, d_hit_ID);

        cudaDeviceSynchronize();
        CUDA_CHECK_ERROR(cudaPeekAtLastError());  // if there was an error inside the kernel, it will show up here

        //---- triangle kernel ----//
        intersectTriangles<<<dimGrid, dimBlock>>>(
            N, Npulse, scan_origin, d_raydir, exit_diameter, beam_divergence, Ntriangles, Npatches, d_tri_vertex,
            d_tri_textureID, Ntextures, d_masksize, masksize_max, d_maskdata, d_tri_uv, d_hit_t, d_hit_fnorm, d_hit_ID);
        ...
        for (size_t r = 0; r < N; r++) {
            ...
            if (t_pulse.size() == 1) {  // this is discrete-return data, or we only had one hit for this pulse
                ...
                for (size_t hit = 1; hit <= t_pulse.size(); hit++) {
                    ...
                    if (hit == t_pulse.size()) {
                        std::vector<float> v{
                            d / float(count), f / float(Npulse), float(count),
                            t_pulse.at(hit - 1).at(
                                2)};  // Note: the last index of t_pulse (.at(2)) is the object identifier. We don't
                                      // want object identifiers to be averaged, so we'll assign the hit identifier
                                      // based on the last ray in the group
                        t_hit.push_back(v);

                        // else if the current ray is more than the pulse threshold distance from t0,  it is part of the
                        // next hitpoint so output the previous hitpoint and reset
                    } else if (t_pulse.at(hit).at(0) - t0 > pulse_distance_threshold) {
                        std::vector<float> v{d / float(count), f / float(Npulse), float(count),
                                             t_pulse.at(hit - 1).at(2)};  // included the ray count here
                        // Note: the last index of t_pulse (.at(2)) is the object identifier. We don't want object
                        // identifiers to be averaged, so we'll assign the hit identifier based on the last ray in the
                        // group
                        t_hit.push_back(v);
                        ...
                        // or else the current ray is less than pulse threshold and is part of current hitpoint; add it
                        // to the current hit point and continue on
                    } else {
                        count++;
                        t0 = t_pulse.at(hit).at(0);
                        d += t_pulse.at(hit).at(0);
            ...
            for (size_t hit = 0; hit < t_hit.size(); hit++) {
                ...
                if (UUID >= 0 && context->doesPrimitiveExist(uint(UUID))) {
                    ...
                    if (context->doesPrimitiveDataExist(uint(UUID), "object_label") &&
                        context->getPrimitiveDataType(uint(UUID), "object_label") == helios::HELIOS_TYPE_INT) {
                        int label;
...
std::vector<float> LiDARcloud::calculateSyntheticLeafArea(helios::Context* context) {
    ...
    dim3 dimBlock(64, 1);
    dim3 dimGrid(ceil(N / 64.f));
    insideVolume<<<dimGrid, dimBlock>>>(N, d_prim_xyz, getGridCellCount(), d_grid_size, d_grid_center, d_grid_anchor,
                                        d_grid_rotation, d_prim_vol);
...
__global__ void LIDAR_CUDA::intersectPatches(const size_t Npulse, const int rays_per_pulse, const float3 origin,
                                             float3* d_raydir, const float exit_diameter, const float beam_divergence,
                                             const int Npatches, float3* d_patch_vertex, int* d_patch_textureID,
                                             const int Ntextures, int2* d_masksize, int2 masksize_max, bool* d_maskdata,
                                             float2* d_patch_uv, float* d_hit_t, float* d_hit_fnorm, int* d_hit_ID) {
    ...
    float zenith =
        acos_safe(d_raydir[idx].z / sqrt(d_raydir[idx].x * d_raydir[idx].x + d_raydir[idx].y * d_raydir[idx].y +
                                         d_raydir[idx].z * d_raydir[idx].z));
            ...
            if (ddota > 0.0 && ddota < a * a) {
                ...
                if (ddotb > 0.0 && ddotb < b * b) {
                    if (d_patch_textureID[p] < 0) {  // no texture mask
                        ...
                        int2 ind = make_int2(roundf(float(sz.x - 1) * (uvmin.x + fabs(uv.x) * duv.x)),
                                             roundf(float(sz.y - 1) * (uvmin.y + fabs(uv.y) * duv.y)));
...
__global__ void LIDAR_CUDA::intersectTriangles(const size_t Npulse, const int rays_per_pulse, const float3 origin,
                                               float3* d_raydir, const float exit_diameter, const float beam_divergence,
                                               const int Ntriangles, const int Npatches, float3* d_tri_vertex,
                                               int* d_tri_textureID, const int Ntextures, int2* d_masksize,
                                               const int2 masksize_max, bool* d_maskdata, float2* d_tri_uv,
                                               float* d_hit_t, float* d_hit_fnorm, int* d_hit_ID) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
            ...
            if (gamma > 0.0 && beta + gamma < 1.0) {
                ...
                if (t > 1e-6 && t < tmin) {
                    if (d_tri_textureID[tri] < 0) {  // no texture mask
                        ...
                        uint2 ind =
                            make_uint2(roundf(float(sz.x - 1) * fabs(uv.x)), roundf(float(sz.y - 1) * fabs(uv.y)));
