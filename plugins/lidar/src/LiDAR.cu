...
void LiDARcloud::calculateLeafAreaGPU_synthetic(helios::Context *context, bool beamoutput, bool fillAnalytic) {
        ...
        for (uint c = 0; c < Ncells; c++) {
            ...
            for (int k = 0; k < Nbeams; k++) {
                ...
                if (beamoutput) {
                    ...
                    file_beam << s << "," << c << "," << k << "," << R_before << "," << R_inside << "," << R_after << "," << R_miss << "," << E_before << "," << E_inside << "," << E_after << "," << E_miss << "," << sin_theta << "," << drrx << ","
                              << last_drr << "," << I_before << "," << I_inside << "," << I_after << "," << I_miss << std::endl;
                }
    ...
    file_output
            << "cell, grid_center_x, grid_center_y, grid_center_z, grid_size_x, grid_size_y, grid_size_z, Nbeams, LAD_ref, G_ref, dr_bar, dr_var, P_ref, G, P_first, P_equal, P_intensity, P_ideal, P_exact, LADGref_refcheck, LADGref_first, LADGref_equal, LADGref_intensity, LADGref_ideal,LADGref_exact, LAD_refcheck, LAD_first, LAD_equal, LAD_intensity, LAD_ideal, LAD_exact"
            << std::endl;
    ...
    for (uint c = 0; c < Ncells; c++) {
        ...
        file_output << c << "," << grid_center.x << "," << grid_center.y << "," << grid_center.z << "," << grid_size.x << "," << grid_size.y << "," << grid_size.z << "," << voxel_beam_count_tot.at(c) << ","
                    << LA_ref[c] / (grid_size.x * grid_size.y * grid_size.z) << "," << Gtheta_ref[c] << "," << dr_bar_ref[c] << "," << dr_var_ref[c] << "," << P_ref[c] << "," << Gtheta[c] << "," << P_first[c] << "," << P_equal[c] << ","
                    << P_intensity[c] << "," << P_ideal[c] << "," << P_exact[c] << "," << LAD_refcheck_Gref[c] << "," << LAD_first_Gref[c] << "," << LAD_equal_Gref[c] << "," << LAD_intensity_Gref[c] << "," << LAD_ideal_Gref[c] << ","
                    << LAD_exact_Gref[c] << "," << LAD_refcheck[c] << "," << LAD_first[c] << "," << LAD_equal[c] << "," << LAD_intensity[c] << "," << LAD_ideal[c] << "," << LAD_exact[c] << std::endl;
    }
    file_output.close();
...
__global__ void LIDAR_CUDA::intersectGridcell(const size_t Nhitsbb, const float3 origin, float3 *d_scan_xyz, float *d_scan_weight, const float3 center, const float3 anchor, const float3 size, const float rotation, float *d_dr, float *hit_before,
                                              float *hit_after) {
...
__global__ void LIDAR_CUDA::intersectGridcell_synthetic(const size_t Nhitsbb, const float3 origin, float3 *d_scan_xyz, float *d_scan_weight, const float3 center, const float3 anchor, const float3 size, const float rotation, float *d_dr,
                                                        float *hit_before, float *hit_after, uint *d_hit_location, const float scanner_range) {
...
void LiDARcloud::trunkReconstruction(const helios::vec3 &box_center, const helios::vec3 &box_size, float Lmax, float max_aspect_ratio) {
...
__global__ void LIDAR_CUDA::intersectPatches(const size_t Npulse, const int rays_per_pulse, const float3 origin, float3 *d_raydir, const float exit_diameter, const float beam_divergence, const int Npatches, float3 *d_patch_vertex,
                                             int *d_patch_textureID, const int Ntextures, int2 *d_masksize, int2 masksize_max, bool *d_maskdata, float2 *d_patch_uv, float *d_hit_t, float *d_hit_fnorm, int *d_hit_ID) {
...
__global__ void LIDAR_CUDA::intersectTriangles(const size_t Npulse, const int rays_per_pulse, const float3 origin, float3 *d_raydir, const float exit_diameter, const float beam_divergence, const int Ntriangles, const int Npatches,
                                               float3 *d_tri_vertex, int *d_tri_textureID, const int Ntextures, int2 *d_masksize, const int2 masksize_max, bool *d_maskdata, float2 *d_tri_uv, float *d_hit_t, float *d_hit_fnorm, int *d_hit_ID) {
...
void LiDARcloud::calculateLeafAreaGPU_equal_weighting(bool beamoutput, bool fillAnalytic) {
    ...
    for (uint c = 0; c < Ncells; c++) {
        ...
        file_output << c << "," << grid_center.x << "," << grid_center.y << "," << grid_center.z << "," << grid_size.x << "," << grid_size.y << "," << grid_size.z << "," << voxel_beam_count_tot.at(c) << "," << dr_bar[c] << "," << Gtheta[c] << ","
                    << P_equal[c] << "," << LAD_equal[c] << std::endl;
    }
    file_output.close();
