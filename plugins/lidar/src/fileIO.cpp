...
void LiDARcloud::loadXML( const char* filename ){
  ...
  for (pugi::xml_node s = helios.child("scan"); s; s = s.next_sibling("scan")){
    ...
    if( thetaMax-1e-5>M_PI ){
      cerr << "ERROR (loadXML): thetaMax cannot be greater than 180 degrees. It is possible you accidentally specified thetaMax in degrees instead of radians." << endl;
      exit(EXIT_FAILURE);
    }
    ...
    if( phiMax-1e-5>4.f*M_PI ){
      cerr << "ERROR (loadXML): phiMax cannot be greater than 720 degrees. It is possible you accidentally specified phiMax in degrees instead of radians." << endl;
      exit(EXIT_FAILURE);
    }
    ...
    ScanMetadata scan(origin, size.x, thetaMin, thetaMax, size.y, phiMin, phiMax );
	...
	for( uint i=0; i<column_format.size(); i++ ){
	  ...
	  }else if( column_format.at(i).compare("theta")==0 ){
	    datafile >> temp_direction.zenith;
	  }else if( column_format.at(i).compare("phi")==0 ){
	    datafile >> temp_direction.azimuth;
	  }else if( column_format.at(i).compare("theta_rad")==0 ){
	    datafile >> temp_direction.zenith;
	    temp_direction.zenith *= 180.f/M_PI;
	  }else if( column_format.at(i).compare("phi_rad")==0 ){
	    datafile >> temp_direction.azimuth;
	...
	if( distanceFilter>0 ){
	  vec3 r = temp_xyz-origin;
	  if( r.magnitude()>distanceFilter ){
	    continue;
	  }
	}
	...
	if( temp_direction.zenith<scans.at(scanID).thetaMin ){
	  temp_direction.zenith=scans.at(scanID).thetaMin;
	}else if( temp_direction.zenith<scans.at(scanID).thetaMin ){
	  temp_direction.zenith=scans.at(scanID).thetaMax;
	}

	temp_direction = cart2sphere(temp_xyz-origin);

	//add hit point to the scan
	addHitPoint( scanID, temp_xyz, temp_direction, temp_rgb, data );
	//addHitPoint( scanID, temp_xyz, make_int2(temp_row,temp_column), temp_rgb, data );
