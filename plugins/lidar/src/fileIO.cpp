...
void LiDARcloud::loadXML(const char* filename) { loadXML(filename, false); }
...
void LiDARcloud::loadXML(const char* filename, const bool load_grid_only) {
    ...
    if (!result) {
        cout << "failed." << endl;
        cerr << "XML  file " << filename << " parsed with errors, attribute value: ["
    ...
    if (load_grid_only == false) {
        ...
        for (pugi::xml_node s = helios.child("scan"); s; s = s.next_sibling("scan")) {
            ...
            vec3 origin = string2vec3(
                origin_str);  // note: pugi loads xml data as a character.  need to separate it into 3 floats
            ...
            if (strlen(size_str) == 0) {
                ...
                size = string2int2(
                    size_str);  // note: pugi loads xml data as a character.  need to separate it into 2 ints
            }
            if (size.x <= 0 || size.y <= 0) {
                cerr << "failed.\nERROR (loadXML): The scan size must be positive (check scan #" << scan_count << ")."
                     << endl;
                exit(EXIT_FAILURE);
            }
            ...
            if (strlen(offset_str) > 0) {
                translation = string2vec3(
                    offset_str);  // note: pugi loads xml data as a character.  need to separate it into 3 floats
            }
            ...
            if (strlen(rotation_str) > 0) {
                vec3 rotation = string2vec3(
                    rotation_str);  // note: pugi loads xml data as a character.  need to separate it into 3 floats
            ...
            if (strlen(thetaMin_str) == 0) {
                // cerr << "WARNING (loadXML): A minimum zenithal scan angle was not specified for scan #" << scan_count
                // << "...assuming thetaMin = 0." << flush;
                thetaMin = 0.f;
            } else {
            ...
            if (strlen(thetaMax_str) == 0) {
                // cerr << "WARNING (loadXML): A maximum zenithal scan angle was not specified for scan #" << scan_count
                // << "...assuming thetaMax = PI rad." << flush;
                thetaMax = M_PI;
            } else {
            ...
            if (strlen(phiMin_str) == 0) {
                // cerr << "WARNING (loadXML): A minimum azimuthal scan angle was not specified for scan #" <<
                // scan_count << "...assuming phiMin = 0." << flush;
                phiMin = 0.f;
            } else {
            ...
            if (strlen(phiMax_str) == 0) {
                // cerr << "WARNING (loadXML): A maximum azimuthal scan angle was not specified for scan #" <<
                // scan_count << "...assuming phiMax = 2PI rad." << flush;
                phiMax = 2.f * M_PI;
            } else {
            ...
            if (!data_filename.empty()) {
                ...
                if (!f.good()) {
                    ...
                    if (!f.good()) {
                        cout << "failed.\n ERROR (loadXML): Data file `" << str << "' given for scan #" << scan_count
                             << " does not exist." << endl;
                        exit(EXIT_FAILURE);
                    }
                    f.close();
                ...
                while (datafile.good()) {  // loop through file to read scan data
                    ...
                    if (!datafile.good()) {  // if the whole line was not read successfully, stop
                        if (hit_count == 1) {
                            std::cerr
                                << "WARNING: Something is likely wrong with the data file " << filename
                                << ". Check that the format is consisten with that specified in the XML metadata file."
                                << std::endl;
                        }
                        break;
                    }
                    ...
                    // hit point
                    if (temp_xyz.x == -9999) {
                        std::cerr << "ERROR (loadXML): x-coordinate not specified for hit point #" << hit_count - 1
                                  << " of scan #" << scan_count << std::endl;
                        exit(EXIT_FAILURE);
                    } else if (temp_xyz.y == -9999) {
                        std::cerr << "ERROR (loadXML): t-coordinate not specified for hit point #" << hit_count - 1
                                  << " of scan #" << scan_count << std::endl;
                        exit(EXIT_FAILURE);
                    } else if (temp_xyz.z == -9999) {
                        std::cerr << "ERROR (loadXML): z-coordinate not specified for hit point #" << hit_count - 1
                                  << " of scan #" << scan_count << std::endl;
                        exit(EXIT_FAILURE);
                    }
    ...
    for (pugi::xml_node s = helios.child("grid"); s; s = s.next_sibling("grid")) {
        ...
        vec3 center =
            string2vec3(center_str);  // note: pugi loads xml data as a character.  need to separate it into 3 floats
        ...
        vec3 gsize =
            string2vec3(gsize_str);  // note: pugi loads xml data as a character.  need to separate it into 3 floats
        ...
        for (int k = 0; k < Nz; k++) {
            ...
            for (int j = 0; j < Ny; j++) {
                ...
                for (int i = 0; i < Nx; i++) {
                    ...
                    if (printmessages) {
                        cout << "Adding grid cell #" << count << " with center " << subcenter_rot.x + center.x << ","
                             << subcenter_rot.y + center.y << "," << subcenter.z + center.z << " and size "
                             << gsubsize.x << " x " << gsubsize.y << " x " << gsubsize.z << endl;
                    }

                    addGridCell(subcenter + center, center, gsubsize, gsize, rotation * M_PI / 180.f,
                                make_int3(i, j, k), make_int3(Nx, Ny, Nz));
    ...
    if (printmessages) {
        ...
        cout << "Successfully read " << getScanCount() << " scan(s), which contain " << total_hits
...
void LiDARcloud::exportPointCloud(const char* filename) {
    if (getScanCount() == 1) {
        ...
        for (int i = 0; i < getScanCount(); i++) {
            ...
            if (dotindex == filename_a.size() - 1 ||
                filename_a.size() - 1 - dotindex > 4) {  // no file extension was provided
...
void LiDARcloud::exportPointCloud(const char* filename, const uint scanID) {
    if (scanID > getScanCount()) {
        std::cerr << "ERROR (LiDARcloud::exportPointCloud): Cannot export scan " << scanID
