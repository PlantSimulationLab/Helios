...
ScanMetadata::ScanMetadata(const helios::vec3 __origin, const uint __Ntheta, const float __thetaMin,
                           const float __thetaMax, const uint __Nphi, const float __phiMin, const float __phiMax,
                           const float __exitDiameter, const float __beamDivergence,
                           const std::vector<std::string> __columnFormat) {
...
int LiDARcloud::selfTest(void) {
    ...
    vec3 dx = make_vec3((gsize.x - 2.f * diskR) / float(N.x), (gsize.y - 2.f * diskR) / float(N.y),
                        (gsize.z - 2.f * diskR) / float(N.z));

    for (int kk = 0; kk < N.z; kk++) {
        for (int jj = 0; jj < N.y; jj++) {
            for (int ii = 0; ii < N.x; ii++) {
                vec3 shift = make_vec3((context_2.randu() + ii) * dx.x, (context_2.randu() + jj) * dx.y,
                                       (context_2.randu() + kk) * dx.z);

                float theta = acos_safe(1.f - 2.f * context_2.randu());
                float phi = context_2.randu() * 2.f * M_PI;

                context_2.addPatch(gcenter - 0.5 * gsize + shift + 2.f * make_vec3(diskR, diskR, diskR),
                                   2.f * make_vec2(diskR, diskR), make_SphericalCoord(theta, phi));
            }
        }
    }
    ...
    gsize = make_vec3(1, 1, 1);

    float dTheta = 0.5f * M_PI / 8.f;
    std::vector<float> leafAngleCDF;
    leafAngleCDF.resize(8);
    std::vector<float> leafAnglePDF;
    leafAnglePDF.resize(8);
    leafAnglePDF.at(0) = 0.0446;
    leafAnglePDF.at(1) = 0.1786;
    leafAnglePDF.at(2) = 0.3125;
    leafAnglePDF.at(3) = 0.1964;
    leafAnglePDF.at(4) = 0.1339;
    ...
    dx = make_vec3((gsize.x - 2.f * diskR) / float(N.x), (gsize.y - 2.f * diskR) / float(N.y),
                   (gsize.z - 2.f * diskR) / float(N.z));

    for (int kk = 0; kk < N.z; kk++) {
        for (int jj = 0; jj < N.y; jj++) {
            for (int ii = 0; ii < N.x; ii++) {
                vec3 shift = make_vec3((context_3.randu() + ii) * dx.x, (context_3.randu() + jj) * dx.y,
                                       (context_3.randu() + kk) * dx.z);
                ...
                for (int i = 0; i < leafAngleCDF.size(); i++) {
                    if (rt < leafAngleCDF.at(i)) {
                        theta = (i + context_3.randu()) * dTheta;
                        break;
                    }
                }
                ...
                context_3.addPatch(gcenter - 0.5 * gsize + shift + 2.f * make_vec3(diskR, diskR, diskR),
                                   2.f * make_vec2(diskR, diskR), make_SphericalCoord(theta, phi));
            }
        }
    }
...
void LiDARcloud::disableMessages(void) { printmessages = false; }

void LiDARcloud::enableMessages(void) { printmessages = true; }

void LiDARcloud::validateRayDirections(void) {
        ...
        for (int j = 0; j < getScanSizePhi(s); j++) {
            for (int i = 0; i < getScanSizeTheta(s); i++) {
                if (getHitIndex(s, i, j) >= 0) {
                    ...
                    float err_theta =
                        max(fabs(direction1.zenith - direction2.zenith), fabs(direction1.zenith - direction3.zenith));

                    float err_phi = max(fabs(direction1.azimuth - direction2.azimuth),
                                        fabs(direction1.azimuth - direction3.azimuth));
...
uint LiDARcloud::getScanCount(void) { return scans.size(); }

void LiDARcloud::addScan(ScanMetadata newscan) {
    float epsilon = 1e-5;

    if (newscan.thetaMin < 0) {
        std::cerr << "WARNING (LiDARcloud::addScan): Specified scan minimum zenith angle of " << newscan.thetaMin
                  << " is less than 0. Truncating to 0." << std::endl;
        newscan.thetaMin = 0;
    }
    if (newscan.phiMin < 0) {
        std::cerr << "WARNING (LiDARcloud::addScan): Specified scan minimum azimuth angle of " << newscan.phiMin
                  << " is less than 0. Truncating to 0." << std::endl;
        newscan.phiMin = 0;
    }
    if (newscan.thetaMax > M_PI + epsilon) {
        std::cerr << "WARNING (LiDARcloud::addScan): Specified scan maximum zenith angle of " << newscan.thetaMax
                  << " is greater than pi. Truncating to pi. Did you mistakenly use degrees instead of radians?"
                  << std::endl;
        newscan.thetaMax = M_PI;
    }
    if (newscan.phiMax > 2.f * M_PI + epsilon) {
        std::cerr << "WARNING (LiDARcloud::addScan): Specified scan maximum azimuth angle of " << newscan.phiMax
                  << " is greater than 2pi. Truncating to 2pi. Did you mistakenly use degrees instead of radians?"
                  << std::endl;
...
void LiDARcloud::addHitPoint(const uint scanID, const helios::vec3 xyz, const helios::SphericalCoord direction,
                             const helios::RGBcolor color, const std::map<std::string, double> data) {
    // error checking
    if (scanID >= scans.size()) {
        cerr << "ERROR (addHitPoint): Hit point cannot be added to scan #" << scanID << " because there have only been "
             << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
void LiDARcloud::addHitPoint(const uint scanID, const helios::vec3 xyz, const helios::int2 row_column,
                             const helios::RGBcolor color, const std::map<std::string, double> data) {
    ScanMetadata scan = scans.at(scanID);
...
void LiDARcloud::deleteHitPoint(const uint index) {
    if (index >= hits.size()) {
        cerr << "WARNING (deleteHitPoint): Hit point #" << index
             << " cannot be deleted from the scan because there have only been " << hits.size() << " hit points added."
             << endl;
        return;
    }
...
uint LiDARcloud::getHitCount(void) const { return hits.size(); }

helios::vec3 LiDARcloud::getScanOrigin(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanOrigin): Cannot get origin of scan #" << scanID << " because there have only been "
             << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
uint LiDARcloud::getScanSizeTheta(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanSizeTheta): Cannot get theta size for scan #" << scanID
             << " because there have only been " << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
uint LiDARcloud::getScanSizePhi(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanSizePhi): Cannot get phi size for scan #" << scanID << " because there have only been "
             << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
helios::vec2 LiDARcloud::getScanRangeTheta(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanRangeTheta): Cannot get theta range for scan #" << scanID
             << " because there have only been " << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
helios::vec2 LiDARcloud::getScanRangePhi(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanRangePhi): Cannot get phi range for scan #" << scanID << " because there have only been "
             << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
float LiDARcloud::getScanBeamExitDiameter(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanBeamExitDiameter): Cannot get exit diameter for scan #" << scanID
             << " because there have only been " << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
float LiDARcloud::getScanBeamDivergence(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanBeamDivergence): Cannot get beam divergence for scan #" << scanID
             << " because there have only been " << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
std::vector<std::string> LiDARcloud::getScanColumnFormat(const uint scanID) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (getScanColumnFormat): Cannot get column format for scan #" << scanID
             << " because there have only been " << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
helios::vec3 LiDARcloud::getHitXYZ(const uint index) const {
    if (index >= hits.size()) {
        cerr << "ERROR (getHitXYZ): Hit point index out of bounds. Requesting hit #" << index << " but scan only has "
             << hits.size() << " hits." << endl;
        exit(EXIT_FAILURE);
    }
...
helios::SphericalCoord LiDARcloud::getHitRaydir(const uint index) const {
    if (index >= hits.size()) {
        cerr << "ERROR (getHitRaydir): Hit point index out of bounds. Requesting hit #" << index
             << " but scan only has " << hits.size() << " hits." << endl;
        exit(EXIT_FAILURE);
    }
...
void LiDARcloud::setHitData(const uint index, const char* label, const double value) {
    if (index >= hits.size()) {
        cerr << "ERROR (setHitScalarData): Hit point index out of bounds. Tried to set hit #" << index
             << " but scan only has " << hits.size() << " hits." << endl;
        exit(EXIT_FAILURE);
    }
...
double LiDARcloud::getHitData(const uint index, const char* label) const {
    if (index >= hits.size()) {
        cerr << "ERROR (getHitData): Hit point index out of bounds. Requesting hit #" << index << " but scan only has "
             << hits.size() << " hits." << endl;
        exit(EXIT_FAILURE);
    }
...
RGBcolor LiDARcloud::getHitColor(const uint index) const {
    if (index >= hits.size()) {
        cerr << "ERROR (getHitColor): Hit point index out of bounds. Requesting hit #" << index << " but scan only has "
             << hits.size() << " hits." << endl;
        exit(EXIT_FAILURE);
    }
...
int LiDARcloud::getHitIndex(const uint scanID, const uint row, const uint column) const {
    if (scanID >= scans.size()) {
        cerr << "ERROR (deleteHitPoint): Hit point cannot be deleted from scan #" << scanID
             << " because there have only been " << scans.size() << " scans added." << endl;
        exit(EXIT_FAILURE);
    }
...
int LiDARcloud::getHitGridCell(const uint index) const {
    if (index >= hits.size()) {
        cerr << "ERROR (getHitGridCell): Hit point index out of bounds. Requesting hit #" << index
             << " but scan only has " << hits.size() << " hits." << endl;
        exit(EXIT_FAILURE);
    } else if (hits.at(index).gridcell == -2) {
        cerr << "WARNING (getHitGridCell): hit grid cell for point #" << index
             << " was never set.  Returning a value of `-1'.  Did you forget to call calculateHitGridCell[*] first?"
             << endl;
        return -1;
    }
...
void LiDARcloud::setHitGridCell(const uint index, const int cell) {
    if (index >= hits.size()) {
        cerr << "ERROR (setHitGridCell): Hit point index out of bounds. Tried to set hit #" << index
             << " but scan only has " << hits.size() << " hits." << endl;
        exit(EXIT_FAILURE);
    }
...
void LiDARcloud::coordinateRotation(const float rotation, const helios::vec3 line_base,
...
uint LiDARcloud::getTriangleCount(void) const { return triangles.size(); }

Triangulation LiDARcloud::getTriangle(const uint index) const {
    if (index >= triangles.size()) {
        cerr << "ERROR (getTriangle): Triangle index out of bounds. Tried to get triangle #" << index
             << " but point cloud only has " << triangles.size() << " triangles." << endl;
        exit(EXIT_FAILURE);
    }
...
void LiDARcloud::addHitsToVisualizer(Visualizer* visualizer, const uint pointsize, const char* color_value) const {
    if (printmessages && scans.size() == 0) {
        std::cout << "WARNING (addHitsToVisualizer): There are no scans in the point cloud, and thus there is no "
                     "geometry to add...skipping."
                  << std::endl;
        return;
    }
...
void LiDARcloud::addGridToVisualizer(Visualizer* visualizer) const {
    if (printmessages && scans.size() == 0) {
        std::cout << "WARNING (addGridToVisualizer): There are no scans in the point cloud, and thus there is no "
                     "geometry to add...skipping."
                  << std::endl;
        return;
    }
...
void LiDARcloud::addTrianglesToVisualizer(Visualizer* visualizer) const {
    if (printmessages && scans.size() == 0) {
        std::cout << "WARNING (addGeometryToVisualizer): There are no scans in the point cloud, and thus there is no "
                     "geometry to add...skipping."
                  << std::endl;
        return;
    }
...
void LiDARcloud::addTrianglesToVisualizer(Visualizer* visualizer, const uint gridcell) const {
    if (printmessages && scans.size() == 0) {
        std::cout << "WARNING (addTrianglesToVisualizer): There are no scans in the point cloud, and thus there is no "
                     "geometry to add...skipping."
                  << std::endl;
        return;
    }
    ...
    for (uint i = 0; i < triangles.size(); i++) {
        ...
        if (tri.gridcell == gridcell) {
            visualizer->addTriangle(tri.vertex0, tri.vertex1, tri.vertex2, tri.color,
...
void LiDARcloud::addGrid(helios::vec3 gcenter, helios::vec3 gsize, helios::int3 ndiv, float rotation) {
    ...
    // add cells to grid
    vec3 gsubsize =
        make_vec3(float(gsize.x) / float(ndiv.x), float(gsize.y) / float(ndiv.y), float(gsize.z) / float(ndiv.z));
    ...
    for (int k = 0; k < ndiv.z; k++) {
        ...
        for (int j = 0; j < ndiv.y; j++) {
            ...
            for (int i = 0; i < ndiv.x; i++) {
                ...
                if (printmessages) {
                    cout << "Adding grid cell #" << count << " with center " << subcenter_rot.x + gcenter.x << ","
                         << subcenter_rot.y + gcenter.y << "," << subcenter.z + gcenter.z << " and size " << gsubsize.x
                         << " x " << gsubsize.y << " x " << gsubsize.z << endl;
                }

                addGridCell(subcenter + gcenter, gcenter, gsubsize, gsize, rotation * M_PI / 180.f, make_int3(i, j, k),
                            ndiv);
...
void LiDARcloud::addGridWireFrametoVisualizer(Visualizer* visualizer) const {
    for (int i = 0; i < getGridCellCount(); i++) {
        ...
        visualizer->addLine(make_vec3(boxmin.x, boxmin.y, boxmin.z), make_vec3(boxmin.x, boxmin.y, boxmax.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);
        visualizer->addLine(make_vec3(boxmin.x, boxmax.y, boxmin.z), make_vec3(boxmin.x, boxmax.y, boxmax.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);
        visualizer->addLine(make_vec3(boxmax.x, boxmin.y, boxmin.z), make_vec3(boxmax.x, boxmin.y, boxmax.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);
        visualizer->addLine(make_vec3(boxmax.x, boxmax.y, boxmin.z), make_vec3(boxmax.x, boxmax.y, boxmax.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);

        // horizontal top edges
        visualizer->addLine(make_vec3(boxmin.x, boxmin.y, boxmax.z), make_vec3(boxmin.x, boxmax.y, boxmax.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);
        visualizer->addLine(make_vec3(boxmin.x, boxmin.y, boxmax.z), make_vec3(boxmax.x, boxmin.y, boxmax.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);
        visualizer->addLine(make_vec3(boxmax.x, boxmin.y, boxmax.z), make_vec3(boxmax.x, boxmax.y, boxmax.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);
        visualizer->addLine(make_vec3(boxmin.x, boxmax.y, boxmax.z), make_vec3(boxmax.x, boxmax.y, boxmax.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);

        // horizontal bottom edges
        visualizer->addLine(make_vec3(boxmin.x, boxmin.y, boxmin.z), make_vec3(boxmin.x, boxmax.y, boxmin.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);
        visualizer->addLine(make_vec3(boxmin.x, boxmin.y, boxmin.z), make_vec3(boxmax.x, boxmin.y, boxmin.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);
        visualizer->addLine(make_vec3(boxmax.x, boxmin.y, boxmin.z), make_vec3(boxmax.x, boxmax.y, boxmin.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);
        visualizer->addLine(make_vec3(boxmin.x, boxmax.y, boxmin.z), make_vec3(boxmax.x, boxmax.y, boxmin.z),
                            RGB::black, 1, Visualizer::COORDINATES_CARTESIAN);
    }
}
...
void LiDARcloud::addLeafReconstructionToVisualizer(Visualizer* visualizer) const {
    ...
    for (size_t g = 0; g < Ngroups; g++) {
        visualizer->addRectangleByCenter(reconstructed_alphamasks_center.at(g), reconstructed_alphamasks_size.at(g),
                                         reconstructed_alphamasks_rotation.at(g),
                                         reconstructed_alphamasks_maskfile.c_str(), Visualizer::COORDINATES_CARTESIAN);
    }
}
...
std::vector<uint> LiDARcloud::addLeafReconstructionToContext(Context* context, const helios::int2 subpatches) const {
    ...
    if (subpatches.x > 1 || subpatches.y > 1) {
        UUID_leaf_template = context->addTile(make_vec3(0, 0, 0), make_vec2(1, 1), make_SphericalCoord(0, 0),
                                              subpatches, reconstructed_alphamasks_maskfile.c_str());
    }
    ...
    for (size_t g = 0; g < Ngroups; g++) {
        ...
        if (reconstructed_alphamasks_size.at(g).x > 0 && reconstructed_alphamasks_size.at(g).y > 0) {
            ...
            if (subpatches.x == 1 && subpatches.y == 1) {
                UUIDs_leaf.push_back(context->addPatch(
                    reconstructed_alphamasks_center.at(g), reconstructed_alphamasks_size.at(g),
                    reconstructed_alphamasks_rotation.at(g), reconstructed_alphamasks_maskfile.c_str()));
            } else {
                UUIDs_leaf = context->copyPrimitive(UUID_leaf_template);
                context->scalePrimitive(UUIDs_leaf, make_vec3(reconstructed_alphamasks_size.at(g).x,
                                                              reconstructed_alphamasks_size.at(g).y, 1));
...
void LiDARcloud::getHitBoundingBox(helios::vec3& boxmin, helios::vec3& boxmax) const {
    if (printmessages && hits.size() == 0) {
        std::cout << "WARNING (getHitBoundingBox): There are no hit points in the point cloud, cannot determine "
                     "bounding box...skipping."
                  << std::endl;
        return;
    }
...
void LiDARcloud::getGridBoundingBox(helios::vec3& boxmin, helios::vec3& boxmax) const {
    if (printmessages && getGridCellCount() == 0) {
        std::cout << "WARNING (getGridBoundingBox): There are no grid cells in the point cloud, cannot determine "
                     "bounding box...skipping."
                  << std::endl;
        return;
    }
...
bool sortcol0(const std::vector<double>& v0, const std::vector<double>& v1) { return v0.at(0) < v1.at(0); }

bool sortcol1(const std::vector<double>& v0, const std::vector<double>& v1) { return v0.at(1) < v1.at(1); }
...
void LiDARcloud::maxPulseFilter(const char* scalar) {
    ...
    for (std::size_t r = 0; r < getHitCount(); r++) {
        if (!doesHitDataExist(r, "timestamp")) {
            std::cout << "ERROR (maxPulseFilter): Hit point " << r
...
void LiDARcloud::minPulseFilter(const char* scalar) {
    ...
    for (std::size_t r = 0; r < getHitCount(); r++) {
        if (!doesHitDataExist(r, "timestamp")) {
            std::cout << "ERROR (minPulseFilter): Hit point " << r
                      << " does not have scalar data "
                         "timestamp"
                         "."
                      << std::endl;
...
void LiDARcloud::firstHitFilter(void) {
    ...
    for (std::size_t r = 0; r < target_index.size(); r++) {
        if (!doesHitDataExist(r, "target_index")) {
            std::cout << "ERROR (firstHitFilter): Hit point " << r
                      << " does not have scalar data "
                         "target_index"
                         "."
                      << std::endl;
...
void LiDARcloud::lastHitFilter(void) {
    ...
    for (std::size_t r = 0; r < target_index.size(); r++) {
        if (!doesHitDataExist(r, "target_index")) {
            std::cout << "ERROR (lastHitFilter): Hit point " << r
                      << " does not have scalar data "
                         "target_index"
                         "."
                      << std::endl;
...
void LiDARcloud::triangulateHitPoints(const float Lmax, const float max_aspect_ratio) {
    if (printmessages && getScanCount() == 0) {
        cout
            << "WARNING (triangulateHitPoints): No scans have been added to the point cloud.  Skipping triangulation..."
            << endl;
        return;
    } else if (printmessages && getHitCount() == 0) {
        cout << "WARNING (triangulateHitPoints): No hit points have been added to the point cloud.  Skipping "
                "triangulation..."
             << endl;
        return;
    }
    ...
    for (uint s = 0; s < getScanCount(); s++) {
        ...
        for (int t = 0; t < triads.size(); t++) {
            ...
            if (printmessages && gridcell == -2) {
                cout << "WARNING (triangulateHitPoints): You typically want to define the hit grid cell for all hit "
                        "points before performing triangulation."
                     << endl;
            }
...
void LiDARcloud::addTrianglesToContext(Context* context) const {
    if (scans.size() == 0) {
        if (printmessages) {
            std::cout << "WARNING (addTrianglesToContext): There are no scans in the point cloud, and thus there are "
                         "no triangles to add...skipping."
                      << std::endl;
        }
        return;
    }
...
uint LiDARcloud::getGridCellCount(void) const { return grid_cells.size(); }
...
void LiDARcloud::addGridCell(const helios::vec3 center, const helios::vec3 global_anchor, const helios::vec3 size,
                             const helios::vec3 global_size, const float rotation, const helios::int3 global_ijk,
...
helios::vec3 LiDARcloud::getCellCenter(const uint index) const {
    if (index >= getGridCellCount()) {
        cout << "ERROR (getCellCenter): grid cell index out of range.  Requested center of cell #" << index
             << " but there are only " << getGridCellCount() << " cells in the grid." << endl;
        exit(EXIT_FAILURE);
    }
...
helios::vec3 LiDARcloud::getCellGlobalAnchor(const uint index) const {
    if (index >= getGridCellCount()) {
        cout << "ERROR (getCellGlobalAnchor): grid cell index out of range.  Requested anchor of cell #" << index
             << " but there are only " << getGridCellCount() << " cells in the grid." << endl;
        exit(EXIT_FAILURE);
    }
...
helios::vec3 LiDARcloud::getCellSize(const uint index) const {
    if (index >= getGridCellCount()) {
        cout << "ERROR (getCellCenter): grid cell index out of range.  Requested size of cell #" << index
             << " but there are only " << getGridCellCount() << " cells in the grid." << endl;
        exit(EXIT_FAILURE);
    }
...
float LiDARcloud::getCellRotation(const uint index) const {
    if (index >= getGridCellCount()) {
        cout << "ERROR (getCellRotation): grid cell index out of range.  Requested rotation of cell #" << index
             << " but there are only " << getGridCellCount() << " cells in the grid." << endl;
        exit(EXIT_FAILURE);
    }
...
float LiDARcloud::getCellLeafArea(const uint index) const {
    if (index >= getGridCellCount()) {
        cout << "ERROR (getCellLeafArea): grid cell index out of range. Requested leaf area of cell #" << index
             << " but there are only " << getGridCellCount() << " cells in the grid." << endl;
        exit(EXIT_FAILURE);
    }
...
float LiDARcloud::getCellLeafAreaDensity(const uint index) const {
    if (index >= getGridCellCount()) {
        cout << "ERROR (getCellLeafAreaDensity): grid cell index out of range. Requested leaf area density of cell #"
             << index << " but there are only " << getGridCellCount() << " cells in the grid." << endl;
        exit(EXIT_FAILURE);
    }
...
float LiDARcloud::getCellGtheta(const uint index) const {
    if (index >= getGridCellCount()) {
        cout << "ERROR (getCellGtheta): grid cell index out of range. Requested leaf area of cell #" << index
             << " but there are only " << getGridCellCount() << " cells in the grid." << endl;
        exit(EXIT_FAILURE);
    }
...
void LiDARcloud::floodfill(size_t t, std::vector<Triangulation>& t_triangles, std::vector<int>& fill_flag,
                           std::vector<std::vector<int> >& nodes, const int tag, const int depth, const int maxdepth) {
...
void LiDARcloud::leafReconstructionAlphaMask(const float minimum_leaf_group_area, const float maximum_leaf_group_area,
                                             const float leaf_aspect_ratio, const char* mask_file) {
    leafReconstructionAlphaMask(minimum_leaf_group_area, maximum_leaf_group_area, leaf_aspect_ratio, -1.f, mask_file);
}

void LiDARcloud::leafReconstructionAlphaMask(const float minimum_leaf_group_area, const float maximum_leaf_group_area,
                                             const float leaf_aspect_ratio, const float leaf_length_constant,
    ...
    if (triangles.size() == 0) {
        ...
        std::cerr << "There are no triangulated points.  Either the triangulation failed or 'triangulateHitPoints()' "
                     "was not called."
                  << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    if (file.substr(file.find_last_of(".") + 1) != "png") {
        std::cerr << "ERROR (leafReconstructionAlphaMask): Mask data file " << mask_file << " must be PNG image format."
                  << std::endl;
        exit(EXIT_FAILURE);
    }
    std::vector<std::vector<bool> > maskdata = readPNGAlpha(mask_file);
    if (maskdata.size() == 0) {
        std::cerr << "ERROR (leafReconstructionAlphaMask): Could not load mask file " << mask_file
                  << ". It contains no data." << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    for (int v = 0; v < getGridCellCount(); v++) {
        ...
        // std::partial_sort( group_areas.at(v).begin(), group_areas.at(v).begin()+Navg,group_areas.at(v).end(),
        // std::greater<float>() );
    ...
    for (int group = 0; group < reconstructed_triangles.size(); group++) {
        ...
        for (int t = 0; t < reconstructed_triangles.at(group).size(); t++) {
            position = position + reconstructed_triangles.at(group).at(t).vertex0 /
                                      float(reconstructed_triangles.at(group).size());
        }
        ...
        helios::vec3 normal = cross(
            reconstructed_triangles.at(group).at(gind).vertex1 - reconstructed_triangles.at(group).at(gind).vertex0,
            reconstructed_triangles.at(group).at(gind).vertex2 - reconstructed_triangles.at(group).at(gind).vertex0);
        reconstructed_alphamasks_rotation.push_back(
            make_SphericalCoord(cart2sphere(normal).zenith, cart2sphere(normal).azimuth));
...
void LiDARcloud::backfillLeavesAlphaMask(const std::vector<float> leaf_size, const float leaf_aspect_ratio,
                                         const float solidfraction, const std::vector<bool> group_filter_flag) {
    if (printmessages) {
    ...
    for (uint v = 0; v < Ncells; v++) {
        ...
        if (leaf_area_total == 0 || reconstructed_alphamasks_size.size() == 0) {  // no leaves in gridcell
            ...
            while (leaf_area_current.at(v) < leaf_area_total) {
                ...
                helios::vec3 shift = cellcenter + rotatePoint(helios::make_vec3((randu() - 0.5) * cellsize.x,
                                                                                (randu() - 0.5) * cellsize.y,
                                                                                (randu() - 0.5) * cellsize.z),
                                                              0, rotation);
                ...
                leaf_area_current.at(v) +=
                    reconstructed_alphamasks_size.back().x * reconstructed_alphamasks_size.back().y * solidfraction;
            }
            ...
            while (leaf_area_current.at(v) > leaf_area_total) {
                ...
                leaf_area_current.at(v) -= reconstructed_alphamasks_size.at(group_index).x *
                                           reconstructed_alphamasks_size.at(group_index).y * solidfraction;
            }
            ...
            while (leaf_area_current.at(v) < leaf_area_total) {
                ...
                // helios::vec3 shift = reconstructed_alphamasks_center.at(group_index) + helios::make_vec3(
                // 0.45*(randu()-0.5)*cellsize.x, 0.45*(randu()-0.5)*cellsize.y, 0.45*(randu()-0.5)*cellsize.z );
                // //uniform shift about group
                helios::vec3 shift =
                    reconstructed_alphamasks_center.at(group_index) +
                    helios::make_vec3(0.25 * randn(generator) * cellsize.x, 0.25 * randn(generator) * cellsize.y,
                                      0.25 * randn(generator) * cellsize.z);  // Gaussian shift about group
                // helios::vec3 shift = cellcenter + helios::make_vec3( (randu()-0.5)*cellsize.x,
                // (randu()-0.5)*cellsize.y, (randu()-0.5)*cellsize.z ); //uniform shift within voxel
                shift = rotatePointAboutLine(shift, cellanchor, make_vec3(0, 0, 1), rotation);

                if (group_index >= reconstructed_alphamasks_center.size()) {
                    std::cout << "FAILED: " << group_index << " " << reconstructed_alphamasks_center.size() << " "
                              << randi << std::endl;
                    exit(EXIT_FAILURE);
    ...
    for (uint v = 0; v < Ncells; v++) {
        ...
        for (uint i = 0; i < reconstructed_alphamasks_size.size(); i++) {
            if (reconstructed_alphamasks_gridcell.at(i) == v) {
                current_area +=
                    reconstructed_alphamasks_size.at(i).x * reconstructed_alphamasks_size.at(i).y * solidfraction;
            }
        }
    }
    ...
    for (int i = deleted_groups.size() - 1; i >= 0; i--) {
        ...
        if (group_index >= 0 && group_index < reconstructed_alphamasks_center.size()) {
            ...
            std::swap(reconstructed_alphamasks_direct_flag.at(group_index),
                      reconstructed_alphamasks_direct_flag.back());
...
void LiDARcloud::calculateLeafAngleCDF(const uint Nbins, std::vector<std::vector<float> >& CDF_theta,
                                       std::vector<std::vector<float> >& CDF_phi) {
    uint Ncells = getGridCellCount();
    ...
    for (size_t g = 0; g < reconstructed_triangles.size(); g++) {
        for (size_t t = 0; t < reconstructed_triangles.at(g).size(); t++) {
            ...
            helios::vec3 normal =
                cross(reconstructed_triangles.at(g).at(t).vertex1 - reconstructed_triangles.at(g).at(t).vertex0,
                      reconstructed_triangles.at(g).at(t).vertex2 - reconstructed_triangles.at(g).at(t).vertex0);
