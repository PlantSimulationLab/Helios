...
void PlantArchitecture::subtractShootMaintenanceCarbon(float dt ){
    ...
    for( auto &plant: plant_instances ){
        ...
        for( auto &shoot: *shoot_tree ){
            ...
            for(auto &phytomer : shoot->phytomers){
                for(uint obj_ID : phytomer->internode_objIDs){
                    shoot->carbohydrate_pool_molC -= context_ptr->getConeObjectVolume(obj_ID)* rho_cw * stem_maintainance_respiration_rate * dt;
                }
